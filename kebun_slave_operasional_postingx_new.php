...
$str = "SELECT * FROM " . $dbname . ".kebun_prestasi_detail where notransaksi='" . $param['notransaksi'] . "'"; #exit('error'.$str);
$ceckD = fetchData($str);
...
$strx = selectQuery($dbname, 'kebun_aktifitas', "*", "notransaksi='" . $param['notransaksi'] . "'");
$dtx = fetchData($strx);
...
try {
		...
		foreach ($valKegX as $kegiatanX => $valBlokX) {
			foreach ($valBlokX as $blokX) {
				...
				if (!in_array($dataH[0]['kodeorg'], $arrDetailAkses)) {
					throw new PDOException("Anda tidak memiliki detail akses ke unit " . $dataH[0]['kodeorg'] . ".<br>
				Silakan hubungi administrator untuk mengatur detail akses anda.");
				}
				...
				$queryAbs = "SELECT a.jhk,a.umr,a.insentif,a.penalty,a.nik FROM " . $dbname . ".kebun_kehadiran_detail a
				left join " . $dbname . ".kebun_prestasi_detail b on a.notransaksi=b.notransaksi and a.nik=b.nikpemel and a.nourut=b.nourut 
				where a.notransaksi='" . $notranX . "' and b.kodekegiatan='" . $kegiatanX . "' and b.kodeorg='" . $blokX . "'";
				// exit("warning: ".$queryAbs);
				...
				# Ambil data dari  kebun_pakaimaterial_detail
				$queryM = selectQuery($dbname, 'kebun_pakaimaterial_detail', "*", "notransaksi='" . $notranX . "' and kodekegiatan='" . $kegiatanX . "' and kodeorg='" . $blokX . "'");
				...
				// Jika Ada data karyawan histori di periode gaji dan berdasarkan karyawanid di prestasi pekerja, maka pakailah yang datakaryawan_hist 
				if ($counthst > 0) {
					$sDr = "select a.nik, lokasitugas,sum(a.umr) as uphtot,sum(a.insentif) as premi, sum(a.penalty) as penalty,kodeorganisasi as kodept 
					from " . $dbname . ".kebun_kehadiran_detail a 
					left join " . $dbname . ".datakaryawan_hist b on a.nik=b.karyawanid AND b.periodegaji='{$prdgj}' AND b.version_type = 'b' 
					left join " . $dbname . ".kebun_prestasi_detail c on a.notransaksi=c.notransaksi and a.nourut=c.nourut 
					where a.notransaksi='" . $notranX . "' and c.kodekegiatan='" . $kegiatanX . "' and c.kodeorg='" . $blokX . "' 
					group by lokasitugas, a.nik";
				} else {
					$sDr = "select a.nik, lokasitugas,sum(a.umr) as uphtot,sum(a.insentif) as premi, sum(a.penalty) as penalty,kodeorganisasi as kodept 
					from " . $dbname . ".kebun_kehadiran_detail a 
				...
				$queryJ = selectQuery(
					...
					'keu_5kelompokjurnal',
					'nokounter',
					"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnal . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
				); #exit("error".$queryJ);
				$tmpKonter = fetchData($queryJ);
				if (count($tmpKonter) == 0) {
					throw new PDOException("Kelompok Jurnal M0 untuk kodeorg " . $_SESSION['org']['kodeorganisasi'] . ", kodeunit : " . $dataH[0]['kodeorg'] . ", periode : " . $prdbln . " silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");
				}
				...
				$res = fetchdata($str);
				if (count($res) > 0) {
					$str = "select max(convert(substring_index(nojurnal,'/',-1),unsigned integer)) as nomor from keu_jurnalht where kodejurnal='" . $kodeJurnal . "' and nojurnal like '%" . $dataH[0]['kodeorg'] . "%' and tanggal like '" . $prdbln . "%'";
					$res = fetchdata($str);
				...
				$kodeJurnalpen = 'PPRWT';
				# Get Journal Counter
				$queryJpen = selectQuery(
					$dbname,
					'keu_5kelompokjurnal',
					'nokounter',
					"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnalpen . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
				); #exit("error".$queryJpen);
				$tmpKonterpen = fetchData($queryJpen);
				if (count($tmpKonterpen) == 0) {
					throw new PDOException("Kelompok Jurnal PPRWT untuk kodeorg " . $_SESSION['org']['kodeorganisasi'] . ", kodeunit : " . $dataH[0]['kodeorg'] . ", periode : " . $prdbln . "  silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");
				}
				...
				$res = fetchdata($str);
				if (count($res) > 0) {
					$str = "select max(convert(substring_index(nojurnal,'/',-1),unsigned integer)) as nomor from keu_jurnalht where kodejurnal='" . $kodeJurnalpen . "' and nojurnal like '%" . $dataH[0]['kodeorg'] . "%' and tanggal like '" . $prdbln . "%'";
					$res = fetchdata($str);
				...
				$kodeJurnalPre = 'M9';
				# Get Journal Counter
				$queryJpre = selectQuery(
					$dbname,
					'keu_5kelompokjurnal',
					'nokounter',
					"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnalPre . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
				); #exit("error".$queryJpre);
				$tmpKonterPre = fetchData($queryJpre);
				if (count($tmpKonterPre) == 0) {
					throw new PDOException("Kelompok Jurnal M9 untuk kodeorg " . $_SESSION['org']['kodeorganisasi'] . ", kodeunit : " . $dataH[0]['kodeorg'] . ", periode : " . $prdbln . " silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");
				}
				...
				$res = fetchdata($str);
				if (count($res) > 0) {
					$str = "select max(convert(substring_index(nojurnal,'/',-1),unsigned integer)) as nomor from keu_jurnalht where kodejurnal='" . $kodeJurnalPre . "' and nojurnal like '%" . $dataH[0]['kodeorg'] . "%' and tanggal like '" . $prdbln . "%'";
					$res = fetchdata($str);
				...
				$kodenyagudang = '';
				$strt = "SELECT a.kodegudang FROM " . $dbname . ".`kebun_pakaimaterial_detail` a WHERE a.notransaksi='" . $notranX . "' and a.kodekegiatan='" . $kegiatanX . "' and a.kodeorg='" . $blokX . "'";
				...
				if (count($lstUnit) != 0) {
					foreach ($lstUnit as $rw => $rwUnit) {
						if ($rwUnit != $dataH[0]['kodeorg']) {
							...
							$aknHtg = makeOption($dbname, 'keu_5caco', 'kodeorg,akunhutang', "kodeorg='" . $dataH[0]['kodeorg'] . "' and jenis='" . $jenis . "'");
							...
							if (count($tmpKonter2) == 0) {
								throw new PDOException("Kelompok Jurnal M untuk kodeorg " . $lstPt[$rwUnit] . ", kodeunit : " . $rwUnit . ", periode : " . $prdbln . " silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");
							}
				...
				if (count($lstUnit) != 0) {
					foreach ($lstUnit as $rw => $rwUnit) {
						if ($rwUnit != $dataH[0]['kodeorg']) {
							...
							if (count($tmpKonter2pre) == 0) {
								throw new PDOException("Kelompok Jurnal M untuk kodeorg " . $lstPt[$rwUnit] . ",   kodeunit : " . $rwUnit . ", periode : " . $prdbln . " silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");
							}

							# Bug nya jika ada Jurnal RK
							// @$konterpre = addZero($tmpKonter2pre[0]['nokounter']+1,3);
							// @$counterDtpre[$rwUnit]=intval($tmpKonter2pre[0]['nokounter'])+1;
							# End

							# Ganti pengambilan jurnal ke nokonter pake,
							# Jurnal di atasnya
							@$konterpre = addZero($konter + 1, 3);
							@$counterDtpre[$rwUnit] = intval($konter) + 1;
							# End
				...
				#Upah
				if (@count($lstUnit[$dataH[0]['kodeorg']]) != 0 and ($costRawat - $totRpRK) != 0) {
					foreach ($dataD as $row) {
					...
					# Detail (Kredit)
					foreach ($listnik as $nik) {
						if ($datanik[$dataH[0]['kodeorg']][$nik] != '') {
							$noUrut++;
							$dataRes['detail'][] = array(
								'nojurnal'    => $nojurnal,
								'tanggal'     => $dataH[0]['tanggal'],
				...
				#Penalty
				if (@count($lstUnit[$dataH[0]['kodeorg']]) != 0 and ($penaltyRawat - $totRpRKPen) != 0) {
					foreach ($dataD as $row) {
						if ($nmorg[substr($row['kodeorg'], 0, 4)] != '') {
							$kodeorgx = substr($row['kodeorg'], 0, 4);
						} else {
				...
				#Premi
				if (@count($lstUnit[$dataH[0]['kodeorg']]) != 0 and ($costRawatPre - $totRpRKPre) != 0) {
					foreach ($dataD as $row) {
						if ($nmorg[substr($row['kodeorg'], 0, 4)] != '') {
							$kodeorgx = substr($row['kodeorg'], 0, 4);
						} else {
					...
					# Detail (Kredit)
					foreach ($listnik as $nik) {
						if ($datanik[$dataH[0]['kodeorg']][$nik] != '' and ($costRawatPreNik[$nik] - $totRpRKPreNik[$nik]) != 0) {
							$dataResPre['detail'][] = array(
								'nojurnal'    => $nojurnalpre,
								'tanggal'     => $dataH[0]['tanggal'],
								'nourut'      => $noUrutPre,
				...
				if (count($lstPent) != 0) {
					$queryKonter = updateQuery(
						$dbname,
						'keu_5kelompokjurnal',
						array('nokounter' => $tmpKonterpen[0]['nokounter'] + 1),
						"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnalpen . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
					);
					$owlPDO->exec($queryKonter);
				}
				...
				if (count($lstPre) != 0) {
					$queryKonter = updateQuery(
						$dbname,
						'keu_5kelompokjurnal',
						array('nokounter' => $tmpKonterPre[0]['nokounter'] + 1),
						"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnalPre . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
					);
					$owlPDO->exec($queryKonter);
				}
	...
	while ($bar3 = $res3->fetch()) {
		$dataXX2[$bar3['notransaksi']][$bar3['kodekegiatan']][$bar3['kodeorg']][$bar3['kodebarang']] = $bar3['kodebarang'];
		$dataXJurnal2[$bar3['notransaksi']][$bar3['kodekegiatan']][$bar3['kodeorg']] = $bar3['kodeorg'];
		@$adamat2 += 1;
	}
	#jalankan jika ada material
	if ($adamat2 != '') {
		$rpmatperbarang2 = array();
		#ini insert jurnal material
		foreach (@$dataXJurnal2 as $notranXXJ => $valKegXXJ) {
				...
				where a.notransaksi='" . $notranXXJ . "' and a.kodegudang!='' and a.kodekegiatan='" . $kegiatanXXJ . "' and a.kodeorg='" . $blokXXJ . "'";
					...
					while ($bab = $resx->fetch()) {
						if (($bab->kwantitas <= 0) || ($bab->kwantitas == '')) {
							...
							$errorDB .= "Induk Blok: " . $bab->kodeorg . " (" . getIndukBlok($bab->kodeorg) . ")<br>";
							$errorDB .= "Kegiatan: " . $bab->kodekegiatan . " (" . getNamaKeg($bab->kodekegiatan) . ")<br>";
							$errorDB .= "Material: " . $bab->kodebarang . " (" . getNamaBrg($bab->kodebarang) . ")<br>";
							$errorDB .= "Jumlah Material: " . $bab->kwantitas . " " . $bab->satuan . "<br>";
							throw new PDOException("Qty " . $pesanError . "<br>" . $errorDB);
						}
						...
						if (isset($akunbarang[substr($bab->kodebarang, 0, 3)]) and $akunbarang[substr($bab->kodebarang, 0, 3)] != '') {
							...
							$rpmatperbarang2[$bab->kodegudang][$bab->kodebarang] += $nilairound;
			...
			foreach ($dataXX2 as $notranXX => $valKegXX) {
				$noxz = 0;
						...
						foreach ($valBrgXX as $barangXX) {
							...
							foreach ($gud as $keygud => $valgud) {
								foreach ($brg[$valgud] as $keybrg => $valbrg) {
									...
									$ttlrpgudang += $rpmatperbarang2[$valgud][$valbrg];
								}
							}
						}
					}
	...
	#cek apakah 1 no transaksi terdiri dari beberapa keg dan blok (Menggunakan Blok Kecil)
	$str3 = "SELECT * FROM " . $dbname . ".kebun_pakaimaterial_detail where notransaksi='" . $param['notransaksi'] . "'"; #exit('error'.$str3);
	$res3 = $owlPDO->query($str3) or die(print " Gagal: " . PDOException::getMessage());
	$res3->setFetchMode(PDO::FETCH_ASSOC);
	$adamat = 0;
	while ($bar3 = $res3->fetch()) {
		...
		foreach (@$dataXJurnal as $notranXXJ => $valKegXXJ) {
			foreach ($valKegXXJ as $kegiatanXXJ => $valBlokXXJ) {
				foreach ($valBlokXXJ as $blokXXJ) {
					...
					// GET LUAS BLOK TERKINI
					$sblok = "SELECT kodeorg, luasareaproduktif FROM $dbname.setup_blok_tahunan WHERE tahun='" . str_replace("-", "", $prdbln) . "' AND kodeorg='" . $blokXXJ . "'";
					$rblok = fetchData($sblok)[0];
					$countblok = count(fetchData($sblok));
					if ($countblok == 0) {
						$sblok = "SELECT kodeorg, luasareaproduktif FROM $dbname.setup_blok WHERE kodeorg='" . $blokXXJ . "'";
						$rblok = fetchData($sblok)[0];
					}
					...
					$queryJ = selectQuery(
						...
						'keu_5kelompokjurnal',
						'nokounter',
						"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnal1 . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
					); #exit("error".$queryJ);
					...
					$res = fetchdata($str);
					if (count($res) > 0) {
						$str = "select max(convert(substring_index(nojurnal,'/',-1),unsigned integer)) as nomor from keu_jurnalht where kodejurnal='" . $kodeJurnal1 . "' and nojurnal like '%" . $dataH[0]['kodeorg'] . "%' and tanggal like '" . $prdbln . "%'";
						$res = fetchdata($str);
					...
					# Detail (kredit)
					$str = "select a.*,b.namabarang,b.satuan from " . $dbname . ".kebun_pakaimaterial_detail a
				  left join " . $dbname . ".log_5masterbarang b on a.kodebarang=b.kodebarang
				  where a.notransaksi='" . $notranXXJ . "' and a.kodegudang!='' and a.kodekegiatan='" . $kegiatanXXJ . "' and a.kodeorg='" . $blokXXJ . "'";
					// exit("error".$str);
					$noUrut = 1;
					...
					$resx->setFetchMode(PDO::FETCH_OBJ);
					while ($bab = $resx->fetch()) {
						$luasblokkecilx 	= $rblok['luasareaproduktif'];
						$blokkecilx			= $bab->kodeorg;

						if ($luasblokkecilx > 0) {
							if (($bab->kwantitas <= 0) || ($bab->kwantitas == '')) {
								$pesanError = "Kuantitas Barang Tidak Boleh Kosong atau nol\n:";
								$errorDB .= "Blok Kecil: " . $bab->kodeorg . " (" . getNamaOrg($bab->kodeorg) . ")<br>";
								$errorDB .= "Kegiatan: " . $bab->kodekegiatan . " (" . getNamaKeg($bab->kodekegiatan) . ")<br>";
								$errorDB .= "Material: " . $bab->kodebarang . " (" . getNamaBrg($bab->kodebarang) . ")<br>";
								$errorDB .= "Jumlah Material: " . $bab->kwantitas . " " . $bab->satuan . "<br>";
								throw new PDOException("Qty " . $pesanError . "<br>" . $errorDB);
					...
					if ($namabarang != '') { // kalo transaksi BKM tanpa material, ga usah eksekusi jurnal barangnya
						...
						$queryKonter = updateQuery(
							$dbname,
							'keu_5kelompokjurnal',
							array('nokounter' => $tmpKonter1[0]['nokounter'] + 1),
							"kodeorg='" . $_SESSION['org']['kodeorganisasi'] . "' and kodekelompok='" . $kodeJurnal1 . "' and kodeunit='" . $dataH[0]['kodeorg'] . "' and periode='" . $prdbln . "'"
						);
						$owlPDO->exec($queryKonter);
					}
			...
			foreach ($dataXX as $notranXX => $valKegXX) {
				...
				foreach ($valKegXX as $kegiatanXX => $valBlokXX) {
					foreach ($valBlokXX as $blokXX => $valBrgXX) {
						// GET LUAS BLOK TERKINI
						$sblok = "SELECT kodeorg, luasareaproduktif FROM $dbname.setup_blok_tahunan WHERE tahun='" . str_replace("-", "", $prdbln) . "' AND kodeorg='" . $blokXX . "'";
						$rblok = fetchData($sblok)[0];
						$countblok = count(fetchData($sblok));
						if ($countblok == 0) {
							$sblok = "SELECT kodeorg, luasareaproduktif FROM $dbname.setup_blok WHERE kodeorg='" . $blokXX . "'";
							$rblok = fetchData($sblok)[0];
						}
						...
						and a.kodekegiatan='" . $kegiatanXX . "' and a.kodeorg='" . $blokXX . "'";
							...
							foreach ($res as $bar) {
								$brg[$bar['kodegudang']][$bar['kodebarang']] = $bar['kodebarang'];
								$gud[$bar['kodegudang']] = $bar['kodegudang'];
								$luasblokkecilm 	= $rblok['luasareaproduktif'];
								$blokkecilm			= $bar['kodeorg'];
							}
							...
							if ($luasblokkecilm > 0) {
								...
								$nomor = array();
								$str = "select distinct kodegudang from " . $dbname . ".kebun_pakaimaterial_detail where notransaksi='" . $notranXX . "' and kodegudang!='' and kodekegiatan='" . $kegiatanXX . "' and kodeorg='" . $blokXX . "' and kodebarang='" . $barangXX . "'";
								$resc = fetchData($str);
								foreach ($resc as $bar1) {
									...
									$str = "select CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(notransaksi, '-', 1), 'M', -1) AS UNSIGNED) AS notransaksi from " . $dbname . ".log_transaksiht where tipetransaksi=5 and kodegudang='" . $gudang . "' and tanggal>=" . $_SESSION['gudang'][$bar1['kodegudang']]['start'] . " and tanggal<=" . $_SESSION['gudang'][$bar1['kodegudang']]['end'] . " and (notransaksireferensi!='' AND notransaksireferensi IS NOT NULL) order by notransaksi desc limit 1";
									$res = fetchData($str);
									foreach ($res as $bar) {
										$num = $bar['notransaksi'];
							...
							and a.kodekegiatan='" . $kegiatanXX . "' and a.kodeorg='" . $blokXX . "' and a.kodebarang='" . $barangXX . "'";
								...
								$str = "select induk from " . $dbname . ".organisasi where kodeorganisasi = '" . $dataH[0]['kodeorg'] . "'";
								$kodeorganisasi = "";
								$res = fetchData($str);
								foreach ($res as $bar) {
								...
								$awlheader = 1;
								$str = "select a.*,b.namabarang,b.satuan,a.kodekegiatan from " . $dbname . ".kebun_pakaimaterial_detail a left join " . $dbname . ".log_5masterbarang b on a.kodebarang=b.kodebarang where a.notransaksi='" . $notranXX . "' and a.kodegudang!='' and a.kodekegiatan='" . $kegiatanXX . "' and a.kodeorg='" . $blokXX . "' and a.kodebarang='" . $barangXX . "'";
								...
								#periksa apakah saldo mencukupi:
								$str = "select a.notransaksi, a.kodebarang, a.kodegudang, sum(a.kwantitas) as kwantitas,b.namabarang,b.satuan from " . $dbname . ".kebun_pakaimaterial_detail a
							left join " . $dbname . ".log_5masterbarang b on a.kodebarang=b.kodebarang
							where a.notransaksi='" . $notranXX . "' and a.kodegudang='" . $kodegudangxz[$blokXX] . "' and a.kodebarang='" . $barangXX . "' group by a.kodebarang, a.notransaksi,a.kodegudang 
							";
								$errsaldo = '';
						...
						foreach ($gud as $keygud => $valgud) {
							foreach ($brg[$valgud] as $keybrg => $valbrg) {
								...
								set hargarata='" . $harga[$valgud][$valbrg] . "' where kodebarang='" . $valbrg . "' and notransaksi in (
									...
									$stup = "update " . $dbname . ".kebun_pakaimaterial_detail set hargasatuan=" . $harga[$valgud][$valbrg] . " where kodegudang='" . $valgud . "' and kodebarang='" . $valbrg . "' and notransaksi='" . $param['notransaksi'] . "'";
									$owlPDO->exec($stup);
					...
					and a.kodekegiatan='" . $kegiatanXX . "' and a.kodeorg='" . $blokXX . "'";
						...
						foreach ($gud as $keygud => $valgud) {
							foreach ($brg[$valgud] as $keybrg => $valbrg) {
								...
								$xsisa = round($nilaisaldoakhir[$valgud][$valbrg] - $rpmatperbarang2[$valgud][$valbrg]);
								// exit("Warning:<br>Nilai Saldo Akhir".$nilaisaldoakhir[$valgud][$valbrg]."<br>Rupiah Per Barang".$rpmatperbarang2[$valgud][$valbrg]."");
								// sebelumnya diginiin, hasilnya jadi -0 < 0
								// if(round($nilaisaldoakhir[$valgud][$valbrg]-($jumlah[$valgud][$valbrg]*$harga[$valgud][$valbrg]))<0){
								if ($xsisa < 0) {
									//throw new PDOException($xsisa." Nilai rupiah saldo tidak mencukupi.\nRupiah saldo : ".number_format($nilaisaldoakhir[$valgud][$valbrg])."\nRupiah pakai : ".number_format($jumlah[$valgud][$valbrg]*$harga[$valgud][$valbrg])."\nNama barang : ".$optnmbrg[$valbrg]."");
									throw new PDOException($xsisa . " Nilai rupiah saldo tidak mencukupi.\nRupiah saldo : " . number_format($nilaisaldoakhir[$valgud][$valbrg]) . "\nRupiah pakai : " . number_format($rpmatperbarang2[$valgud][$valbrg]) . "");
								}
								...
								#ambil rupiah dari rp jurnal
								$nilairound = $rpmatperbarang2[$valgud][$valbrg];
								...
								if ($qtyout < 0) {
									throw new PDOException("Nilai qty keluar salah, proses dibatalkan.\n\nNama barang : " . $optnmbrg[$valbrg] . "\nNilai : " . $qtyout . "");
								}
								if ($rpout < 0) {
