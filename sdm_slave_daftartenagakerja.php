...
switch($proses) {
	...
	echo"<table cellpadding=1 cellspacing=1 border=0 class=sortable width=100%><thead><tr align=center>";
        ...
        $rowdata=owlBaris($qdata);
        $saks="select distinct * from ".$dbname.".setup_remotetimbangan 
               where lokasi='HRDJKRT'";		
        $qaks=$owlPDO->query($saks) or die(print " Gagal: ".PDOException::getMessage());
        $qaks->setFetchMode(PDO::FETCH_ASSOC);
        $jaks=$qaks->fetch();
        ...
        try {
          $owlPDO2 = new PDO('mysql:host='.$dbserver2.';dbname='.$dbdt, $uname2, $passwd2, array(PDO::ATTR_PERSISTENT => true));
          $owlPDO2->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
        }
        ...
        while($rdata=  $qdata->fetch()){
            ...
            $qdt=$owlPDO2->query($sdt) or die(print " Gagal: ".PDOException::getMessage());
   ...
   //exit("error:".$scek2);
   $qcek2=$owlPDO2->query($scek2) or die(print " Gagal: ".PDOException::getMessage());
   ...
   $rcek2=$qcek2->fetch();
    ...
    $qdt->setFetchMode(PDO::FETCH_ASSOC);
    $rdt=$qdt->fetch();
    $qdata=$owlPDO->query($sdt) or die(print " Gagal: ".PDOException::getMessage());
    $qdata->setFetchMode(PDO::FETCH_ASSOC);
    $rowdata=owlBaris($qdata);
...
//    exit("error:".$slisihhari);
    ...
    $qdtip=$owlPDO->query($sdtip) or die(print " Gagal: ".PDOException::getMessage());
    $qdtip->setFetchMode(PDO::FETCH_ASSOC);
    $rdtip=$qdtip->fetch();
                $dbserver2=$rdtip['ip'];
                $dbport2  =$rdtip['port'];
               ...
               //print_r($rdtip);
                try {
                  $owlPDO2 = new PDO('mysql:host='.$dbserver2.';dbname='.$dbname2, $uname2, $passwd2, array(PDO::ATTR_PERSISTENT => true));
                  $owlPDO2->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
                }
                catch (PDOException $e) {
                       print " Gagal, could not connect\n"; 
                ...
                $qed=$owlPDO2->query($sed) or die(print " Gagal: ".PDOException::getMessage());
                $qed->setFetchMode(PDO::FETCH_ASSOC);
                $red=owlBaris($qed);
                if($red==0){
	...
	global $posting;
		...
		global $test;
			...
			//ambil nama pt
			   ...
			   $res1=$owlPDO2->query($str1) or die(print " Gagal: ".PDOException::getMessage());
