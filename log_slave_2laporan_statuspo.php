...
## GET NOPO
$str="select distinct nopo,tanggal,lokalpusat from ".$dbname.".log_po_vw where nopo!='' ".$whr." order by nopo asc";
$res=fetchdata($str);
foreach($res as $val){
    $data[] = $val['nopo'];
    $lstTgl[$val['nopo']] = $val['tanggal'];
    $lstStat[$val['nopo']] = $val['lokalpusat'];
}

$inarrpo = implode("','", $data);
## CEK DARI TRANSAKSI DT
$str="select a.notransaksi, b.nopo, a.nopp,a.kodebarang ,a.jumlah from ".$dbname.".log_transaksidt a left join ".$dbname.".log_transaksiht b on a.notransaksi=b.notransaksi where a.nopo in ('".$inarrpo."')";
$res=fetchdata($str);
foreach($res as $val){
	$lstJmlh[$val['nopo']][$val['kodebarang']] += $val['jumlah'];
    $lstNotrans[$val['nopo']][$val['kodebarang']] = $val['notransaksi'];
}

## CEK DARI BA SERVICE
$str="select a.nopo,a.kodebarang, a.jumlahpesan as jumlah, b.noba as notransaksi from ".$dbname.".log_podt a left join ".$dbname.". log_baservis b on a.nopo=b.noso where a.nopo in ('".$inarrpo."') and b.posting='1'";
$res=fetchdata($str);
foreach($res as $val){
	$lstJmlh[$val['nopo']][$val['kodebarang']] = $val['jumlah'];
    $lstNotrans[$val['nopo']][$val['kodebarang']] = $val['notransaksi'];
...
$str="select a.nopo,a.kodebarang, a.notransaksi, a.jumlah from ".$dbname.".log_penerimaanpodt a left join ".$dbname.".log_penerimaanpoht b on a.notransaksi=b.notransaksi where a.nopo in ('".$inarrpo."')";
$res=fetchdata($str);
foreach($res as $val){
	$lstJmlh[$val['nopo']][$val['kodebarang']] = $val['jumlah'];
    $lstNotrans[$val['nopo']][$val['kodebarang']] = $val['notransaksi'];
...
## CEK DARI NON-INVENTORY
$str="select nopo,kodebarang,jumlah,notransaksi from ".$dbname.".log_noninventorydt_vw where nopo in ('".$inarrpo."')";
$res=fetchdata($str);
foreach($res as $val){
	$lstJmlh[$val['nopo']][$val['kodebarang']] = $val['jumlah'];
    $lstNotrans[$val['nopo']][$val['kodebarang']] = $val['notransaksi'];
...
$tab.="<tr>";
$tab.="<th align=center " . $bgcoloraja . ">No.</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['nopo'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['tanggal'] . " PO</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['status'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['kodebarang'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['namabarang'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['jumlah'] . " PO</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['satuan'] . " </th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['bapb'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['jumlah'] . " " . $_SESSION['lang']['diterima'] . "</th>";
$tab.="<th align=center " . $bgcoloraja . ">" . $_SESSION['lang']['sisa'] . "</th>";
$tab.="</tr></thead><tbody>";
...
if ($brsdt != 0) {
    ...
    foreach ($data as $dataNopo) {
		...
		$qdtPo=$owlPDO->query($sdtPo) or die(print " Gagal: ".PDOException::getMessage());
		$qdtPo->setFetchMode(PDO::FETCH_ASSOC);
        while ($rdtPo = $qdtPo->fetch()) {
            ...
            if (!isset($lstNotrans[$rdtPo['nopo']][$rdtPo['kodebarang']]))
                $lstNotrans[$rdtPo['nopo']][$rdtPo['kodebarang']] = '';
            if (!isset($lstJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']]))
                $lstJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']] = 0;
            if (!isset($sisaJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']]))
                $sisaJmlh[$rdtPo['nopo']][$rdtPo['kodebarang']] = 0;
