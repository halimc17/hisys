...
$kodeline = checkPostGet('kodeline', '');
$width = checkPostGet('width', '');
...
{
	case'insert':
		$str="INSERT INTO ".$dbname.".`bi_5warna` (`tipe`,`fill`,`line`,`width`,`updateby`)
			VALUES ('".$tipe."','".$kodefill."','".$kodeline."', '".$width."','".$_SESSION['standard']['userid']."')";
		try{
			$owlPDO->exec($str);
	...
	case'delete':
		...
		$str="delete from ".$dbname.".`bi_5warna` where tipe='".$tipe."' ";
		try{
			$owlPDO->exec($str);
		}catch (PDOException $e){
			echo "DB Error : ".$e->getMessage();
	...
	case'update':
		$str="update ".$dbname.".bi_5warna set fill='".$kodefill."',line='".$kodeline."',width='".$width."',
				updateby='".$_SESSION['standard']['userid']."' where tipe='".$tipe."' ";
		try{
			$owlPDO->exec($str);
		}catch (PDOException $e){
			echo "DB Error : ".$e->getMessage();
	...
	case'loaddata':
        ...
        $ql2="select count(*) as jmlhrow from ".$dbname.".bi_5warna";
        ...
        $no=0;
        $str="SELECT * from  ".$dbname.".bi_5warna where 1=1 ".$where." limit ".$offset.",".$limit."";
        $tab="";
        ...
        while($bar=$res->fetch())
        {
			$optNamaTipe = makeOption($dbname,'bi_5tipepeta','id_tipepeta,keterangan',"id_tipepeta='".$bar['tipe']."'");
            $no+=1;
            $tab.="<tr class=rowcontent>";
            $tab.="<td align=center>".$no."</td>";
			$tab.="<td align=left>".$bar['tipe']." - ".$optNamaTipe[$bar['tipe']]."</td>";
            $tab.="<td align=left>".$bar['fill']."</td>";
            $tab.="<td align=left>".$bar['line']."</td>";
            $tab.="<td align=right bgcolor=".$bar['fill']."></td>";
			$tab.="<td align=right bgcolor=".$bar['line']."></td>";
			 $tab.="<td align=left>".$bar['width']."</td>";
			$tab.="
			<td align=center>
				<img src=images/application/application_edit.png class=resicon title='Edit' 
					 onclick=\"edit('".$bar['tipe']."','".$bar['fill']."','".$bar['line']."','".$bar['width']."');\">
				<img src=images/application/application_delete.png class=resicon title='Delete' 
					 onclick=\"del('".$bar['tipe']."');\">
