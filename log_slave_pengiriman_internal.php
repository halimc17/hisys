...
$arrd=array("1"=>$_SESSION['lang']['expeditor'],"2"=>$_SESSION['lang']['internal']);
	switch($method)
	{
                ...
                $sDtPt="select distinct a.*,b.tanggal,b.tipetransaksi,sum(jumlah-jumlahlalu) as jmlhdata,nopo from ".$dbname.".log_lpbdt a  left join ".$dbname.".log_lpbht b
                 ...
                 //exit("Error:".$sDtPt);
                $tempNotrans='';
                  $qDtPt=$owlPDO->query($sDtPt) or die(print " Gagal: ".PDOException::getMessage());
                  $qDtPt->setFetchMode(PDO::FETCH_ASSOC);

                while($rDtPt=$qDtPt->fetch())
                {
                    $no++;
		...
		<button name=cancel id=cancel onclick=normalView()>". $_SESSION['lang']['cancel']."</button></td></tr>";
                ...
                case'createTable':
                    ...
                    else
                    {
                        $sHeader="select distinct * from ".$dbname.".log_pengiriman_ht where nosj='".$nosj."'";
                          $qHeader=$owlPDO->query($sHeader) or die(print " Gagal: ".PDOException::getMessage());
                          $qHeader->setFetchMode(PDO::FETCH_ASSOC);

                        $rHeader=$qHeader->fetch();
                ...
                if(substr($supplierd,0,1)=='S'){
                    ...
                    $sKary2="select  karyawanid,namakaryawan from ".$dbname.".datakaryawan 
                             where tipekaryawan in ('0','1','2') and sistemgaji='Bulanan' and statuskaryawan != 'Keluar' and 
                             tanggalkeluar='0000-00-00' and karyawanid!='".$_SESSION['standard']['userid']."' order by namakaryawan asc";
                      $qKary2=$owlPDO->query($sKary2) or die(print " Gagal: ".PDOException::getMessage());
                      $qKary2->setFetchMode(PDO::FETCH_ASSOC);

                    while($rKary2=$qKary2->fetch())
                    {
                        if($dert!=''){
                ...
                else
                {
                    ...
                    while($rData=$qData->fetch())
                    {
                        ...
                        $qJmlhTotal=$owlPDO->query($sJmlhTotal) or die(print " Gagal: ".PDOException::getMessage());
                        $qJmlhTotal->setFetchMode(PDO::FETCH_ASSOC);

                        $rJmlhTotal=$qJmlhTotal->fetch();
		...
		case'delData':
                ...
                $qUpdate=$owlPDO->query($sUpdate) or die(print " Gagal: ".PDOException::getMessage());
                $qUpdate->setFetchMode(PDO::FETCH_ASSOC); 

                $rUpdate=$qUpdate->fetch();
		...
		break;
                case'getLokasi':
                    ...
                    $qLokTgs=$owlPDO->query($sLokTgs) or die(print " Gagal: ".PDOException::getMessage());
                    $qLokTgs->setFetchMode(PDO::FETCH_ASSOC); 

                    $rLokTgs=$qLokTgs->fetch();
