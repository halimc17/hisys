...
function preview() {
    ...
    param += '&tgl1=' + tgl1;
    param += '&tgl2=' + tgl2;
    tujuan = 'kebun_slave_2bkm_mobile.php';
...
function detailData(notransaksi,numRow,ev,tipe,jenis){
	...
	title="Data Detail";
	if(tipe=='BKM'){		
		alertify.popuppdf("Preview","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='kebun_slave_operasional_print_detailx_mobile.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('80%','70%');
	}else{
		alertify.popuppdf("Preview","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='kebun_slave_operasional_print_detail_panen_mobile.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('80%','70%');
	}
}

const mobiletoerp = (nobkm) => {
	param='method=mobiletoerp&nobkm='+nobkm;
