...
function updateUnitId() {
    ...
    document.getElementById('unitId4').value = selected.join(',');
}
</script>
...
OPEN_BOX('','<span class=judul>Detail Produksi TBS<br></span>');
...
echo"<tr><td>".$_SESSION['lang']['unit']."</td><td>:</td><td><select multiple class=select2 id=unitSelect onchange=updateUnitId() style=width:200px>".$optUniDt."</select><input type=hidden id=unitId4 /></td></tr>";
...
<button onclick=\"zPreview('kebun_slave_DetProduksiTbs','".$arr6."','printContainer2');showheader();\" class=\"mybutton\" name=\"preview\" id=\"preview\">".$_SESSION['lang']['preview']."</button>
<button onclick=\"zExcel(event,'kebun_slave_DetProduksiTbs.php','".$arr6."')\" class=\"mybutton\" name=\"excel2\" id=\"excel2\">".$_SESSION['lang']['excel']."</button>
