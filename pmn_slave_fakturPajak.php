...
case 'kodetimb':
	...
	$q_timb=$owlPDO->query($s_timb) or die(print " Gagal: ".PDOException::getMessage());
	$q_timb->setFetchMode(PDO::FETCH_ASSOC);
    $r_cust=$q_timb->fetch();
...
case 'loadkontrak':
	...
	$q_kontrak=$owlPDO->query($s_kontrak) or die(print " Gagal: ".PDOException::getMessage());
	$q_kontrak->setFetchMode(PDO::FETCH_ASSOC);
    while($r_kontrak=$q_kontrak->fetch())
    {
...
case 'loadcurr':
    ...
    $q_curr=$owlPDO->query($s_curr) or die(print " Gagal: ".PDOException::getMessage());
	$q_curr->setFetchMode(PDO::FETCH_ASSOC);
    $r_curr=$q_curr->fetch();
...
case 'loadfaktur':
	...
	$q_npwp=$owlPDO->query($s_npwp) or die(print " Gagal: ".PDOException::getMessage());
	$q_npwp->setFetchMode(PDO::FETCH_ASSOC);
    $r_npwp=$q_npwp->fetch();
    $npwp=$r_npwp['npwp'];
	...
	$q_data=$owlPDO->query($s_data) or die(print " Gagal: ".PDOException::getMessage());
	$q_data->setFetchMode(PDO::FETCH_ASSOC);
    $r_data=$q_data->fetch();
...
case 'loadvol':  
    ...
    if($biaya=='Uang Muka'){
		...
		$q_qtykontrak=$owlPDO->query($s_qtykontrak) or die(print " Gagal: ".PDOException::getMessage());
		$q_qtykontrak->setFetchMode(PDO::FETCH_ASSOC);
        $r_qtykontrak = $q_qtykontrak->fetch();
        ...
        if($timbangan=='Sendiri'){
			...
			$q_wb=$owlPDO->query($s_wb) or die(print " Gagal: ".PDOException::getMessage());
			$q_wb->setFetchMode(PDO::FETCH_ASSOC);
            $r_wb=$q_wb->fetch();
            $vol=$r_wb['total'];
        }
        else{
    ...
    if($curr!='IDR'){
		...
		$q_harga=$owlPDO->query($s_harga) or die(print " Gagal: ".PDOException::getMessage());
		$q_harga->setFetchMode(PDO::FETCH_ASSOC);
        $r_harga=$q_harga->fetch();
    ...
    if($biaya!='Uang Muka'){
		...
		$q_uangmuka=$owlPDO->query($s_uangmuka) or die(print " Gagal: ".PDOException::getMessage());
		$q_uangmuka->setFetchMode(PDO::FETCH_ASSOC);
        $r_uangmuka=$q_uangmuka->fetch();
...
case 'daftarfaktur':
   ...
   */     
	...
	$q_daftar=$owlPDO->query($s_daftar) or die(print " Gagal: ".PDOException::getMessage());
	$q_daftar->setFetchMode(PDO::FETCH_ASSOC);
    echo "<table cellspacing=1 border=0 class='sortable'>
              <thead>
                <tr class=rowcontent>
                    <td align=center>No.</td>
...
case 'posting':
    ...
    $q_cekpost=$owlPDO->query($s_cekpost) or die(print " Gagal: ".PDOException::getMessage());
	$r_cekpost=owlBaris($q_cekpost);
    if($r_cekpost>0)
    {
        $s_posting="update ".$dbname.".pmn_faktur set post='1' where nofaktur='".$nofaktur."'";
        try{
			$owlPDO->exec($s_posting);
		}catch (PDOException $e){
			echo "error : ".$e->getMessage();
		}
    }
...
case 'pdf':
	...
	$q_pdf=$owlPDO->query($s_pdf) or die(print " Gagal: ".PDOException::getMessage());
	$q_pdf->setFetchMode(PDO::FETCH_ASSOC);
    while($r_pdf=$q_pdf->fetch()){
        $no+=1;
