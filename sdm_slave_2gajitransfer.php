...
$sumberrek=checkPostGet('sumberrek','');
...
switch($method)
{
    ...
    ####################data  
       ...
       $sSlip="select distinct a.*,b.tipekaryawan,b.statuspajak,b.tanggalmasuk,b.nik,b.namakaryawan,b.bagian,c.namajabatan,b.norekeningbank,d.realkode from 
        ...
        if($rCek>0)
        {
            ...
            $sKomp="select distinct id,name from ".$dbname.".sdm_ho_component  a
               ...
               and b.kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$pt."')  and a.id in (".$gjthnlu.")
               and b.karyawanid like '%".$karyawan."%' order by a.id";
            $qKomp=$owlPDO->query($sKomp) or die(print " Gagal: ".PDOException::getMessage());
            $qKomp->setFetchMode(PDO::FETCH_ASSOC);
    ...
    {
        $amountplus+=$TarrJmlh[$idKompPls];
    }


    foreach($arrIdKompMin as $idKompMin){
        $amountmin+=$TarrJmlh[$idKompMin];
    }

    $ttlamount=$amountplus-$amountmin;


    $kodebanksumber = "";
    $str="select ifnull(b.realkode,'') as realkode from ".$dbname.".keu_5akunbank a
    left join ".$dbname.".keu_5daftarbank b on a.namabank = b.kodebank
    where a.noakun = '".$sumberrek."' limit 1";
    $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
    $res->setFetchMode(PDO::FETCH_ASSOC);
    $bar=$res->fetch(); 
    $kodebanksumber = $bar['realkode'];

    $optreklbu=  makeOption($dbname, 'sdm_rekgajitransfer', 'kodebank,kodelbu');
    $optrekrbu=  makeOption($dbname, 'sdm_rekgajitransfer', 'kodebank,koderbu');
    $optkombinasinama=  makeOption($dbname, 'sdm_rekgajitransfer', 'kodebank,kombinasinama');
    ...
    $stream.="<td align=center>P</td>";
    $stream.="<td>".$tanggal."</td>";
    $stream.="<td align=center>".$petik."".$sumberrek."</td>";
    $stream.="<td align=center>".$ttlbaris."</td>";
    $stream.="<td align=center>".ceil($ttlamount)."</td>";
    ...
    foreach($arrKary as $dtKary){
        if($arrNmKary[$dtKary]!=''){
            ...
            $stream.="<td align=right>".$arrNorek[$dtKary]."</td>";
            ...
            $stream.="<td align=right>".ceil($gajiBersih)."</td>";
            $stream.="<td></td>";#defaultnya kosong
            $stream.="<td></td>";#defaultnya kosong
            if ($kodebank[$dtKary]== $kodebanksumber) {
                ...
                if ($gajiBersih<100000000) {
                   ...
                   $stream.="<td>LBU</td>";
                   $stream.="<td align=right>".@$optreklbu[$kodebank[$dtKary]]."</td>";
               }
               else
               {
                  $stream.="<td>RBU</td>";
                  $stream.="<td align=right>".@$optrekrbu[$kodebank[$dtKary]]."</td>";
              }
          }


          $stream.="<td>".@$optkombinasinama[$kodebank[$dtKary]]."</td>";
            $stream.="<td></td>";#defaultnya kosong
            $stream.="<td></td>";#defaultnya kosong
            $stream.="<td></td>";#defaultnya kosong
            $stream.="<td></td>";#defaultnya kosong
  ...
  break;
    ...
    case 'getkar':
            ...
            $str="select noakun,namabank,rekening,pemilik from ".$dbname.".keu_5akunbank where pemilik in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$pt."') ";
            $res=fetchData($str);
            foreach ($res as $val) {
            $namabank=makeOption($dbname,'keu_5daftarbank','kodebank,namabank',"kodebank='".$val['namabank']."'");
                    $optrek.="<option value='".$val['noakun']."'>".$val['rekening']."-".$val['pemilik']."(". $namabank[$val['namabank']].")</option>";                  
