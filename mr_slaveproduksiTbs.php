...
$periodeDt = checkPostGet('periodeDt','');
...
elseif($kdPt!='')
{
	...
	$qKod=$owlPDO->query($sKod) or die(print " Gagal: ".PDOException::getMessage());
	$rTot=owlBaris($qKod);
	$qKod->setFetchMode(PDO::FETCH_ASSOC);
	while($rKod=$qKod->fetch())
	{	
        @$nord+=1;
...
if($proses!='getData')
{
        ...
        $sLuasMrh="select sum(luasareaproduktif) as luas,substr(kodeorg,1,6) as afd,tahuntanam 
                   from ".$dbname.".setup_blok where ".$whreblok." and statusblok in ('TM','TBM') group by substr(kodeorg,1,6),tahuntanam order by substr(kodeorg,1,6)";

		$qLuasMrh=$owlPDO->query($sLuasMrh) or die(print " Gagal: ".PDOException::getMessage());
		$qLuasMrh->setFetchMode(PDO::FETCH_ASSOC);
		while($rLuasMrh=$qLuasMrh->fetch()){	
        ...
        $sLuasBgt="select sum(hathnini) as luasbgt,thntnm,substr(kodeblok,1,6) as afd from ".$dbname.".bgt_blok where 
			...
			$qLuasBgt=$owlPDO->query($sLuasBgt) or die(print " Gagal: ".PDOException::getMessage());
			$qLuasBgt->setFetchMode(PDO::FETCH_ASSOC);
			while($rLuasBgt=$qLuasBgt->fetch()){
		...
		$qBrtBgt=$owlPDO->query($sBrtBgt) or die(print " Gagal: ".PDOException::getMessage());
		$qBrtBgt->setFetchMode(PDO::FETCH_ASSOC);
		while($rBrtBgt=$qBrtBgt->fetch()){	
		...
		$qBrtBgt2=$owlPDO->query($sBrtBgt2) or die(print " Gagal: ".PDOException::getMessage());
		$qBrtBgt2->setFetchMode(PDO::FETCH_ASSOC);
		while($rBrtBgt2=$qBrtBgt2->fetch()){		
		...
		$qAktualBln=$owlPDO->query($sAktualBln) or die(print " Gagal: ".PDOException::getMessage());
		$qAktualBln->setFetchMode(PDO::FETCH_ASSOC);
		while($rAktualBln=$qAktualBln->fetch())		
        {
