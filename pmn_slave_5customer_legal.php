...
$tab.="<table class='sortable' cellspacing='1' border='0' width=100%>
    ...
    $str="select namafilelegal, kodecustomer from ".$dbname.".pmn_4customer where kodecustomer = '".$kodecustomer."'";
    $res=fetchData($str);
    if(empty($res))
    {
      ...
      foreach($res as $key=>$val)
      {
        ...
        if ($val['namafilelegal']!='')
        {
          ...
          $tab.="<tr id='ppDetailTable' class=rowcontent>
            ...
            <td style='text-align:left'>".$val['namafilelegal']."</td>
            <td align=center>
              <a href='fileupload/customer/".$val['namafilelegal']."' download><img src=images/uploader/dwnld8.png class=resicon  title='download'></a>
            </td>
          </tr>";
        }
      }
