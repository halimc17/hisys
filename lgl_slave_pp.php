...
// $tujuan = replaceEnter(checkPostGet('tujuan', ''));
...
switch ($method) {
    case'html':
		...
		$tab.= "<fieldset style=min-width:900px><legend><img src=images/excel.jpg class=resicon class=zImgBtn height='30'  title='View HTML' onclick=\"previewexcel('" . $bar['notransaksi'] . "','" . $bar['kodeorg'] . "','" . tanggalnormal($bar['tanggal']) . "','excel');\" >
				</legend>";
		...
		$tab.= "<tr class=rowcontent>
				<td>".$nmkar[$bar['updateby']]."<br>
					".$bar['updatetime']."</td>";
			for($i=1;$i<=$countApprove;$i++){
				@$arrApp = detailApprove($i,$notransaksi,$jenisApp);

				$strap="select tipe from ".$dbname.".setup_approval where jenispersetujuan='".$jenisApp."' and kodeunit='".$bar['kodeorg']."' and level='".$i."'";
				...
				if(($arrApp['karyawanid']!='')&&($arrApp['karyawanid']!=0)){
					if ($arrApp['status']==0 && $barap['tipe']==1){
						$arrApp['nama']='Direksi';
					}
					$tab.= "<td>".$arrApp['nama']."
	...
	case'getnotransaksi':
		...
		$kdpt=makeOption($dbname,'organisasi','kodeorganisasi,induk',"kodeorganisasi='".$kodeorg."'");
		$tempnotran = "/PP/".$kdpt[$kodeorg]."/".$kodeorg."/".$tempPrd[1]."/".$tempPrd[0];
		
		$str=" select max(left(notransaksi,3)) as notransaksi from ".$dbname.".lgl_bansos where notransaksi like '%".$tempnotran."' and jenis='".$jenistrk."' order by notransaksi desc limit 1";
		$res=fetchdata($str);
		if(intval($res[0]['notransaksi'])==0){
			$noawal=1;
		}else{
			$noawal = intval($res[0]['notransaksi'])+1;
		}
		...
		$notranbaru=addZero($noawal,3)."".$tempnotran;
    ...
    case'insert':
        #cek data
		if(strlen($notransaksi)!=23){
			exit("Warning : Nomor Transaksi Salah !!!");
		}
        ...
        $str = "insert into " . $dbname . ".lgl_bansos (`notransaksi`,`kodeorg`,`tanggal`,`kategori`,`namapemesan`,`lokasipemesan`,`tujuan`,`deskripsi`,`rupiah`,`statuspersetujuan`,`createby`,`createtime`,`updateby`,`updatetime`,`rekening`,`tipebayar`,`atasnama`,`jenis`)
        values ('".$notransaksi."','".$kodeorg."','".$tanggal."','".$kategori."','".$namapemesan."','".$lokasipemesan."','".$tujuan."','".$deskripsi."','".$rupiah."','0','" . $_SESSION['standard']['userid'] . "','".$todayhis."','" . $_SESSION['standard']['userid'] . "','".$todayhis."','".$rekening."','".$tipebayar."','".$atasnama."','".$jenistrk."')";
    ...
    case'deletedetail':
        ...
        $str = "delete from " . $dbname . ".lgl_bansos where notransaksi='" . $notransaksi . "' and kodeorg ='".$kodeorg."' and kategori='".$kategori."' and tujuan='" . $tujuan . "' and deskripsi='".$deskripsi."' and tanggal='".$tanggal."'";
        try {
            $owlPDO->exec($str);
        } catch (PDOException $e) {
            print " Gagal  !: " . $e->getMessage() . "\n";
	...
	case'form_ajukan';
		...
		$optKry = "";
		$listApprove = listApprove('1',$jenisApp,$kodeorg);
		...
		if($row==0){
			...
			while ($bar = $res->fetch()) {
				...
				$tab.="<img src=images/application/application_delete.png class=resicon  title='Delete' 
					onclick=\"deletedetail('" . $bar['notransaksi'] . "','" . $bar['kodeorg'] . "','" . $bar['kategori'] . "','" . $bar['tujuan'] . "','" . $bar['deskripsi'] . "','" . tanggalnormal($bar['tanggal']) . "');\" >
				</td>";
				@$trupiah+=$bar['rupiah'];
			}
        $tab.="</tr>";
    ...
    while($barr=$res->fetch())
    {
        ...
        $sql = "select count(*) as jmlhrow from " . $dbname . ".lgl_bansos a left join ".$dbname.".keu_kasbankdt b on a.notransaksi=b.nodok where jenis='".$jenistrk."' " . $where . " group by a.notransaksi, a.kodeorg";
        #exit('error '.$sql);
        $res = $owlPDO->query($sql) or die(print " Gagal: " . PDOException::getMessage());
        $jlhbrs = owlBaris($res);
        $no = 0;
        ...
        $str = "SELECT sum(a.rupiah) as biaya,a.notransaksi, a.kodeorg, a.tanggal, a.updateby, a.posting,a.kategori,a.namapemesan,a.lokasipemesan,a.tujuan,a.rekening,a.tipebayar,a.atasnama,b.nodok,b.notransaksi as kas, a.statuspersetujuan,a.posting FROM " . $dbname . ".lgl_bansos  a left join ".$dbname.".keu_kasbankdt b on a.notransaksi=b.nodok 
		where a.jenis='".$jenistrk."'  " . $where . " group by a.notransaksi, a.kodeorg order by a.createtime desc, a.kodeorg asc, a.notransaksi desc limit " . $offset . "," . $limit . "";
        ...
        while ($bar = $res->fetch()) {
			...
			$status='';
			if($bar['posting']=='0' and $bar['statuspersetujuan']=='0'){
				$status="Perlu Persetujuan";
			}elseif($bar['posting']=='1' and $bar['statuspersetujuan']=='0'){
				$status="Dalam Proses Persetujuan";
			}elseif($bar['posting']=='1' and $bar['statuspersetujuan']=='1'){
				$status="Disetujui";
			}elseif($bar['posting']=='1' and $bar['statuspersetujuan']=='2'){
				$status="Dikoreksi";
			}elseif($bar['posting']=='1' and $bar['statuspersetujuan']=='3'){
				$status="Ditolak";
			}
			...
			for($i=1;$i<=$countApp;$i++){
				...
				if($tgl[$bar['notransaksi']][$i]==''||$tgl[$bar['notransaksi']][$i]=='0000-00-00 00:00:00'){
            		$tngl='';
            	}else{
            		$tngl=tanggalnormal($tgl[$bar['notransaksi']][$i]);
            	}
				
				$tab.="<td style='text-align:center'>".$arrDetail['nama']."
            	<br>".$arrHsl[$stat[$bar['notransaksi']][$i]]."
            	<br>".$komentar[$bar['notransaksi']][$i]."
            	<br>".$tngl."
            	</td>";
				// $tab.="<td align=left>".$arrDetail['nama']." ".(($arrDetail['status']=='9'||$arrDetail['status']=='')?"":"(".$arrDetail['namastatus'].")")."</td>";
			}

            // $level=$arrayName = array('1' => '1' , '2' => '2', '3' => '3');
            // foreach ($level as $keylevel => $vallevel) {

            	// if($tgl[$bar['notransaksi']][$vallevel]==''||$tgl[$bar['notransaksi']][$vallevel]=='0000-00-00 00:00:00'){
            		// $tngl='';
            	// }else{
            		// $tngl=tanggalnormal($tgl[$bar['notransaksi']][$vallevel]);
            	// }

            	// $tab.="<td>".$nma[$bar['notransaksi']][$vallevel]."
            	// <br>".$arrHsl[$stat[$bar['notransaksi']][$vallevel]]."
            	// <br>".$komentar[$bar['notransaksi']][$vallevel]."
            	// <br>".$tngl."
            	// </td>";
            ...
            if ($bar['posting'] == 0) {
				...
				onclick=\"edit('" . $bar['kodeorg'] . "','" . tanggalnormal($bar['tanggal']) . "','" . $bar['notransaksi'] . "','" . $bar['kategori'] . "','" . $bar['namapemesan'] . "','" . $bar['lokasipemesan'] . "','".str_replace('####','\n',$bar['tujuan'])."','".$bar['rekening']."','".$bar['tipebayar']."','".$bar['atasnama']."');\" ></td>";
                ...
                $strjns="select tipe from ".$dbname.".setup_approval where jenispersetujuan='".$jenisApp."' and level='1' and kodeunit='".$bar['kodeorg']."'";
				$resjns=$owlPDO->query($strjns) or die(print " Gagal: ".PDOException::getMessage());
				$resjns->setFetchMode(PDO::FETCH_ASSOC);
				$barjns=$resjns->fetch();
				if ($barjns['tipe']==1) {
					$isi.="<td align=center><img src=images/skyblue/submit.jpg class=resicon class=zImgBtn height='30'  title='Ajukan ???' 
                    onclick=\"ajukandireksi('".$bar['notransaksi']."','" . $bar['kodeorg'] . "');\" ></td>";
				}else{
					$isi.="<td align=center><img src=images/skyblue/submit.jpg class=resicon class=zImgBtn height='30'  title='Ajukan ???' 
	...
	case 'submitfile':
		...
		$res=fetchData($sql);
		if(count($res)==0){
			exit('Warning : Silahkan isikan dan save detail Pengajuan Pembayaran terlebih dahulu !');
		}
			...
			if($_FILES['file']['error']==0){	
				...
				if(($filetype=='.jpeg')||($filetype=='.jpg')||($filetype=='.png')||($filetype=='.pdf')||($filetype=='.xls')||($filetype=='.xlsx')||($filetype=='.doc')||($filetype=='.docx')){
					if($_FILES['file']['size'] <= 1000000){
						...
						exit("warning : Ukuran file upload maksimal 1Mb");
					}
				}else{
	...
	$strkegiatan="select a.*,b.namakaryawan, b.karyawanid from ".$dbname.".approval a left join ".$dbname.".datakaryawan b on b.karyawanid=a.karyawanid where a.notransaksi='".$notransaksi."'";
	$res=fetchdata($strkegiatan);
	foreach($res as $barr){
		$nma[$barr['notransaksi']][$barr['level']]=$barr['namakaryawan'];
		...
		$tab.="<tr><td align=center><font size=5><b><u>FORM PENGAJUAN PEMBAYARAN</u></b></font></td></tr>";
		$tab.="<tr><td>&nbsp;</td></tr>";
		$tab.="<tr><td>&nbsp;</td></tr>";
		...
		$tab.="<tr><td align=left>Kategori Pengajuan Pembayaran : <b>".($barx['kategori'])."</b></td></tr>";
		$tab.="<tr><td>";
		...
		if($karyid[$barx['notransaksi']]['2']==''){
			...
			$tab.="<tr>
						<td align=center colspan=".$cols1.">Disetujui Oleh,</td>
						<td ".$hide1."></td>
						<td ".$hide." ></td>
			...
			$tab.="<tr><td colspan=5>&nbsp;</td></tr>";
			$tab.="<tr>
			<td align=center><u>".$nma[$barx['notransaksi']]['1']."</u></td>
			<td align=center><u>".$nma[$barx['notransaksi']]['2']."</u></td>
			<td align=center><u>".$nma[$barx['notransaksi']]['3']."</u></td>
			<td align=center><u>".getNamaKaryawan($barx['updateby'])."</u>
			</td>
			</tr>";
			$tab.="<tr>
			<td align=center>".$tglarr[$barx['notransaksi']]['1']."</td>
			<td align=center>".$tglarr[$barx['notransaksi']]['2']."</td>
			<td align=center>".$tglarr[$barx['notransaksi']]['3']."</td>
		...
		## Print Out
		if($urlefil=='0'){
			$dompdf->stream("PengajuanPembayaran",array("Attachment"=>0));
		}else{
