...
OPEN_BOX('', '<span class=judul>'.getMenu('kebun_2rkbvsrealisasi').'</span><br>');
?>
...
for($x=0;$x<13;$x++){
	$dt=mktime(0,0,0,date('m')-$x+2,12,date('Y'));
	$optper.="<option value=".date("Y-m",$dt).">".date("m-Y",$dt)."</option>";
}
$arr = "##kdUnit##periode##divisi";
        ...
        <legend>Form</legend>
            <table border=0 cellpadding=1 cellspacing=1>
                ...
                <tr>
                    <td colspan=2></td>
                    <td colspan=4>
                    <button onclick=zPreview('kebun_slave_2rkbvsrealisasi','" . $arr . "','printContainer') class=mybutton name=preview id=preview>" . $_SESSION['lang']['preview'] . "</button>
                    <button onclick=zExcel(event,'kebun_slave_2rkbvsrealisasi.php','" . $arr . "') class=mybutton name=preview id=preview>" . $_SESSION['lang']['excel'] . "</button>
                    </td>
                </tr>
            </table>
</fieldset>";
...
<div id='printContainer' style='overflow:auto;height:420px;max-width:100%'; >
</div></div>";
CLOSE_BOX();
echo close_body();
?>