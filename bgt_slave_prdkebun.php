...
switch ($method) {
	case 'del':
		try {
			...
			$str = "delete from " . $dbname . ".bgt_produksi_kebun  where tahunbudget='" . $param['tahun'] . "' and kodeunit='" . $param['kodeorg'] . "' " . $wh . "";
			$owlPDO->exec($str);
	...
	case 'posting':
		try {
			$owlPDO->beginTransaction();
			$str = "select * from " . $dbname . ".bgt_produksi_kebun where tahunbudget = '" . $param['tahun'] . "' and kodeunit='" . $param['kodeorg'] . "'";
			$res = fetchdata($str);
			foreach ($res as $bar) {
			...
			$str = "update " . $dbname . ".bgt_produksi_kebun set tutup='1' where tahunbudget = '" . $param['tahun'] . "' and kodeunit='" . $param['kodeorg'] . "'"; #exit("error".$str);
			$owlPDO->exec($str);
	...
	case 'unposting':
		try {
			...
			$str = "update " . $dbname . ".bgt_produksi_kebun set tutup='0' where tahunbudget = '" . $param['tahun'] . "' and kodeunit='" . $param['kodeorg'] . "'"; #exit("error".$str);
			$owlPDO->exec($str);
	...
	case 'simpan':
		try {
			...
			$str = "select * from " . $dbname . ".bgt_produksi_kebun where tahunbudget='" . $param['tahun'] . "' and kodeunit='" . $param['kodeorg'] . "' and tutup='1'";
			$res = fetchdata($str);
			if (count($res) > 0) {
			...
			$str = "select * from " . $dbname . ".bgt_produksi_kebun where tahunbudget='" . $param['tahun'] . "' and kodeblok='" . $param['blok'] . "'";
			$res = fetchdata($str);
			if (count($res) > 0) {
				$str = "delete from " . $dbname . ".bgt_produksi_kebun  where tahunbudget='" . $param['tahun'] . "' and kodeblok='" . $param['blok'] . "'";
				$owlPDO->exec($str);
			}
			...
			if ($param['ttljjg'] > 0 and $param['ttlkg'] > 0) {
				$str = "insert into " . $dbname . ".bgt_produksi_kebun (`tahunbudget`, `kodeunit`, `kodeblok`, `tahuntanam`, `intiplasma`, `jenisbudget`, `jjgperpkk`, `totaljjg`, `totalkg`, `totalkgbruto`, `updateby`";
				for ($i = 1; $i <= 12; $i++) {
					$str .= ",`jjg" . addZero($i, 2) . "`";
					$str .= ",`kg" . addZero($i, 2) . "`";
	...
	case 'adddata':
		...
		$str = "select * from " . $dbname . ".bgt_produksi_kebun where 1=1 " . $where . " and tutup='1'";
		$res = fetchdata($str);
		if (count($res) > 0) {
		...
		if ($param['sumber'] == 'upload') {
			...
			$str = "select * from " . $dbname . ".bgt_produksi_kebun where 1=1 " . $where . " order by substr(kodeblok,1,6) asc,tahuntanam asc,kodeblok asc";
			$res = fetchdata($str);
			foreach ($res as $bar) {
				#$dtblok[$bar['kodeblok']]=$bar['kodeblok'];
		...
		if (count($dtblok) > 0) {
			...
			foreach ($dtblok as $blok) {
				...
				$tab .= "<td " . $rowspan . " align=center >" . ($nmorg[$blok] ?? substr($blok, 6, 4)) . "</td>";
	...
	case 'showposting':
		...
		$bulan = range(1, 12);
		$dtunit = array();
		$str = "select * from " . $dbname . ".bgt_produksi_kebun where 1=1 " . $where . " and substr(kodeunit,1,4) in (" . getOrgDetail(2) . ") order by tahunbudget desc,kodeunit asc";
		$res = fetchdata($str);
		foreach ($res as $bar) {
	...
	case 'loaddata':
		...
		$sql = "select count(*) as jmlhrow from " . $dbname . ".bgt_produksi_kebun where substr(kodeunit,1,4) in (" . getOrgDetail(2) . ") " . $where . "";
		$res = fetchdata($sql);
		...
		$str = "select * from " . $dbname . ".bgt_produksi_kebun where substr(kodeunit,1,4) in (" . getOrgDetail(2) . ") " . $where . " order by tahunbudget desc,kodeunit asc,tahuntanam asc " . $lmt . "";
		$res = fetchdata($str);
		if (count($res) > 10000) {
		...
		if (count($res) > 0) {
			foreach ($res as $key => $val) {
				...
				$tab .= "<td " . $rowspan . " style='text-align:center;vertical-align:top;'>" . ($nmorg[$val['kodeblok']] ?? substr($val['kodeblok'], 6, 4)) . "</td>";
		...
		if ($_FILES['file']['error'] == 0) {
			...
			if ($filetype == '.xlsx') {
				...
				if (count($tahunlist) != 1) {
					$validasiht .= "Tahun Budget tidak boleh lebih dari satu.<br>";
					$err++;
				}
				...
				foreach ($sheets as $noitem => $sheet) {
					if ($noitem == 1) {
						...
						if ($uploadtahunbudget == '') {
							$validasi .= "Tahun Kosong.<br>";
							$err++;
						}
						if (strlen($uploadtahunbudget) != 4) {
							$validasi .= "Panjang Tahun Budget tidak sesuai.<br>";
							$err++;
						}
						if ($uploadkodeunit == '') {
							$validasi .= "Kode Org	anisasi tidak boleh Kosong.<br>";
							$err++;
						}
						if (strlen($uploadkodeunit) != 4) {
