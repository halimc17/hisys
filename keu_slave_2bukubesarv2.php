...
$halaman= checkPostGet('halaman','0');
...
$periode2_first = date("Y-m-01",strtotime("+1 month", strtotime($periode2."-01")));
$strQuery= " FROM ".$dbname.".keu_jurnaldt_vw WHERE tanggal >= '".$periode."-01' and tanggal < '".$periode2_first."' and noakun >= '".$akundari."' and noakun <=  '".$akunsampai."' and noakun!='".$clm."' and jumlah!=0 ".
$wheregudang."";
$res=$owlPDO->query("SELECT count(*) totalrows ".$strQuery) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
$datarows = $res->fetch();
$jmldata = $datarows->totalrows;
$limit = 500;
$jmlhalaman = ceil($jmldata/$limit);
$offset = $halaman*$jmlhalaman;
$str = "SELECT * ".$strQuery." order by noakun,tanggal LIMIT ".$offset.",".$limit;
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()) {
