<?php
require_once('master_validation.php');
require_once('lib/zLib.php');
include_once('lib/HtmlExcel.php');

$method   = checkPostGet('method', '');
$param = $_POST;if(count($param)==0){$param = $_GET;}

$arrstatus=array('1'=>'AKTIF','0'=>'NON AKTIF');
$arrkomponen=makeOption($dbname, 'sdm_ho_component', 'id,name');
$arrkomponen2=makeOption($dbname, 'sdm_ho_component', 'id,name','','','',true);
//$arrgolongan=makeOption($dbname, 'sdm_5golongan', 'kodegolongan,namagolongan','aktif=1');

// $strx="select kodegolongan,namagolongan from ".$dbname.".sdm_5golongan where aktif=1 order by cast(kodegolongan as int) asc";
// $res=fetchdata($strx);
// $arrgolongan2='';
// foreach ($res as $bar) {
// 	$arrgolongan2.="<option value='".$bar['kodegolongan']."'>".$bar['namagolongan']."</option>";
// }
//$arrgolongan2=makeOption($dbname, 'sdm_5golongan', 'kodegolongan,namagolongan','aktif=1','','',true);

switch($method){
	case 'loaddata':
		$tab.="<table id=mytable class='sortable'  cellspacing='1' cellpadding='5' border='0' width=100%>
		<thead>
			<tr class=rowheader>
				<th align=center>".$_SESSION['lang']['nourut']."</th>
				<th align=center>".$_SESSION['lang']['kodept']."</th>
				<th align=center>".$_SESSION['lang']['namakomponen']."</th>
				<th align=center>".$_SESSION['lang']['status']."</th>
				<th align=center>".$_SESSION['lang']['updateby']."</th>
				<th align=center>".$_SESSION['lang']['updatetime']."</th>
				<th align=center>".$_SESSION['lang']['action']."</th>
			</tr>
		</thead>
		<tbody>";
		
		$no=0;
		$str= "select * from ".$dbname.".sdm_5komponenlemburpengali order by kodeorg asc, status desc, komponengaji asc";
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
			$tab.="<tr class=rowcontent>";
			$tab.="<td style='text-align:center;'>".$no."</td>";
			$tab.="<td style='text-align:left;'>".getNamaOrg($val['kodeorg'])."</td>";
			$tab.="<td style='text-align:center;'>".$arrkomponen[$val['komponengaji']]."</td>";
			$tab.="<td style='text-align:center;'>".$arrstatus[$val['status']]."</td>";
			$tab.="<td style='text-align:center;'>".getNamaKaryawan($val['updateby'])."</td>";
			$tab.="<td style='text-align:center;'>".tanggalnormald($val['updatetime'])."</td>";
			$tab.="<td style='text-align:center;width:25px'>
				<img src='images/application/application_edit.png' class='resicon' title='Edit' onclick=\"editdata('edit','".$val['kodeorg']."','".$val['komponengaji']."','".$val['status']."')\";>
			</td>";
			$tab.="</tr>";
		}
		
		$tab.="</tbody>
		<tfoot>
		</tfoot>
		</table>";
		echo $tab;
	break;
	
	case 'addnew':
		$tab="";
		foreach($arrstatus as $key => $val){
			if($key=='1'){
				@$optstatus.="<option value=".$key." selected>".$val."</option>";							
			}else{
				@$optstatus.="<option value=".$key.">".$val."</option>";							
			}
		}
		
		
		## GET Kode PT
		$arrorgdet = getOrgDetail(3);
		foreach($arrorgdet as $key=>$val){
			$optpt.="<option value='".$key."'>".$key." - ".$val."</option>";	
		}
		$tab.="
			 <table border=0 cellpadding=3 cellspacing=1>
				<tr>
					<td>".$_SESSION['lang']['unit']."</td>
					<td>:</td>
					<td>
						<select class='select2' id='kodeorg' >".$optpt."</select>
					</td>
				</tr>
				<tr>
					<td>".$_SESSION['lang']['namakomponen']."</td>
					<td>:</td>
					<td>
						<select class='select2' id='komponen'>".$arrkomponen2."</select>
					</td>
				</tr>
				<tr>
					<td>".$_SESSION['lang']['status']."</td>
					<td>:</td>
					<td>
						<select class='select2' id=status >".$optstatus."</select>
					</td>
				</tr>
                <tr>
                    <td><input type=hidden id=method value=insert></td>
                    <td colspan=4>
						<button onclick=simpan(); style='width:150px;height:30px' class=mybutton>Save</button>
                    </td>
                </tr>
            </table>";
		
		echo $tab;
	break;
	
	case 'insert':
		try{
			$owlPDO->beginTransaction();
			
			## VALIDATE
			$str="select count(kodeorg) as jlhitem from ".$dbname.".sdm_5komponenlemburpengali where kodeorg='".$param['kodeorg']."' and komponengaji='".$param['komponen']."' ";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
				throw new PDOException("Komponen Gaji sudah pernah terdaftar di PT ini.");
			}
			
			$data = array(
				'kodeorg'		=> $param['kodeorg'],
				'komponengaji' 	=> $param['komponen'],
				'status' 		=> $param['status'],
				'updateby' 		=> $_SESSION['standard']['userid'],
				'updatetime'	=> date('Y-m-d H:i:s')
			);
			
			$cols = array();
			foreach($data as $key=>$row) {
				$cols[] = $key;
			}

			$str = insertQuery($dbname,'sdm_5komponenlemburpengali',$data,$cols);
			$owlPDO->exec($str);
		
			$owlPDO->commit();
		}catch (PDOException $e) {$owlPDO->rollback(); echo "warning, " . addslashes($e->getMessage()); die();}	
	break;
	
	case 'update':
		try {
			$owlPDO->beginTransaction();
			
			$data = array(
				'status' 		=> $param['status'],
				'updateby' 		=> $_SESSION['standard']['userid']
			);
			$where = "kodeorg='".$param['kodeorg']."' and komponengaji='".$param['komponen']."' ";
			$str = updateQuery($dbname,'sdm_5komponenlemburpengali',$data,$where);
			$owlPDO->exec($str);
				
			$owlPDO->commit();
		} catch (PDOException $e) {$owlPDO->rollback(); echo "Errorcode, " . addslashes($e->getMessage()); die();}	
	break;	
}
?>
<script>
	getSelect2();
</script>