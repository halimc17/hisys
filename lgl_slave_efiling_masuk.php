...
$arrEnumTipeSurat=getEnum($dbname,'lgl_efiling_masuk','tipesurat');
...
switch($method){
	case 'loaddata':
		...
		<tbody>";
		$no=0;
		$str= "select * from ".$dbname.".lgl_efiling_masuk";
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
		...
		$tab.="
			 ...
			 <input disabled hidden class=myinputtext id='idx' style=width:247px>
                ...
                <tr>
					...
					<td>:</td>
					<td>
						<input class=myinputtext id='jabatan' style=width:247px>
					</td>
				</tr>
                <tr>
					<td>Untuk</td>
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(nosurat) as jlhitem from ".$dbname.".lgl_efiling_masuk where nosurat='".$param['nosurat']."' ";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			$str = insertQuery($dbname,'lgl_efiling_masuk',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':
		try {
			...
			$where = " id ='".$param['idx']."'";
			$str = updateQuery($dbname,'lgl_efiling_masuk',$data,$where);
			$owlPDO->exec($str);
	...
	case 'deletedata':
		try {
			...
			$where = " id ='".$param['idx']."'";
			$str = deleteQuery($dbname,'lgl_efiling_masuk',$where);
			$owlPDO->exec($str);
