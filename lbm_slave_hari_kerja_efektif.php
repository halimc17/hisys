...
$qHari = $owlPDO->query($sHari) or die(print " Gagal: " . PDOException::getMessage());
$qHari->setFetchMode(PDO::FETCH_ASSOC);
$rHari = $qHari->fetch();
...
$sAbsensi = "select distinct count(absensi) as jmlhAbsen,b.jeniskelamin,absensi from " . $dbname . ".sdm_absensidt a left join 
           ...
           and b.tipekaryawan in (3,4) and absensi not in ('AS','BK','IDT','IPC')
...
$qKehadiran = $owlPDO->query($sKehadiran) or die(print " Gagal: " . PDOException::getMessage());
$qKehadiran->setFetchMode(PDO::FETCH_ASSOC);
while ($rKehadiran = $qKehadiran->fetch()) {
...
$qKabsensi = $owlPDO->query($sKabsensi) or die(print " Gagal: " . PDOException::getMessage());
$qKabsensi->setFetchMode(PDO::FETCH_ASSOC);
while ($rKabsensi = $qKabsensi->fetch()) {
