...
<script type="text/javascript" src="js/budget_budget_pks_rev.js?v=<?php echo time(); ?>"></script>
<?php
...
$optstationx.="<option value=''>".$_SESSION['lang']['all']."</option>";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
	$optstation.="<option value='".$bar->kodeorganisasi."'>".$bar->namaorganisasi."</option>";
	$optstationx.="<option value='".$bar->kodeorganisasi."'>".$bar->namaorganisasi."</option>";
}
...
echo"<br /><fieldset style='float:left;width:275px;'><legend>".$_SESSION['lang']['form']."</legend><table cellspacing=1 border=0>
    ...
    <tr><td>".$_SESSION['lang']['station']."</td><td>:</td><td colspan=3>
        <select name=station id=station  style='width:150px;'>".$optstation."</select></td></tr>
    <tr hidden><td>".$_SESSION['lang']['mesin']."</td><td>:</td><td colspan=3>
    ...
    <fieldset style='width:250px;'><legend>".$_SESSION['lang']['tutup']."</legend>
		...
		<input hidden id=nokunci>
		<input hidden id=nomesin>
...
$frm[1].="<table cellspacing=1 border=0><thead>
    ...
    <tr><td>".$_SESSION['lang']['jumlah']."</td><td>:</td><td>
        <input type=text class=myinputtextnumber onblur=\"jumlahkan1();\" id=jumlah1 name=jumlah1 onkeypress=\"return angka_doang(event);\" maxlength=10 style=width:145px; disabled=false/>
...
$sOrgs="select kodebudget,nama from ".$dbname.".bgt_kode where kodebudget in ('PKSM','SERVICE') order by kodebudget asc";
$res=fetchdata($sOrgs);
foreach($res as $rOrgs){
	if($rOrgs['kodebudget']=='PKSM'){		
		@$optKdbdgt_S.="<option value='".$rOrgs['kodebudget']."' selected>".$rOrgs['nama']."</option>";
	}else{
...
$frm[2].="<table cellspacing=1 border=0>
    <tr><td>".$_SESSION['lang']['kodeanggaran']."</td><td>:</td><td>
		<select id='kodebudget2' style='width:155px;' onchange=getaruskas(this.id,'aruskasmain','x');getkodews(this.id,'kodews');>".$optKdbdgt_S."</select>
        </td></tr>
	<tr>
		<td>".$_SESSION['lang']['aruskas']."</td><td>:</td>
		<td><select id='aruskasmain' style='width:155px;'>".$optaruskasx."</select></td>
	</tr>
	<tr>
		<td>".$_SESSION['lang']['kdWorks']."</td><td>:</td>
		<td><select id='kodews' style='width:155px;' disabled></select></td>
	</tr>
	<tr><td>Jam ".$_SESSION['lang']['setahun']." </td><td>:</td><td>
        <input type=text class=myinputtextnumber onkeyup=getjumlahws(); id=jamws name=jamws disabled onkeypress=\"return angka_doang(event);\" maxlength=20 style=width:150px;></td>
	</tr>
	
	
    <tr><td>".$_SESSION['lang']['jumlahpertahun']." </td><td>:</td><td>
        <input type=text class=myinputtextnumber id=jumlahpertahun2 name=jumlahpertahun2 onkeypress=\"return angka_doang(event);\" maxlength=20 style=width:150px; /></td></tr>
    <tr><td></td><td></td><td>
...
$frm[3].="<table cellspacing=1 border=0><thead>
    ...
    <tr><td>".$_SESSION['lang']['kodeanggaran']."</td><td>:</td><td>
		...
		<td>".$_SESSION['lang']['aruskas']."</td><td>:</td>
		<td><select id='aruskasvhc' style='width:150px;'></select></td>
	</tr>
...
$frm[4].="<fieldset style=float:left><legend>Find</legend><table><tr class=rowcontent>
              <td>".$_SESSION['lang']['station']."</td><td><select id=AfdSebaran onchange=load_mesin('sebaran')>".$optstationx."</select></td>
			  ...
			  <td>Jumlah Baris</td><td><input type=text id=limit style=width:50px class=myinputtextnumber nkeypress=\"return tanpa_kutip(event);\" onkeypress='return angka_doang(event)' value='50'></td>
