...
switch($method){
	case 'loaddata':
		...
		$no=0;
		$str= "select * from ".$dbname.".sdm_5setuppenilaian order by judul asc, status desc";
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
			...
			$tab.="<td style='text-align:center;width:25px'>
				<img src='images/application/application_edit.png' class='resicon' title='Edit' onclick=\"editdata('edit','".$val['judul']."','".$val['text']."','".$val['nilai']."','".$val['status']."')\";>
			</td>";
			$tab.="</tr>";
		}
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5setuppenilaian where judul='".$param['judul']."'  and text='".$param['text']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			$str = insertQuery($dbname,'sdm_5setuppenilaian',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':
		try {
			...
			$str = updateQuery($dbname,'sdm_5setuppenilaian',$data,$where);
			$owlPDO->exec($str);
