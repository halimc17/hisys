...
// kamus akun
$sakun=$owlPDO->query("select noakun, namaakun from ".$dbname.".keu_5akun  where substr(noakun,1,3) in ('121', '122')");
...
$ssumber=$owlPDO->query("select noreferensi from ".$dbname.".keu_jurnaldt
...
$notrantu="(";
$ssumber->setFetchMode(PDO::FETCH_ASSOC);
while($rsumber=$ssumber->fetch()){    
...
$stujuan=$owlPDO->query("select a.nojurnal, substr(a.tanggal,1,7) as periode, a.noakun, a.noreferensi, a.jumlah, a.kodebarang, b.tipetransaksi, b.notransaksireferensi, b.kodegudang, b.gudangx from ".$dbname.".keu_jurnaldt a
...
$stujuan->setFetchMode(PDO::FETCH_ASSOC);
while($rtujuan=$stujuan->fetch()){    
...
// remove AK-%
$sremove=$owlPDO->query("select * from ".$dbname.".log_transaksidt  where notransaksi in ".$notrantu." ");
$sremove->setFetchMode(PDO::FETCH_ASSOC);
while($rremove=$sremove->fetch()){    
