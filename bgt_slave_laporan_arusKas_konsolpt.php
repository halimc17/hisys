...
$sNoakun="select distinct noakun from ".$dbname.".keu_5akun where substr(noakun,1,1)='5' or substr(noakun,1,1)='9' order by noakun asc";
...
$sSum2="select distinct a.noakun  AS dnoakun,sum(setahun) as setahun,unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a 
    left join ".$dbname.".organisasi b on a.unit=b.kodeorganisasi
    where tahunbudget='".$thnBudget."' and  substr(a.noakun,1,1)='5' or substr(a.noakun,1,1)='9' and a.noakun!=0 
    group by  a.noakun,b.induk order by  a.noakun asc";
/*
...
$qSumPeng=$owlPDO->query($sSumPeng) or die(print " Gagal: ".PDOException::getMessage());
$qSumPeng->setFetchMode(PDO::FETCH_ASSOC);
while($rSumPeng=$qSumPeng->fetch())
{
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun,unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a 
        left join ".$dbname.".organisasi b on a.unit=b.kodeorganisasi
        where tahunbudget='".$thnBudget."' and (substr(a.noakun,1,3)>'125' and substr(a.noakun,1,3)<'129') and a.noakun!=0 
		group by substr(a.noakun,1,3),induk order by substr(a.noakun,1,3) asc";
//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
$qSum2=$owlPDO->query($sSum2) or die(print " Gagal: ".PDOException::getMessage());
$qSum2->setFetchMode(PDO::FETCH_ASSOC);
$rupiahBulan=array();
while($rSum2=$qSum2->fetch())
{
   // $dtNoakun[]=$rSum2['dnoakun'];
    // $rupAkunStaon2[$rSum2['induk']][$rSum2['dnoakun']]+=$rSum2['setahun'];
    $rupAkunStaon2[$rSum2['dnoakun']][$rSum2['induk']]+=$rSum2['setahun'];
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun, unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a  
        ...
        tahunbudget='".$thnBudget."' and substr(a.noakun,1,1)='6' and a.noakun!=0 
		group by substr(a.noakun,1,3),induk order by substr(a.noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
$rupiahBulan2=array();
$qSum2=$owlPDO->query($sSum2) or die(print " Gagal: ".PDOException::getMessage());
$qSum2->setFetchMode(PDO::FETCH_ASSOC);
while($rSum2=$qSum2->fetch())
...
$sSum2="select distinct substring(a.noakun, 1, 3) AS dnoakun,sum(setahun) as setahun, unit,b.induk from ".$dbname.".bgt_summary_biaya_vw a  
        left join ".$dbname.".organisasi b on a.unit=b.kodeorganisasi where
        tahunbudget='".$thnBudget."'and substr(a.noakun,1,1)='7' or substr(a.noakun,1,1)='8' and a.noakun!=0 
		group by substr(a.noakun,1,3),induk order by substr(a.noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
$qSum2=$owlPDO->query($sSum2) or die(print " Gagal: ".PDOException::getMessage());
$qSum2->setFetchMode(PDO::FETCH_ASSOC);
while($rSum2=$qSum2->fetch())
...
if($proses!='PDF')
{ 
            $tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable width=1000px><thead>";
            ...
            foreach($noAkun as $listNoakun)
            {
               ...
               // $tab.="<td   align=right >".number_format($totalDes[$listNoakun],2)."</td>";
                foreach($listUnit as $listBln)
                {
				   ...
				   @$grTotalpt[$listBln]+=$rupAkunStaon[$listNoakun][$listBln];
                }
			...
			foreach($listUnit as $listBln)
			{
			   $tab.="<td  align=right>".number_format($grTotalpt[$listBln],2)."</td>";
			}
