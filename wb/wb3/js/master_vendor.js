function loaddata() {
    ...
    tujuan = 'master_vendor_slave.php';
...
function editdata(jenis,kode,deskripsi,supplier,transportir,alamat,kota,stat){
	param  = '';
	param += '&jenis=' + jenis;
	param += '&kode=' + kode;
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					if(transportir=='1'){
						document.getElementById('transportir').checked=true;
					}
					setValue2('alamat',alamat);
					setValue2('kota',kota);
...
function simpan(){
	...
	supplier='0';
	transportir='0';
	if(document.getElementById('supplier').checked==true){
		supplier='1';
		counttipe++;
	}
	if(document.getElementById('transportir').checked==true){
		transportir='1';
		counttipe++;
	}
	...
	validate([
        ["kode","Kode Vendor harus diisi"],
        ["deskripsi","Nama Vendor harus diisi"],
        ["status","Status tidak boleh kosong"]
	]);
	
	if(counttipe==''){
		alertify.alert("Tipe Vendor harus dipilih.");
		return false;
	}
