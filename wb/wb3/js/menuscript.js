...
if(typeof sessionStorage.menuActive === 'undefined'){
	sessionStorage.menuActive = "0";
}
if(typeof sessionStorage.menuid === 'undefined'){
	sessionStorage.menuid = "0";
}
function toActiveParent(parentIdName){
	if(parentIdName !== "0"){
		if(document.getElementById('menu_'+parentIdName)){
			var menuParent = document.getElementById('menu_'+parentIdName);
			if(typeof menuParent !== 'undefined' && menuParent.classList.contains('active') !== true){
				menuParent.classList.add("active");
				parentid = menuParent.getAttribute('parentid');
				toActiveParent(parentid);
			}
		}
	}
}
function activedMenu(){
	var menu = document.getElementById('menu_'+sessionStorage.menuid);
	if(menu){
		if(menu.classList.contains('active') !== true){
			menu.classList.add("active");
			parentid = menu.getAttribute('parentid');
			toActiveParent(parentid);
		}
	}
}
window.addEventListener("load", activedMenu);
function do_load(dest,menuid){
  sessionStorage.menuActive = dest;
  if(dest=='master.php'){
...
function openusersetting(){
	param = '&method=openusersetting';
	post_response_text('slave_usersetting.php', param, respon);
		...
		if(con.readyState == 4){
			if(con.status == 200){
				...
				if(!isSaveResponse(con.responseText)){
					...
					alertify.popuserset(con.responseText).set().resizeTo('50%','50%');
					// document.getElementsByClassName('ajs-close').style.display='none';
				}
			}else{
				busy_off();
