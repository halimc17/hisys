...
echo"<table style=\"width:99.5%;border-collapse:collapse;margin-left:auto;margin-right:auto;\">
	<tr>
		<td>
			<strong>
				<font size=3 color=#191919 font-family: Verdana, Arial, Helvetica, sans-serif>Terima TBS Inti / Plasma</font>
			</strong>
		</td>
...
## GET UNIT
$str="select unitcode,unitname from ".$dbname.".msunit where compcode='".$compcode."' and tipeunit!='' and tipe='KEBUN' and unitstatus='1'";
$res=fetchdata($str);
foreach ($res as $val) {
...
echo"<table border=0 cellpadding=3 style=width:100%>
	...
	<td style=vertical-align:top;width:50%>
		...
		<td>
				...
				<input tabindex=1 class=myinputtext style='width:80%;height:28px' type=text name=qrcode id=qrcode onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' maxlength='14' onclick=\"this.select();\" value=''>
			</td>

			<td>
				<label class=label style='font-weight:bold'>Kernet 1</label>
				<input tabindex=7 class=myinputtext style='width:80%' type=text name=kernet1 id=kernet1 onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' value=''>
			</td>
		...
		<tr>
			<td>
				...
				<select tabindex=2 class='select2' style='width:85%;height:32px;' id='unit' tabindex=2>".$optunit."</select>
			</td>

			<td>
				<label class=label style='font-weight:bold'>Kernet 2</label>
				<input tabindex=8 class=myinputtext style='width:80%' type=text name=kernet1 id=kernet2 onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' value=''>
			</td>
		...
		<tr>
			<td>
				...
				<select tabindex=3 class='select2' style='width:85%;height:32px;' id='divisi' tabindex=3>".$optdivisi."</select>
			</td>
			<td>
				<label class=label style='font-weight:bold'>Jumlah Janjang</label>
				<input tabindex=9 class=myinputtext style='width:80%;text-align:right' type=text id=jjg onkeypress=\"return isNumberKey2(event);\" onkeydown='upperCaseF(this)' onblur=calcgrad() value='' placeholder='0'>
			</td>
		...
		<tr>
			<td>
				...
				<select tabindex=4 class='select2' style='width:85%;height:32px;' id='transportir' tabindex=5>".$opttransportir."</select>
			</td>
			<td>
				<label class=label style='font-weight:bold'>Brondolan</label>
				<input tabindex=10 class=myinputtext style='width:80%;text-align:right' type=text id=brondol onkeypress=\"return isNumberKey(event);\" onkeydown='upperCaseF(this)' value='' placeholder='0'>
			</td>
		...
		<tr>
			<td>
				...
				<input tabindex=5 class=myinputtext style='width:80%' type=text name=nokendaraan id=nokendaraan onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' value=''>
			</td>	
			<td>
				<label class=label style='font-weight:bold'>Keterangan</label>
				<input tabindex=11 class=myinputtext style='width:80%' type=text id=keterangan onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' value='' disabled>
			</td>
		</tr>
	...
	<td style='width:50%'>
		<div id='showgrading' style='display:none'>";
			$tabindx=14;
			$str="select * from ".$dbname.".msgrading where status='1' order by nourut asc";
			$res=fetchdata($str);
			if(count($res) > 0){
						...
						if($valx['jjg']!='' and $valx['persen']!='' and $valx['kg']!=''){
							...
							if($valx['jjg']!=''){
								...
								echo"<td style='text-align:center'>
									<input tabindex='".$tabindx."' class=myinputtext type=text style='text-align:right;width:50%' onkeypress='return isNumberKey(event);' name='jjg' id='".$valx['kode']."__".$valx['jjg']."' value='' onblur=calcgrad(this.id) placeholder='0'>
								</td>";
							}
							...
							if($valx['persen']!=''){
								$tabindx++;
								echo"<td style='text-align:center'>
									<input tabindex='".$tabindx."' class=myinputtext type=text style='text-align:right;width:50%' onkeypress='return isNumberKey(event);' name='persen' id='".$valx['kode']."__".$valx['persen']."' value='' onblur=calcgrad(this.id) placeholder='0'>
								</td>";
							}
							...
							if($valx['kg']!=''){
								$tabindx++;
								echo"<td style='text-align:center'>
									<input tabindex='".$tabindx."' class=myinputtext type=text style='text-align:right;width:50%' onkeypress='return isNumberKey(event);' name='kg' id='".$valx['kode']."__".$valx['kg']."' value='' onblur=calcgrad(this.id) placeholder='0'>
								</td>";
							}
							echo"</tr>";
						}
		...
		echo"<div id='showsortasi' style='display:none'>";
			$tabindx=14;
			$str="select * from ".$dbname.".mssortasi where status='1' order by nourut asc";
			$res=fetchdata($str);
			if(count($res) > 0){
				echo"<table cellspacing=0 cellpadding=3 style='border:1px solid #FFFFFF'>
					<tr>
		...
		<td align='center' style='vertical-align:top;'>
			...
			<table border=0 cellspacing=0>
				<tr>
					...
					<td>:</td>
					<td>
						<input class=myinputtext type=text name=datein id=datein style='width:85%;font-size:18px;text-align:right;' $disabledinput>
					</td>
					<td rowspan=2>
		...
		<td align='center' style='vertical-align:top;'>
			...
			<table border=0 cellspacing=0>
				<tr>
					...
					<td>:</td>
					<td>
						<input class=myinputtext type=text name=dateout id=dateout style='width:85%;font-size:18px;text-align:right;' $disabledinput>
					</td>
					<td rowspan=2>
...
<script language=javascript1.2 src='js/trx_intbsinti.js?v=<?php echo time(); ?>'></script>
<script>
	getSelect2();
</script>
<?php
