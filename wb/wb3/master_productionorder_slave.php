	...
	case 'loaddata':
		...
		<thead>
			<tr class=rowheader>
				...
				<th style='text-align:center;'>No Production Order</th>
		...
		$no=0;
		$str= "select * from ".$dbname.".msproductionorder order by tanggalmulai desc";
		$res= fetchdata($str);
		foreach($res as $val){
			## GET NAMA PT
			$optunit=makeOption($dbname,'msunit','unitcode,unitname',"unitcode='".$val['unitcode']."'");
			
			if($val['status']=='1'){$checked='checked';}else{$checked='';}
			$no+=1;
			$tab.="<tr class=rowcontent>";
			$tab.="<td style='text-align:center;'>".$no."</td>";
			$tab.="<td style='text-align:left;'>".$val['no']."</td>";
			$tab.="<td style='text-align:left;'>".$optunit[$val['unitcode']]."</td>";
