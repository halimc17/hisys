...
switch($method){
	case 'loaddata':
		...
		if($param['unit']!=''){
			...
			$where.=" and unitcode in (select unitcode from ".$dbname.".msunit where compcode='".$pt."')";
		}
		...
		$str= "select * from ".$dbname.".wb where netto > 0 and tipeunit='PLASMA' and kodebarang='".$kodeproduktbs."' and sumber='".$sumber."' ".$where." order by notransaksi asc";
		$res= fetchdata($str);
		$no=0;
		foreach($res as $val){
			$no+=1;
		...
		$tab.="<td style='text-align:center;' colspan=6>T O T A L</td>";
