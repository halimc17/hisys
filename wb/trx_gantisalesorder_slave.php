...
switch($method){
	case 'generatenotiket':
		...
		$arrhasil['tiket']=generatenotiket();
...
function generatenotiket(){
    ...
    $res2=fetchdata($str2);
    $idwb=$res2[0]['idwb'];
    $compcode=$res2[0]['compcode'];

    $tipe="S";

    $tahun=date('y');

    $str="select distinct RIGHT(notransaksi,5) as notiket from ".$dbname.".wb";
    $res=fetchdata($str);
    if(!$res)
    {
        $no_1=1;
        $no=str_pad($no_1,5,"0",STR_PAD_LEFT);
    }
    else
    {   
        $str2="select RIGHT(notransaksi,5) as notiket from ".$dbname.".wb where notransaksi like '%".$compcode."%' order by notiket desc limit 1";
        $res2=fetchdata($str2);
        if ($res2){
        ...
        {
            $no3=1;
            $no=str_pad($no3,5,"0",STR_PAD_LEFT);
        }
    }
    return $tahun."".$compcode."D".$tipe."WB".$no;
}
