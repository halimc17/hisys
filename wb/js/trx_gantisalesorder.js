...
function getso(product='',customer='',so='',transportir='',nokendaraan='',newFunc){
    ...
    tujuan='trx_gantisalesorder_slave.php';
...
function fillfield() {
	ticketno = document.getElementById('tiketref').value;
        ...
        if(con.readyState==4){
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					setValue2('datein','');
					setValue2('wei1st',arrlist['beratkeluar']);
					setValue2('dateout','');
					setValue2('wei2nd',arrlist['beratmasuk']);
...
const tiketrefEl = document.getElementById('tiketref');
if (tiketrefEl) {
  tiketrefEl.addEventListener('blur', function () {
    fillfield();
  });
}
