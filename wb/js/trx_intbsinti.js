...
document.addEventListener('keypress', e=>{
    if (e.keyCode===13){
        if (code) {
        	let pisahdata='';
  			pisahdata=code.split('#');
  			document.getElementById('qrcode').value=pisahdata[0];
  			document.getElementById('supir').value=pisahdata[1];
  			document.getElementById('nokendaraan').value=pisahdata[2];
  			setValue2('unit',pisahdata[6]);
  			getdivisi(pisahdata[7],'','','');
        }
  }else{
       code+=e.key;
  }
...
function getdivisi(divisi='',productionorder='',so='',newFunc) {
	unit = getValue('unit');
	divisi = divisi;
    param='method=getdivisi&unit='+unit+'&divisi='+divisi;
    tujuan='trx_intbsinti_slave.php';
...
function simpan(){
	...
	kernet1 = document.getElementById('kernet1').value;
	kernet2 = document.getElementById('kernet2').value;
	...
	param='ticketno='+ticketno+'&qrcode='+qrcode+'&unit='+unit+'&divisi='+divisi+'&transportir='+transportir+'&nokendaraan='+nokendaraan+'&supir='+supir;
	param+='&jjg='+jjg+'&brondol='+brondol+'&keterangan='+keterangan;
	param+='&kernet1='+kernet1+'&kernet2='+kernet2;
...
function batal(){
	...
	document.getElementById('kernet1').disabled=false;
	document.getElementById('kernet2').disabled=false;
        ...
        if(con.readyState==4){
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					getdivisi(arrlist['divcode'],arrlist['nopo'],arrlist['kontrakbeli']);
					...
					setValue2('kernet1',arrlist['kernet1']);
					setValue2('kernet2',arrlist['kernet2']);
					...
					document.getElementById('kernet1').disabled=true;
					document.getElementById('kernet2').disabled=true;
...
function calcgrad(idx=''){
	...
	if(document.getElementById('showsortasi').style.display=='none'){
		document.getElementById('kgpotongan').value=Math.round(ttlkg);
	}else{
		document.getElementById('kgpotongan').value=0;
	}
