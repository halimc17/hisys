...
document.addEventListener('keypress', e=>{
    if (e.keyCode===13){
        if (code) {     
			...
			if(document.getElementById('nospb').value == ''){
				alert('NOSPB masih kosong...');
				return false;
			}
...
function getdivisi(divisi='',customer='',so='',newFunc) {
    ...
    tujuan='trx_tbskebunx_slave_isi.php';
        ...
        if(con.readyState==4){
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					setValue2('tahuntanam',arrlist['tahuntanam']);
