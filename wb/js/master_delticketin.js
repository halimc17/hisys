...
function hapus(){
	...
	tujuan='master_delticketin_slave.php';
	alertify.confirm("Konfirmasi","Apakah anda yakin hapus tiket masuk "+notiket+"??",
	function(){
        ...
        if(con.readyState==4){
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					batal();
					alertify.alert("Info","Berhasil, No. Tiket masuk "+notiket+" sudah dihapus dari sistem.");
                }
            } else {
                busy_off();
