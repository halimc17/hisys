...
$tanggal=date("Y-m-d");
$jlhkendaraan=array();
$str="select waktumasuk, waktukeluar from ".$dbname.".wb where in_out='I' and (waktumasuk LIKE '".$tanggal."%' or waktukeluar = '".$tanggal."%') and kodebarang = '".$kodeproduktbs."' and tipeunit in ('INTERNAL','PLASMA') and sumber='KEBUN'";
$res=fetchdata($str);
foreach($res as $val){
	if($val['waktumasuk']!=''){
		@$jlhkendaraan['0']+=1;
...
echo"<table style=\"width:99.5%;border-collapse:collapse;margin-left:auto;margin-right:auto;\">
	<tr>
		<td>
			<strong>
				<font size=5 color=#191919 font-family: Verdana, Arial, Helvetica, sans-serif>TBS Kebun (Timbang Awal Kosong)</font>
			</strong>
		</td>
...
// jika ada datapanen yang tidak ada di wb maka hapus, ini bisa saja terjadi ketika mereka sudah scan panen, lalu mati lampu, maka data panen tersimpan tapi data timbangan tidak tersimpan
$str="delete from ".$dbname.".wb_datapanen where notiket not in (select notransaksi from ".$dbname.".wb)";
try
{
	$owlPDO->exec($str);
}
...
$optkendaraan=$optcustomer=$optunit=$optdivisi=$opttransportir="<option value=''>Silahkan pilih</option>";
## GET UNIT
$str="select unitcode,unitname from ".$dbname.".msunit where compcode='".$compcode."' and tipeunit!='' and unitstatus='1' and unitname not like '%PLASMA%' ";
$res=fetchdata($str);
foreach ($res as $val) {
...
## GET optcustomer
$str="select custcode,custname from ".$dbname.".mscustomer where custstatus ='1' ";
$res=fetchdata($str);
foreach ($res as $val) {
...
$tipeangkutan="<option value=''>Silahkan pilih</option>";
$tipeangkutan.="<option value='1'>Internal</option>";
$tipeangkutan.="<option value='0'>External</option>";
...
echo"<table border=0 cellpadding=3 style=width:100%>
	...
	<td style=vertical-align:top;width:50%>
		...
		<tr>
			...
			<td style='width:50%;'>
				<label class=label style='font-size:12px;font-weight:bold'>No SPB <span style='color:red'>*</span></label><br>
				<input tabindex=1 class=myinputtext style='width:80%;height:28px' type=text name=nospb id=nospb onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' maxlength='50' value='' disabled>
			</td>
		</tr>

		<tr>
			<td>
				<label class=label style='font-weight:bold'>Unit <span style='color:red'>*</span></label>
				<select class='select2' style='width:85%;height:32px;' id='unit' tabindex=2>".$optunit."</select>
			</td>
			<td style='width:50%;'>
				<label class=label style='font-size:12px;font-weight:bold'>Tahun Tanam <span style='color:red'>*</span></label><br>
				<input class=myinputtext style='width:80%;text-align:right' type=text id=tahuntanam onkeypress=\"return isNumberKey2(event);\" onkeydown='upperCaseF(this)' value='' maxlength='4' disabled>
			</td>
		</tr>
		
		<tr>
			<td>
				<label class=label style='font-weight:bold'>Divisi <span style='color:red'>*</span></label>
		...
		<tr>
			<td>
			<label class=label style='font-weight:bold'>Customer <span style='color:red'>*</span></label>
				<select class='select2' style='width:85%;height:32px;' id='customer' tabindex=3>".$optcustomer."</select>
			</td>
		...
		<tr>
			<td>
				<label class=label style='font-size:12px;font-weight:bold'>Kontrak <span style='color:red'>*</span></label><br>
			...
			<td>
				<label class=label style='font-weight:bold'>Jumlah Janjang <span style='color:red'>*</span></label>
		...
		<tr>
			...
			<td>
				<label class=label style='font-weight:bold'>Tipe Angkutan <span style='color:red'>*</span></label>
				<select class='select2' style='width:85%;height:32px;' id='tipe_angkutan' tabindex=5>".$tipeangkutan."</select>
			</td>
			
			
			<td>
				<label class=label style='font-weight:bold'>Brondolan <span style='color:red'>*</span></label>
		...
		<td>
			<label class=label style='font-weight:bold'>Transportir <span style='color:red'>*</span></label>
			<select class='select2' style='width:85%;height:32px;' id='transportir' tabindex=5>".$opttransportir."</select>
		</td>
			
			
			<td>
				<label class=label style='font-weight:bold'>Adjustment Brondolan <span style='color:red'>*</span></label>
				<input tabindex=14 class=myinputtext type='number' style='width:80%;text-align:right' type=text id=adj_brondol value='0' disabled>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<label class=label style='font-weight:bold'>Adjustment Jumlah Janjang</label>
				<input tabindex=14 class=myinputtext type='number' style='width:80%;text-align:right' type=text id=adj_jjg value='0' disabled>
			</td>
		</tr>
		<tr>
		<td>
		<label class=label style='font-weight:bold'>No Kendaraan <span style='color:red'>*</span></label>
			<!--<input tabindex=7 class=myinputtext style='width:80%' type=text name=nokendaraan id=nokendaraan onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' maxlength='11' value=''>-->
			<select class='select2' style='width:85%;height:32px;' id='nokendaraan' tabindex=7>".$optkendaraan."</select>
		</td>		
		
		<td>
			<label class=label style='font-weight:bold'>Keterangan</label>
			<input tabindex=15 class=myinputtext style='width:80%' type=text id=keterangan onkeypress=\"return tanpa_kutip(event);\" onkeydown='upperCaseF(this)' value='' disabled>
		</td>
		</tr>
		</table>
	</td>
	<td style='width:50%'>
		...
		// tiket panen
		echo "
			<div id='dataPanen'></div>

		";
		// echo "
		// 	<div class='datapanen' style='border: 1px solid red'>
...
<script language=javascript1.2 src='js/trx_tbskebunx_kosong.js?v=<?php echo time(); ?>'></script>
<script>
	getSelect2();
	stopreminder();
</script>
