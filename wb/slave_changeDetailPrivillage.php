...
try {
	$owlPDO->beginTransaction();
		...
		$str=$owlPDO->query("select * from ".$dbname.".auth  where uname='".$uname."'  and menuid=".$menuid); 
		...
		if(!$status and $action=='remove'){
			$str="insert into ".$dbname.".auth(uname,menuid,status,lastuser) 
		...
		if($status and $action=='remove'){
			...
			where uname='".$uname."'and menuid=".$menuid;
			$owlPDO->exec($str);
			$s=2;
...
function addchild($param){
	...
	foreach($_SESSION['idchild'] as $key => $idmenu){		
		...
		if(count($res)>0){
			...
			foreach($res as $bar){			
				$jlh = 0;
				$str = "select * from ".$dbname.".auth where uname='".$param['uname']."' and menuid = '".$bar['id']."'";
				$res = fetchdata($str);
				$jlh = count($res);
				if($param['action']=='add' and $jlh == '0'){				
					...
					$str="update ".$dbname.".auth set status=1,lastuser='".$_SESSION['standard']['username']."'  
					where uname='".$param['uname']."' and menuid=".$bar['id'];	
					$owlPDO->exec($str); 
				}
				if($param['action']=='remove' and $jlh > '0'){
					$str="delete from ".$dbname.".auth where uname='".$param['uname']."' and menuid='".$bar['id']."'";
					$owlPDO->exec($str); 
				}
...
function addparent($param){
	...
	if($induk>0){
		...
		$jlh = 0;
		$str = "select * from ".$dbname.".auth where uname='".$param['uname']."' and menuid = '".$induk."'";
		$res = fetchdata($str);
		$jlh = count($res);
		if($param['action']=='add' and $jlh == '0'){				
			$str="insert into ".$dbname.".auth (uname,menuid,status,lastuser) 
			...
			$str="update ".$dbname.".auth set status=1,lastuser='".$_SESSION['standard']['username']."'  
			where uname='".$param['uname']."' and menuid=".$induk;	
			$owlPDO->exec($str); 
			$_SESSION['menuid']=$induk;
		}
		if($param['action']=='remove' and $jlh > '0'){				
