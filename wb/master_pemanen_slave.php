...
switch($method){
	case 'loaddata':
		...
		$no=0;
		$str= "select * from ".$dbname.".msdatapemanen order by unitcode asc, divcode asc";
		$res= fetchdata($str);
		foreach($res as $val){
			if($val['status']=='1'){$checked='checked';}else{$checked='';}
			## GET NAMA WILAYAH
			$optnmunit=makeOption($dbname,'msunit','unitcode,unitname');
			$optndivisi=makeOption($dbname,'msdivisi','divcode,divname');
			
			$e=$optnmunit[$val['unitcode']];
			if(@$e!=@$o){
			...
			$tab.="<td id='kodeform_".$no."' style='display:none'>".$val['karyawanid']."</td>";
			$tab.="<td style='text-align:center;'>".$val['namakaryawan']."</td>";
	...
	case 'addnew':
		...
		$tab.="
			 <table border=0 cellpadding=2 cellspacing=1>
				...
				<tr>
					<td style=min-width:150px>Kode blok</td>
					<td>
	...
	case 'insert':
		try{
			...
			$query = insertQuery($dbname,'msblok',$data,$cols);
			$owlPDO->exec($query);
	...
	case 'update':
		try {
			$owlPDO->beginTransaction();
				...
				$query = updateQuery($dbname,'msblok',$data,$where); #exit("warningcode".$query);
				$owlPDO->exec($query);
	...
	case'getvalstt':
		try {
			$owlPDO->beginTransaction();
				...
				$where = "karyawanid='".$param['kode']."'";
				$query = updateQuery($dbname,'msdatapemanen',$data,$where); #exit("warningcode".$query);
				$owlPDO->exec($query);
