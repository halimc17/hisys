	...
	case 'loaddata':
		...
		<thead>
			<tr class=rowheader>
				...
				<th style='text-align:center;'>Kode Induk Blok</th>
				<th style='text-align:center;'>Nama Induk Blok</th>
		...
		$no=0;
		$str= "select * from ".$dbname.".msbloktph order by divisi asc, indukblok asc";
		$res= fetchdata($str);
		foreach($res as $val){
			if($val['status']=='1'){$checked='checked';}else{$checked='';}
			## GET NAMA WILAYAH
			// $optnmperusahaan=makeOption($dbname,'mscompany','compcode,compname');
			$optnmunit=makeOption($dbname,'msdivisi','divcode,divname');
			...
			$tab.="<td id='kodeform_".$no."'>".$val['indukblok']."</td>";
			$tab.="<td style='text-align:left;'>".$val['namaindukblok']."</td>";
	...
	case'getvalstt':
		try {
			$owlPDO->beginTransaction();
				...
				$where = "indukblok='".$param['kode']."'";
				$query = updateQuery($dbname,'msbloktph',$data,$where); #exit("warningcode".$query);
				$owlPDO->exec($query);
