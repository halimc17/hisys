...
OPEN_BOX('','<span class=judul>'.getMenu('wb_2gradingtbsint').'</span><br>');

$str="select compcode from ".$dbname.".mssystem limit 1";
$res=fetchData($str);
$pt=$res[0]['compcode'];

$optunit="<option value=''>Seluruhnya</option>";
$str="select unitcode,unitname from ".$dbname.".msunit where compcode='".$pt."' and tipeunit in ('INTERNAL','PLASMA')";
$res=fetchdata($str);
foreach($res as $val){
...
echo"<fieldset style='min-width:100px;float:left'>
	<table border=0 cellpadding=3>
		<tr>
			...
			<td>:</td>
			<td>
				<select class='select2' style='width:200px' id='unit' tabindex=2>".$optunit."</select>
			</td>
		</tr>
		<tr>
			<td>Periode</td>
...
?>
<script language=javascript src='js/wb_2gradingtbsint.js?v=<?php echo time(); ?>'></script>
<script>
	getSelect2();
</script>
<?
