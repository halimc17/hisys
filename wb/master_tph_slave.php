	...
	case 'loaddata':
		...
		<thead>
			<tr class=rowheader>
				...
				<th style='text-align:center;'>Kode TPH</th>
				<th style='text-align:center;'>Nama TPH</th>
		...
		$no=0;
		$str= "select * from ".$dbname.".mstph order by indukblok asc, kode asc";
		$res= fetchdata($str);
		foreach($res as $val){
			if($val['status']=='1'){$checked='checked';}else{$checked='';}
			## GET NAMA WILAYAH
			// $optnmperusahaan=makeOption($dbname,'mscompany','compcode,compname');
			$optnmunit=makeOption($dbname,'msbloktph','indukblok,namaindukblok');
			...
			$d=$optnmunit[$val['indukblok']];
			if(@$d!=@$n){
				$tab.="<tr class=rowcontent style='background-color:#e8e8e8;font-weight:bold'>";
				$tab.="<td style='text-align:center;'></td>";
				$tab.="<td style='text-align:left;'>&nbsp;&nbsp;&nbsp;&nbsp;".$d." - ".$val['indukblok']."</td>";
				$tab.="<td></td><td></td>";
				$tab.="</tr>";
			}
			...
			$tab.="<td id='kodeform_".$no."'>".$val['kode']."</td>";
	...
	case'getvalstt':
		try {
			$owlPDO->beginTransaction();
				...
				$query = updateQuery($dbname,'mstph',$data,$where); #exit("warningcode".$query);
				$owlPDO->exec($query);
