...
function stagesblok(){
	...
	## NEW
	$str="select * from ".$dbnameerp.".organisasi where `tipe` = 'BLOK' group by indukblok ";
	$res=fetchdatax($str);
	if(count($res) > 0){
		foreach($res as $val){
			try{
				...
				$strx="select * from ".$dbname.".msbloktph where indukblok='".$val['indukblok']."'";
				$resx=fetchdata($strx);
				if(count($resx) <= 0){
					$data = array(
						'divisi'=>$val['induk'],
						'indukblok'=>$val['indukblok'],
						// 'descode1'=>$val['descode1'],
						'namaindukblok'=>$val['namaindukblok'],
						'status'=>'1',
					);
					$cols = array();	
					foreach($data as $key=>$row) {
					...
					$strt = insertQuery($dbname,'msbloktph',$data,$cols);
					$owlPDO->exec($strt);
				}else{
					$data = array(
						'divisi'=>$val['induk'],
					...
					$strt = updateQuery($dbname,'msbloktph',$data,"indukblok='".$val['indukblok']."'");
					$owlPDO->exec($strt);
				}
...
function stagestph(){
	...
	## NEW
	$str="select * from ".$dbnameerp.".kebun_5tph";
	$res=fetchdatax($str);
	if(count($res) > 0){
		foreach($res as $val){
			try{
				...
				$strx="select * from ".$dbname.".mstph where kode='".$val['kode']."'";
				$resx=fetchdata($strx);
				if(count($resx) <= 0){
					$data = array(
						'indukblok'=>$val['kodeorg'],
						'kode'=>$val['kode'],
						// 'descode1'=>$val['descode1'],
					...
					$strt = insertQuery($dbname,'mstph',$data,$cols);
					$owlPDO->exec($strt);
				}else{
					$data = array(
						'indukblok'=>$val['kodeorg'],
					...
					$strt = updateQuery($dbname,'mstph',$data,"kode='".$val['kode']."'");
					$owlPDO->exec($strt);
				}
...
function stagespemanen(){
	...
	// cek paramater aplikasi
	$str = "select nilai from ".$dbnameerp.".setup_parameterappl where kodeparameter='PNN'"; 
	$res = fetchdata($str);
	$PNN = explode(',', $res[0]['nilai']);
	if(count($PNN) > 0){

			$str="select * from ".$dbnameerp.".datakaryawan where kodejabatan in ('".implode("','",$PNN)."') and `tanggalkeluar` = '0000-00-00'";
			$res=fetchdatax($str);
			if(count($res) > 0){
				foreach($res as $val){
				try{
					...
					$strx="select * from ".$dbname.".msdatapemanen where karyawanid='".$val['karyawanid']."'";
					$resx=fetchdata($strx);
					if(count($resx) <= 0){
						$data = array(
							'unitcode'=>$val['lokasitugas'],
							'divcode'=>$val['subbagian'],
						...
						$strt = insertQuery($dbname,'msdatapemanen',$data,$cols);
						$owlPDO->exec($strt);
					}else{
						$data = array(
							'unitcode'=>$val['lokasitugas'],
						...
						$strt = updateQuery($dbname,'msdatapemanen',$data,"karyawanid='".$val['karyawanid']."'");
						$owlPDO->exec($strt);
					}
...
function stagesso(){
	...
	$str="select * from ".$dbnameerp.".pmn_kontrakjual WHERE posting='1'";
	$res=fetchdatax($str);
	$arrdata=$res;
	if(count($arrdata) > 0){
		foreach($arrdata as $val){
