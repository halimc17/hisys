...
switch($method){
	...
	case 'loadData':
		...
		$where = "and netto='0' and kodebarang='".$kodeproduktbs."' and tipeunit in ('EXTERNAL','PLASMA') and sumber='KEBUN' and in_out='O' and spb != '' ";
	...
	case'timbang2':
		try{
			...
			$data = array(
				...
				'adjjjg'=>$param['adj_jjg'],
				'adjbrondol'=>$param['adj_brondol']
			);
			// 'qr'=>$qrcode,
	...
	case'tambahPanen':
		...
		$res1=fetchdata($str1);
		if(count($res1) > 0){
			exit("warning : QR CODE '".$qrcode."' ,Blok ".$optnamablok[$blok].", TPH ".$optnamatph[$tph].", Pemanen ".$optnamapemanen[$pemanen].", Sesi '".$sesi."' Data sudah ada di no tiket ".$res1[0]['notiket']." ");
		}
