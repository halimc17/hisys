...
{
	case 'insert':
		$str = "select count(id) as count from ".$dbname.".pmn_5daerahkontrak where UPPER(lokasi) = '".strtoupper($lokasi)."' or inisial='".$inisial."'";
		...
		{
			$str = "insert into ".$dbname.".pmn_5daerahkontrak values ('','".$lokasi."','".$inisial."','".$status."','".$_SESSION['standard']['userid']."','".date('Y-m-d H:i')."','".$_SESSION['standard']['userid']."','')";
			try
			{
				$owlPDO->exec($str);
			}
    ...
    case 'update':
		$str = "update ".$dbname.".pmn_5daerahkontrak set status='".$status."', updateby='".$_SESSION['standard']['userid']."' where lokasi = '".$lokasi."' and inisial='".$inisial."'";
        try
		{
			$owlPDO->exec($str);
		}
    ...
    case'loaddata':
		...
		$no = 0;
		$str = "select * from ".$dbname.".pmn_5daerahkontrak order by lokasi";
		$res = $owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
        while ($bar = $res->fetch())
		{
