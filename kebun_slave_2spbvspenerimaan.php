...
#ambil spb kebun
$str="SELECT sum(a.kgwb) as kgwb, a.nospb,sum(a.jjg) as jjg,b.tanggal,substr(a.nospb,9,6) as afdeling,sum(kgwbnetto) as kgwbnetto FROM ".$dbname.".kebun_spbdt a
...
# ambil hasil timbangan kebun
$str="select nokendaraan,supir,nospb,(jumlahtandan1+jumlahtandan2+jumlahtandan3) as jjgpabrik,(beratmasuk -beratkeluar) as beratbersih,(beratbersih) as beratnormal,
...
while($bar=$res->fetch()){
    ...
    @$kgkebunnetto[$bar->nospb]+=$bar->beratbersih.' ';
    @$kgkebun[$bar->nospb]+=$bar->beratnormal.' ';

}

#ambil spb hasil timbangan pabrik
$str="select nokendaraan,supir,nospb,(jumlahtandan1+jumlahtandan2+jumlahtandan3) as jjgpabrik,(beratmasuk -beratkeluar) as beratbersih,(beratbersih) as beratnormal,
                notransaksi,left(tanggal,10) as tanggal from ".$dbname.".pabrik_timbangan where 
          left(tanggal,10)!='' ".$whrpab." and nospb!='' and millcode = 'EXTM'
          and left(tanggal,10) like '".$periode."%' order by tanggal,nospb";
...
$bgcoloraja="";
$tab="
<table class=sortable cellspacing=1 cellpadding=5 border=".$brd." >
<thead>
<tr>
	<th align=center colspan=7>".$_SESSION['lang']['kebun']."</th>
...
if(isset($nospb)){
    ...
    foreach($nospb as $spb=>$val){
		...
		@$varkg=$kgkebunnetto[$spb]-$beratnormal[$spb];
		if(abs($varkgnetto)>=1){
		...
		@$totalberatnormal+=$beratnormal[$spb];     
          ...
          $tab.="<tr class=rowcontent>
            ...
            <td colspan=4 align=center bgcolor=".$colorkebun.">Total</td>
