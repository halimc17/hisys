...
<script language=javascript src='js/vhc_3mandortraksi.js?v=<?php echo time(); ?>'></script>
<?
...
$optafd2="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
...
foreach ($rOrg as $val) {
    ...
    $optunitx.="<option value=".$val['kodeorganisasi'].">".$val['kodeorganisasi']." - ".$val['namaorganisasi']."</option>";
}

$str="select distinct(substr(tanggal,1,7)) as periode from ".$dbname.".vhc_runht order by tanggal desc";
...
OPEN_BOX('','<span class=judul>'.getMenu('vhc_3mandortraksi').'</span><br>');
echo"<table>
     <tr valign=middle>
	 ...
	 <td>
		...
		<tr>
			...
			<td hidden>Kode Traksi</td>
			<td hidden>:</td>
			<td hidden><select class=select2 id=afdlist onchange='loaddata(0)' style='width:153px;'>".$optafd2."</select></td>
		</tr>";
	...
	<table class='sortable' cellspacing=1 cellpadding=5 border=0 width=100%>
		<thead>
			<tr class=rowheader>
				...
				<th align=center>Total Premi (Rp)</th>
...
$arr="##prd##unit##afd##tglmulai##tglakhir";
echo"<tr>
        <td colspan=2></td>
		<td colspan=3>
		<button onclick=zPreview('vhc_slave_3mandortraksi','".$arr."','printContainer') class=mybutton name=preview id=preview>".$_SESSION['lang']['preview']."</button>
...
echo"<table>
	...
	<tr>
		<td>Urutan melakukan proses premi Mandor Traksi adalah :<br>
			   1. Posting Pekerjaan Traksi<br>
			   2. Lakukan Proses premi Mandor Traksi<br>
			   3. Proses premi diambil dari hasil kerja berdasarkan mandor
	   </td>
	</tr>
</table>";
