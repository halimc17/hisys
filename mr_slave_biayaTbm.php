...
// preview excel pdf    
	...
	$query=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$query->setFetchMode(PDO::FETCH_ASSOC);
	while($res=$query->fetch()){	        // cuman nyari luas aja
...
//    echo "</pre>";
    ...
    $tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable >
        ...
        <td align=right colspan=3 ".$bg.">Luas (Ha)</td>";
        if(!empty($adatt))foreach($adatt as $tata){
            if($tata==0){
...
//                $luastotal+=$luas[$tata];
        ...
        <tr>
        <td align=center colspan=2 ".$bg.">No Akun</td>
        
        <td align=center colspan=1 ".$bg.">Nama Akun</td>";
        if(!empty($adatt))foreach($adatt as $tata){
            if($tata==0){
    ...
    // total TBM ===============================================================
    $tab.="<thead class=rowheader><tr class=rowtitle>
