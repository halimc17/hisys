...
$smbrData = checkPostGet('smbrData', '');
$statDt = checkPostGet('statDt', '');
...
$qKap = $owlPDO->query($sKap) or die(print " Gagal: " . PDOException::getMessage());
$qKap->setFetchMode(PDO::FETCH_ASSOC);
while ($rKap = $qKap->fetch()) {
...
$qYkap = $owlPDO->query($sYkap) or die(print " Gagal: " . PDOException::getMessage());
$qYkap->setFetchMode(PDO::FETCH_ASSOC);
while ($rYkap = $qYkap->fetch()) {
...
switch ($proses) {
    ...
    case'getDetBrgKap':
       ...
       " . $dbname . ".bgt_kapital_vw where tahunbudget='" . $tahun . "' and jeniskapital='" . $klmpkbrg . "'";
        ...
        $qaresta = $owlPDO->query($aresta) or die(print " Gagal: " . PDOException::getMessage());
        $qaresta->setFetchMode(PDO::FETCH_ASSOC);
        while ($raresta = $qaresta->fetch()) {
