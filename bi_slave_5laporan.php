...
switch ($proses){
	case 'simpan':
		...
		if($namafile == ''){
			exit("Warning : Nama File harus diisi.");
		}
		...
		if($method == 'insert'){
			$idlap = getID();
			$str = "insert into ".$dbname.".bi_5laporan (idlap,tipe,namalaporan,namafile,updateby) values ('".$idlap."','".$tipelaporan."','".$namalaporan."','".$namafile."','".$_SESSION['standard']['userid']."')";
		
			try{
				$owlPDO->exec($str);
				foreach($_SESSION['mswarna'] as $row){
					$str = "insert into ".$dbname.".bi_5warnalaporan (idlap,opawal,nilaiawal,opakhir,nilaiakhir,warna,keterangan) VALUES ('".$idlap."','".$row['operationawal']."','".$row['nilaiawal']."','".$row['operationakhir']."','".$row['nilaiakhir']."','".$row['kodefill']."','".$row['keterangan']."')";
					try{
						$owlPDO->exec($str);
			...
			$str = "delete from ".$dbname.".bi_5warnalaporan where idlap = '".$idlap."'";
			try{
				$owlPDO->exec($str);
			}catch(PDOException $e){
				echo "error : ".$e->getMessage();
			...
			$str = "update ".$dbname.".bi_5laporan set tipe='".$tipelaporan."', namalaporan = '".$namalaporan."', namafile = '".$namafile."', updateby = '".$_SESSION['standard']['userid']."' where idlap = '".$idlap."'";
			try{
				$owlPDO->exec($str);
	...
	case 'loaddata':
		...
		$str = "select * from ".$dbname.".bi_5laporan";
		...
		if($jlhbrs == 0){
			...
			while($bar = $res->fetch()){
				$result .= "<tr class=rowcontent>
					...
					<td>".$bar['idlap']."</td>
					<td>".ucwords($bar['tipe'])."</td>
					<td>".$bar['namalaporan']."</td>
					<td>".$bar['namafile']."</td>
					<td style='text-align:center'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar['idlap']."','".$bar['tipe']."','".$bar['namalaporan']."','".$bar['namafile']."')\">
					</td>
					<td style='text-align:center'>
						<img src='images/skyblue/delete.png' class='resicon' title='Delete' onclick=\"deletefield('".$bar['idlap']."')\">
					</td>
					<td style='text-align:center'>
						<img src='images/skyblue/zoom.png' class='resicon' title='Detail' onclick=\"detailfield('".$bar['idlap']."',event)\">
					</td>
				</tr>";
			}
		}
	...
	case 'fillfield':
		$result = "";
		$_SESSION['mswarna'] = array();
		$str = "select * from ".$dbname.".bi_5warnalaporan where idlap = '".$idlap."' order by idwarna";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$no = -1;
		while($bar = $res->fetch()){
	...
	case 'deletefield':
		$str = "delete from ".$dbname.".bi_5laporan where idlap = '".$idlap."'";
		try{
			$owlPDO->exec($str);
		}catch (PDOException $e){
			echo "error : ".$e->getMessage();
...
function getID(){
	...
	$str = "select idlap from ".$dbname.".bi_5laporan order by idlap DESC LIMIT 1";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$numrow=owlBaris($res);
	...
	if($numrow == 0){
		...
		$bar = $res->fetch();
		$expNo = explode('LAP',$bar['idlap']);
		$idno = 'LAP'.addZero(intval($expNo[1]) + 1,7);
	}
