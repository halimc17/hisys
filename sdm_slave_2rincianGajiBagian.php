...
$periode=explode('-',$periodeGaji);
$kdBag = checkPostGet('kdBag','');
$sisGaji = checkPostGet('sisGaji','');
...
switch($proses)
{
        ...
        foreach($kodeBagian as $hslBrs => $hslAkhir)
        {
			...
			$qGetData=$owlPDO->query($sGetKary) or die(print " Gagal: ".PDOException::getMessage());
			$qGetData->setFetchMode(PDO::FETCH_ASSOC);
			$rowData=owlBaris($qGetData);
            $tmpRow=$rowData-1;
            if($tmpRow!=-1)
            {

            while($rData=  $qGetData->fetch())
            {
