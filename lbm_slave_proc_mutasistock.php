...
for ($i = 1; $i <= 12; $i++) {
    ...
    while ($rData = $qData->fetch()) {
        ...
        $rData['hargarata'] = isset($rData['hargarata']) ? $rData['hargarata'] : '0';
        if ($i == 1) {
        ...
        if ($pilih == 'nilai')
            if ($mayor == '')
                $resData[$rData['kodebarang']]['totem']+=$rData['qtymasuk'] * $rData['hargarata'];
        @$resData[$rData['kodebarang']]['tokel']+=$rData['qtykeluar'];             // total keluar
        if ($pilih == 'nilai')
            if ($mayor == '')
                $resData[$rData['kodebarang']]['tokel']+=$rData['qtykeluar'] * $rData['hargarata'];
