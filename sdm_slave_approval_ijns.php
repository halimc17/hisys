...
case 'getdetail':
	case'IJNS':
		$tab.="<fieldset>
            ...
            if ($lvlapr!=0) {
                ...
                $countApp = getCountApproval('IJNS');
            }
		...
		$tab.="</tr>
            ...
            $str = "select * from ".$dbname.".approval a
                ...
                where a.jenispersetujuan='IJNS' and a.status='0' and a.karyawanid='".$karyawanid."' order by a.tanggal asc";
                ...
                if ($showaction!=$level || $level==1) {
                    $tab.="<td style='text-align:center'>
                        <button class=mybutton onclick=\"getdataijns('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['approve']."</button>
                        </td>
                        <td style='text-align:center'>
                        <button class=mybutton onclick=\"tolakijns('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['ditolak']."</button>
                        </td>";
                } else {
                    $tab.="<td colspan=2>&nbsp;</td>";
                }
                ...
                for ($i = 1; $i <= $countApp; $i++) {
                    $arrDetail = detailApprove($i, $bar['notransaksi'], 'IJNS');
                    
                    $strpo="select * from ".$dbname.".setup_approval where jenispersetujuan='IJNS' and kodeunit='".$kodeorg."' and level='".$i."'";
	...
	case'get_form_approval_ijns':
		...
		$countApp = getCountApproval('IJNS',$koderorg);
			...
			foreach($resx as $keyx=>$valx){
				if($karyawanid==$valx['karyawanid']){
					if($i == $countApp){
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									<td colspan=3 align=center>
										<button id=Ajukan class=mybutton onclick=nextapprovalijns('approved') >Approved</button>
									</td>
								</tr>
							</table>
						</div>";
                        ...
                        a.karyawanid!='".$_SESSION['standard']['userid']."' and a.jenispersetujuan='IJNS' and a.level='".$level."' and a.kodeunit='".$koderorg."'  order by b.namakaryawan asc";
						...
						$tab.="<div id=test style=display:block>
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									...
									<td colspan=2></td>
									<td>
										<button class=mybutton onclick=nextapprovalijns() title=\" Submit to the next level\" id=Ajukan >".$_SESSION['lang']['diajukan']."</button>
									</td>
								</tr>
							</table>
	...
	case 'insert_nextapprovalijns':
		...
		if ($bar['approval'] == 1 || $bar['approval'] == 2 ) {
			...
			$arrDetail = detailApprove($kolom, $notransaksi, 'IJNS');
			$level = $kolom + 1;
			...
			if ($kolom != $countApp) {
				if ($userid == $arrDetail['karyawanid']) {
					...
					if($tipeapp=='1'){
						...
						$str = "insert into ".$dbname.".approval values ('','".$notransaksi."','IJNS','".$level."','".$userid."','0','','','')";
						$owlPDO->exec($str);
					}
						...
						foreach ($arrDate as $dateCuti) {
							...
							if (count($qLibur) > 0) {
								...
								if ($cekAbsensi == 0) {
									...
									$nilaiHK = $hkIjin[$idjenis];
									$umr = getUpahKary(substr($dateCuti, 0, 7), $karyawancuti) * $hkIjin[$idjenis];
									...
									$cekAbsensidt = getCountRows($dbname, "sdm_absensidt", "kodeorg='" . $subbagian . "' AND tanggal='" . $dateCuti . "' and karyawanid ='".$karyawancuti."'");
									if($cekAbsensidt > 0){
										exit("Warning : Karyawan sudah ada di absensi pada tanggal = '".$dateCuti."' ");
									}
	...
	echo"<div id=rejected_form>
		...
		<tr>
            <td colspan=3 align=center>
	            <button class=mybutton onclick=\"inserttolakijns(".$_POST['kolom'].")\" >".$_SESSION['lang']['ditolak']."</button>
		    </td>
        </tr>
        </table>
		</div>";
	break;
	case 'inserttolakijns':        
        $sDat = selectQuery($dbname,"datakaryawan","bagian,kodegolongan,lokasitugas", "karyawanid = '".$_SESSION['standard']['userid']."'");
		$qDat = fetchData($sDat);
		$koderorg = $qDat[0]['lokasitugas'];
