...
switch($_POST['proses']) {
    case 'add' :
        ...
        unset($data['satuan']);
        $data['tipeanggaran']='Material';
        $data['topografi']='D1';
        $col=array("kodeorg","kodekegiatan","kelompok","kodebarang","tipeanggaran","topografi");
        # Make Query
        $query = insertQuery($dbname,'setup_kegiatannorma',$data,$col);
        //exit('warning:'.$query);
        # Insert Data
		try{
			$owlPDO->exec($query);
        ...
        foreach($field as $row) {
            if($row=='topografi') {
                ...
                $content .= "<td>".makeElement($row."_".$j,'txt','',
                    array('style'=>'width:200px','readonly'=>'readonly'))."</td>";
