...
$optTipe=makeOption($dbname, 'organisasi', 'kodeorganisasi,tipe');
    switch($method)
    {
            ...
            if($pilUn_1=='7')
            {
               $sUpdate="update ".$dbname.".bgt_blok set closed=0  where kodeblok like '".substr($unitId,0,4)."%' and tahunbudget='".$thnBudget."'";
               try{$owlPDO->exec($sUpdate); break;}catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n"; die(); }
            }
             if($pilUn_1=='8')
            {
            ...
            while($rData=$qData->fetch())
            {
                ...
                switch($pilUn_1)
                {
                    ...
                    $qharga=$owlPDO->query($sharga) or die(print " Gagal: ".PDOException::getMessage());
                    $qharga->setFetchMode(PDO::FETCH_ASSOC);
                    $rharga=$qharga->fetch();
            ...
            $qKegt=$owlPDO->query($skegt) or die(print " Gagal: ".PDOException::getMessage());
            $qKegt->setFetchMode(PDO::FETCH_ASSOC);
            while($rkegt= $qKegt->fetch())
            {
            ...
            if($optTipe[$unitId]=='KEBUN')
            {
                ...
                $qthn=$owlPDO->query($thn) or die(print " Gagal: ".PDOException::getMessage());
      ...
      if(strlen($_POST['nmBrg'])<4)
      {
        ...
        $qRupiah=$owlPDO->query($sRupiah) or die(print " Gagal: ".PDOException::getMessage());
        $qRupiah->setFetchMode(PDO::FETCH_ASSOC);
        $rRupiah=$qRupiah->fetch();
...
$qdt=$owlPDO->query($dt) or die(print " Gagal: ".PDOException::getMessage());
...
if($rdt['totrupiah']!=0){
    ...
    $strupdate="update ".$dbname.".bgt_distribusi set
                ...
                where kunci='".$idx."'
                   ";
    try{$owlPDO->exec($strupdate); }catch (PDOException $e) {print " Gagal  !: " . $e->getMessage(); }
}
?>