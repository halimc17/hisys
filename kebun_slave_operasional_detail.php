...
switch($proses) {
    ...
    case 'cegatKegiatan': // by dz March 11, 2014
        ...
        // $where = "blok = '".$kodeorg."' and periode <= '".substr($tanggal,0,7)."' 
		// and substr(periode,4,4) = '".substr($tanggal,0,4)."' and posting ='1'";
...
$whereKaryBukanPemanen = " and kodejabatan != '45'"; // selain pemanen
		...
		$cols = "kodekegiatan,kodeorg,hasilkerja,jumlahhk,upahkerja,upahpremi,keterangan";
		...
		foreach($dataShow as $key=>$row) {
			...
			if(!empty($data)){
					...
					$theForm2->addEls('kodeorg',$_SESSION['lang']['blok'],'','selectwithsearch','L',30,$optOrg);
			...
			$theForm2->addEls('keterangan',$_SESSION['lang']['keterangan'],'','text','L',50);
		...
		if(!empty($data)){
			...
			// $theForm1->_elements[1]->_attr['disabled'] = 'disabled';
		}
		else
		{
			$theForm1->addEls('nik',$_SESSION['lang']['nama'],'','selectsearch','L',30,$optKary);
		...
		$theForm1->_elements[3]->_attr['onchange'] = "getpremibkm()";
		...
		$formTab1->_noEnable = '##nourut##insentif##umr';
    ...
    case 'gatKarywanAFD':
        if($param['tipe']=='perpt')
        {
            ...
            $str="select karyawanid,namakaryawan,nik,subbagian from ".$dbname.".datakaryawan where lokasitugas in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['empl']['kodeorganisasi']."' and tipe='KEBUN') and statuskaryawan != 'Keluar' and (tanggalkeluar = '0000-00-00' or tanggalkeluar > ".$_SESSION['org']['period']['start'].")
                and tipekaryawan in('1','2','3','4','6')  order by namakaryawan";
				$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_OBJ);
			while($bar=$res->fetch())
			{
        ...
        else if($param['tipe']=='afdeling')
        {
			...
			$tab = "<fieldset>
				...
				<table class=sortable cellspacing=1 cellpadding=3 border=0>
					...
					<tbody>";
			$arrKarDetail = array();
			$str = "select t1.nourut, t2.nik, t1.absensi, t1.jhk, t1.umr, t1.insentif,t1.hasilkerja, t2.namakaryawan, t2.subbagian, t2.karyawanid from ".$dbname.".kebun_kehadiran t1 
			...
			while($bar = $res->fetch()){
				...
				$arrKarDetail[$bar['nik']]['hasilkerja'] = $bar['hasilkerja'];
			...
			while($bar = $res->fetch()){
				if($arrKarDetail[$bar['nik']]['nik'] == $bar['nik']){
					...
					$arrKarDetail[$bar['nik']]['hasilkerja'] = 0;
			...
			foreach($arrKarDetail as $key=>$val){
				...
				if($val['status'] == 0){
					$tab .= "<tr class=rowcontent>
						...
						<td style='text-align:right'>".($val['hasilkerja'] == 0 ? 0 : number_format($val['hasilkerja'],2))."</td>
					...
					$tab .= "<tr id=row_".$nourutSave." class=rowcontent>
						...
						<td style='text-align:right'><input style=width:75px type=text id=kehadiranumr_".$nourut." disabled class=myinputtextnumber onKeyPress=\"return angka_doang(event);\" value='".($val['umr'] == 0 ? 0 : number_format($val['umr'],2))."' onclick=\"opendisabled('".$nourut."')\" /></td>
						<td style='text-align:right'><input style=width:75px type=text id=kehadiranhasilkerja_".$nourut." onchange=getpremibkmkemandoran('".$nourut."') class=myinputtextnumber onKeyPress=\"return angka_doang(event);\" value='".($val['hasilkerja'] == 0 ? 0 : number_format($val['hasilkerja'],2))."' onclick=\"opendisabled('".$nourut."')\" /></td>
						<td style='text-align:right'><input style=width:75px type=text id=kehadiranpremi_".$nourut." disabled class=myinputtextnumber onKeyPress=\"return angka_doang(event);\" value='".($val['insentif'] == 0 ? 0 : number_format($val['insentif'],2))."' onclick=\"opendisabled('".$nourut."')\" /></td>
			...
			$tab .= "<tr>
				<td colspan=8 style='text-align:center'><button class=mybutton onclick=simpankehadiranAll('".$nourutSave."')>".$_SESSION['lang']['saveall']."</button></td>
			</tr>";
			$tab .= "</tbody>
				</table>
			</fieldset>";
       ...
       break;  
        ...
        case 'getDivisi':
           ...
           $whereOrg = " tipe='BIBITAN' and length(kodeorganisasi)>6 and left(kodeorganisasi,4)='".$param['chKdOrg']."'";
                        ...
                        $whereOrg = " kodeorganisasi in (select distinct kodeorg from ".$dbname.".setup_blok where left(kodeorg,4)='".$param['chKdOrg']."' and luasareaproduktif>0 ".$whereBlok.")
	...
	case 'simpankehadiran':
		...
		$kehadiranhasilkerja = $param['kehadiranhasilkerja'];
		$kehadiranpremi = $param['kehadiranpremi'];
		$nik = $param['nik'];
		$notransaksi = $param['notransaksi'];
		$hasilkerja = $param['kehadiranhasilkerja'];
		...
		$str = "select nilaihk from ".$dbname.".sdm_absensidt_vw where karyawanid='".$param['nik']."' and tanggal='".$param['tanggal']."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hkabs=$bar['nilaihk'];
        ...
        if(($datr['jum']+$param['kehadiranhk'])>1)
        {
            $not='';
			$jhk='';
		...
		#cek mandor
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor='".$param['nik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek mandor1
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor1='".$param['nik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek kerani
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where keranimuat='".$param['nik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek nikasisten
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikasisten='".$param['nik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
			
		if($jumtrans>0 and ($param['kehadiranumr']>0 || $param['kehadiranhk']>0)){
		...
		#cek vhc kegiatan
		$jlhvhc='';
		$str = "select count(*) as jlhvhc from ".$dbname.".vhc_runhk where idkaryawan='".$param['nik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jlhvhc+=$bar['jlhvhc'];
		...
		if($nowJlhHk > 0){
			...
			if(($kehadiranhk == 0 || $kehadiranhk == "") && ($kehadiranpremi == 0 || $kehadiranpremi == "")){
				...
				$str = "insert into ".$dbname.".kebun_kehadiran (notransaksi,nourut,nik,absensi,jhk,umr,insentif,hasilkerja) 
				values ('".$notransaksi."','".$noUrut."','".$nik."','H','".$kehadiranhk."','".$kehadiranumr."','".$kehadiranpremi."','".$kehadiranhasilkerja."')";
			}
		}
		try{
			$owlPDO->exec($str); 
	...
	case 'simpankehadiranAll':
		...
		$kehadiranhasilkerja = checkPostGet('kehadiranhasilkerja','');
		...
		//cek apakah sudah ada HK di sdm_absensidt
		$str = "select nilaihk from ".$dbname.".sdm_absensidt_vw where karyawanid='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hkabs=$bar['nilaihk'];
		...
		#==============periksa apakah sudah ada kehadiran pada hari yang sama
        $str="select sum(jhk) as jum from ".$dbname.".kebun_kehadiran_vw where tanggal='".$tanggal."'
              and karyawanid='".$param['kehadirannik']."' group by karyawanid";
        $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
        $res->setFetchMode(PDO::FETCH_ASSOC);
        $datr=$res->fetch();
      
        if(($datr['jum']+$kehadiranhk)>1)
        {
            $not='';
			$jhk='';
            $str="select * from ".$dbname.".kebun_kehadiran_vw where tanggal='".$tanggal."'
                  and karyawanid='".$param['kehadirannik']."'";
            $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
            $res->setFetchMode(PDO::FETCH_OBJ);
			echo "Karyawan tersebut sudah memiliki absen lebih dari satu HK :\n";
            while($bar=$res->fetch())
		...
		#cek mandor
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek mandor1
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor1='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek kerani
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where keranimuat='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
		...
		#cek nikasisten
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikasisten='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jumtrans+=$bar['jumkar'];
			
		if($jumtrans>0 and ($kehadiranumr>0 || $kehadiranhk>0)){
			exit("Warning:Upah karyawan sudah terdaftar sebagai mandor/mandor1/kerani, hanya diperbolehkan menginput premi saja dengan umr harus di-0-kan");
		}
		
		#cek vhc kegiatan
		$jlhvhc='';
		$str = "select count(*) as jlhvhc from ".$dbname.".vhc_runhk where idkaryawan='".$param['kehadirannik']."' and tanggal='".$tanggal."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$jlhvhc+=$bar['jlhvhc'];
		...
		#cari hasilkerja di kehadiran / sudah disave
		$strorg="select * from ".$dbname.".kebun_kehadiran where notransaksi = '".$param['notransaksi']."' and nik != '".$param['kehadirannik']."'";
		$queorg=$owlPDO->query($strorg) or die(print " Gagal: ".PDOException::getMessage());
		$queorg->setFetchMode(PDO::FETCH_ASSOC);                   
		while($roworg=$queorg->fetch()){
			$hslkerja_hdr+=$roworg['hasilkerja'];	
		...
		#cek apakah hasil kerja di prestasi vs kehadiran
		if(($kehadiranhasilkerja+$hslkerja_hdr)>$hslkerja_pr){
			exit("Error: Hasil kerja melebihi jumlah Hasil kerja di Tab Prestasi :\nPrestasi => ".$hslkerja_pr." Kehadiran => ".($hasilkerja+$hslkerja_hdr)." Varian => ".($hslkerja_pr - ($hasilkerja+$hslkerja_hdr))."");
		}
		...
		// exit("error : ".$str);
		if($nowJlhHk > 0 || $kehadiranpremi>0){
			if(($kehadiranhk == 0 || $kehadiranhk == "") && ($kehadiranpremi == 0 || $kehadiranpremi == "")){
				$str = "delete from ".$dbname.".kebun_kehadiran where notransaksi = '".$notransaksi."' and nik = '".$nik."'";				
			}else{
				$str = "update ".$dbname.".kebun_kehadiran 
				set jhk = '".$kehadiranhk."', umr = '".$kehadiranumr."', insentif = '".$kehadiranpremi."',hasilkerja='".$kehadiranhasilkerja."' 
	...
	case'getpremibkm':
		...
		#cari kodekegiatan
		$str = "select kodekegiatan, hasilkerja from ".$dbname.".kebun_prestasi where notransaksi = '".$notransaksi."'";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar = $res->fetch();
			$kodekegiatan=$bar['kodekegiatan'];
		...
		#cari basis
		$str = "select basis,premibasis,premilebihbasis from ".$dbname.".kebun_5premibkm where kodekegiatan = '".$kodekegiatan."' and unit='".$unit."'";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$row=$res->rowCount();
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar = $res->fetch();
			...
			$premilebihbasis=$bar['premilebihbasis'];
			
		#Ambil UMR
		$str = "select * from ".$dbname.".sdm_5gajipokok where karyawanid='".$karyawanid."' and tahun='".substr($tgl,0,4)."' and idkomponen='1'"; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$umrperhari+=$bar['jumlah']/25;
		
		if($umrperhari==''){
			exit("Error : Gaji Pokok untuk tahun ".substr($tgl,0,4)." belum ada !.$str");
		}
		
		#cek sudah ada HK belum, jika sudah pernah dapat HK maka pek selanjutnya tanpa basis
		$hksebelumnya='';
		$rpsebelumnya='';
		#Rawat
		$str = "select * from ".$dbname.".kebun_kehadiran_vw where karyawanid='".$karyawanid."' and tanggal='".$tgl."' "; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['jhk'];
			$rpsebelumnya+=$bar['umr'];
			
		#Panen
		$str = "select * from ".$dbname.".kebun_prestasi_vs_hk where karyawanid='".$karyawanid."' and tanggal='".$tgl."' "; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['hkpanenperhari'];
			$rpsebelumnya+=$bar['tupah'];
			
		#Traksi
		$str = "select * from ".$dbname.".vhc_runhk_vw where idkaryawan='".$karyawanid."' and tanggal='".$tgl."' "; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['hk'];
			$rpsebelumnya+=$bar['upah'];
		
		#SDM
		$str = "select * from ".$dbname.".sdm_absensidt where karyawanid='".$karyawanid."' and tanggal='".$tgl."' "; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['hk'];
		...
		#cek mandor
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor='".$karyawanid."' and tanggal='".$tgl."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['jumkar'];
		
		#cek mandor1
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikmandor1='".$karyawanid."' and tanggal='".$tgl."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['jumkar'];
		...
		#cek kerani
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where keranimuat='".$karyawanid."' and tanggal='".$tgl."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['jumkar'];
		...
		#cek nikasisten
		$str = "select count(*) as jumkar from ".$dbname.".kebun_aktifitas where nikasisten='".$karyawanid."' and tanggal='".$tgl."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$hksebelumnya+=$bar['jumkar'];
		
		if($row>0){//jika ada dalam setup
			if($libur==true){//jika hari libur
				$premi=$hasilkerja*$premilebihbasis;
			}else{
				if($hksebelumnya!=''||$rpsebelumnya>=$umrperhari){
					$premi=$hasilkerja*$premilebihbasis;
				}else{
					$lebihbasis=$hasilkerja-$basis;
					if($lebihbasis>0){
						$premi=$lebihbasis*$premilebihbasis;
					}else{
						$premi=0;
					}
				}
	...
	case'getstpremi':
		...
		$str = "select kodekegiatan from ".$dbname.".kebun_prestasi where notransaksi='".$notransaksi."'  ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
			$kegsimpan=$bar['kodekegiatan'];
		...
		$str = "select premi from ".$dbname.".setup_kegiatan where kodekegiatan='".$kdkeg."'  ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
