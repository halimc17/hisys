...
switch ($proses) {
    ...
    case'loadData':
        ...
        $ql2 = "select count(a.karyawanid) as jmlhrow from " . $dbname . ".sdm_ijin a, " . $dbname . ".datakaryawan b
		where a.karyawanid = b.karyawanid and a.tanggal between '" . $pAwal . "' and '" . $pAkhir . "' 
		and a.idjenis like '%" . $jnsCuti . "%' " . $tmbWhere . " order by a.tanggal desc";
		//echo $ql2;
        ...
        $slvhc = "select a.* from " . $dbname . ".sdm_ijin a, " . $dbname . ".datakaryawan b where a.karyawanid = b.karyawanid and a.tanggal between '" . $pAwal . "' and '" . $pAkhir . "' and a.idjenis like '%" . $jnsCuti . "%' " . $tmbWhere . " order by a.tanggal desc limit " . $offset . "," . $limit . " ";
        ...
        break;
    case'appSetuju':
        $sket = "select distinct idjenis,stpersetujuan1,persetujuan1,hrd,tanggal from " . $dbname . ".sdm_ijin where " . $where . "";
		...
		if ($stat == 2) {
	        $str = "update " . $dbname . ".sdm_ijin  set stpersetujuanhrd='2', komenst2= 'Di tolak oleh Atasan 1 ".$nmkar[$_SESSION['standard']['userid']]."' where " . $where . "";
			try {
            $owlPDO->exec($str);
				} catch (PDOException $e) {
	...
	case'appSetuju4':
        $sket = "select distinct idjenis,stpersetujuan4,persetujuan4,hrd,tanggal from " . $dbname . ".sdm_ijin where " . $where . "";
		$qKet=$owlPDO->query($sket) or die(print " Gagal: ".PDOException::getMessage());
		$qKet->setFetchMode(PDO::FETCH_ASSOC);
        $rKet = $qKet->fetch();
        ...
        $sUpdate = "update " . $dbname . ".sdm_ijin  set stpersetujuan4='" . $stat . "',komenst4='" . $ket . "' where " . $where . "";
		try{
			$owlPDO->exec($sUpdate); 
		...
		if ($stat == 2) {
	        $str = "update " . $dbname . ".sdm_ijin  set stpersetujuanhrd='2', komenst2= 'Di tolak oleh Atasan 2 ".$nmkar[$_SESSION['standard']['userid']]."' where " . $where . "";
			try {
            $owlPDO->exec($str);
				} catch (PDOException $e) {
					print " Gagal  !: " . $e->getMessage() . "\n";
    ...
    case 'appSetujuHRD':
        $sket = "select distinct darijam,sampaijam,jumlahhari,idjenis,stpersetujuanhrd,hrd,tanggal,periodecuti from " . $dbname . ".sdm_ijin where " . $where . "";
		$qKet=$owlPDO->query($sket) or die(print " Gagal: ".PDOException::getMessage());
		$qKet->setFetchMode(PDO::FETCH_ASSOC);
        $rKet = $qKet->fetch();
        ...
        if ($stat == 1) 
		{
			...
			$optCuti = makeOption($dbname,'sdm_5jenisijin','idjenis,nilaihk',"idjenis='".$rKet['idjenis']."'");
			$cekHkCuti = $optCuti[$rKet['idjenis']];

            if ($cekHkCuti == '0') 
			{
				//insert to cuti
                $str = "insert into " . $dbname . ".sdm_cutidt (kodeorg,karyawanid,periodecuti,daritanggal,sampaitanggal,jumlahcuti,keterangan)
				values
				('" . $kodeorg . "'," . $krywnId . ",'" . $rKet['periodecuti'] . "','" . substr($rKet['darijam'], 0, 10) . "','" . substr($rKet['sampaijam'], 0, 10) . "'," . $rKet['jumlahhari'] . ",'" . $rKet['idjenis'] . "')";
				try
				{
					$owlPDO->exec($str); 
    ...
    case'prevPdf':
        ...
        while ($bar = $res->fetch()) {
            $strc = "select a.namakaryawan,a.karyawanid,a.bagian,b.namajabatan,a.nik,a.lokasipenerimaan,a.kodeorganisasi,a.lokasitugas from " . $dbname . ".datakaryawan a left join  " . $dbname . ".sdm_5jabatan b on a.kodejabatan=b.kodejabatan where a.karyawanid=" . $bar->karyawanid;
            ...
            $alamatcuti = $bar->alamatcuti;
            $nohp = $bar->nohp;
            $statuspersetujuan_cancel = $bar->statuspersetujuan_cancel;
            $statuspersetujuan = $bar->statuspersetujuan;
            $alasanbatal = $bar->alasanbatal;

            if ($bar->darijamreal=='0000-00-00 00:00:00' || $bar->sampaijamreal=='0000-00-00 00:00:00') {
                $drjm=$jmDr;
                $smpjam=$jmSmp;
            }
            else
            {
                $drjm=$bar->darijamreal;
                $smpjam=$bar->sampaijamreal;
            }
            $tglAwal = explode("-",$jmDr);
            $tglSmp = explode("-",$jmSmp);
            ...
            $potongatautidak = makeOption($dbname, 'sdm_5jenisijin', 'idjenis,statuspotongan');
            ...
            $perbagian4 = '';
            $pernama4 = '';
            $strf = "select a.bagian,b.namajabatan,a.namakaryawan from " . $dbname . ".datakaryawan a left join " . $dbname . ".sdm_5jabatan b on a.kodejabatan=b.kodejabatan where karyawanid=" . $persetujuan4;
            $resf=$owlPDO->query($strf) or die(print " Gagal: ".PDOException::getMessage());
            $resf->setFetchMode(PDO::FETCH_OBJ);
            while ($barf = $resf->fetch()) {
        ...
        $pdf->ln(10);
        $pdf->Cell(50, 5,'Dengan ini saya mengajukan permohonan cuti : ', 0,1, 'J');
        $pdf->SetFont('Arial', '', 10);
        $pdf->SetX(20);
        $pdf->Cell(8, 6,'V', 1, 0, 'C');
        $pdf->Cell(65, 8,getnamajeniscuti($jnsCuti,$datajeniscuti), 0, 0, 'L');

        $pdf->ln(15);
        $pdf->Cell(50, 5,"Keperluan", 0, 'J');
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        $pdf->Cell(120, 5,$kperluan, 'B',1, 'J');
        $pdf->Cell(50, 5,"No HP", 0, 'J');
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        $pdf->Cell(120, 5,$nohp, 'B',1, 'J');
        $pdf->Cell(50, 5,"Alamat Cuti", 0, 'J');
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        $pdf->Cell(120, 5,$alamatcuti, 'B',1, 'J');
        ...
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        $pdf->Cell(120, 5,$drjm, 'B',1, 'J');
        $pdf->Cell(50, 5,"Tanggal & Waktu Berangkat", 0, 'J');
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        $pdf->Cell(120, 5,$tglberangkat, 'B',1, 'J');
        ...
        $pdf->Cell(50, 5,"Periode Cuti", 0, 'J');
        $pdf->Cell(5, 5," : ", 0,0, 'J');
        ...
        $pdf->SetX(150);
        $pdf->Cell(40, 5,"Paraf ", 'B',1, 'C');
        $awaly=$pdf->GetY();
        $pdf->Cell(140, 5,"Catatan / Verifikasi Personalia : ", 'R',1, 'J');

        $str="select min(notransaksi) as notrans from ".$dbname.".sdm_ijin where karyawanid='".$karyawanid."' and statuspersetujuan = '1' and periodecuti='".$periode."' ";
        $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
        $res->setFetchMode(PDO::FETCH_ASSOC);
        $bar = $res->fetch();
        $ygpertama = $bar['notrans'];

        #mahe Cek cuti non staff
        $str="select sum(jumlahhari) as jumlahhari from ".$dbname.".sdm_ijinnonstaff where karyawanid='".$karyawanid."' and periodecuti='".$periode."' and idjenis='CUTI08'";
        $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
        $res->setFetchMode(PDO::FETCH_ASSOC);
              $bar = $res->fetch();
         
         $sisanonstaff +=$bar['jumlahhari'];
        ...
        foreach ($res as $data) {
            if($data['statuspotongan'] == '0'){
                $jlhhari = $jumlahhari;
                $sisacutiper = $sisaprdsblmnya;
            }else{
                $jlhhari = $jumlahhari;
                $sisacutiper = $sisaprdsblmnya-$jumlahhari;
            }
        }
        ...
        $sisatotal = $sisaprdsblm - $jumlahhari;
        ...
        $arr_jenis_pemotong = makeOption($dbname,'sdm_5jenisijin','idjenis,idjenis',"statuspotongan = 0");
        $note = "";
        #jika cuti bukan pemotong ya jangan di potong dong
        if(in_array($jnsCuti,$arr_jenis_pemotong)) {
            $sisatotal =$sisaprdsblm;
            $note = "*Jenis ijin tidak memotong hak cuti";
        }
        if ($statuspersetujuan==2) {
            $sisatotal = $sisaprdsblm;
            $note = "*Cuti Ditolak dan tidak memotong hak cuti";
        }
        if ($statuspersetujuan_cancel==1) {
            $sisatotal = $sisaprdsblm;
            $note = "*Ijin dibatalkan dan tidak memotong hak cuti";
        }
        ...
        $pdf->Cell(5, 5, " ", 0, 0, 'C');
        $pdf->Cell(63, 5, "hari", 'R', 1, 'L');
        $pdf->Cell(50, 5, "Rencana Pengambilan Cuti ", 0, 0, 'L');
        $pdf->Cell(6, 5, " = ", 0, 0, 'L');
        $pdf->Cell(16, 5,$jumlahhari, 'B', 0, 'R');
        ...
        $pdf->Cell(11, 5, " = ", 0, 0, 'R');
        $pdf->Cell(16, 5,$sisatotal, 0, 0, 'R');
        $pdf->Cell(5, 5, " ", 0, 0, 'C');
        $pdf->Cell(63, 5, "hari", 'R', 1, 'L');
        $pdf->SetFont('Arial', 'I', 10);
        $pdf->Cell(63, 5, $note, 0, 0, 'R');
        $akhiry=$pdf->GetY();
        $pdf->Line(190, $awaly,190, $akhiry);
        $pdf->Line(150, $akhiry,190, $akhiry);

        // $pdf->ln(7);
        // $pdf->Cell(40, 5,"Kolom Persetujuan : ", 0,1, 'L');
        // $pdf->Cell(100, 5, "Pemohon,", 0, 0, 'L');
        // $pdf->Cell(40, 5,"Disetujui,", 0,1, 'J');
        // $pdf->ln(20);
        // $pdf->SetFont('Arial', '', 10);
        // $pdf->Cell(100, 5, "Karyawan", 0, 0, 'L');
        // $pdf->Cell(40, 5,"Atasan", 0,1, 'J');
        // $pdf->ln(15);
		...
		$pdf->Ln(5);
        if($statuspersetujuan_cancel!=0)
        {
            $pdf->Cell(40, 5,"Alasan Pembatalan : ", 0,1, 'L');
            $pdf->SetFont('Arial', '', 10);
            $pdf->Cell(170, 5, $alasanbatal, 0,1, 'L');
            $pdf->Ln(7);
        }
		...
		if(count($res)){
			...
			foreach($res as $val){
				if($stf==0){
					...
					$detailApprove=detailApprove($val['level'],$nomortrans,'IJS');
                    $detailApprove2=detailApprove($val['level'],$nomortrans,'IJSC');   					
				}
        ...
        //footer================================
        $pdf->Ln();
		$namafile=checkPostGet('namafile','0');
		if($namafile=='0'){
			$pdf->Output();
		}else{
			$pdf->Output($namafile);
    ...
    case'getExcel':
        ...
        $slvhc = "select a.* from " . $dbname . ".sdm_ijin a, " . $dbname . ".datakaryawan b where a.karyawanid = b.karyawanid and a.tanggal between '" . $pAwal . "' and '" . $pAkhir . "' and a.idjenis like '%" . $jnsCuti . "%' " . $tmbWhere . " order by a.tanggal desc ";
		$qlvhc=$owlPDO->query($slvhc) or die(print " Gagal: ".PDOException::getMessage());
		$qlvhc->setFetchMode(PDO::FETCH_ASSOC);
