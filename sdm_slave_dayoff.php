...
use Dompdf\Dompdf;
	...
	switch ($method) {	
		...
		case 'loadData':
	        ...
	        $where = "a.notransaksi like '%/STAFF/%' and a.karyawanid='".$_SESSION['standard']['userid']."'";
	        $footd = "";
	            ...
	            while($bar=$res->fetch()){
					...
					for($i=1;$i<=$countApp;$i++){
	                    ...
	                    if ($bar->status==0 and $stat==0){
							$tab.="<td align=center>
									<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"editdt('".$bar->notransaksi."','".tanggalnormal($bar->tanggalpengajuan)."','".tanggalnormal($bar->tanggalmulai)."','".$bar->jumlahharidayoff."', `".$bar->keterangan."`)\">
									</td>
		...
		case 'viewpdf':
			...
			# datakaryawan dayoff
			$sKaryawan = selectQuery($dbname, 'datakaryawan', '*', "karyawanid='".$rDayoff[0]['karyawanid']."'");
			$rKaryawan = fetchData($sKaryawan);
			...
			$url_explode = explode('/', $server.$uri);
			$url = $url_explode[0].'/'.$url_explode[1];
			$namapt = $arrHead['nama'];
			...
			$dokumen .= '.header-content {margin-left: 10px;margin-bottom: 5px;}';
			...
			$dompdf->render();
			$dompdf->stream('Dayoff_Staff.pdf', array('Attachment' => 0));
		break;
		...
		case 'insertht':
				...
				// $str="select notransaksi from ".$dbname.".sdm_dayoff_dt_vw where karyawanid='".$_SESSION['standard']['userid']."' and status!='2' 
				// and (tanggaldayoff>='".$tglAwal."' and tanggaldayoff<='".$tglEnd."')   order by notransaksi desc";
				$str="select notransaksi from ".$dbname.".sdm_dayoff_dt_vw where karyawanid='".$_SESSION['standard']['userid']."' and status!='2' and tanggaldayoff = '".$tglAwal."' order by notransaksi desc";
				$bar = fetchData($str);
				if(count($bar)>0){
				...
				$notransaksi=generatenotrans();
					...
					// $str = "insert into ".$dbname.".sdm_dayoff (notransaksi,karyawanid,tanggalpengajuan,tanggalmulai,tanggalsampai,tanggalkerja,jumlahharidayoff,keterangan,status,createdby, createdat) values ('".$notransaksi."','".$_SESSION['standard']['userid']."','".$tanggalpengajuan."','".$tglAwal."','".$tglEnd."','".$tanggalkerja."','".$jmldayoff."','".$keterangan."','0', '".$_SESSION['standard']['userid']."', '".$createdat."')";
					$str = "insert into ".$dbname.".sdm_dayoff (notransaksi,karyawanid,tanggalpengajuan,tanggalmulai,jumlahharidayoff,keterangan,status,createdby, createdat) values ('".$notransaksi."','".$_SESSION['standard']['userid']."','".$tanggalpengajuan."','".$tglAwal."','".$jmldayoff."','".$keterangan."','0', '".$_SESSION['standard']['userid']."', '".$createdat."')";
					// echo $str;
					// exit('error');
					try{
						$owlPDO->exec($str); 

						// $rangetgl = rangeTanggal($tglAwal,$tglEnd);
						// foreach($rangetgl as $tglx){
						//     $date1 = new DateTime($tglx);
						//     $date2 = new DateTime($tglx);
						// 	$date1->modify('+2 month');
						// 	$date2->modify('last day of second month');
						// 	if($date1>=$date2)
						// 	{
						// 		$strdt = "insert into ".$dbname.".sdm_dayoff_dt (notransaksi,karyawanid,tanggaldayoff,tanggalberlakusampai,jumlahharidayoff) values ('".$notransaksi."','".$_SESSION['standard']['userid']."','".$tglx."','".$date2->format('Y-m-d')."','1')";
						// 		//echo $date2->format('Y-m-d');

						// 	}
						// 	else
						// 	{
						// 		$strdt = "insert into ".$dbname.".sdm_dayoff_dt (notransaksi,karyawanid,tanggaldayoff,tanggalberlakusampai,jumlahharidayoff) values ('".$notransaksi."','".$_SESSION['standard']['userid']."','".$tglx."','".$date1->format('Y-m-d')."','1')";
						...
						$strdt = "insert into ".$dbname.".sdm_dayoff_dt (notransaksi,karyawanid,tanggaldayoff,tanggalberlakusampai,jumlahharidayoff) values ('".$notransaksi."','".$_SESSION['standard']['userid']."','".$tglAwal."','".$berlakusampai."','1')";
	    ...
	    case'deleteht':
	        ...
	        try {
				...
				$strdelapp="delete from ".$dbname.".approval where jenispersetujuan='DOF' and notransaksi='".$notransaksi."'";
				try {
	    ...
	    case 'updateht':
	    		...
	    		// $str="select notransaksi from ".$dbname.".sdm_dayoff_dt_vw where karyawanid='".$_SESSION['standard']['userid']."' and status!='2' and (tanggaldayoff>='".$tglAwal."' and tanggaldayoff<='".$tglEnd."')   order by notransaksi desc";
	    		$str="select notransaksi from ".$dbname.".sdm_dayoff_dt_vw where karyawanid='".$_SESSION['standard']['userid']."' and status!='2' and tanggaldayoff = '".$tglAwal."' order by notransaksi desc";
				$bar = fetchData($str);
				if(count($bar)>0){
					exit('Warning : untuk tanggal day off yang dipilih sudah ada dalam tahap pengajuan atau telah disetujui , silahkan pilih tanggal lain');
	        ...
	        $strht = "update ".$dbname.".sdm_dayoff set tanggalpengajuan='".$tanggalpengajuan."',tanggalmulai='".$tglAwal."',tanggalsampai='".$tglEnd."',tanggalkerja='".$tanggalkerja."',jumlahharidayoff='".$jmldayoff."', keterangan='".$keterangan."' where notransaksi='".$notransaksi."'";
	...
	function generatenotrans(){
		...
		$tempno=substr($tanggalpengajuan,0,6)."/"."STAFF"."/".$_SESSION['empl']['lokasitugas'];
