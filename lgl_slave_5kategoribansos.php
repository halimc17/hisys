...
case 'getKode':
        ...
        $str = "select max(right(kode,2)) as nomorurut  from " . $dbname . ".lgl_kategoribansos where akun ='".$noakun."' order by right(kode,2) desc";
...
case 'update':	
	$str="update ".$dbname.".lgl_kategoribansos set nama='".$namakegiatan."'
	...
	break;
case 'insert':
	$str="insert into ".$dbname.".lgl_kategoribansos (jenis,kode,akun,nama)
	...
	break;
case 'delete':
	$str="delete from ".$dbname.".lgl_kategoribansos 
	where kode='".$kodekegiatan."'";
        try{$owlPDO->exec($str); }
catch (PDOException $e) {
    print " Gagal  !: " . $e->getMessage() . "\n"; 
