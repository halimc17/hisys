...
switch($proses){
    ...
    case'savedata':
		...
		$row=count($res);
		if($row>0){
			exit("Error : Transaksi tanggal ".tanggalnormal($tglpnn)." sudah ada");
		}
	...
	case'loaddata':
		...
		#cek jurnal
		$str = "select nojurnal, noreferensi, sum(debet) as rpj from ".$dbname.".keu_jurnaldt_vw where noreferensi like '%/PNN02/%' ".$whj." group by nojurnal, noreferensi";
		$res    =fetchData($str);
		foreach($res as $val){			
        ...
        while ($bar = $res->fetch()) {
			...
			// $strmin="select min(tanggalpanen) as tglawal,max(tanggalpanen) as tglakhir from ".$dbname.".kebun_3premipemanen where notransaksi='".$bar['notransaksi']."'"; 
			// $barmin=fetchData($strmin);
			// $tglmin = tanggalnormal($barmin[0]['tglawal']);
			// $tglmax = tanggalnormal($barmin[0]['tglakhir']);
            ...
            $tab.="<td align=center>".$tglmin[$bar['notransaksi']]." - ".$tglmax[$bar['notransaksi']]."</td>";
			...
			$lsupdate=makeOption($dbname,'kebun_3premipemanen','notransaksi,updateby',"notransaksi='".$bar['notransaksi']."' and periode='".$bar['periode']."' and tahap='".$bar['tahap']."'");
			
            $tab.="<td>" . getNamaKaryawan($lsupdate[$bar['notransaksi']]) . "</td>";
			...
			if ($bar['posting'] == 0) {
                $isi.="<td align=center width=20px><img src=images/application/application_delete.png class=zImgBtn  title='Delete' 
                    onclick=\"del('".$bar['notransaksi']."','".$bar['periode']."','".$bar['kodeorg']."','".$tglmin."','".$tglmax."');\" ></td>";
				$post='';
