...
$nPel = $owlPDO->query($iPel) or die(print " Gagal: " . PDOException::getMessage());
$nPel->setFetchMode(PDO::FETCH_ASSOC);
while ($dPel = $nPel->fetch()) {
    $optPel.="<option value='" . $dPel['kodecustomer'] . "'>" . $dPel['namacustomer'] . "</option>";
}
OPEN_BOX('','<span class=judul>'.getMenu('pmn_2suratperintahpengiriman').'</span>');
...
echo"<td><input type=text class=myinputtext id=tanggaldari readonly onmousemove=setCalendar(this.id) onkeypress=return false;  style=width:70px;  maxlength=10 /></td>";
// echo"</tr>";
echo"<td>" . $_SESSION['lang']['sd'] . "</td>
          <td><input type=text class=myinputtext id=tanggalsampai readonly onmousemove=setCalendar(this.id) onkeypress=return false;  style=width:70px;  maxlength=10 /></td>
          </tr>";

echo"<tr><td>" . $_SESSION['lang']['komoditi'] . "</td>
          <td colspan=3><select id=komoditi style=width:175px;>" . $optBar . "</select></td>
          </tr>";

echo"<tr><td>" . $_SESSION['lang']['penjual'] . "</td>
          <td colspan=3><select id=penjual style=width:175px;>" . $optPel . "</select></td>
          </tr>";
