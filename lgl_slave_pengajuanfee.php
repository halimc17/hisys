...
$instansi = checkPostGet('instansi', '');
$tipe = checkPostGet('tipe', '');
/*if ($tipe) {
	$penerima = checkPostGet('penerima1', '');
}
else
{
	$penerima = checkPostGet('penerima0', '');
}*/
...
switch ($method) {
	...
	$tab.= "<table cellspacing=1 border=0>
			...
			<tr valign=top>
				...
				<td>:</td>
				<td width=300px align='justify'>".$bar['uraian']."</td>
				<td>&nbsp;&nbsp;&nbsp;</td>
	...
	case'getnotransaksi':
		...
		$str=" select notransaksi from ".$dbname.".lgl_pengajuanfee where  kodeorg='".$kodeorg."' and tanggal like '".substr($tanggal,0,7)."%' order by notransaksi desc limit 1 "; //exit('error'.$str);
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
		...
		$notranbaru=addZero($noawal,3)."/INT/LGL/".$nmpt[$kodeorg]."/".$kodeorg."/".romawi($tempPrd[1])."/".$tempPrd[0];
        
		echo $notranbaru;
    break;

    case'gocaripenerima':
			$textnama = checkPostGet('txtnama','');
			$tab="";
			...
			$tab.="<table class=sortable cellspacing=1 border=0>
				<thead>
					...
					<td align=center>".$_SESSION['lang']['supplier']." ID</td>
					<td align=center>".$_SESSION['lang']['namasupplier']."</td>
				</tr>
				</thead>
				<tbody>";
				
			$str="select * from ".$dbname.".keu_tagihanht a left join log_5supplier b on a.kodesupplier=b.supplierid where a.kodesupplier in (select distinct supplierid from log_5supkelompok where tipe in ('LAWYER','NOTARIS','KOPERASI','SEWA')) and unit='".$kodeorg."' and namasupplier like '%".$textnama."%' group by supplierid";
			//exit('error'.$str);
			$res=fetchData($str);
			$no=0;
			foreach($res as $key=>$val)
			{
				...
				$tab.=" <tr class=rowcontent style='cursor:pointer' onclick=\"fillpenerima('".$val['namasupplier']."')\">
					<td>".$no."</td>
    ...
    case'detail':
    	...
    	$arrakun=makeOption($dbname,'keu_5akun', 'noakun,namaakun',"substr(noakun,1,1) in (7,8)");
    	//print_r($arrakun);
    	$optakun='';
    	foreach ($arrakun as $key => $val) {
	...
	echo"<fieldset>
        ...
        <thead><tr class=rowheader>
            ...
            <td align=center width=350px>" . $_SESSION['lang']['noakun'] . "</td>
        ...
        <tr class=rowcontent>
			...
			<td><select id=noakun>".$optakun."</select></td>
    ...
    case'insert':
        #cek data
        $sql = "select count(*) as jmlhrow from " . $dbname . ".lgl_pengajuanfee where "
		. " notransaksi='" . $notransaksi . "' and kodeorg ='".$kodeorg."' and instansi='".$instansi."' and keterangan='" . $keterangan . "' and deskripsi='".$deskripsi."' and tanggal='".$tanggal."'";
        ...
        $str = "insert into " . $dbname . ".lgl_pengajuanfee (`notransaksi`,`kodeorg`,`tanggal`,`instansi`,`tipe`,`penerima`,`keterangan`,`uraian`,`bank`,`rekening`,`deskripsi`,`rupiah`,`statuspersetujuan`,`createby`,`createtime`,`updateby`,`updatetime`)
        values ('".$notransaksi."','".$kodeorg."','".$tanggal."','".$instansi."','".$tipe."','".$penerima."','".$keterangan."','".$uraian."','".$bank."','".$rekening."','".$deskripsi."','".$rupiah."','0','" . $_SESSION['standard']['userid'] . "','".$todayhis."','" . $_SESSION['standard']['userid'] . "','".$todayhis."')";
        //exit('error'.$penerima);
        try {$owlPDO->exec($str);} catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n";die();}

	break;
    case'delete':
        $str = "delete from " . $dbname . ".lgl_pengajuanfee where notransaksi='".$notransaksi."' and kodeorg='" . $kodeorg . "' and tanggal='" . $tanggal . "'"; //exit('error'.$str);
        try {
            $owlPDO->exec($str);
    ...
    case'ajukan':
		...
		# update flag menjadi 1
        $str = "update " . $dbname . ".lgl_pengajuanfee set posting='1' ,postingdate='" . date('Y-m-d H:i:s') . "',"."postingby='" . $_SESSION['standard']['userid'] . "' where notransaksi = '" . $notransaksi . "'";
        try {
            $owlPDO->exec($str);
			...
			$str = "insert into " . $dbname . ".approval (`nourut`,`notransaksi`, `jenispersetujuan`, 
					`level`, `karyawanid`, `status`, `komentar`, `keterangan`, `tanggal`)
					 values ('','".$notransaksi."','FEE','1','" . $kepada."','0','','','')";
			try {$owlPDO->exec($str);
    ...
    case'loaddatadetail':
		...
		if($row==0){
			...
			while ($bar = $res->fetch()) {
				$arrakun=makeOption($dbname,'keu_5akun', 'noakun,namaakun',"noakun='".$bar['noakun']."'");
				$no+=1;
				$a=$no%2;
				$xx='';
				if($a==0){
				...
				$tab.="<td align=center>" . $no . "</td>";
				$tab.="<td align=left>" . $bar['deskripsi'] . "</td>";
				$tab.="<td align=left>" . $arrakun[$bar['noakun']] . "</td>";
				$tab.="<td align=right>" . hidezerodecimal($bar['rupiah'],2) . "</td>";
				$tab.="<td align=center>";
				$tab.="<img src=images/application/application_delete.png class=resicon  title='Delete' 
					onclick=\"deletedetail('" . $bar['notransaksi'] . "','" . $bar['kodeorg'] . "','" . $bar['instansi'] . "','" . $bar['keterangan'] . "','" . $bar['deskripsi'] . "','" . tanggalnormal($bar['tanggal']) . "');\" >
				</td>";
				@$trupiah+=$bar['rupiah'];
			}
        $tab.="</tr>";
    ...
    case'loaddata':
        ...
        $sql = "select count(*) as jmlhrow from " . $dbname . ".lgl_pengajuanfee where 1=1 " . $where . "";
        $res = $owlPDO->query($sql) or die(print " Gagal: " . PDOException::getMessage());
        $jlhbrs = owlBaris($res);
        $no = 0;

        $str = "SELECT * FROM " . $dbname . ".lgl_pengajuanfee
		where 1=1 " . $where . " group by notransaksi  order by notransaksi desc limit " . $offset . "," . $limit . "";
        //exit('error '.$str);
		$tab = "";
        $no = $maxdisplay;
        $res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
        ...
        while ($bar = $res->fetch()) {
            ...
            if ($bar['posting'] == 0) {
				...
				onclick=\"edit('" . $bar['kodeorg'] . "','" . tanggalnormal($bar['tanggal']) . "','" . $bar['notransaksi'] . "','" . $bar['instansi'] . "','" . $bar['tipe'] . "','" . $bar['penerima'] . "','" . $bar['keterangan'] . "','" . $bar['uraian'] . "','".$bar['bank']."','".$bar['rekening']."');\" ></td>";
