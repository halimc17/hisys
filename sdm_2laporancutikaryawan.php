...
<script language=javascript src='js/sdm_2laporancutikaryawan.js?v=<?php echo time(); ?>'></script>

<?php

OPEN_BOX('','<span class=judul>'.getMenu('sdm_2laporancutikaryawan').'</span><br>');
...
## GET PERIODE
$optPeriode="";
$str="select periodecuti from ".$dbname.".sdm_cutiht group by periodecuti order by periodecuti desc";
$res=fetchdata($str);
foreach($res as $val){
	$optPeriode.="<option value='".$val['periodecuti']."'>".$val['periodecuti']."</option>";
}
...
<fieldset style=float:left>
	<table cellspacing=1 cellpadding=2>
		...
		<tr>
			<td>".$_SESSION['lang']['periodecuti']."</td>
			<td>:</td>
			<td>
