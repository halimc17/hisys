...
OPEN_BOX('','<span class=judul>'.getMenu('sdm_uploadabsensihov2').'</span>');
...
<script language="javascript" src="js/sdm_uploadabsensihov2.js?v=<?php echo time(); ?>"></script>
<?php
...
$optunit = "";
$sOrg = "SELECT kodeorganisasi, namaorganisasi FROM $dbname.organisasi WHERE kodeorganisasi IN (".getOrgDetail(2).") and tipe='HOLDING' ORDER BY kodeorganisasi";
$qOrg = fetchData($sOrg);
foreach ($qOrg as $rOrg) {
	$selected = ($_SESSION['empl']['lokasitugas'] == $rOrg['kodeorganisasi']) ? "selected" : "";
	$optunit .= "<option value='".$rOrg['kodeorganisasi']."' $selected>".$rOrg['kodeorganisasi']." - ".$rOrg['namaorganisasi']."</option>";
}
...
echo"<fieldset>
	...
	<tr>
			<td style='vertical-align:top'>No. Transaksi</td>
			...
			<td style='padding-left:20px;vertical-align:top' rowspan=5>
					...
					<table cellspacing='1' border='0'>
						<tr>
							<td>
							=> No. Transaksi Akan otomatis terisi</b>
 							<br>=> <b>Klik untuk download format </b>&nbsp;<a href='fileupload/upload_absensi_ho_detail.xlsx'  title='upload_absensi_ho_detail.xlsx'>Klik disini untuk mendapatkan contoh file  </a>
							</td>
						</td>
						</tr>
					</table>
		...
		<tr>
			...
			<td>:</td>
			<td>
				<input type='text' class='myinputtext' id='tanggal' value='".tanggalnormal($as)."' onmousemove=\"setCalendar(this.id)\" onkeypress=\"return tanpa_kutip(event)\" readonly style='width:195px;text-align:center' />
			</td>
		</tr>
		<tr>
