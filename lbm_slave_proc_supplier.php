...
$statKurs = checkPostGet('statKurs', '');
...
foreach ($dtBrg as $dtKlmpBrg) {
    ...
    $tab.="<td>" . $ert . "</td>";
    $optKlmpk[$dtKlmpBrg] = isset($optKlmpk[$dtKlmpBrg]) ? $optKlmpk[$dtKlmpBrg] : '';
...
switch ($proses) {
    ...
    case'getDetail':
        ...
        foreach ($dtOrg as $lstOrg2) {
            $lstOrg3 = isset($lstOrg3) ? $lstOrg3 : '';
            $lstMatauang2 = isset($lstMatauang2) ? $lstMatauang2 : '';
            $artt+=1;
    ...
    case'getDetPtEx':
        ...
        foreach ($dtOrg as $lstOrg2) {
            ...
            $tabc.="<td>" . $optNmBrg[$lstOrg2] . "</td>";
            $tabc.="<td align=right>" . @number_format($jmlhPsn[$lstOrg2], 0) . "</td>";
            $dtMatuang = isset($dtMatuang) ? $dtMatuang : '';
            foreach ($dtMatuang as $lstMataung) {
                $tabc.="<td align=right>" . @number_format($nilBrg[$lstOrg2 . $lstMataung], 0) . "</td>";
                @$total[$lstMataung]+=$nilBrg[$lstOrg2 . $lstMataung];
            }
