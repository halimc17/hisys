...
$sSum2="select distinct(noakun) AS dnoakun,sum(setahun) as setahun, sum(rp01)  as rp01, sum(rp02) as rp02, sum(rp03) as rp03,
    ...
    where tahunbudget='".$thnBudget."' and  (substr(noakun,1,1)='5' or substr(noakun,1,1)='9') and noakun!=0 ".$where2."
    group by  noakun order by substr(noakun,1,3) asc";
	// echo $sSum2;
    ...
    for($arAwal=1;$arAwal<13;$arAwal++)
    {
        if(strlen($rSum2['dnoakun'])==3)
        {
            ...
            else
            {
             ...
             $rupiahBulan2[$rSum2['dnoakun']][$arAwal]=$rSum2['rp'.$dtke];
             $byLangsung+=$rSum2['rp'.$dtke];
        }
    }
}
...
$arrBln=array("1"=>"Jan","2"=>"Feb","3"=>"Mar","4"=>"Apr","5"=>"Mei","6"=>"Jun","7"=>"Jul","8"=>"Aug","9"=>"Sep","10"=>"Okt","11"=>"Nov","12"=>"Des");
            ...
            $tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable width=2000px><thead>";
            ...
            foreach($noAkun as $listNoakun)
            {
                ...
                for($ter=1;$ter<13;$ter++)
                {
				   ...
				   $grTotalbulan[$ter]+=$rupiahBulanPen[$listNoakun][$ter];
                }
			...
			for($ter=1;$ter<13;$ter++)
			{
			   $tab.="<td  align=right>".number_format($grTotalbulan[$ter],2)."</td>";
			}
