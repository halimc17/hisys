...
switch($proses){
	case'prosespph':
		...
		if($tipegaji == '0'){
			...
			$str="select distinct(sudahproses) from ".$dbname.".sdm_5periodegaji_kecil where kodeorg='".$unit."' and periode='".$per."'";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			$bar=$res->fetch();
			$aktifperiodegaji=$bar['sudahproses'];
			...
			if($aktifperiodegaji==0){
				throw new PDOException('Periode gaji kecil '.$per.' belum ditutup.');
			}
		}
		...
		$datakaryawan_gajikecilx = "";
		$str = "select * from ".$dbname.".sdm_gaji_kecil where periodegaji = '".$per."' and kodeorg = '".$unit."' group by karyawanid";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
			if($datakaryawan_gajikecilx ==''){
				$datakaryawan_gajikecilx ="'".$bar['karyawanid']."'";
			}else{
				$datakaryawan_gajikecilx .=",'".$bar['karyawanid']."'";
			}
		} 

		$whereKaryawanGajiKecilx = '';
		$whereKaryawanGajiKecily = '';
		if($datakaryawan_gajikecilx != ''){
			$whereKaryawanGajiKecilx = "and karyawanid in (".$datakaryawan_gajikecilx.")";
			$whereKaryawanGajiKecily = "and karyawanid not in (".$datakaryawan_gajikecilx.")";
		}
		...
		if($baris=='1'){	
			...
			if($tipegaji == '1'){
				...
				$str = "delete from " . $dbname . ".sdm_gaji where kodeorg='" . $unit . "' and idkomponen='42' and periodegaji='".$per."' ".$whereKaryawanGajiKecilx." ";
				$owlPDO->exec($str);

				$str = "delete from " . $dbname . ".sdm_pph21 where kodeorg='" . $unit . "' and periode='".$per."' ".$whereKaryawanGajiKecilx."";
				$owlPDO->exec($str);

				$str = "delete from " . $dbname . ".sdm_gaji_kecil where kodeorg='" . $unit . "' and idkomponen='42' and periodegaji='".$per."'";
				$owlPDO->exec($str);
			
			## Gaji Besar
			}else{

				$str = "delete from " . $dbname . ".sdm_gaji where kodeorg='" . $unit . "' and idkomponen='42' and periodegaji='".$per."' ".$whereKaryawanGajiKecily."";
				$owlPDO->exec($str);

				$str = "delete from " . $dbname . ".sdm_pph21 where kodeorg='" . $unit . "' and periode='".$per."' ".$whereKaryawanGajiKecily." ";
				$owlPDO->exec($str);
			}
		}
		...
		if($pph21x != 0){
			...
			if($tipegaji == '1'){
				$str = "insert into " . $dbname . ".sdm_gaji_kecil (`karyawanid`,`kodeorg`,`idkomponen`,`jumlah`,`pengali`,`hk`,`periodegaji`) values ('".$karyawanid."','".$unit."','42','".$pph21x."','1','0','".$per."') ";
				$owlPDO->exec($str);
			}
		}
    ...
    case 'preview':
		...
		$dataGajiPakai = "";
		## Cek gaji apa yang dipakai
		if($tipegaji == 0){

			## Cek udah ada proses gaji nya belum (kecualikan yang ada di gaji kecil)
			...
			$dataGajiPakai = "sdm_gaji";
			
		}else{
			
			## Cek udah ada proses gaji kecil nya belum
			$sCount = "select count(*) as jmlhrow from ".$dbname.".sdm_gaji_kecil where periodegaji = '".$per."' and kodeorg = '".$unit."'";
			$qCount = $owlPDO->query($sCount) or die(print " Gagal: " . PDOException::getMessage());
			$qCount->setFetchMode(PDO::FETCH_OBJ);
			while ($rCount = $qCount->fetch()) {
				$jmlbrs = $rCount->jmlhrow;
			...
			if($jmlbrs == 0){
				exit("Warning : Data gaji kecil tidak ada untuk periode ".$per." di unit ".$unit.", Silahkan proses gaji kecil dan tutup periode penggajian...");
			}

			$dataGajiPakai = "sdm_gaji_kecil";
		}


		$datakaryawan_gajikecil='';
	
		#= buat ambil karyawan dengan gaji kecil biar gak ke proses di pph21 normal
		$str = "select * from ".$dbname.".sdm_gaji_kecil where periodegaji = '".$per."' and kodeorg = '".$unit."' group by karyawanid";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
			if($datakaryawan_gajikecil==''){
				$datakaryawan_gajikecil="'".$bar['karyawanid']."'";
			}else{
				$datakaryawan_gajikecil.=",'".$bar['karyawanid']."'";
			}
		} 

		$whereKaryawanGajiKecil = '';
		if($datakaryawan_gajikecil != ''){
			$whereKaryawanGajiKecil = "and karyawanid not in (".$datakaryawan_gajikecil.")";
		}
		...
		if($tipegaji == 0){
			...
			## Jika ada sudah di proses gaji kecil jangan masukin lagi 
			if($dakarbulanan==1){
				$strawal = "select * from ".$dbname.".datakaryawan_hist where approval_status='8' and version_type='B' and lokasitugas='".$unit."' and periodegaji='".$per."' and statuspajak <> ''  and (periodeakhirgaji>='" . $per . "' or periodeakhirgaji='') and ( tanggalmasuk<='" . $per . "-31' or tanggalmasuk='0000-00-00' or tanggalmasuk is null) and tipekaryawan !=0 ".$whereKaryawanGajiKecil." "; 
			}else{
				$strawal="select * from ".$dbname.".datakaryawan where lokasitugas='".$unit."' and statuspajak <> '' and (periodeakhirgaji>='" . $per . "' or periodeakhirgaji='') and ( tanggalmasuk<='" . $per . "-31' or tanggalmasuk='0000-00-00' or tanggalmasuk is null) and tipekaryawan !=0 ".$whereKaryawanGajiKecil." ";
			}
		...
		## DT Tunjangan tetap
		$str="select a.*,plus from ".$dbname.".".$dataGajiPakai." a left join ".$dbname.".sdm_ho_component b on a.idkomponen=b.id 
		...
		$str="select a.*,plus from ".$dbname.".sdm_gaji a left join ".$dbname.".sdm_ho_component b on a.idkomponen=b.id 
			where 1=1 and idkomponen not in (".$expph.",".$komponentjtetap.",".$komponenBPJSper.") and periodegaji = '".$per."' ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
			if($bar['plus']==1){
		...
		foreach ($dtkarid as $karid){	
			...
			if($tipe != 'excel') {
				...
				## Ambil BPJS JP dan kesehatan perusahaan
				foreach ($dtbpjsper as $komplus){
					if($komplus == '71' ||  $komplus == '80'){
						@$ttlbpjs_jpkesehatan[$karid]+=$rupiah[$karid][$komplus];
					}
				}

				## Biaya jabatan
				$biaya_jabatan[$karid] = ($ttjtetap[$karid] + $ttdtkompus[$karid] + $$ttlbpjs_jpkesehatan[$karid] + ($ttjtetap[$karid] * 0.84 / 100)) * ($persenjabatan / 100);
