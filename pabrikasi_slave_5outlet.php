...
switch($method)
{
	...
	break;
	case 'update':
		$str="update ".$dbname.".pabrikasi_5outlet set nama='".$nama."' where kode='".$kode."'";
		try{$owlPDO->exec($str); }
			catch (PDOException $e) {
				print " Gagal  !: " . $e->getMessage() . "\n"; 
				die(); 
	...
	case 'delete':
		...
		$str="delete from ".$dbname.".pabrikasi_5outlet where kode='".$kode."'";
		try{$owlPDO->exec($str); }
		catch (PDOException $e) {
			print " Gagal  !: " . $e->getMessage() . "\n"; 
			die(); 
	...
	case'loaddata':
		...
		$str="select * from ".$dbname.".pabrikasi_5outlet";
