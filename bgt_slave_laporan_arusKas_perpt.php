...
{
  if($rNoakun['noakun']!='5'&&$rNoakun['noakun']!='9')
  {
    $noAkun[]=$rNoakun['noakun'];
  }
}
$rupiahBulanPen=array();
$sSum2="select  noakun  AS dnoakun,sum(setahun) as setahun,unit from ".$dbname.".bgt_summary_biaya_vw 
    where tahunbudget='".$thnBudget."' and  substr(noakun,1,1)='5' or substr(noakun,1,1)='9' and noakun!=0 and
    unit in (select distinct kodeorganisasi from ".$dbname.".organisasi where induk='".$kdUnit."')
    group by noakun,unit order by substr(noakun,1,3) asc";
...
$sSum2="select distinct substring(noakun, 1, 3) AS dnoakun,sum(setahun) as setahun,unit from ".$dbname.".bgt_summary_biaya_vw where 
        ...
        (substr(noakun,1,3)>'125' and substr(noakun,1,3)<'129') and noakun!=0 group by substr(noakun,1,3),unit order by substr(noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
$rupiahBulan=array();
$qSum2=$owlPDO->query($sSum2) or die(print " Gagal: ".PDOException::getMessage());
$qSum2->setFetchMode(PDO::FETCH_ASSOC);
while($rSum2=$qSum2->fetch())
{
   // $dtNoakun[]=$rSum2['dnoakun'];
    ...
    $rupAkunStaon2[$rSum2['dnoakun']][$rSum2['unit']]+=$rSum2['setahun'];
...
$sSum2="select distinct substring(noakun, 1, 3) AS dnoakun,sum(setahun) as setahun, unit from ".$dbname.".bgt_summary_biaya_vw where 
        tahunbudget='".$thnBudget."' and  unit in (select distinct kodeorganisasi from ".$dbname.".organisasi where induk='".$kdUnit."') and
        substr(noakun,1,1)='7' or substr(noakun,1,1)='8' and noakun!=0 group by substr(noakun,1,3),unit order by substr(noakun,1,3) asc";//and substr(noakun,1,1)<'4'  group by noakun order by noakun asc";
$rupiahBulan2=array();
$qSum2=$owlPDO->query($sSum2) or die(print " Gagal: ".PDOException::getMessage());
$qSum2->setFetchMode(PDO::FETCH_ASSOC);
while($rSum2=$qSum2->fetch())
...
{ 
            ...
            $tab.="<td   colspan=".$jumunit." align=center >&nbsp;</td>";
            $tab.="</tr>";
            $tab.="<tr class=rowcontent>";
            ...
            foreach($noAkun as $listNoakun)
            {
                if($stNoakun!=substr($listNoakun,0,1))
                  ...
                  if($brsDt==1)
                  {
                        ...
                        $tab.="<td   colspan=".$jumunit." align=center  >&nbsp</td>";
                        $tab.="</tr>";
                  }
                $tab.="<tr class=rowcontent>";
               ...
               // $tab.="<td   align=right >".number_format($totalDes[$listNoakun],2)."</td>";
                foreach($listUnit as $listBln)
                {
				   ...
				   $grTotalunit[$listBln]+=$rupAkunStaon[$listNoakun][$listBln];
                }
			...
			foreach($listUnit as $listBln)
			{
			   $tab.="<td  align=right>".number_format($grTotalunit[$listBln],2)."</td>";
			}
			$tab.="<td  align=right>".number_format($grTotal,2)."</td>";
            $tab.="</tr>";
            $tab.="<tr class=rowcontent>";
