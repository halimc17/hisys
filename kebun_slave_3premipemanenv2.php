...
if($jlhbss==0){
	$str="select * from ".$dbname.".kebun_5basispanen2 where afdeling='".$unit."' and tahun<='".$prd."' order by tahun asc";	
}
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
if($proses != 'excel'){
	...
	$tab.="<table><td><b>".$_SESSION['lang']['notransaksi']."</b></td><td><b>:</b></td>
					 <td><input disabled class=myinputtext style=width:170px value='".$notr."' id=notransaksi></td>
	...
	foreach($dtbrd as $blokbrd){
		...
		$tab.="<tr class=rowcontent>";
		$tab.="<td align=center>".$no."</td>";
		$tab.="<td align=center>".$blokbrd."</td>";
		foreach($rangetgl as $rtgl){
			$i="";
...
if ($proses != 'excel' and $ttlrestant==0 and $row==0 and $rowp==0 and $afd!='%%') {
	...
	$tab.="<button class=mybutton id=proses onclick=deleteTrans(".$no.");>".$_SESSION['lang']['proses']."</button>";
}
