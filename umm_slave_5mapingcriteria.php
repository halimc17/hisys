...
switch ($method) 
{
	case 'insert':
		try{	
			...
			$str="select * from ".$dbname.".fil_5mapcriteria where lower(kriteria)='".strtolower($kriteria)."' or lower(id)='".strtolower($id)."'";
			$res=fetchData($str);
			...
			$str = "insert into ".$dbname.".fil_5mapcriteria values ('".$id."','".$kriteria."','".$modul."','".$status."')";
			//exit('Error '.$str);
			$owlPDO->exec($str);
    ...
    case 'update':
		try{
			$owlPDO->beginTransaction();
			$str="select * from ".$dbname.".fil_5mapcriteria where lower(kriteria)='".strtolower($kriteria)."' and modul='".$modul."' and status='".$status."'";
			$res=fetchData($str);
			...
			$str = "update ".$dbname.".fil_5mapcriteria set kriteria='".$kriteria."', modul='".$modul."', status='".$status."' where id='".$id."'";
			$owlPDO->exec($str);
			$owlPDO->commit();
		}catch(PDOException $e){
			$owlPDO->rollback();
	...
	case'delete';		
		try{
			$countdata = 0;
			$owlPDO->beginTransaction();
			$str="select * from ".$dbname.".fil_5mapdt where kriteriadet='".$id."'";
			$res=fetchdata($str);
			$countdata = $countdata + count($res);
			
			$str="select * from ".$dbname.".fil_5mapdt2 where kriteriadet='".$id."'";
			$res=fetchdata($str);
			$countdata = $countdata + count($res);
			
			if($countdata > 0){
				throw new PDOException("Gagal, Item sudah pernah digunakan di setup Mapping E-Filing");
			}
			
			$str = "delete from ".$dbname.".fil_5mapcriteria where id='".$id."'";
			$owlPDO->exec($str);
			$owlPDO->commit();
		}catch(PDOException $e){
			$owlPDO->rollback();
    ...
    case'loaddata':
		...
		$no = 0;
		$str = "select * from ".$dbname.".fil_5mapcriteria order by kriteria asc";
        ...
        while ($bar = $res->fetch())
		{
            ...
            $tab.="<td>".$bar['kriteria']."</td>";
            $tab.="<td align=center>".$arrstatus[$bar['status']]."</td>";
            $tab.="<td align=center>".$otpModul[$bar['modul']]."</td>";
			$tab.="<td align=center>
				<img src=images/application/application_edit.png class=resicon  caption='Edit' onclick=\"edit('".$bar['id']."','".$bar['kriteria']."','".$bar['modul']."','".$bar['status']."');\">
				<img src=images/application/application_delete.png class=resicon  caption='Delete' onclick=\"del('".$bar['id']."','".$bar['kriteria']."');\">
			</td>";
