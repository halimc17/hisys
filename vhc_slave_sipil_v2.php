...
@$param['kodebarang']   	= str_replace(",","",$param['kodebarang']);
@$param['jumlahbarang']   	= str_replace(",","",$param['jumlahbarang']);
...
switch ($method) {
	...
	case'getprdcari':
		...
		// }
		// $str="select DISTINCT (substr(tanggal,1,7)) as prd from ".$dbname.".vhc_spl_aktifitas where kodeorg = '".$_SESSION['empl']['lokasitugas']."' ".$wh." order by prd desc";
		// $res = fetchData($str);
	...
	case'simpanheader':
		...
		$arrthn = explode("-",$tgl);
		if($arrthn[0]>'2019'){
			exit("Error : Penginputan BKM Sipil Sipil untuk periode tahun 2020 masih belum di perbolehkan.");
		} */
		
		#cek status awal
		stsawal($param);
		
		##cek apakah sudah diinput di detail BKM Sipil belum
		$str1 = "select * from " . $dbname . ".vhc_spl_kehadiran_vw where 
		( nik = '".$mandor."' or nik = '".$mandor1."' or nik = '".$kerani."') 
		...
		if($numrows>0){
			echo "HK Mandor / Mandor 1 / Kerani tidak boleh di input pada detail BKM Sipil.<br><br>Karyawan tersebut dibawah ini sudah terdaftar pada transaksi :<br>";
			$no=0;
			while($bar=$res->fetch()){
			   $no+=1;
        ...
        if ($mode=='edit') {
            ...
            $str = "update " . $dbname . ".vhc_spl_aktifitas set `nikmandor`='".$mandor."', `nikmandor1`='".$mandor1."',`keranimuat`='".$kerani."',`nikasisten`='".$asst."' where `notransaksi`='".$notransaksi."' and `nobkm`='".$nobkm."'"; #exit("error".$str);
			try {$owlPDO->exec($str);} catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n";die();}
        }else{
			#===== buat nomor BKM Sipil =====
			$data = $_POST;
			# Data Capture & Reform			
			...
			$fQuery = selectQuery($dbname,'vhc_spl_aktifitas','nobkm',$fWhere);
			$tmpNo = fetchData($fQuery);
			
			# Generate No Transaksi
			if(count($tmpNo)==0) {
				$notransaksi = $data['tgl']."/".$data['kodeorg']."/SPL/001";
			} else {
				# Get Max No Urut
				$maxNo = 1;
				...
				$notransaksi = $data['tgl']."/".$data['kodeorg']."/SPL/".$currNo;
			}
			#=== End buat nomor transaksi ===
			if($statusblok==''){
				exit("Warning : Tipe transaksi masih kosong.");
			}
			$str = "insert into " . $dbname . ".vhc_spl_aktifitas (`notransaksi`, `tipetransaksi`, `tanggal`, `nobkm`, `kodeorg`,`divisi`, `nikmandor`, `nikmandor1`, `nikasisten`, `keranimuat`, `jurnal`, `nospk`, `updateby`)
    ...
    case'detail':
		...
		$str="select * from ".$dbname.".kebun_5asistensi where kodeorgtujuan ='".$param['kodeorg']."' and tanggal<='".$param['tgl']."' and tanggalsampai>='".$param['tgl']."' and posting='1' and (tipetrans='SPL' or tipetrans='')";
		$resx=fetchdata($str);
		foreach($resx as $res){
		...
		foreach($arrUnit as $key=>$val){
			...
			$optKebon.="<option value='".$key."' ".$sel.">".$key." - ".$val."</option>";			
		...
		// $sDiv="select kode,nama from ".$dbname.".project where pekerjaan = 'Internal' and posting = '0' and kodeorg='".$kodeorg."' and SUBSTRING(kode,1,5) ='AK-BG'";
        ...
        $frm[0]="<table><td valign=top>
			<fieldset style=float:left><legend>Filter</legend>
				<table height=25px>
					...
					<td><select style=\"width:150px;\" onchange=\"getdata(this.value)\" id=filterdivisi>".$optDivisi."</select></td>
					<td>Filter " . $_SESSION['lang']['kebun'] . "</td>
					<td><select style=\"width:150px;\"  title=\"Untuk menampilkan data karyawan dari unit lain.\" onchange=\"getdata(this.value)\" id=filterkebon>".$optKebon."</select></td>
					
					<td hidden>&nbsp;</td>
					<td hidden><input type=checkbox onchange=\"getdatamandor()\" id=showpermandor></td>
					<td hidden>Per Mandor</td>
				</table>
			</fieldset>
			</td><td valign=top>
			...
			$frm[0].="<th align=center ".$rows." width=20px>No</th>
				<th align=center ".$rows." colspan=2><font color=red><b>* </font></b>".$_SESSION['lang']['namakaryawan']."</th>
				<th align=center ".$rows."  colspan=2><font color=red><b>* </font></b>".$_SESSION['lang']['alokasi'] . "</th>
				...
				<th align=center ".$rows." width=30px hidden>".$_SESSION['lang']['luas'] . "</th>
				<th align=center ".$rows." width=50px>".$_SESSION['lang']['hasilkerja2'] . "</th>
				<th align=center colspan=3 width=50px>".$_SESSION['lang']['jumlah']."</th>
			...
			<tr>
				...
				<th align=center width=50px hidden>".$_SESSION['lang']['basic']."</th>
				<th align=center width=50px hidden>".$_SESSION['lang']['rpsat']."</th>
				<th align=center width=50px hidden>".$_SESSION['lang']['lebihbasis']."</th>
			</tr>
		...
		$frm[1].="<tbody id=inputdetailabsensi>
				...
				<td  align=right width=30px><input type=hidden id=method value='insert'>
	...
	case'loaddataabsensi':
		...
		#absensi dari BKM Sipil
		$tab.="<tr class=rowcontent>";
		$tab.="<td colspan=10 bgcolor=#2C3E50></td>";
		$tab.="</tr>";
		$thkb=$tumrb=$tpremib=0;
		$str = "select a.nobkm, a.nik, sum(c.jhk) as jhk, sum(c.umr) as umr, sum(c.insentif) as insentif, c.absensi
			from " . $dbname . ".vhc_spl_prestasi a 
			left join " . $dbname . ".vhc_spl_kehadiran c on a.nik=c.nik and a.notransaksi=c.notransaksi and a.nourut=c.nourut
			where a.nobkm='" . $nobkm . "' group by a.nik"; #exit('error'.$str);
		$res = fetchdata($str);
		$no=0;
		foreach($res as $bar){
			$no++;
			...
			$tab.="<td align=center>".$no."</td>";
			$tab.="<td align=left colspan=2>" . $bar['nik']." - ".getKary($bar['nik']) . "</td>";
			$tab.="<td align=center>".$bar['absensi']."</td>";
		...
		$tab.="<tr class=rowcontent style=background-color:#AED6F1;font-weight:bold;height:25px>";
		$tab.="<td align=center>".numb_format($no)."</td>";
		$tab.="<td align=center colspan=3>Sub Total (BKM Sipil)</td>";
		...
		$tab.="<td align=center colspan=3>Grand Total (Absensi + BKM Sipil)</td>";
		...
		if($_SESSION['empl']['subbagian']==''){
			$str = "select substr(alokasi,1,6) as divisi from ".$dbname.".vhc_spl_prestasi where notransaksi='".$param['notransaksi']."' group by divisi";
			$res = fetchdata($str);
			foreach($res as $bar){
	...
	case 'caribarang':
		...
		if(count($res)>0){			
			foreach($res as $val){
				...
				$queryM="select sum(kwantitas) as jlh from ".$dbname.".kebun_pakaimaterial where kodegudang='".$kodegudang."' and kodebarang='".$val['kodebarang']."' and notransaksi in (select notransaksi from ".$dbname.".vhc_spl_aktifitas where jurnal='0' and kodeorg='".substr($kodegudang,0,4)."')";
	...
	case'inputdetailmaterial':
		$str = "select sum(hasilkerja) as hasilkerja, kodekegiatan, kodeorg,notransaksi from ".$dbname.".vhc_spl_prestasi where 1=1 and notransaksi in (select notransaksi from ".$dbname.".vhc_spl_aktifitas where nobkm='".$notransaksi."') and kodekegiatan in (select kodekegiatan from ".$dbname.".setup_kegiatannorma) group by kodekegiatan, kodeorg order by kodekegiatan asc, kodeorg asc"; //exit('error'.$str);
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$no=0;
		while($bar=$res->fetch()){
	...
	case'loaddatadetailmaterial':
		...
		$str = "select * from ".$dbname.".kebun_pakaimaterial where 1=1 and notransaksi in (select notransaksi from ".$dbname.".vhc_spl_aktifitas where nobkm='".$notransaksi."') order by kodekegiatan, kodeorg";
		$res=fetchdata($str);
		foreach($res as $bar){
		...
		foreach($datamat as $kdkeg => $valkdorg){
			foreach($valkdorg as $kdorg => $valbrg){
				foreach($valbrg as $kdbrg => $kodebrg){
					...
					$strv = "select sum(hasilkerja) as hasilkerja from ".$dbname.".vhc_spl_prestasi where notransaksi='".$notrans[$kdkeg][$kdorg][$kdbrg]."' and kodekegiatan='".$kdkeg."' and kodeorg='".$kdorg."'"; //exit('error'.$strv);
					$barv=fetchdata($strv);
					...
					@$ttljlhmat[$kdbrg]+=$jlhmat[$kdkeg][$kdorg][$kdbrg];
						...
						$optpost=makeOption($dbname,'vhc_spl_aktifitas','notransaksi,jurnal',"notransaksi='".$notrans[$kdkeg][$kdorg][$kdbrg]."'");
	...
	echo"<td id=no align=center>1</td>";
		...
		## Material
	$optMaterial="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
	$str="select * from ".$dbname.".log_5masterbarang where kelompokbarang = '311'";
	$res=fetchdata($str);
	foreach($res as $bar){
		$optMaterial.="<option value='".$bar['kodebarang']."'>".$bar['kodebarang']." - ".$bar['namabarang']."</option>";
	}

	$optKary = getoptkary($param);
	$optBlok = getoptblok($param);
	echo"<td colspan=2><select style=width:205px;align:right; onchange=getDataDetail('','changekeg') id=karyawanid class='select2'>".$optKary."</select></td>

			<td hidden>
				<input id=luas disabled class=myinputtextnumber style=\"width:35px;\">
			</td>
			<td colspan=2>
				<select style=width:150px onchange=getDataDetail('','changekeg') id=blok class='select2'>".$optBlok."</select>
			</td>
			<td colspan=2>
				<select style=width:180px onchange=getDataDetail() id=kegiatan class='select2'>".@$optKeg."</select>
			</td>
			<td>
				<input id=keterangan class=myinputtext style=\"width:140px;align:center\">
			</td>
			<td>
				<input id=satuan class=myinputtext disabled style=\"width:40px;align:center\">
			</td>
			<td>
				<input id=prestasi onkeyup=\"getDataDetail();\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber  onkeypress=\"return angka_doang(event);\" style=\"width:50px;\">
			</td>
			<td>
				<input id=jhk title=\"Double click untuk mengisi HK menjadi 1\" onkeyup=\"getumr('','i');\" ondblclick=\"getumr('','d');\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:35px;\">
			</td>
			<td>
				<input id=upah onkeyup=gethk(this.id,'jhk','karyawanid',''); nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" class=myinputtextnumber style=\"width:60px;\">
			</td>

			<td><input id=premi onkeyup=\"numberFormat2('premi',0)\"  class=myinputtextnumber style=\"width:60px;\" maxlength=10></td>

			<td>
				<select style=width:150px  id=kodebarang class='select2'>".$optMaterial."</select>
			</td>

			<td>
				<input id=jumlahbarang nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber  onkeypress=\"return angka_doang(event);\" style=\"width:50px;\">
			</td>

			<td hidden><input id=basis disabled onkeyup=\"z.numberFormat('basis',2)\"  class=myinputtextnumber style=\"width:50px;\"></td>
			<td hidden><input id=rpsat disabled onkeyup=\"z.numberFormat('rpsat',2)\"  class=myinputtextnumber style=\"width:50px;\"></td>
			<td hidden><input id=upahperhari disabled onkeyup=\"z.numberFormat('upahperhari',2)\"  class=myinputtextnumber style=\"width:50px;\"></td>
	...
	$tab="<table id=tabledt cellpadding=3 cellspacing=1 ".$border." class=sortable style=min-width:960px>
			...
			<th align=center ".$rows." width=25px>No</th>
				<th align=center ".$rows.">".$_SESSION['lang']['notransaksi'] . "</th>
				<th align=center ".$rows.">".$_SESSION['lang']['nik2']." - ".$_SESSION['lang']['divisi']." - ".$_SESSION['lang']['namakaryawan']."</th>
				<th align=center ".$rows.">".$_SESSION['lang']['alokasi'] . "</th>
				<th align=center ".$rows.">".$_SESSION['lang']['kegiatan'] . "</th>
				<th align=center ".$rows.">".$_SESSION['lang']['keterangan'] . "</th>
				<th align=center ".$rows." width=45px>".$_SESSION['lang']['satuan'] . "</th>
				<th align=center ".$rows." width=30px hidden>".$_SESSION['lang']['luas'] . "</th>
				<th align=center ".$rows." width=50px>".$_SESSION['lang']['hasilkerja2'] . "</th>
				<th align=center colspan=3 width=50px>".$_SESSION['lang']['jumlah']."</th>
		...
		if($namakary!=''){
			$where.=" and (a.nik like '%".$namakary."%' or a.nik in (select karyawanid from ".$dbname.".datakaryawan where karyawanid like '%".$namakary."%' or nik like '%".$namakary."%' or namakaryawan like '%".$namakary."%'))";
			$wherexz.=" and (karyawanid like '%".$namakary."%' or nik like '%".$namakary."%' or namakaryawan like '%".$namakary."%')";
		}
		if($blok!=''){
			$where.=" and a.alokasi like '%".$blok."%'";
		}
		if($kegiatan!=''){
		...
		$stsblok2 = array();
		$strx = "SELECT b.statusblok FROM $dbname.vhc_spl_prestasi a LEFT JOIN $dbname.setup_blok b
		ON a.alokasi = b.indukblok WHERE a.nobkm = '".$nobkm."' group by b.statusblok";
		$resx = fetchData($strx);
		...
		$res = fetchdata($str);
		foreach($res as $bar){
			$luasblok2[$bar['indukblok']][$bar['statusblok']]+=$bar['luasareaproduktif'];
		}


		// exit("warning: ".print_r($stsblok2));

		$str = "select a.*,d.kelompok,c.jhk, c.umr, c.insentif,e.tipetransaksi from " . $dbname . ".vhc_spl_prestasi a 
		left join " . $dbname . ".vhc_spl_kehadiran c on a.nik=c.nik and a.notransaksi=c.notransaksi and a.nourut=c.nourut
		left join " . $dbname . ".vhc_kegiatan d on a.kodekegiatan=d.kodekegiatan 
		left join " . $dbname . ".vhc_spl_aktifitas e on a.notransaksi =e.notransaksi 
		...
		if($row==0){
			...
			foreach ($res as $bar) {
				if(getHistKary($bar['nik'],substr($bar['notransaksi'],0,4)."-".substr($bar['notransaksi'],4,2),'namakaryawan')!=''){
					$bar['namakaryawan']= getHistKary($bar['nik'],substr($bar['notransaksi'],0,4)."-".substr($bar['notransaksi'],4,2),'namakaryawan');
					$bar['subbagian']   = getHistKary($bar['nik'],substr($bar['notransaksi'],0,4)."-".substr($bar['notransaksi'],4,2),'subbagian');
					$bar['nik2']        = getHistKary($bar['nik'],substr($bar['notransaksi'],0,4)."-".substr($bar['notransaksi'],4,2),'nik');
				}else{
					$bar['namakaryawan']= getKary($bar['nik'],'namakaryawan');
					$bar['subbagian']   = getKary($bar['nik'],'subbagian');
					$bar['nik2']        = getKary($bar['nik'],'nik');
				}
				
				
				#cek apakah sudah di posting	
				$optpost=makeOption($dbname,'vhc_spl_aktifitas','notransaksi,jurnal',"notransaksi='".$bar['notransaksi']."'");
				
				$bgcolor=$title=$asstensi=$infomat='';
				$bgc="style=height:25px;";
				$strx = "select count(nik) as jmlkary, nik from " . $dbname . ".vhc_spl_prestasi where notransaksi='".$bar['notransaksi']."' and nik='".$bar['nik']."' group by nik";
				...
				$tab.="<td align=left ".$asstensi." >" . $bar['nik2'].$bar['subbagian'].$bar['namakaryawan'] . "</td>";

				$tiperumah=makeOption($dbname,'sdm_perumahanht','norumah,tipe');
				$namaRumah=makeOption($dbname,'sdm_5jenis_prasarana','jenis,nama');

				if($nmorg[$bar['alokasi']] == ''){
					$text =$namaRumah[$tiperumah[$bar['alokasi']]]; 
				}else{
					$text =$nmorg[$bar['alokasi']]; 
				}

				$tab.="<td align=center ".$bgcolor." >".$bar['alokasi']." - " . $text. "</td>";
				
				$tab.="<td align=left ".$infomat." >".$bar['kodekegiatan']." - " . $nmkeg[$bar['kodekegiatan']]. "</td>";
				$tab.="<td align=center>" . $bar['keterangan'] . "</td>";
				$tab.="<td align=center>" . $nmsat[$bar['kodekegiatan']] . "</td>";
				$tab.="<td align=right ".$luasclr.">" . @numb_format($bar['total_hasilkerja'],2) . "</td>";
				$tab.="<td align=right>" . @numb_format($bar['jhk'],2) . "</td>";
				$tab.="<td align=right>" . @numb_format($bar['umr'],2) . "</td>";
				...
				$text = "";
				if($bar['kodebarang'] != 0){
					$text = $bar['kodebarang']." - ".getNamaBrg($bar['kodebarang']);
				}else{
					$text = "";
				}

				$tab.="<td align=right>".$text."</td>";
				$tab.="<td align=right>" . @numb_format($bar['jumlahbarang'],2) . "</td>";
				...
				if($optpost[$bar['notransaksi']]==0){
					...
					if($tipe!='excel'){
						...
						if ($kodeproject == "AK") {
							...
							onclick=\"editdetail('".$bar['notransaksi']."','".$bar['nik']."','".$bar['kodekegiatan']."','".$bar['alokasi']."','".$luasblok[$bar['alokasi']]."','".$nmsat[$bar['kodekegiatan']]."','".$bar['total_hasilkerja']."','".$bar['jhk']."','".$bar['umr']."','".$bar['insentif']."','".$no."','".getNamaKaryawan($bar['nik'])."','".$nmorg[$bar['alokasi']]. "','".$fil."','".$bar['keterangan']."','".$bar['kodebarang']."','".$bar['jumlahbarang']."','".$bar['tipetransaksi']."');\" >";	
						} else {
							$tab.="<img src=images/application/application_edit.png class=zImgBtn  title='Edit' 
							onclick=\"editdetail('".$bar['notransaksi']."','".$bar['nik']."','".$bar['kodekegiatan']."','".$bar['alokasi']."','".$luasblok2[$bar['alokasi']][$bar['kelompok']]."','".$nmsat[$bar['kodekegiatan']]."','".$bar['total_hasilkerja']."','".$bar['jhk']."','".$bar['umr']."','".$bar['insentif']."','".$no."','".getNamaKaryawan($bar['nik'])."','".$nmorg[$bar['alokasi']]. "','".$fil."','".$bar['keterangan']."','".$bar['kodebarang']."','".$bar['jumlahbarang']."','".$bar['tipetransaksi']."');\" >";
						}
						
						$tab.="</td>";	
						$tab.="<td align=center width=20px>";	
						$tab.="<img src=images/application/application_delete.png class=zImgBtn  title='Delete' 
						onclick=\"deletedetail('" . $bar['notransaksi'] . "','" . $bar['nik'] . "','" . $bar['alokasi'] . "','".$bar['kodekegiatan']."');\" >";
					}
					$tab.="</td>";
				}else{
				...
				@$preskerja[$bar['kodeorg']][$bar['kodekegiatan']]+=$bar['total_hasilkerja'];
	...
	#========================== Setup Kegiatan ============================
		...
		$str = "select * from ".$dbname.".vhc_kegiatan where kodekegiatan='".$kegiatan."'"; 
		$res=fetchdata($str);
		foreach($res as $bar){
			$stspremi=$bar['premi'];
			if($satkegiatan ==''){
	...
	#========================== Setup Kegiatan ============================
	#========================== Premi Kegiatan ============================
		$basis =$premibasis=$premilebihbasis=$luasblok=$rppremilebihbasis=0;
	...
	#====================== Ambil Daftar Kegiatan =========================
		...
		if($filterdivisi == 'PROJECT'){
			...
			$str = "select kegiatan, namakegiatan from ".$dbname.".project_dt where kodeproject = '".$filterdivisi."' order by namakegiatan";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while($bar = $res->fetch()){
			...
			## Cek dulu dia blok atau bukan
			if(getTipeOrganisasi($blok,'tipe') == 'BLOK'){

				## Cek status blok nya 
				if(getDataIndukBlok($blok,'statusblok') == 'TB' or getDataIndukBlok($blok,'statusblok') == 'TBM' ){
					$str = "select kodekegiatan, namakegiatan from ".$dbname.".vhc_kegiatan where (tipe = 'blok') and (kodekegiatan LIKE '126%' OR kodekegiatan LIKE '12601%' OR kodekegiatan LIKE '12606%') AND status='1'";
					$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
					$res->setFetchMode(PDO::FETCH_ASSOC);
					while($bar = $res->fetch()){
						$optKeg .= "<option value='".$bar['kodekegiatan']."'>".$bar['namakegiatan']." (".$bar['kodekegiatan'].")</option>";
					}

				}else{
					$str = "select kodekegiatan, namakegiatan from ".$dbname.".vhc_kegiatan where (tipe = 'blok') and kodekegiatan LIKE '62%' AND status='1'";
					$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
					$res->setFetchMode(PDO::FETCH_ASSOC);
					while($bar = $res->fetch()){
						$optKeg .= "<option value='".$bar['kodekegiatan']."'>".$bar['namakegiatan']." (".$bar['kodekegiatan'].")</option>";
				...
				if(getTipeOrganisasi($blok,'tipe') == 'STATION'){

					$str = "select kodekegiatan, namakegiatan from ".$dbname.".vhc_kegiatan where (tipe = 'sipil') and kodekegiatan like '63%' AND status='1'";
					$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
					$res->setFetchMode(PDO::FETCH_ASSOC);
					while($bar = $res->fetch()){
						$optKeg .= "<option value='".$bar['kodekegiatan']."'>".$bar['namakegiatan']." (".$bar['kodekegiatan'].")</option>";
					...
					$str = "select kodekegiatan, namakegiatan from ".$dbname.".vhc_kegiatan where (tipe = 'sipil') and kodekegiatan like '7%' AND status='1'";
					$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
					$res->setFetchMode(PDO::FETCH_ASSOC);
					while($bar = $res->fetch()){
						$optKeg .= "<option value='".$bar['kodekegiatan']."'>".$bar['namakegiatan']." (".$bar['kodekegiatan'].")</option>";
		...
		if($tgl == ''){
			...
			$upahhar = getUpahKary(substr(tanggalsystemn($tgl),0,7),$karyawanid);
		}
	#====================== Ambil Daftar Kegiatan =========================

	echo $optKeg."######".$basis."######".$premibasis."######".$premilebihbasis."######".$tipeKary."######".$luasblok."######".$satkegiatan."######".$rppremilebihbasis."######".$stspremi."######".$upahhar;
	break;
	...
	// exit("error".getKary($karyawanid,'tipekaryawan'));
	...
	$owlPDO->beginTransaction();
		...
		#ambil transaksi belum posting di BKM Sipil
		$str="select sum(kwantitas) as kwantitas from ".$dbname.".kebun_pakaimaterial a left join ".$dbname.".vhc_spl_aktifitas b on a.notransaksi=b.notransaksi where a.kodebarang='".$param['kodebarang']."' and a.kodegudang='".$param['kodegudang']."' and b.jurnal='0'";
		$res=fetchData($str);
		...
		if($param['notransaksi']!='' and $param['kegiatan']!='' and $param['blok']!='' and $param['kodebarang']!='' and $param['qtymat']!='0' and $param['kodegudang']!=''){
			...
			#cek apakah sudah di posting	
			$optpost=makeOption($dbname,'vhc_spl_aktifitas','notransaksi,jurnal',"notransaksi='".$notransaksi."'");
			if($optpost[$notransaksi]==1){
				$ststransblok = explode("/",$notransaksi);
				throw new PDOException("Transaksi dengan nomor ".$notransaksi." (jenis : ".$ststransblok[2].") sudah diposting, silahkan buat Transaksi dengan No BKM Sipil yang baru untuk menambahkan.");
			}
	...
	$owlPDO->beginTransaction();
		...
		# Jika ada datanya maka exe, jika tidak maka lewatkan
		if($param['notransaksi']!='' and $param['karyawanid']!='' and $param['kegiatan']!='' and $param['prestasi']!='0' and (($param['jhk']!='0' and $param['upah']!='0') or $param['premi']!='0')){
			if($param['filterdivisi']=='PROJECT' && $param['blok']==''){
				throw new PDOException("Jika PROJECT maka alokasi harus diisi.");
			}
			# Cari status blok
			$optstsblok = makeOption($dbname,'setup_blok','indukblok,statusblok',"indukblok='".$param['blok']."' and statusblok IN (select kelompok from ".$dbname.".setup_kegiatan where kodekegiatan='".$param['kegiatan']."')");
			#===== buat nomor transaksi =====
			# Data Capture & Reform
			if($param['filterdivisi']=='PROJECT'){
				$param['tipetransaksi'] = "PRJ";
			}else{				
				// $param['tipetransaksi'] = $optstsblok[$param['blok']];saya comment karena ini bawaa bkm rawat jadi dibawah di patok SPL saja kalau umum
				$param['tipetransaksi'] = 'SPL';
			}
			
			$upahharian = 0;
			$tahun = substr(tanggalsystemn($param['tgl']),0,7);
			$str = "select sum(jumlah) as nilai from ".$dbname.".sdm_5gajipokok where karyawanid='".$param['karyawanid']."' and tahun='".$tahun."' and idkomponen in ('1')"; #exit("Warning :".$str);
			$res = fetchdata($str);
			...
			#=== Generate No Transaksi
			# Get Existing Data
			$fWhere = "tanggal='".$tgl."' and kodeorg='".$param['kodeorg'].
				"' and tipetransaksi='".$param['tipetransaksi']."'";
			$fQuery = selectQuery($dbname,'vhc_spl_aktifitas','notransaksi',$fWhere);
			$tmpNo = fetchData($fQuery);
			
			# Generate No Transaksi
			if(count($tmpNo)==0) {
				$notransaksi = $tgl."/".$param['kodeorg']."/".$param['tipetransaksi']."/001";
			} else {
				# Get Max No Urut
				$maxNo = 1;
				foreach($tmpNo as $row) {
				...
				$currNo = addZero($maxNo+1,3);
				$notransaksi = $tgl."/".$param['kodeorg']."/".$param['tipetransaksi']."/".$currNo;
			}
			#=== End buat nomor transaksi ===
			#==== Update nomor transaksi ====
			
			# Cek sudah ada atau belum ???
			$str = "select * from " . $dbname . ".vhc_spl_prestasi where nobkm='".$param['nobkm']."'";
			$resp=count(fetchData($str));
			
			# Cek aktifitas sudah ada atau belum ???
			$str = "select * from " . $dbname . ".vhc_spl_aktifitas where nobkm='".$param['nobkm']."' and tipetransaksi='SPL'";
			$resa=count(fetchData($str));
			
			# Cek aktifitas sudah ada atau belum ???
			$str = "select * from " . $dbname . ".vhc_spl_aktifitas where nobkm='".$param['nobkm']."' and tipetransaksi='".$param['tipetransaksi']."'";
			$rexx = fetchData($str); 
			$resx=count($rexx);
			...
			if($resp==0 and $resa>0){
				# pertama kali input
				$str = "update " . $dbname . ".vhc_spl_aktifitas set `notransaksi`='".$notransaksi."', `tipetransaksi`='".$param['tipetransaksi']."' where `nobkm`='".$param['nobkm']."' and tipetransaksi='SPL'"; #exit("error".$str);
				$owlPDO->exec($str);
				...
				$str = "insert into " . $dbname . ".vhc_spl_aktifitas (`notransaksi`, `tipetransaksi`, `tanggal`, `nobkm`, `kodeorg`, `nikmandor`, `nikmandor1`, `nikasisten`, `keranimuat`, `jurnal`, `nospk`, `updateby`, `divisi`)
				values ('".$notransaksi."','".$param['tipetransaksi']."','".$tgl."','" . $param['nobkm'] . "','" . $kodeorg . "','".$mandor."','".$mandor1."','".$asst."','".$kerani."','0',null,'" . $_SESSION['standard']['userid'] . "','".$param['divisi']."')";
				$owlPDO->exec($str);
				
				$param['notransaksi']=$notransaksi;
			}else{
				$str = "update " . $dbname . ".vhc_spl_aktifitas set `nikmandor`='".$mandor."', `nikmandor1`='".$mandor1."',`keranimuat`='".$kerani."',`nikasisten`='".$asst."' where `nobkm`='".$param['nobkm']."'"; #exit("error".$str);
				$owlPDO->exec($str);
			...
			#cek apakah sudah di posting	
			$optpost=makeOption($dbname,'vhc_spl_aktifitas','notransaksi,jurnal',"notransaksi='".$param['notransaksi']."'");
			if($optpost[$param['notransaksi']]==1){
				$ststransblok = explode("/",$param['notransaksi']);
				throw new PDOException("Transaksi dengan nomor ".$param['notransaksi']." (jenis : ".$ststransblok[2].") sudah diposting, silahkan buat Transaksi dengan No BKM Sipil yang baru untuk menambahkan.");
			}
			...
			#validasi penginputan
			validasiInput($kodeorg,substr($param['blok'],0,6),'SPL',tanggalsystemn(tanggalnormal($tgl)),$exit='0');
			
			#validasi maksimal HK BHL
			cekmaxnilaihk($param['karyawanid'],tanggalsystemn(tanggalnormal($tgl)),'1','0','new',$exit='0');
			
			# Buat nomor urut
			$sql = "select max(nourut) as nourut from " . $dbname . ".vhc_spl_kehadiran where notransaksi='".$param['notransaksi']."' limit 1"; 
			$res=fetchData($sql);
			...
			# ==========================================================================================
			$cols = array(
						'nobkm','notransaksi','nourut','nik','kodekegiatan','alokasi','total_hasilkerja','total_hk','total_premi','keterangan','kodebarang','jumlahbarang'
					);
			$data = array(
						$param['nobkm'],$param['notransaksi'],($res[0]['nourut']+1),$param['karyawanid'],$param['kegiatan'],$param['blok'],$param['prestasi'],$param['jhk'],$param['premi'],$param['keterangan'],$param['kodebarang'],$param['jumlahbarang']
					);
			
			# Insert vhc_spl_prestasi
			$query = insertQuery($dbname,'vhc_spl_prestasi',$data,$cols);
			#exit("error".$query);
			$owlPDO->exec($query);
			# ==========================================================================================
			...
			# Insert vhc_spl_kehadiran
			$query = insertQuery($dbname,'vhc_spl_kehadiran',$data,$cols);
			$owlPDO->exec($query);
			# ==========================================================================================
	...
	$owlPDO->beginTransaction();
		...
		if($param['notransaksi']!='' and $param['karyawanid']!='' and $param['kegiatan']!='' and $param['prestasi']!='0' and (($param['jhk']!='0' and $param['upah']!='0') or $param['premi']!='0')){
			...
			$str = "select * from " . $dbname . ".vhc_spl_prestasi where notransaksi='".$param['notransaksi']."' and nik='".$param['karyawanid']."' and alokasi='".$param['blok']."' and kodekegiatan='".$param['kegiatan']."'";
			$res=fetchData($str);
			$nourut=$res[0]['nourut'];
			...
			# ==========================================================================================
			$data = array(
						'total_hasilkerja'=>$param['prestasi'],
						'total_hk'=>$param['jhk'],
						'total_premi'=>$param['premi'],
						'keterangan'=>$param['keterangan'],
						'kodebarang'=>$param['kodebarang'],
						'jumlahbarang'=>$param['jumlahbarang']
					);
			$where = "notransaksi='".$param['notransaksi']."' and nik='".$param['karyawanid']."' and alokasi='".$param['blok']."' and kodekegiatan='".$param['kegiatan']."'";
			
			# Update vhc_spl_prestasi
			$query = updateQuery($dbname,'vhc_spl_prestasi',$data,$where);
			$owlPDO->exec($query);
			# ==========================================================================================
			$data = array(
			...
			# Update vhc_spl_kehadiran
			$query = updateQuery($dbname,'vhc_spl_kehadiran',$data,$where);
			$owlPDO->exec($query);
			# ==========================================================================================
		}
		#execute
    ...
    case'delete':
		...
		$str="select * from ".$dbname.".vhc_spl_aktifitas where nobkm='".$nobkm."'";
		$res=fetchData($str);
		if(count($res)>1){
        ...
        $str = "delete from " . $dbname . ".vhc_spl_aktifitas where notransaksi='".$notransaksi."'";
		$owlPDO->exec($str);
        
		$str = "SELECT * FROM " . $dbname . ".listfileupload where notransaksi ='".$param['notransaksi']."' and kriteriaefil='SPL'";
		$res = fetchData($str);
		if(count($res)>0){
			foreach($res as $bar){
    ...
    case'deletedetail':
		...
		# Ambil nomor urut kary
		$str = "select * from " . $dbname . ".vhc_spl_prestasi where notransaksi ='".$notransaksi."' and alokasi='" . $blok . "' and nik='" . $karyawanid . "' and kodekegiatan='".$kegiatan."'";
		$res=fetchData($str);
		$nourut=$res[0]['nourut'];
			
		#hapus kehadiran
		$str = "delete from " . $dbname . ".vhc_spl_kehadiran where notransaksi ='".$notransaksi."' and nik='" . $karyawanid . "' and nourut='".$nourut."'";
		try {$owlPDO->exec($str);
			#hapus prestasi	
			$str = "delete from " . $dbname . ".vhc_spl_prestasi where notransaksi ='".$notransaksi."' and alokasi='" . $blok . "' and nik='" . $karyawanid . "' and kodekegiatan='".$kegiatan."'";
			try {$owlPDO->exec($str);
    ...
    break;
	 ...
	 case'loaddataexcel':
		...
		*/
		...
		$fileup=array();
        $sql = "select distinct(notransaksi) as notransaksi from " . $dbname . ".listfileupload where 1=1 and notransaksi like '%".$_SESSION['empl']['lokasitugas']."%' and notransaksi like '".str_replace("-","",$periodesch)."%' and kriteriaefil='SPL'";
        $res = fetchdata($sql);
		foreach ($res as $bar) {
			$fileup[$bar['notransaksi']]=$bar['notransaksi'];
		}
		
		$sql = "select count(distinct a.notransaksi) as notr from " . $dbname . ".vhc_spl_aktifitas a where 1=1 and a.tipetransaksi not in ".$wh." " . $where . "";
        $res = fetchdata($sql);
        $jlhbrs = $res[0]['notr'];
		if($jlhbrs==0){
			$tab.="<tr class=rowcontent>";
		...
		$strx = "select sum(umr) as umr, sum(jhk) as jhk, sum(insentif) as insentif, notransaksi from ".$dbname.".vhc_spl_kehadiran where notransaksi in (select notransaksi from " . $dbname . ".vhc_spl_aktifitas a where 1=1 and a.tipetransaksi not in ".$wh." ".$wh3.") group by notransaksi"; 
		$resn = fetchdata($strx);
		foreach ($resn as $bar) {
		...
		$notrx=1;
        //$str = "SELECT a.*, substr(b.kodeorg,1,6) as divisipres FROM " . $dbname . ".vhc_spl_aktifitas a left join " . $dbname . ".vhc_spl_prestasi b on a.notransaksi=b.notransaksi where 1=1 and a.tipetransaksi not in ".$wh." " . $where . " group by a.notransaksi order by a.nobkm desc, a.notransaksi desc limit " . $offset . "," . $limit . ""; 
		...
		$tab.="<table class='sortable' cellspacing='1' cellpadding='5' border='1' width=100%>
			<thead>
				<tr class=rowheader>
					...
					<th align=center >No BKM Sipil</th>
		...
		$str = "SELECT * FROM " . $dbname . ".vhc_spl_aktifitas a where 1=1 and a.tipetransaksi not in ".$wh." " . $where . " group by a.notransaksi order by a.nobkm desc, a.notransaksi desc"; 
		// echo $str;
		// exit('error'.$str);
        $res = fetchdata($str);
        foreach ($res as $bar) {
    ...
    case'loaddata':
		...
		*/
		...
		$strx = "select sum(umr) as umr, sum(jhk) as jhk, sum(insentif) as insentif, notransaksi from ".$dbname.".vhc_spl_kehadiran where notransaksi like '".str_replace("-","",$periodesch)."%".$_SESSION['empl']['lokasitugas']."%' group by notransaksi"; 
		$resn = fetchdata($strx);
		foreach ($resn as $bar) {
			$umr[$bar['notransaksi']]=$bar['umr'];
			$hkp[$bar['notransaksi']]=$bar['jhk'];
		...
		notransaksi like '".str_replace("-","",$periodesch)."%".$_SESSION['empl']['lokasitugas']."%' order by nourut asc";
		...
		$str = "SELECT * FROM " . $dbname . ".vhc_spl_aktifitas a where 1=1 and a.tipetransaksi not in ".$wh." " . $where . " group by a.notransaksi order by a.nobkm desc, a.notransaksi desc limit " . $offset . "," . $limit . ""; 
		// echo $str;
		// exit('error'.$str);
        $res = fetchdata($str);
        foreach ($res as $bar) {
			// echo "<pre>";
			// print_r(implode(",",$verifiedby2[$bar['notransaksi']]));
			// print_r(getKary($verifiedby2[$bar['notransaksi']]));
			// echo "</pre>";
            ...
            if ($bar['jurnal'] == 0) {
				if($subbg=='isi' and $bar['updateby']!=$_SESSION['standard']['userid']){
					...
					$isi.="<td align=center style=width:20px><img src=images/application/application_edit.png class=zImgBtn  title='Edit' onclick=\"edit('".$bar['notransaksi']."','".tanggalnormal($bar['tanggal'])."','".$bar['kodeorg']."','".$bar['nobkm']."','".$bar['nikmandor']."','".$bar['nikmandor1']."','".$bar['nikasisten']."','".$bar['keranimuat']."','".$no."');\" ></td>";
        ...
        $footd = "";
        $footd.="<tr><td colspan=24 align=center>";
        if ($page == '0') {
	...
	case'edithead':
		...
		foreach($res as $bar){
			if($bar['divisi']==''){
				$sql="select substr(kodeorg,1,6) as divisi from ".$dbname.".vhc_spl_prestasi where notransaksi='".$notransaksi."'";
				$req=fetchData($sql);
				foreach($req as $val){
	...
	case 'submitfile':
		...
		if($data['fileupload']!=''){
			if($_FILES['file']['error']==0){
				...
				//$filename = "BKM Sipil".date("YmdHis");
				...
				if(($filetype=='.jpeg')||($filetype=='.jpg')||($filetype=='.png')||($filetype=='.pdf')||($filetype=='.xls')||($filetype=='.xlsx')||($filetype=='.doc')||($filetype=='.docx')){
					...
					values ('".$param['notransaksi']."','".$filename."','".$filetype."','SPL','1','".$_SESSION['standard']['userid']."','".date('Y-m-d H:i:s')."')";
					$owlPDO->exec($str);
	...
	break;
	case 'loadfiles':
		$str= "select * from ".$dbname.".vhc_spl_aktifitas where notransaksi = '".$param['notransaksi']."'";
		$res= fetchData($str);
		$jurnal = $res[0]['jurnal'];
	...
	try {
		...
		$queryH= selectQuery($dbname,'vhc_spl_aktifitas',"*","notransaksi='".$param['notransaksi']."'");
		...
		if($statusfp==1){
			validasifp($tipevalidasi,$detval,'TRK',$arrUpload,$tanggal,'0');
			// $countUpload=0;
			// foreach($arrUpload as $row){
				// $str = "select karyawanid from ".$dbname.".upload_absensi where karyawanid='".$row['nik']."' and tanggalabsen='".$tanggal."' limit 1";
				// $bar = fetchdata($str);
				// if($row['nik'] != $bar['karyawanid']){
					// $no++;
					// $errorUpload .= $no.". ".getKary($row['nik'],'nik')." = ".getKary($row['nik'])."<br>";
					// $countUpload = $countUpload + 1;
				// }
			// }
			// if($countUpload > 0){
				// throw new PDOException("Absen fingerprint untuk karyawan dg NIK : <br>".$errorUpload."belum ada.<br>Silahkan lakukan proses melalui menu : SDM - Proses - Fingerprint.");
			// }
		}

		$strupd=" update ".$dbname.".vhc_spl_aktifitas set jurnal='1' where notransaksi='".$param['notransaksi']."'";
		$owlPDO->exec($strupd);			
	...
	// case 'insertDetails':
	// 	// Cek dulu datanya ada atau gak vhc_spl_prestasi_detail 
	// 	$cekPres = selectQuery($dbname,"vhc_spl_prestasi_detail","*","notransaksi='".$param['notransaksi']."'");
	// 	$rcPres = fetchData($cekPres);
	// 	$countPres = count($rcPres);
	// 	// Jika ada di delete dulu datanya
	// 	if ($countPres > 0) {
	// 		$delPres = deleteQuery($dbname,"vhc_spl_prestasi_detail","notransaksi='".$param['notransaksi']."'");
	// 		try {
	// 			$owlPDO->exec($delPres);
	// 		} catch (PDOException $e) {
	...
	// 	// Cek dulu datanya ada atau gak vhc_spl_kehadiran_detail
	// 	$cekHadir = selectQuery($dbname,"vhc_spl_kehadiran_detail","*","notransaksi='".$param['notransaksi']."'");
	// 	$rcHadir = fetchData($cekHadir);
	// 	$countHadir = count($rcHadir);
	// 	// Jika ada di delete dulu datanya
	// 	if ($countHadir > 0) {
	// 		$delHadir = deleteQuery($dbname,"vhc_spl_kehadiran_detail","notransaksi='".$param['notransaksi']."'");
	// 		try {
	...
	// 				'alokasi'			=> $param['blokkecil_pres'][$key],
	// 				'total_hasilkerja'	=> $param['hasilkerja_pres'][$key],
	// 				'total_hk'			=> $param['hk_pres'][$key],
	...
	// 			$qInsPres = insertQuery($dbname,"vhc_spl_prestasi_detail",$dtPres,$cols);
	...
	// 			$qInsHadir = insertQuery($dbname,"vhc_spl_kehadiran_detail",$dtHadir,$cols);
...
function cekPrestasi($param) {
	...
	if($param['method']=='insert'){
		$str = "select a.notransaksi,jhk from ".$dbname.".vhc_spl_kehadiran a
				left join ".$dbname.".vhc_spl_aktifitas b on a.notransaksi=b.notransaksi
	...
	#cek mandor
	$str = "select count(*) as jumkar from ".$dbname.".vhc_spl_aktifitas where nikmandor='".$param['karyawanid']."' and tanggal='".$tgl."' ";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$bar=$res->fetch();
		@$jumtrans+=$bar['jumkar'];
	...
	#cek mandor1
	$str = "select count(*) as jumkar from ".$dbname.".vhc_spl_aktifitas where nikmandor1='".$param['karyawanid']."' and tanggal='".$tgl."' ";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$bar=$res->fetch();
		@$jumtrans+=$bar['jumkar'];
	...
	#cek kerani
	$str = "select count(*) as jumkar from ".$dbname.".vhc_spl_aktifitas where keranimuat='".$param['karyawanid']."' and tanggal='".$tgl."' ";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$bar=$res->fetch();
		@$jumtrans+=$bar['jumkar'];
	...
	#cek nikasisten
	$str = "select count(*) as jumkar from ".$dbname.".vhc_spl_aktifitas where nikasisten='".$param['karyawanid']."' and tanggal='".$tgl."' ";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$bar=$res->fetch();
		@$jumtrans+=$bar['jumkar'];
	...
	$str = "select * from " . $dbname . ".setup_blok where indukblok='".$param['blok']."' and statusblok in (select kelompok from ".$dbname.".setup_kegiatan where kodekegiatan='".$param['kegiatan']."')"; 
	$res = fetchData($str);
	foreach($res as $val){
	...
	if($param['method']=='update'){
		$str = "select sum(total_hasilkerja) as hasilkerja from " . $dbname . ".vhc_spl_prestasi where alokasi='".$param['blok']."' and notransaksi='".$param['notransaksi']."' and nik='".$param['karyawanid']."' and kodekegiatan='".$param['kegiatan']."'"; 
		$res = fetchData($str);
		foreach($res as $val){
	...
	$str = "select sum(total_hasilkerja) as hasilkerja,notransaksi,substr(notransaksi,1,8) from " . $dbname . ".vhc_spl_prestasi where alokasi='".$param['blok']."' and substr(notransaksi,1,8) between '".seminggulalu($tgl)."' and '".$tgl."' and kodekegiatan='".$param['kegiatan']."'  group by notransaksi  order by notransaksi desc"; 
	$res = fetchData($str);
	foreach($res as $val){
...
function getmandor($param){
	...
	$dt=array();
	$str="select * from ".$dbname.".kebun_5asistensi where kodeorgtujuan ='".$param['kodeorg']."' and  tanggal<='".tanggalsystemn($param['tgl'])."' and tanggalsampai>='".tanggalsystemn($param['tgl'])."' and posting='1' and (tipetrans='SPL' or tipetrans='')";
	$resx=fetchdata($str);
	foreach($resx as $res){
	...
	$whereKarykerani.=$whereKary; // kalo kerani where-nya ga usah pake a.subbagian!=''
	$whereKary.=" and a.subbagian!=''";
	$str="select * from ".$dbname.".kebun_5pejabatbkm where kodeorg ='".$param['kodeorg']."' and tipe='SPL'";
	$res=fetchdata($str);
	foreach($res as $bar){
		if($bar['kolom']=='mandor'){
			$mdr=$bar['jabatan'];
...
function getoptkary($param){
	...
	$whereKary.= " and lokasitugas = '".$param['filterkebon']."'";
	...
	if($filterdivisi=='PROJECT'){
		...
		if($filterdivisi!=''){
			if(substr($filterdivisi,0,4)!=$kodeorg){
				$whereBlok.= " and substr(a.kodeorganisasi,1,4) = '".$param['filterkebon']."'";
		...
		$nmrmh=makeOption($dbname,'sdm_5jenis_prasarana','jenis,nama');

		$str = "select * from ".$dbname.".sdm_perumahanht where kodeorg = '".$kodeorg."'";
		$res=fetchData($str);

		if(count($res) > 0){
			$optBlok.="<optgroup label='PERUMAHAN'>";
			foreach ($res as $v) {
				$optBlok.="<option value='".$v['norumah']."'>".$v['norumah']." - ".$nmrmh[$v['tipe']]." - ".$v['blok']."</option>";
			}
			$optBlok.="</optgroup>";
		}
		...
		$str = "select * from ".$dbname.".organisasi a  left join ".$dbname.".setup_blok b on a.kodeorganisasi=b.kodeorg where a.tipe in('BLOK','BIBITAN') ".$whereBlok." and b.luasareaproduktif>0 group by a.indukblok order by substr(a.kodeorganisasi,1,6), a.kodeorganisasi, b.tahuntanam asc";
		$res=fetchdata($str);
		foreach($res as $bar){
			...
			$optBlok.="<option value='".$bar['indukblok']."'>".$bar['indukblok']." - ".$getNamaIndukBlok[$bar['indukblok']]."</option>";
