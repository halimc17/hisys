...
switch($mode) {
    ...
    case 'barang':
		...
		$table = "<table class=sortable cellspacing=1 border=0 style='cursor:pointer;'>";
	...
	case 'barangSaldo':
		...
		#ambil material kegiatan setup_kegiatannorma
		if($_POST['notransaksi']!=''){
			$sKeg="select kodekegiatan from ".$dbname.".kebun_prestasi where notransaksi='".$_POST['notransaksi']."'";
			$rKeg=fetchdata($sKeg);
			$where.=" and a.kodebarang in (select distinct kodebarang from ".$dbname.".setup_kegiatannorma where kodekegiatan='".$rKeg[0]['kodekegiatan']."')";	
		}
		...
		$table = "<table class=sortable cellspacing=1 border=0 width=100%  style='cursor:pointer';>";
		$table .= "<thead><tr class='rowheader'>";
		...
		$table .= "<tbody>";
		foreach($data as $key=>$row) {
			$table .= "<tr style='cursor:pointer;' id='inv_tr_".$key."' class='rowcontent' ";
			$table .= "onclick=\"passValue('".$row['kodebarang']."','".$target."');";
			$table .= "passValue('".$row['namabarang']."','".$target."_name');";
