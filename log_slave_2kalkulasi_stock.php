...
for ($i = 1; $i <= $bulan; $i++) {
    ...
    while ($rData = $qData->fetch()) {
        ...
        if (!isset($resData[$rData['kodebarang']]['totem']))
            $resData[$rData['kodebarang']]['totem'] = 0;
	...
	while ($rData = $qData->fetch()) {
        ...
        if (!isset($resData[$rData['kodebarang']]['tokel']))
            $resData[$rData['kodebarang']]['tokel'] = 0;
...
<th rowspan=1 align=center " . $bg . ">No.</th>";
$tab.="<th align=left " . $bg . ">" . STRTOUPPER($_SESSION['lang']['kodebarang']) . "<br>" . STRTOUPPER($_SESSION['lang']['namabarang']) . "<br>" . STRTOUPPER($_SESSION['lang']['satuan']) . "</th>";
$tab.="<th align=left " . $bg . "></th>";
for ($i = 1; $i <= $bulan; $i++) {
    if (strlen($i) == 1)
        $ii = '0' . $i;
    else
        $ii = $i;
    $per = $tahun . "-" . $ii;
    $tab.="<th align=center " . $bg . ">" . $per . "</th>"; // periode
}
$tab.="<th align=center " . $bg . ">" . $tahun . "</th>"; // total setahun
$tab.="</tr>";
$tab.="</thead><tbody>";
...
//    $tab.="<td rowspan=4 bgcolor='AAAAFF' align=right>".number_format($ar[sallu])."</td>";
        ...
        for ($i = 1; $i <= $bulan; $i++) {
            ...
            if ($pilih == 'nilai')
                $tampilan = $ar[$terima] * $ar[$harga]; // nilai penerimaan
//        $tab.="<td bgcolor='AAFFAA' align=right style='cursor:pointer;' onclick=getDetailGudang('GR','".$ar['kobar']."','".$per."',event) title='Klik untuk melihat detail.'>".number_format($tampilan)."</td>"; 
            ...
            if (!isset($ar['totalharga']))
                $ar['totalharga'] = 0;
            if (!isset($ar['totalbarang']))
                $ar['totalbarang'] = 0;
            if (!isset($ar['totalnilai']))
                $ar['totalnilai'] = 0;
        ...
        if ($pilih == 'nilai')
            $tampilan = $ar['totalharga']; // nilai penerimaan
//    $tab.="<td bgcolor='AAFFAA' align=right style='cursor:pointer;' onclick=getDetailGudang('GR','".$ar['kobar']."','".$tahun."',event) title='Klik untuk melihat detail.'><b>".number_format($tampilan)."</b></td>"; 
        ...
        for ($i = 1; $i <= $bulan; $i++) {
            ...
            if ($pilih == 'nilai')
                $tampilan = $ar[$kasih] * $ar[$harga]; // nilai pengeluaran
        ...
        if ($pilih == 'nilai')
            $tampilan = $ar['totalnilai']; // nilai pengeluaran
        ...
        for ($i = 1; $i <= $bulan; $i++) {
            ...
            if ($pilih == 'nilai')
                $tampilan = $ar[$sama] * $ar[$harga]; // nilai saldo akhir
            $tab.="<td bgcolor='' align=right>" . number_format($tampilan) . "</td>";
//            $tab.="<td align=right>".number_format($ar[$per])."</td>";                    saldo akhir DARI DATABASE !!!
        ...
        if ($pilih == 'nilai')
            $tampilan = $ar['salakpondoh'] * $ar[$harga]; // nilai saldo akhir
        $tab.="<td bgcolor='' align=right><b>" . number_format($tampilan) . "</b></td>";
