...
?>
<script language=javascript src='js/keu_2agingAP.js?v=<?php echo time(); ?>'></script>
<script language="javascript" src="js/zSelect2.js?ver=1"></script>
<?
include('master_mainMenu.php');
OPEN_BOX("","<span class=judul>".getMenu('keu_2agingAP')."<br></span>");
...
$str="select * from ".$dbname.".keu_5akun where detail=1 and aktif='1'";
$res=fetchdata($str);
foreach($res as $bar){
...
$str="select * from ".$dbname.".pmn_4customer where 1";
$res=fetchdata($str);
foreach($res as $bar){
...
// $optperiode.="<option value='2022-02'>2022-02</option>";
echo"<fieldset style='float:left;'>
     ...
     <table>
		...
		<td>:</td>		
		<td>
			<input type=text class=myinputtext id=tanggal name=tanggal  style=\"width:125px;\" value=".date('d-m-Y')." readonly onmousemove=setCalendar(this.id) onkeypress=return false;  maxlength=10 style=width:61px;/>	
		</td>
      ...
      <tr>     
        ...
        <td >:</td>
        <td ><select class='select2' id=kodesupplier style='width:125px;'>".$optsupplier."</select></td>	
	  </tr>
	  ...
	  <td hidden>".$_SESSION['lang']['subledger']." </td>
	  
		<td hidden>".$_SESSION['lang']['periode']." ".$_SESSION['lang']['sampai']." </td>
        <td hidden>:</td>
        <td hidden><select class='select2' id=periode2 style='width:125px;'>".$optperiode."</select></td>
		
		
	  
		<td hidden>".$_SESSION['lang']['periode']." ".$_SESSION['lang']['awal']."</td>
        <td hidden>:</td>
        <td hidden><select class='select2' id=periode style='width:125px;'>".$optperiode."</select></td>
	  
        <td hidden>:</td>
        <td hidden><select class='select2' id=subledger style='width:125px;'>".$optsubledger."</select></td>
		
		<td hidden>".$_SESSION['lang']['jumlah']." ".$_SESSION['lang']['bulan']."</td>
		<td hidden>:</td>		
		<td hidden><input class=myinputtextnumber id=bulan value=4 style=\"width:125px;\" onkeypress='return angka_doang(event)' /></td>	
			
		
		
		<td hidden>".$_SESSION['lang']['customer']." </td>
			<td hidden>:</td>
			<td hidden><select class='select2' id=customer style='width:125px;'>".$optcustomer."</select></td>
	  </tr>
