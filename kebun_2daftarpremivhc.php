...
OPEN_BOX('', '<span class=judul>'.getMenu('kebun_2daftarpremivhc').'</span><br>');
?>
...
$arrJenis=array("KD"=>"Premi Kegiatan","AB"=>"Premi Alat Berat");
$jenis="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
foreach($arrJenis as $val => $res)
{
	$jenis.="<option value=".$val.">".$res."</option>";
}
        ...
        <legend>Form</legend>
            <table border=0 cellpadding=1 cellspacing=1>
				...
				<tr>
                    ...
                    <td>:</td>
                    <td><select id=jenis style=\"width:174px;\">" . $jenis . "</select></td>
                </tr>
                <tr>
                    <td colspan=2></td>
                    <td colspan=4>
                    <button onclick=zPreview('kebun_slave_2daftarpremivhc','" . $arr . "','printContainer') class=mybutton name=preview id=preview>" . $_SESSION['lang']['preview'] . "</button>
                    <button onclick=zExcel(event,'kebun_slave_2daftarpremivhc.php','" . $arr . "') class=mybutton name=preview id=preview>" . $_SESSION['lang']['excel'] . "</button>
                    </td>
                </tr>
            </table>
</fieldset>";
