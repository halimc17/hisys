...
if($proses!='excel')$tab.=$judul;
	...
	if(in_array($_SESSION['empl']['kodejabatan'],$postJabatan) && $proses!='excel'  && $periode>=$peraktif)  {
		...
		#$tab.="<td align=center rowspan=4  colspan=2 style='width=270px'>".$_SESSION['lang']['penjelasan']." & Upload File Pendukung</td>";
	}
	$tab.="
    </tr>
    <tr>
...
$str="SELECT noakun, namaakun,namaakun1 FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=5 and  (left(noakun,5) between '12606' and '12616')
    ORDER BY noakun";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
#bentuk akun 7
$str="SELECT noakun, namaakun,namaakun1 FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=7 and  (left(noakun,7) between '1260601' and '1261601')
    ORDER BY noakun";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select kegiatan,noakun,".$addstr." as sdbi,rp".$bulan." as bi,rupiah,tahunbudget,kodeorg,".$addstrvol." as sdbivol,fis".$bulan." as bivol,volume
		from ".$dbname.".bgt_budget_detail where kodeorg like '".$sortunit."%' and tahunbudget = '".$tahun."' and (left(noakun,5) between '12606' and '12616')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select * from ".$dbname.".kebun_perawatan_dan_spk_vw where unit='".$unit."' 
	and kodeorg like '".$sortunit."%' and tanggal like '".$periode."%' and (left(kodekegiatan,5) between '12606' and '12616')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select * from ".$dbname.".kebun_perawatan_dan_spk_vw where unit='".$unit."' 
	and kodeorg like '".$sortunit."%' and tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15') 
	and (left(kodekegiatan,5) between '12606' and '12616')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
	@$textinput[$bar['parameter']]=$bar['text'];
}
...
foreach($akunlima as $noakunlima){
	...
	foreach($akun as $noakun){
		if($noakunlima==substr($noakun,0,5)){
			...
			$tab.="<td  style='cursor:pointer' title='click to show details' onclick=\"detailrow('".$no."','".@$totkeg[$noakunlima][$noakun]."')\"></td>";
			$tab.="<td align=right style='cursor:pointer' title='click to show details' onclick=\"detailrow('".$no."','".@$totkeg[$noakunlima][$noakun]."')\">".@number_format($bgtthnvoltjh[$noakunlima][$noakun])."</td>";
