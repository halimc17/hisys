...
if($pt!=''){
	...
	$where_kap=" and substr(a.kodeunit,1,4) in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$pt."' and tipe='KEBUN')";
...
if($kdorg!=''){
	...
	$where_kap=" and substr(a.kodeunit,1,4) ='".$kdorg."'";
...
if(strlen($akun)>7){
	$whereakun = " and kegiatan like '".$akun."%'";	
}else{
...
switch ($real) {
	...
	case'budget':
		if(substr($akun,0,3)=='127'){
			...
			$optkode=makeOption($dbname,'sdm_5tipeasset','kodetipe,namatipe');
			...
			$str = "select akunaset as noakun, a.*, ".$e." as sdbi,k".$bulan." as bi from ".$dbname.".bgt_kapital a left join ".$dbname.".sdm_5tipeasset b on `a`.`jeniskapital` = `b`.`kodetipe` where 1=1 ".$where_kap." and akunaset like '".$akun."%' and tahunbudget = '".$tahun."' and pta='BGT'"; 
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			$no=0;
			while ($bar = $res->fetch()) {
				...
				$tab.="<tr class=rowcontent>";
				$tab.="<td align=center>".$no."</td>";
				$tab.="<td>".$optkode[$bar['jeniskapital']]."</td>";
				$tab.="<td align=left>".$bar['noakun']." - ".$optakun[$bar['noakun']]."</td>";
				$tab.="<td align=left>".$bar['keterangan']."</td>";
				if(getNamaOrg($bar['lokasi'])!=''){					
					$tab.="<td>".getNamaOrg($bar['lokasi'])."</td>";
				}else{
				...
				$tab.="<td align=right ".$boldthn.">".@number_format($bar['hargatotal'],0)."</td>";
				$tab.="</tr>";
				
				@$bgtbi += $bar['bi'];
				@$bgtsdbi += $bar['sdbi'];
				@$bgtthn += $bar['hargatotal'];
			}
			$tab.="<tr class=rowcontent>";
			...
			$str=" select keterangan, kegiatan,rp01,rp02,rp03,rp04,rp05,rp06,rp07,rp08,rp09,rp10,rp11,rp12,kodevhc,kodebarang,
			...
			where 1=1 ".$where." ".$wh_bgtrp."  and tahunbudget = '".$tahun."' ".$whereakun." order by kodebudget, kegiatan, kodeorg";
			...
			while ($bar = $res->fetch()) {
				...
				if($bar['jumlah']>0){					
					$tab.="<td align=right>".@number_format($bar['rupiah']/$bar['jumlah'],2)."</td>";
				}else{
					$tab.="<td></td>";
				}
				$tab.="</tr>";
