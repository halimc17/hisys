...
while($bar=$str->fetch()){
    ...
    $semuakolomdb='debet01-kredit01+debet02-kredit02+debet03-kredit03+debet04-kredit04+debet05-kredit05+debet06-kredit06+debet07-kredit07+debet08-kredit08+debet09-kredit09+debet10-kredit10+debet11-kredit11+debet12-kredit12';
     $st12=$owlPDO->query("select noakun,(".$semuakolomdb.") as jumlah, awal01 as awal,awal02, awal03, awal04, awal05, awal06, awal07, awal08, awal09, awal10, awal11, awal12,substr(periode,5,2) as bulan
    ...
    if($data['tipe']=='Total'){
		...
		switch($data['nourut']){
			...
			case'300014':
				for($i = 1; $i <= $bulan; $i++){
					if($i<10)$ii='0'.$i; else $ii=$i;
					$sAwl=$owlPDO->query("select sum(awal".$ii.") as awal from ".$dbname.".keu_saldobulanan_ori where noakun='1150201' and left(periode,4)='".$tahun."' and kodeorg ".$where."");
			...
			case'300015':
				for($i = 1; $i <= $bulan; $i++){
					if($i<10)$ii='0'.$i; else $ii=$i;
					$sAwl=$owlPDO->query("select sum(awal".$ii.") as awal from ".$dbname.".keu_saldobulanan_ori where noakun='1150202' and left(periode,4)='".$tahun."' and kodeorg ".$where."");
					$sAwl->setFetchMode(PDO::FETCH_ASSOC); 
					$rAwl=$sAwl->fetch();
					$dzArr[$data['nourut']][$ii]=$rAwl['awal'];
					//$dzArr[$data['nourut']]['sd']+=$rAwl['awal'];
