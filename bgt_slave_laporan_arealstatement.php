...
$str="select sum(hathnini) as hathnini,sum(hanonproduktif) as hanonproduktif,sum(pokokproduksi) as pokokproduksi,
      ...
      order by thntnm, substr(kodeblok,1,6)";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
$rowdata1 = array();
while($bar= $res->fetch()){
	$data[$bar->statusblok][$bar->thntnm]=$bar->thntnm;
	$luas[$bar->statusblok][$bar->thntnm][$bar->afdeling]+=$bar->hathnini;
	$pkk[$bar->statusblok][$bar->thntnm][$bar->afdeling]+=$bar->pokokproduksi;
...
echo"<table class=sortable cellpadding=5 cellspacing=1 border=0>
     <thead>
        <tr class=rowtitle>
            <th rowspan=3 align=center>".$_SESSION['lang']['uraian']."</th>
            <th rowspan=3 align=center>".$_SESSION['lang']['tahuntanam']."</th>
            <th colspan=".$jumlahafdeling." align=center>Data per Divisi</th>";
       echo"<th rowspan=2 colspan=2 align=center >".$_SESSION['lang']['total']."</th>
        </tr>";
    if(!empty($headerdata))foreach($headerdata as $baris){
		echo"<th align=center  colspan=2>".$baris."</th>";
    } 
	
	echo"<tr class=rowtitle>";
    if(!empty($headerdata))foreach($headerdata as $baris){		
		echo"<th align=center>Luas</th>";
...
foreach($data as $stsblok => $vtt){
	foreach($vtt as $tt){
		...
		echo"<td align=left>".$stsblok."</td>";
		echo"<td align=center>".$tt."</td>";
		foreach($headerdata as $divisi){
			echo"<td align=right>".number_format($luas[$stsblok][$tt][$divisi],2)."</td>";
			echo"<td align=right>".number_format($pkk[$stsblok][$tt][$divisi])."</td>";
			$ttlluasknn[$stsblok][$tt]+=$luas[$stsblok][$tt][$divisi];
			$ttlpkkknn[$stsblok][$tt]+=$pkk[$stsblok][$tt][$divisi];
			
			$ttlluasbtm[$stsblok][$divisi]+=$luas[$stsblok][$tt][$divisi];
			$ttlpkkbtm[$stsblok][$divisi]+=$pkk[$stsblok][$tt][$divisi];
			
			$ttlluasknnx[$stsblok]+=$luas[$stsblok][$tt][$divisi];
			$ttlpkkknnx[$stsblok]+=$pkk[$stsblok][$tt][$divisi];
			
			$gtluas[$divisi]+=$luas[$stsblok][$tt][$divisi];
			$gtpkk[$divisi]+=$pkk[$stsblok][$tt][$divisi];
			
			$gtluasx+=$luas[$stsblok][$tt][$divisi];
			$gtpkkx+=$pkk[$stsblok][$tt][$divisi];
		}
		echo"<td align=right>".number_format($ttlluasknn[$stsblok][$tt],2)."</td>";
		echo"<td align=right>".number_format($ttlpkkknn[$stsblok][$tt])."</td>";
	}	
	echo"<tr class=rowcontent style=font-weight:bold>";
	echo"<td align=left colspan=2>Sub Total ".$stsblok."</td>";
	foreach($headerdata as $divisi){		
		echo"<td align=right>".number_format($ttlluasbtm[$stsblok][$divisi],2)."</td>";
		echo"<td align=right>".number_format($ttlpkkbtm[$stsblok][$divisi])."</td>";
	}
	echo"<td align=right>".number_format($ttlluasknnx[$stsblok],2)."</td>";
	echo"<td align=right>".number_format($ttlpkkknnx[$stsblok])."</td>";
}
echo"<tr class=rowcontent style=font-weight:bold>";
echo"<td align=left colspan=2>Grand Total</td>";
foreach($headerdata as $divisi){		
	echo"<td align=right>".number_format($gtluas[$divisi],2)."</td>";
	echo"<td align=right>".number_format($gtpkk[$divisi])."</td>";
}
echo"<td align=right>".number_format($gtluasx,2)."</td>";
	echo"<td align=right>".number_format($gtpkkx)."</td>";
...
// if(!empty($headerdata))foreach($headerdata as $af)
// {
    // echo"<td align=center>Jumlah<br>Pokok</td><td align=center>Pokok<br>Produktif</td>"; 
// }
