...
function ambilUnitPembebananBarang($returntype='array'){
	...
	$list='';
	$option="";
	$str="select distinct kodeorganisasi,namaorganisasi,tipe from ".$dbname.".organisasi where length(kodeorganisasi)=4 and induk!='' and kodeorganisasi in (select kodeunit from ".$dbname.".bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."') order by kodeorganisasi"; #exit("error".$str);
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_OBJ);
	while($bar=$res->fetch()){
		if($bar->tipe=='PT'){
...
function getKegiatanBlok($returntype='array',$blok,$statusblok = ''){
	global $dbname;
	global $conn;
	global $owlPDO;
	$arr=Array();
	$status_blok=Array();
        ...
        if($statusblok != ''){
		$whereStatusBlok = "and statusblok = '".$statusblok."' ";
	}
	...
	$option="";
        // cek apakah ada data
	$str_cek="select statusblok from ".$dbname.".setup_blok where kodeorg='".$blok."'";
        $rData_cek=fetchdata($str_cek);
        if(count($rData_cek)>0){
                // query old
                $str="select statusblok from ".$dbname.".setup_blok where kodeorg='".$blok."'";
        }else{
                // untuk masuk while
                $str="select statusblok from ".$dbname.".setup_blok where kodeorg like '".$blok."%' limit 1 ";
                // ambil status di blok kecil
                $str_kecil="select statusblok from ".$dbname.".setup_blok where kodeorg like '".$blok."%' ".$whereStatusBlok." group by statusblok ";
                $rData_kecil=fetchdata($str_kecil);
                foreach($rData_kecil as $row) {
                        $status_blok[$row['statusblok']] .= $row['statusblok'];
                        $ada_arrstatusblok = '1';
                }
        }
	...
	while($bar1=$res->fetch()){
		$no+=1;
                if($ada_arrstatusblok == '1'){
                        $str="select kodekegiatan,kelompok,namakegiatan, noakun as kelakun from ".$dbname.".setup_kegiatan where kelompok in ('".implode("','",$status_blok)."') and status = '1' order by noakun, kelompok,kodekegiatan"; 
                }else{
                        if($bar1->statusblok=='TM'){
                                $str="select kodekegiatan,kelompok,namakegiatan, noakun as kelakun from ".$dbname.".setup_kegiatan where (kelompok='TM' or kelompok='PNN') and status = '1' order by noakun, kelompok,kodekegiatan";
                        }else{
                                $str="select kodekegiatan,kelompok,namakegiatan, noakun as kelakun from ".$dbname.".setup_kegiatan where kelompok='".$bar1->statusblok."' and status = '1' order by noakun, kelompok,kodekegiatan"; 
                        } 
                }
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_OBJ);
		...
		while($bar=$res->fetch()){
			if($returntype=='array'){
				...
				if($d!=$n){			
					$option.="<optgroup label='".$d." - ".getNamaAkun($d)."'>";
				}
					$option.= "<option value='".$bar->kodekegiatan."'>".substr($bar->kodekegiatan,-2)." - ".$bar->namakegiatan."</option>";
				$n=$d;
				if($d!=$n){			
					$optorg.="</optgroup>";
				}
