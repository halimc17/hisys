...
$sOrg = "select kodeorganisasi,namaorganisasi,tipe,induk from " . $dbname . ".organisasi where kodeorganisasi='".$_SESSION['empl']['lokasitugas']."' order by kodeorganisasi asc";
$qOrg = $owlPDO->query($sOrg) or die(print " Gagal: " . PDOException::getMessage());
$qOrg->setFetchMode(PDO::FETCH_ASSOC);
while ($rOrg = $qOrg->fetch()) {
    if($rOrg['tipe']=='HOLDING' or $rOrg['tipe']=='KANWIL')
    {
        $optOrg = "<option value=''>" . $_SESSION['lang']['all'] . "</option>";
        $sOrg = "select kodeorganisasi,namaorganisasi from " . $dbname . ".organisasi where tipe='PT' order by kodeorganisasi asc";
    }
    else
    {
        $sOrg = "select kodeorganisasi,namaorganisasi from " . $dbname . ".organisasi where kodeorganisasi='".$_SESSION['empl']['lokasitugas']."' order by kodeorganisasi asc";
    }
}
// echo $sOrg;
// exit('error');
...
$arr = "##unit##klmpkBrg##subklmpkBrg##kdBrg##tglDr##tanggalSampai";
?>
...
<script>
    function getBrg()
    {
        ...
        tujuan = "log_slave_2realisasipo.php";
        //alert(param);	
...
<?
OPEN_BOX('','<span class=judul>REALISASI PURCHASE ORDER</span>');
?>
<div>
    <fieldset style="float: left;">
		<legend><b><?php echo $_SESSION['lang']['form'] ?></b></legend>
		<table cellspacing="1" border="0" >
            <tr><td><label><?php echo $_SESSION['lang']['unit'] ?></label></td><td>:</td><td colspan='3'><select id="unit" name="unit" style="width:185px"><?php echo $optOrg ?></select></td></tr>
			...
			<tr><td></td><td><td colspan="3"><button onclick="zPreview('log_slave_2realisasipo', '<?php echo $arr ?>', 'printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button onclick="zExcel(event, 'log_slave_2realisasipo.php', '<?php echo $arr ?>')" class="mybutton" name="preview" id="preview">Excel</button></td></tr>
