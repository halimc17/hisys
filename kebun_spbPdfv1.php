 ...
 class PDF extends FPDF
        {
            function Header() {
                ...
                $arrIntex=array("0"=>"Internal","1"=>"Afiliasi","3"=>"External","4"=>"TPH Besar");
            ...
            function Footer()
            {
                ...
                $this->Cell(0,10,'Dicetak: '.date('d-m-Y H:i:s').' oleh '.$_SESSION['empl']['name'].'     Halaman '.$this->PageNo().' / {nb}',0,0,'L');
            }
        }
        $pdf=new PDF('P','pt','A4');
        $pdf->AliasNbPages();
...
//kebun_spbdt
				$no=0;
                #hasil timbang per blok ada di kebun_spbdt_detail (terisi saat posting); kalau belum ada pakai kebun_spbdt
                $tbldt="kebun_spbdt";
                $cekdet=fetchdata("select count(*) as jml from ".$dbname.".kebun_spbdt_detail where nospb='".$noSpb."'");
                if($cekdet[0]['jml']>0){
                    $tbldt="kebun_spbdt_detail";
                }
                $str="select * from ".$dbname.".".$tbldt."   where nospb='".$noSpb."'"; //echo $str;exit();
