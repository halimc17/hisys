...
$penyusutan = checkPostGet('penyusutan', '');
...
if($region==''){exit("warning : Kode Regional harus di pilih.");}
...
$tab.="
    <thead>
        <tr class=rowheader>
            <th align=center rowspan='2' ".$hide.">Description</th>
            <th align=center rowspan='2'>Description</th>
            <th align=center rowspan='2'>UoM</th>
            <th align=center rowspan='1' colspan=3>This Month</th>
            <th align=center rowspan='1' colspan=3>Year To Date</th>
            <th align=center rowspan='2'>YTD SMLY</th>
            <th align=center rowspan='2'>Bgt This Year</th>
            <th align=center rowspan='2'>% TDTM  vs<br>TD SMLY</th>
            <th align=center rowspan='2'>% TDTM  vs<br>Budget TY</th>
			";
    $tab.="</tr>";
    $tab.="<tr class=rowheader>";
    $tab.="<th align=center >Actual</th>";
    $tab.="<th align=center >Budget</th>";
    $tab.="<th align=center >%Var</th>";
	$tab.="<th align=center >Actual</th>";
    $tab.="<th align=center >Budget</th>";
    $tab.="<th align=center >%Var</th>";
    $tab.="</tr>";
...
foreach($res as $bar){	
	if($bar['statusblok']=='TM'){
		if($bar['intiplasma']=='I'){			
			$data['tmi']['bgtbi']+=$bar['hathnini'];
			$data['tmi']['bgtsdbi']+=$bar['hathnini'];
			$data['tmi']['bgtthn']+=$bar['hathnini'];
			
			$data['ttlplanti']['bgtbi']+=$bar['hathnini'];
			$data['ttlplanti']['bgtsdbi']+=$bar['hathnini'];
			$data['ttlplanti']['bgtthn']+=$bar['hathnini'];
		}elseif($bar['intiplasma']=='P'){
			$data['tmp']['bgtbi']+=$bar['hathnini'];
			$data['tmp']['bgtsdbi']+=$bar['hathnini'];
			$data['tmp']['bgtthn']+=$bar['hathnini'];
			
			$data['ttlplantp']['bgtbi']+=$bar['hathnini'];
			$data['ttlplantp']['bgtsdbi']+=$bar['hathnini'];
			$data['ttlplantp']['bgtthn']+=$bar['hathnini'];
		}
	}elseif($bar['statusblok']=='TBM'){
		if($bar['intiplasma']=='I'){			
			$data['tbmi']['bgtbi']+=$bar['hathnini'];
			$data['tbmi']['bgtsdbi']+=$bar['hathnini'];
			$data['tbmi']['bgtthn']+=$bar['hathnini'];
			
			$data['ttlplanti']['bgtbi']+=$bar['hathnini'];
			$data['ttlplanti']['bgtsdbi']+=$bar['hathnini'];
			$data['ttlplanti']['bgtthn']+=$bar['hathnini'];
		}elseif($bar['intiplasma']=='P'){
			$data['tbmp']['bgtbi']+=$bar['hathnini'];
			$data['tbmp']['bgtsdbi']+=$bar['hathnini'];
			$data['tbmp']['bgtthn']+=$bar['hathnini'];
		...
		$data['tb']['bgtbi']+=$bar['hathnini'];
		$data['tb']['bgtsdbi']+=$bar['hathnini'];
		$data['tb']['bgtthn']+=$bar['hathnini'];
	}elseif($bar['statusblok']=='BBT'){
		$data['bbt']['bgtbi']+=$bar['pokokthnini'];
		$data['bbt']['bgtsdbi']+=$bar['pokokthnini'];
		$data['bbt']['bgtthn']+=$bar['pokokthnini'];
	}
}

#total ha estate
$str = "select * from ".$dbname.".setup_blok a where 1=1 ".$wh2." ".$where." and a.luasareaproduktif>0 and jumlahpokok>0 order by a.kodeorg";
$res = fetchdata($str);
foreach($res as $bar){
	if($bar['statusblok']=='TM'){
		if($bar['intiplasma']=='I'){			
			$data['tmi']['actbi']+=$bar['luasareaproduktif'];
			$data['tmi']['actsdbi']+=$bar['luasareaproduktif'];
			
			$data['ttlplanti']['actbi']+=$bar['luasareaproduktif'];
			$data['ttlplanti']['actsdbi']+=$bar['luasareaproduktif'];
		}elseif($bar['intiplasma']=='P'){
			$data['tmp']['actbi']+=$bar['luasareaproduktif'];
			$data['tmp']['actsdbi']+=$bar['luasareaproduktif'];
			
			$data['ttlplantp']['actbi']+=$bar['luasareaproduktif'];
			$data['ttlplantp']['actsdbi']+=$bar['luasareaproduktif'];
		}
	}elseif($bar['statusblok']=='TBM'){
		if($bar['intiplasma']=='I'){			
			$data['tbmi']['actbi']+=$bar['luasareaproduktif'];
			$data['tbmi']['actsdbi']+=$bar['luasareaproduktif'];
			
			$data['ttlplanti']['actbi']+=$bar['luasareaproduktif'];
			$data['ttlplanti']['actsdbi']+=$bar['luasareaproduktif'];
		}elseif($bar['intiplasma']=='P'){
			$data['tbmp']['actbi']+=$bar['luasareaproduktif'];
			$data['tbmp']['actsdbi']+=$bar['luasareaproduktif'];
			
			$data['ttlplantp']['actbi']+=$bar['luasareaproduktif'];
			$data['ttlplantp']['actsdbi']+=$bar['luasareaproduktif'];
		}
	}elseif($bar['statusblok']=='TB'){
			$data['tb']['actbi']+=$bar['luasareaproduktif'];
			$data['tb']['actsdbi']+=$bar['luasareaproduktif'];
	}
}
#total ha estate smly
$str="select * from ".$dbname.".setup_blok_tahunan a where 1=1 ".$wh2." ".$where." and a.luasareaproduktif>0 and jumlahpokok>0 and tahun='".($tahun-1).$bulan."' order by a.kodeorg";
$res = fetchdata($str);
foreach($res as $bar){
	if($bar['statusblok']=='TM'){
		if($bar['intiplasma']=='I'){			
			$data['tmi']['ytdsmly']+=$bar['luasareaproduktif'];
			
			$data['ttlplanti']['ytdsmly']+=$bar['luasareaproduktif'];
		}elseif($bar['intiplasma']=='P'){
			$data['tmp']['ytdsmly']+=$bar['luasareaproduktif'];
			
			$data['ttlplantp']['ytdsmly']+=$bar['luasareaproduktif'];
		}
	}elseif($bar['statusblok']=='TBM'){
		if($bar['intiplasma']=='I'){			
			$data['tbmi']['ytdsmly']+=$bar['luasareaproduktif'];
			
			$data['ttlplanti']['ytdsmly']+=$bar['luasareaproduktif'];
		}elseif($bar['intiplasma']=='P'){
			$data['tbmp']['ytdsmly']+=$bar['luasareaproduktif'];
			
			$data['ttlplantp']['ytdsmly']+=$bar['luasareaproduktif'];
		}
	}elseif($bar['statusblok']=='TB'){
			$data['tb']['ytdsmly']+=$bar['luasareaproduktif'];
	}
}	
...
#bibitan
$str = "select sum(jumlah) as jumlah,substr(tanggal,1,7) as periode from " . $dbname . ".bibitan_mutasi a  where 1=1 ".$where." and substr(tanggal,1,7) <=  '".$periode2."' and post='1'"; 
$res = fetchdata($str);
foreach($res as $bar){
	$data['bbt']['actbi']+=$bar['jumlah'];
	$data['bbt']['actsdbi']+=$bar['jumlah'];
}

$str = "select sum(jumlah) as jumlah,substr(tanggal,1,7) as periode from " . $dbname . ".bibitan_mutasi a  where 1=1 ".$where." and substr(tanggal,1,7) <=  '".$periodelalu2."' and post='1'"; 
$res = fetchdata($str);
foreach($res as $bar){
	$data['bbt']['ytdsmly']+=$bar['jumlah'];
}

#ambil prd real
$prdton=$prdtontitle=array();
$str = "select sum(kgwb) as kgwb, substr(tanggal,1,7) as periode,substr(tanggal,1,4) as tahun,intiplasma from " . $dbname . ".kebun_spb_vw a  where 1=1 ".$wh_spb." ".$where_spb." and (substr(tanggal,1,7) between '".$periode1."' and  '".$periode2."' or substr(tanggal,1,7) between '".$periodelalu1."' and  '".$periodelalu2."') group by periode,intiplasma"; 
$res = fetchdata($str);
foreach($res as $bar){
	if($bar['tahun']==$tahun){
	#tahun ini
		if($bar['periode']==$prd){
			if($bar['intiplasma']=='P'){
				$data['tbsp']['actbi']+=$bar['kgwb']/1000;
			}else{			
				$data['tbsi']['actbi']+=$bar['kgwb']/1000;
			}
			$data['ttlffb']['actbi']+=$bar['kgwb']/1000;		
		}
		if($bar['intiplasma']=='P'){
			$data['tbsp']['actsdbi']+=$bar['kgwb']/1000;
		}else{		
			$data['tbsi']['actsdbi']+=$bar['kgwb']/1000;
		}
		$data['ttlffb']['actsdbi']+=$bar['kgwb']/1000;		
	}else{
	#tahun lalu
		if($bar['intiplasma']=='P'){
			$data['tbsp']['ytdsmly']+=$bar['kgwb']/1000;
		}else{		
			$data['tbsi']['ytdsmly']+=$bar['kgwb']/1000;
		}
		$data['ttlffb']['ytdsmly']+=$bar['kgwb']/1000;		
	}
}
...
foreach($res as $bar){
	if($bar['tahun']==$tahun){		
		if($bar['periode']==$prd){
			...
			//$data['belitbs']['actbi']+=$bar['totalrp']/1000;
		}
		$data['tbso']['actsdbi']+=$bar['kgbruto']/1000;
		$data['ttlffb']['actsdbi']+=$bar['kgbruto']/1000;		
		//$data['belitbs']['actsdbi']+=$bar['totalrp']/1000;
	}else{
		$data['tbso']['ytdsmly']+=$bar['kgbruto']/1000;
		$data['ttlffb']['ytdsmly']+=$bar['kgbruto']/1000;		
		//$data['belitbs']['ytdsmly']+=$bar['totalrp']/1000;
	}
}

#data sudah diambil di jurnal
#BELI TBS PLASMA
/* $str = "select a.*,substr(tanggalpks,1,7) as periode,substr(tanggalpks,1,4) as tahun from " . $dbname . ".kebun_tbsafiliasi a  where 1=1 ".$whtbsplasma." and (substr(tanggalpks,1,7) between '".$periode1."' and  '".$periode2."' or substr(tanggalpks,1,7) between '".$periodelalu1."' and  '".$periodelalu2."')"; 
$res = fetchdata($str);
foreach($res as $bar){
	if($bar['tahun']==$tahun){		
		if($bar['periode']==$prd){
...
$res = fetchdata($str);
foreach($res as $bar){
	$bar['noakun'] = substr($bar['akun'],0,3);
	$bar['tahun'] = substr($bar['periode'],0,4);
	...
	if($bar['noakun']=='621'){
		if(in_array($bar['akun'],$akunppk)){			
			if($bar['periode']==$prd){
				$showcomment['ppk'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['akun']);
			}
			$showcomment['ppk'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
		}else{
			if($bar['periode']==$prd){
				$showcomment['tmc'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
			}
			$showcomment['tmc'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
		}
	}else if($bar['noakun']=='611'){
		if(in_array($bar['akun'],$akuntrans)){			
			if($bar['periode']==$prd){
				$showcomment['atbs'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['akun']);
			}
			$showcomment['atbs'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
		}else{
			if($bar['periode']==$prd){
				$showcomment['pnn'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
			}
			$showcomment['pnn'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
		}
	}else if(substr($bar['noakun'],0,2)=='63'){
		if($bar['periode']==$prd){
			$showcomment['prosescost'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
		}
		$showcomment['prosescost'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
	}else if(substr($bar['noakun'],0,1)=='7'){		
		if($bar['periode']==$prd){
			if($tipeOrg[$bar['unit']]=='PABRIK'){
				$showcomment['prosescost'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
			}else{
				$showcomment['gc'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
			}
		}
		if($tipeOrg[$bar['unit']]=='PABRIK'){
			$showcomment['prosescost'][$code][]=array('id'=>$bar['id'],'user'=>$bar['createdby'],'comment'=>substr($bar['comment'],0,40),'akun'=>$bar['noakun']);
...
foreach($res as $bar){
	...
	}else if(substr($bar['noakun'],0,3)=='511'){ #akun revenue
		// if(in_array($bar['noakun'],$akunrevcpo)){
			// $data['revenuecpo']['bgtbi']+=$bar['bi']/1000;
			// $data['revenuecpo']['bgtsdbi']+=$bar['sdbi']/1000;
			// $data['revenuecpo']['bgtthn']+=$bar['rupiah']/1000;
		// }else{
			// $data['revenuepk']['bgtbi']+=$bar['bi']/1000;
			// $data['revenuepk']['bgtsdbi']+=$bar['sdbi']/1000;
			// $data['revenuepk']['bgtthn']+=$bar['rupiah']/1000;
		// }
	}
}
...
$arrheadcomment=array(
	'actbi'=>array('act'=>'real','bi'=>'bi'),
	'bgtbi'=>array('act'=>'bgt','bi'=>'bi'),
	'actsdbi'=>array('act'=>'real','bi'=>'sdbi'),
	'bgtsdbi'=>array('act'=>'bgt','bi'=>'sdbi'),
	'bgtthn'=>array('act'=>'bgt','bi'=>'thn')
);
// echo"<pre>";
// print_r($arrheadcomment);
// exit();
...
foreach($descid as $iddesc){
	...
	foreach($arrheader as $head){
		...
		@$data['gcperha'][$head]=($data['gc'][$head]/$data['tmi'][$head])*1000;
		...
		if(isset($data['tmi'][$head])){
			...
			@$data['tmcperha'][$head]=($data['tmc'][$head]/$data['tmi'][$head])*1000;
			@$data['ppkperha'][$head]=($data['ppk'][$head]/$data['tmi'][$head])*1000;
			@$data['ucperha'][$head]=($data['uc'][$head]/$data['tmi'][$head])*1000;
		...
		if(isset($data['tmi'][$head])){
			@$data['dcostestperha'][$head]=($data['dcostest'][$head]/$data['tmi'][$head])*1000;			
		}			
		...
		// if(!empty($data[$iddesc][$head])){			
			// if($style[$iddesc]!=''){
				// $style[$iddesc]=$style[$iddesc]."cursor:pointer; title='Click untuk melihat detail.'";
			// }else{
				// $style[$iddesc]="style=cursor:pointer; title='Click untuk melihat detail.'";
			// }
			// $click="onclick=getdetail('".$iddesc."','".$head."','".$region."','".$prd."')";
		// }else{
			// $click="";
		// }
		...
		$adacomment=""; $flag=2;
		$title="";
		if(!empty($showcomment[$iddesc][$head])){
			$adacomment="class=has_sign";
			$title=" title='".getKary($showcomment[$iddesc][$head][0]['user'])."\n".$showcomment[$iddesc][$head][0]['comment']."\n\nClick Kanan -> Show Comment.'";
			$det[$iddesc][$head]=$title." onmousedown=\"rightclick(event,this,'".$flag."','".$showcomment[$iddesc][$head][0]['akun']."','".$region."','".$prd."','".$arrheadcomment[$head]['bi']."','".$arrheadcomment[$head]['act']."')\"";
		}
		
		
		#tampilkan data
	 	$tab.="<td align=right ".$click." ".$style[$iddesc]." ".$adacomment." ".$det[$iddesc][$head].">".numb_format($data[$iddesc][$head],2)."</td>";
	}
	$tab.="</tr>";
}	
