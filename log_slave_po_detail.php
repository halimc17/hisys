...
if($_POST['proses']=='createTable')
{
	...
	$nopo="/".date('Y')."/PO-HO/".$nopp."/".$res2['kodeorg'];
	...
	$nopo=$counter."/".$bln."/".$thn."/PO-HO/".$nopp."/".$res2['kodeorg'];
	$tglSkrng=date("Y-m-d");
	$sIns="insert into ".$dbname.".log_poht (nopo,tanggal,kodeorg,purchaser,lokalpusat,statuspo,tgledit,kurs,matauang) values 
	('".$nopo."','".$tglSkrng."','".$res2['kodeorg']."','".$_SESSION['standard']['userid']."','0','2','".$tglSkrng."','1','IDR')";
	try{
		$owlPDO->exec($sIns);
...
if($_POST['proses']=='listPp')
{
	...
	while($res2=$query2->fetch())
	{
		...
		<button class=mybutton name=process  id=process onclick=process()>". $_SESSION['lang']['proses']."</button>
		<button  class=mybutton name=cancel id=cancel onclick=displayList()>". $_SESSION['lang']['cancel']."</button>
		</td></tr>";
}
    ...
    if($data!=array()) {
	    ...
	    $table .="<td><input class=myinputtext style=width:200px id=\"spek_brg_".$key."\" cols=\"25\" ></td>";
            ...
            $table .= "<td>".makeElement("harga_satuan_".$key."",'textnum','',
				...
				'onkeyup'=>"calculate('".$key."')",'onblur'=>"periksa_isi(this)",'onblur'=>"display_number('".$key."')",'onfocus'=>"normal_number('".$key."')"))."<br>".makeElement("hidden_harga_satuan_".$key."",'hidden',number_format(isset($row['hargasbldiskon']),2,'.',','),
