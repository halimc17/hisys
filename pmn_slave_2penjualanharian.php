...
$kdBrg1 = checkPostGet('kdBrg1', '');
$idPabrik1 = checkPostGet('idPabrik1', '');
...
if (($tanggalakhir1 - $tanggalmulai1) < 0) {
    ...
    switch ($proses) {
        case'preview':
            echo" <table class=sortable cellspacing=1 border=0  style=width:125% >
          <thead>
              <tr class=rowheader>
              <td align=center rowspan=2>No.</td>
              <td align=center rowspan=2>" . $_SESSION['lang']['NoKontrak'] . "</td>
             <td align=center rowspan=2>" . $_SESSION['lang']['suratjalan'] . "</td>
             ...
             */
            $sql = "select *  from " . $dbname . ".pmn_kontrakjual where (tanggalkontrak between '" . $tanggalmulai1 . "' and '" . $tanggalakhir1 . "') and kodebarang like '%" . $kdBrg1 . "%' and kodept='" . $idPabrik1 . "' order by tanggalkontrak asc";
            ...
            if ($row > 0) {
                ...
                while ($res = $query->fetch()) {
                    ...
                    $sStat = "select penjualan  from " . $dbname . ".pmn_5franco where id_franco='" . $res['franco'] . "'";
                    $qStat = $owlPDO->query($sStat) or die(print " Gagal: " . PDOException::getMessage());
                    $qStat->setFetchMode(PDO::FETCH_ASSOC);
                    $rStat = $qStat->fetch();
                    ...
                    $nSpp = $owlPDO->query($iSpp) or die(print " Gagal: " . PDOException::getMessage());
                    $nSpp->setFetchMode(PDO::FETCH_ASSOC);
                    while ($dSpp = $nSpp->fetch()) {
                        $isi.=$dSpp['nodo'] . ",\n";
                    }


                    $arr2 = "nokontrak" . "##" . $res['nokontrak'];
                    echo"<tr class=rowcontent onclick=\"zDetail(event,'pmn_slave_laporanPemenuhanKontrak.php','" . $arr2 . "')\">
                            ...
                            <td align=center>" . $arrKurs[$res['matauang']] . "</td> 
                            ...
                            // $total2+=$res['hargasatuan'];
                            // $total2+=$rTimb['beratbersih'];
                            // $total3+=$rTimb['kgpembeli'];
                            $total4+=($res['kuantitaskontrak'] * $res['hargasatuan']);
                    ...
                    //     echo"<td align=center>" . $isiMulai . " " . $sd . " " . $isiSd . "</td>";
                    // }
        ...
        case'pdf':
            ...
            class PDF extends FPDF {
                ...
                function Header() {
                    ...
                    global $title;
                    global $owlPDO;
                    global $tanggalmulai1;
                    global $tanggalakhir1;
                    global $kdBrg1;
                    global $idPabrik1;

                    $sBrg = "select namabarang from " . $dbname . ".log_5masterbarang where kodebarang='" . $kdBrg1 . "'";
                   $qBrg = $owlPDO->query($sBrg) or die(print " Gagal: " . PDOException::getMessage());
                    $qBrg->setFetchMode(PDO::FETCH_ASSOC);
                    $rBrg = $qBrg->fetch();

                    $sOrg = "select namaorganisasi from " . $dbname . ".organisasi where kodeorganisasi='" . $idPabrik1 . "'";
                    $qOrg = $owlPDO->query($sOrg) or die(print " Gagal: " . PDOException::getMessage());
                    $qOrg->setFetchMode(PDO::FETCH_ASSOC);
                    $rOrg = $qOrg->fetch();
            ...
            $pdf->SetFont('Arial', '', 8);
            $sDet = "select nokontrak,franco,kodebarang,tanggalkontrak,koderekanan,tanggalkirim,kuantitaskontrak,kodept,hargasatuan,matauang,satuan  from " . $dbname . ".pmn_kontrakjual where (tanggalkontrak between '" . $tanggalmulai1 . "' and '" . $tanggalakhir1 . "') and kodebarang like '%" . $kdBrg1 . "%' and kodept='" . $idPabrik1 . "' order by tanggalkontrak asc ";
        ...
        case'excel':
            ...
            $sBrg = "select namabarang from " . $dbname . ".log_5masterbarang where kodebarang like '%" . $kdBrg1 . "%'";
            $qBrg = $owlPDO->query($sBrg) or die(print " Gagal: " . PDOException::getMessage());
            $qBrg->setFetchMode(PDO::FETCH_ASSOC);
            $rBrg = $qBrg->fetch();
           ...
           $qOrg = $owlPDO->query($sOrg) or die(print " Gagal: " . PDOException::getMessage());
            ...
            $stream.="
                        ...
                        <tr><td colspan=3>" . $_SESSION['lang']['periode'] . "</td><td>" . tanggalnormal($tanggalmulai1) . " s/d " . tanggalnormal($tanggalakhir1) . "</td></tr>
             ...
             <td bgcolor=#DEDEDE align=center rowspan=2>No SPP</td>
              ...
              <td bgcolor=#DEDEDE align=center rowspan=2>" . $_SESSION['lang']['jumlah'] . "</td>
              </tr>
            ...
            $strx = "select *  
                               from " . $dbname . ".pmn_kontrakjual where (tanggalkontrak between '" . $tanggalmulai1 . "' and '" . $tanggalakhir1 . "') and kodebarang like '%" . $kdBrg1 . "%' and kodept='" . $idPabrik1 . "' 
            ...
            if ($row < 1) {
                ...
                while ($barx = $resx->fetch()) {
                    ...
                    $sStat = "select penjualan  from " . $dbname . ".pmn_5franco where id_franco='" . $barx['franco'] . "'";
                    $qStat = $owlPDO->query($sStat) or die(print " Gagal: " . PDOException::getMessage());
                    $qStat->setFetchMode(PDO::FETCH_ASSOC);
                    $rStat = $qStat->fetch();
                   ...
                   $stream.="<tr class=rowcontent>
                                ...
                                <td align=right>" . number_format(($barx['kuantitaskontrak'] * $barx['hargasatuan']), 2) . "</td>";

                            $total1+=$barx['kuantitaskontrak'];
                            // $total2+=$res['hargasatuan'];
                            // $total2+=$rTimb['beratbersih'];
                            // $total3+=$rTimb['kgpembeli'];
                            $total4+=($barx['kuantitaskontrak'] * $barx['hargasatuan']);
