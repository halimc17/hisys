...
<script language=javascript src='js/kebun_tbskud.js?v=<?php echo time(); ?>'></script>
...
// if($_SESSION['empl']['kodeorganisasi']=='KBP' || $_SESSION['empl']['kodeorganisasi']=='SNP'){
	// echo"<font size='10px'>Menu ini Sedang dalam proses penambahan unit region sekadau dan bonti</font>";exit();
// }

$optbuyer=$optbarang=$opttipe=$opttangki=$optpt=$optpelayaran=$optkapal=$optponton=$optfranco=$optkud=$optunit="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
...
$str = "select a.*,b.namaorganisasi from ".$dbname.".kebun_5namakud a 
	left join ".$dbname.".organisasi b on a.afdeling=b.kodeorganisasi where status=1 and b.induk='".$_SESSION['empl']['lokasitugas']."'";
$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
   @$optkud.="<option value='".$bar['afdeling']."'>".$bar['namaorganisasi']."</option>";
}
...
echo"<div>";//buka div
OPEN_BOX('','<span class=judul>'.getMenu('kebun_tbskud').'</span>');
echo"<table>
     <tr valign=middle>
    ...
    echo "
            ...
            <thead>
                <tr class=rowheader>
                    ...
                    <th  align=center>".$_SESSION['lang']['unit']."<br>".$_SESSION['lang']['induk']."</th>
...
<table cellspacing=1 border=0>
	<tr>
		...
		<td>:</td>		
		<td>
			<select id=unit  style=\"width:220px;\" >'".$optunit."'</select>
		</td>
	</tr>
	...
	<tr>
		...
		<td>:</td>		
		<td>
			<select id=divisi  style=\"width:155px;\" onchange=\"getrekening(this.value)\" >'".$optkud."'</select>
		</td>

		<td valign=top rowspan=5>".$_SESSION['lang']['keterangan']."</td> 
			<td valign=top rowspan=5>:</td>
			<td  rowspan=5 valign=top><textarea rows='6' id=keteranganht type=text onkeypress=\"return tanpa_kutip(event)\" style=\"width:200px;\"></textarea></td>
	</tr>
	<tr>
		<td>".$_SESSION['lang']['rekening']."</td>
		<td>:</td>		
		<td>
			<select id=rekening  style=\"width:155px;\" >'".$optrekening."'</select>
		</td>
	</tr>
	<tr>
...
<legend><b>".$_SESSION['lang']['info']."</b></legend>
	<ol>
		...
		<li>Tanggal TBS mengikuti tanggal tbs diterima pabrik</li>
		<li>Jika muncul informasi <font color=red>Harga Belum diinput/disetujui</font>, artinya belum ada harga untuk data tersebut, maka hubungi pihak finance RO, dengan menyebutkan tanggal, tahun tanam, KUD, dan tanggal pabrik</li>
		<li>Jika muncul informasi <font color=red>Silahkan proses ambil kg timbangan</font>, artinya belum melakukan proses ambil kg timbangan dimenu <b>Kebun->Proses->Ambil Kg Timbangan</b></li>
...
// $border='0';
// echo "<div id=formpetani style=display:block>";
// // echo "<div id=detail style=display:block>";
// OPEN_BOX('','<span class=judul>'.$_SESSION['lang']['detail'].'</span>');
// // echo "<fieldset style=float:left>
...
//                     <td  align=center>".$_SESSION['lang']['kode']." Hamparan</td>
//                     <!--td  align=center>".$_SESSION['lang']['nama']."<br>Hamparan</td-->
//                     <td  align=center>No Kavling</td>
//                     <td  align=center>Nama Petani</td>
