...
foreach($arrnourut as $nourut){
	if($tipenourut[$nourut]=='Detail' and (@$jumlahdaftar[$nourut]>0 || @$jumlahdaftar[$nourut]!='')){
		...
		foreach($arrper as $per){
			...
			$str="select ".$kolomthnini." as dtthnini,noakun from ".$dbname.".keu_saldobulanan where 1=1  ".$where."  ".$listakun."  and periode='".$perdata."'";
			$res=fetchdata($str);
			foreach($res as $bar){
			...
			@$dtthniniakun[$nourut][$bar['noakun']][$i[1]]=($dtthniniakun[$nourut][$bar['noakun']][$tahun.'-'.$i[1]]-$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]])/$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]]*100;
...
$stream.="<th align=left style='".$sty."' colspan='".(($cspan*3)+2)."'><b>".strtoupper($namaorg[$param['kodept']]."".$judulunit."<br>".$judullaporan)."</b></th>";
$stream.="</tr>";
	$stream.="<tr class=rowheader>";
			...
			if($tipenourut[$nourut]=='Detail'){
				foreach($arrnoakun as $noakun){
					if($ltnoakun[$nourut][$noakun]){
						$stream.="<tr class=rowcontent>";
						$stream.="<td align=left style='width:1%'></td>"; 
						$stream.="<td align=left style='width:20%'>&nbsp;&nbsp;&nbsp;&nbsp;".$noakun." - ".$namaakun[$noakun]."</td>"; 
						foreach($arrper as $per){
							$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan($dtthniniakun[$nourut][$noakun][$per]),2)."</td>"; 
							// $stream.="<td align=right ".$style.">".hidezerodecimal(fixnan($dtthniniakun[$nourut][$noakun][$per]))."</td>"; 
						}
						$stream.="</tr>";
					}
				}
