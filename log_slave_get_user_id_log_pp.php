...
include_once('lib/zLib.php');
        ...
        if(isset($_POST['rnopp']))
        {
                ...
                $red=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
                $red->setFetchMode(PDO::FETCH_OBJ);
                        ...
                        while($bar= $red->fecth())
                        {
                          ...
                          $rit=$owlPDO->query($sq) or die(print " Gagal: ".PDOException::getMessage());
                          $rit->setFetchMode(PDO::FETCH_OBJ);
                          $opt=$rit->fetch();
        ...
        if(isset($_POST['hnopp']))
        {
                ...
                $riu=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
                $riu->setFetchMode(PDO::FETCH_OBJ);
                        echo"<table class=data cellspacing=1 border=0>
                                 <thead>
                        ...
                        while($bar=$riu->fetch())
                        {
                                ...
                                $riu3=$owlPDO->query($sql) or die(print " Gagal: ".PDOException::getMessage());
                                $riu3->setFetchMode(PDO::FETCH_OBJ);
                                $res3=$riu3->fetch();

                                $sql2="select * from ".$dbname.".`sdm_5jabatan` where kodejabatan='".$res3->kodejabatan."'";
                                $riu2=$owlPDO->query($sql2) or die(print " Gagal: ".PDOException::getMessage());
                                $riu2->setFetchMode(PDO::FETCH_OBJ);
                                $res2=$riu2->fetch();
                    ...
                    else
                    {
                      ...
                      if (owlBaris($query)< 1) 
                      {
                        $ins="insert into ".$dbname.".log_prapoht (`nopp`,`kodeorg`,`tanggal`,`dibuat`) values ('".$nopp."','".$kd_org."','".$tgl."','".$id_user."')";
                          ...
                          try{
                             $owlPDO->exec($ins); //insert hedaer 
                          }catch (PDOException $e){
                            echo " Error insert header: ".$d->getMessage();
                          }
