...
switch ($method) {
	...
	case 'insert':
		...
		$owlPDO->beginTransaction();
			...
			$str = insertQuery($dbname,'kebun_5premibmtbs',$data,$cols);
			$owlPDO->exec($str);
	...
	break;
	case 'delete':
		$str = "delete from ".$dbname.".kebun_5premibmtbs where divisi = '".$divisi."'  and tanggalberlaku='".$tglberlaku."' and jenispremi='".$jenispremi."'";
	...
	case'update':
		...
		$str = "update ".$dbname.".kebun_5premibmtbs set kegiatan='".$kegiatan."',harga='".$rpangkut."',denda='".$denda."',toleransi='".$toleransi."', updateby='".$_SESSION['standard']['userid']."' where divisi = '".$divisi."'  and tanggalberlaku='".$tglberlaku."' and jenispremi='".$jenispremi."'";
        try{$owlPDO->exec($str);}catch(PDOException $e){echo " Gagal," . addslashes($e->getMessage());}
	break;
    case'loaddata':
		...
		$ql2 = "select * from " . $dbname . ".kebun_5premibmtbs where 0=0 ".$where.""; 
        $res = fetchdata($ql2);
		$jlhbrs = count($res);
        
		$no = 0;
		$str = "select * from ".$dbname.".kebun_5premibmtbs where 0=0 ".$where." order by divisi asc, tanggalberlaku desc LIMIT ".$offset.",".$limit."";
        ...
        while ($bar = $res->fetch()){
            ...
            $tab.="<td align=center>".$bar['jenispremi']."</td>";
            
			$tab.="<td align=center width=20px><img src=images/application/application_edit.png class=zImgBtn  title='Edit' onclick=\"edit('".$bar['kodeorg']."','" . $bar['divisi'] . "','".$bar['harga']."','".$bar['denda']."','".$bar['toleransi']."','".tanggalnormal($bar['tanggalberlaku'])."','".$bar['kegiatan']."','".$bar['jenispremi']."');\" >	</td>";
			 
            $tab.="<td align=center width=20px><img src=images/application/application_delete.png class=zImgBtn  title='Delete' onclick=\"del('" . $bar['divisi'] . "','".tanggalnormal($bar['tanggalberlaku'])."','" . $bar['jenispremi'] . "');\" >	</td>";
            $tab.="</tr>";
        }
