...
$qTotalLuas = $owlPDO->query($sTotalLuas) or die(print " Gagal: " . PDOException::getMessage());
$qTotalLuas->setFetchMode(PDO::FETCH_ASSOC);
while ($rTotalLuas = $qTotalLuas->fetch()) {
    $lsAfdeling[$rTotalLuas['afdeling']] = isset($lsAfdeling[$rTotalLuas['afdeling']]) ? $lsAfdeling[$rTotalLuas['afdeling']] : '';
...
$qAfdeling = $owlPDO->query($sAfdeling) or die(print " Gagal: " . PDOException::getMessage());
$qAfdeling->setFetchMode(PDO::FETCH_ASSOC);
while ($rAfdeling = $qAfdeling->fetch()) {
    $dataAfd[] = $rAfdeling['kodeorganisasi'];
}


$grTotal = $TotalKarBag = array(); //total pekerjaa
...
$qDataKary = $owlPDO->query($sDataKary) or die(print " Gagal: " . PDOException::getMessage());
$qDataKary->setFetchMode(PDO::FETCH_ASSOC);
while ($rDataKary = $qDataKary->fetch()) {
    @$grTotal[$kdUnit]+=$rDataKary['total'];
    @$TotalKary[$kdUnit][$rDataKary['jeniskelamin']]+=$rDataKary['total'];
}

$grTotalBag = $TotalKarBag = array();
...
foreach ($dataAfd as $kdAfd) {
    ...
    $qDataKary2 = $owlPDO->query($sDataKary2) or die(print " Gagal: " . PDOException::getMessage());
    $qDataKary2->setFetchMode(PDO::FETCH_ASSOC);
    while ($rDataKary2 = $qDataKary2->fetch()) {
...
$qDataKary25 = $owlPDO->query($sDataKary25) or die(print " Gagal: " . PDOException::getMessage());
$qDataKary25->setFetchMode(PDO::FETCH_ASSOC);
while ($rData = $qDataKary25->fetch()) {
...
$qTotStaff = $owlPDO->query($sTotStaff) or die(print " Gagal: " . PDOException::getMessage());
$qTotStaff->setFetchMode(PDO::FETCH_ASSOC);
while ($rTotStaff = $qTotStaff->fetch()) {
...
foreach ($dataAfd as $kdAfd) {
    ...
    $qKaryStaff = $owlPDO->query($sKaryStaff) or die(print " Gagal: " . PDOException::getMessage());
    $qKaryStaff->setFetchMode(PDO::FETCH_ASSOC);
    while ($rKaryStaff = $qKaryStaff->fetch()) {
        @$grTotStaff[$rKaryStaff['subbagian']]+=$rKaryStaff['total'];
        @$TotStaff[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']]+=$rKaryStaff['total'];
    }
}
$grTotStaff2 = $TotKarKnbesStaff = array();
...
$qKaryStaff2 = $owlPDO->query($sKaryStaff2) or die(print " Gagal: " . PDOException::getMessage());
$qKaryStaff2->setFetchMode(PDO::FETCH_ASSOC);
while ($rDataStaff = $qKaryStaff2->fetch()) {
...
$qTotKhtL = $owlPDO->query($sTotKhtL) or die(print " Gagal: " . PDOException::getMessage());
$qTotKhtL->setFetchMode(PDO::FETCH_ASSOC);
while ($rTotKhtL = $qTotKhtL->fetch()) {
...
foreach ($dataAfd as $kdAfd) {
    ...
    $qKaryPerKht = $owlPDO->query($sKaryPerKht) or die(print " Gagal: " . PDOException::getMessage());
    $qKaryPerKht->setFetchMode(PDO::FETCH_ASSOC);
    while ($rKaryPerKht = $qKaryPerKht->fetch()) {
...
foreach ($dataAfd as $kdAfd) {
    ...
    $qKaryStaff = $owlPDO->query($sKaryStaff) or die(print " Gagal: " . PDOException::getMessage());
    $qKaryStaff->setFetchMode(PDO::FETCH_ASSOC);
    while ($qKaryStaff = $qKaryStaff->fetch()) {
...
$grRowKraniKht = $totRowKraniKht = $totTdkLangJen = array();
$totTdkLang = 0;
$sTotKhtL = "select distinct count(karyawanid) as total,jeniskelamin from " . $dbname . ".datakaryawan where " . $addTmb . " and kodejabatan in(24,35,52,54,118,119,120,121,122,123) 
                   and tipekaryawan in (2,3) and  substr(tanggalmasuk,1,7)<='" . $periode . "' and 
                   (substr(tanggalkeluar,1,7)>'" . $periode . "' or substr(tanggalkeluar,1,7)='0000-00') group by jeniskelamin";
...
$qTotKhtL->setFetchMode(PDO::FETCH_ASSOC);
while ($rTotKhtL = $qTotKhtL->fetch()) {
    $grRowKraniKht[$kdUnit] = isset($grRowKraniKht[$kdUnit]) ? $grRowKraniKht[$kdUnit] : '';
    $totRowKraniKht[$kdUnit][$rTotKhtL['jeniskelamin']] = isset($totRowKraniKht[$kdUnit][$rTotKhtL['jeniskelamin']]) ? $totRowKraniKht[$kdUnit][$rTotKhtL['jeniskelamin']] : '';
    $totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']] = isset($totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']]) ? $totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']] : '';
...
$grRowKraniKht3 = $totRowKraniKht3 = $totKanBesJen = array();
$totKanBes = 0;
$sKaryStaff2 = "select distinct count(karyawanid) as total,jeniskelamin from " . $dbname . ".datakaryawan where " . $addTmb . " and 
                      subbagian not in (select distinct kodeorganisasi from " . $dbname . ".organisasi where induk='" . $kdUnit . "') and substr(tanggalmasuk,1,7)<='" . $periode . "' and 
                      (substr(tanggalkeluar,1,7)>'" . $periode . "' or substr(tanggalkeluar,1,7)='0000-00')
...
$qKaryStaff2->setFetchMode(PDO::FETCH_ASSOC);
while ($rDataStaff = $qKaryStaff2->fetch()) {
    $grRowKraniKht3[$kdUnit] = isset($grRowKraniKht3[$kdUnit]) ? $grRowKraniKht3[$kdUnit] : '';
    $totRowKraniKht3[$kdUnit][$rDataStaff['jeniskelamin']] = isset($totRowKraniKht3[$kdUnit][$rDataStaff['jeniskelamin']]) ? $totRowKraniKht3[$kdUnit][$rDataStaff['jeniskelamin']] : '';
    $totKanBesJen[$kdUnit][$rDataStaff['jeniskelamin']] = isset($totKanBesJen[$kdUnit][$rDataStaff['jeniskelamin']]) ? $totKanBesJen[$kdUnit][$rDataStaff['jeniskelamin']] : '';
...
//$grRowLainBul3=$totRowLainBul3=$totKanBesJen=array();
$totKanBes = 0;
$sKaryStaff2 = "select distinct count(karyawanid) as total,jeniskelamin from " . $dbname . ".datakaryawan where " . $addTmb . " and 
                      subbagian not in (select distinct kodeorganisasi from " . $dbname . ".organisasi where induk='" . $kdUnit . "') and substr(tanggalmasuk,1,7)<='" . $periode . "' and 
                      (substr(tanggalkeluar,1,7)>'" . $periode . "' or substr(tanggalkeluar,1,7)='0000-00') and 
...
$grRowLainKht = $totRowLainKht = $totTdkLangJen = array();
$sTotKhtL = "select distinct count(karyawanid) as total,jeniskelamin from " . $dbname . ".datakaryawan where " . $addTmb . " and kodejabatan NOT in(24,35,37,38,48,52,53,54,55,118,119,120,121,122,123,132,136,138,60,168,45,88) 
                   and tipekaryawan in (2,3) and substr(tanggalmasuk,1,7)<='" . $periode . "' and 
                   (substr(tanggalkeluar,1,7)>'" . $periode . "' or substr(tanggalkeluar,1,7)='0000-00')  group by jeniskelamin";
//echo $sTotKhtL;
...
while ($rTotKhtL = $qTotKhtL->fetch()) {
    /*
      $grRowLainKht[$kdUnit]=isset($grRowLainKht[$kdUnit])?$grRowLainKht[$kdUnit]:'';
      $totRowLainKht[$kdUnit][$rTotKhtL['jeniskelamin']]=isset($totRowLainKht[$kdUnit][$rTotKhtL['jeniskelamin']])?$totRowLainKht[$kdUnit][$rTotKhtL['jeniskelamin']]:'';
      $totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']]=isset($totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']])?$totTdkLangJen[$kdUnit][$rTotKhtL['jeniskelamin']]:'';
     */
...
$grRowLainKht2 = $totRowLainKht2 = $totAfdTdkLang = $totAfdTdkLangJen = array();
foreach ($dataAfd as $kdAfd) {
    $sKaryStaff = "select distinct count(karyawanid) as total,jeniskelamin,subbagian from " . $dbname . ".datakaryawan where " . $addTmb . " and subbagian='" . $kdAfd . "' 
    ...
    while ($rKaryStaff = $qKaryStaff->fetch()) {
        /*
          $grRowLainKht2[$rKaryStaff['subbagian']]=isset($grRowLainKht2[$rKaryStaff['subbagian']])?$grRowLainKht2[$rKaryStaff['subbagian']]:'';
          $totRowLainKht2[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']]=isset($totRowLainKht2[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']])?$totRowLainKht2[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']]:'';
          $totAfdTdkLang[$rKaryStaff['subbagian']]=isset($totAfdTdkLang[$rKaryStaff['subbagian']])?$totAfdTdkLang[$rKaryStaff['subbagian']]:'';
          $totAfdTdkLangJen[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']]=isset($totAfdTdkLangJen[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']])?$totAfdTdkLangJen[$rKaryStaff['subbagian']][$rKaryStaff['jeniskelamin']]:'';
...
$grRowLainKht3 = $totRowLainKht3 = $totKanBesJen = array();
$sKaryStaff2 = "select distinct count(karyawanid) as total,jeniskelamin from " . $dbname . ".datakaryawan where " . $addTmb . " and 
                      subbagian not in (select distinct kodeorganisasi from " . $dbname . ".organisasi where induk='" . $kdUnit . "') and substr(tanggalmasuk,1,7)<='" . $periode . "' and 
                      (substr(tanggalkeluar,1,7)>'" . $periode . "' or substr(tanggalkeluar,1,7)='0000-00') and 
...
//$grRowLainKhl2=$totRowLainKhl2=$totRowLainKhl2=array();
...
$qKaryLangsung = $owlPDO->query($sKaryLangsung) or die(print " Gagal: " . PDOException::getMessage());
$qKaryLangsung->setFetchMode(PDO::FETCH_ASSOC);
while ($rKaryLangsung = $qKaryLangsung->fetch()) {
...
$qKaryLangsung2 = $owlPDO->query($sKaryLangsung2) or die(print " Gagal: " . PDOException::getMessage());
$qKaryLangsung2->setFetchMode(PDO::FETCH_ASSOC);
while ($rKaryLangsung2 = $qKaryLangsung2->fetch()) {
    ...
    case'pdf':
        ...
        class PDF extends FPDF {
            ...
            function Header() {
                ...
                foreach ($dataAfd as $kdAfd) {
                    //$lsAfdeling[$kdAfd]=isset($lsAfdeling[$kdAfd])?$lsAfdeling[$kdAfd]:'';
                    $this->Cell(33, 20, $_SESSION['lang']['luas'], 1, 0, 'C', 1);
