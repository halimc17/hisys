...
switch($method)
{
	...
	case'detail1produksi':
		...
		while($bar=$res->fetch()){	
			...
			$arrunit[$bar['kodeorg']]=$bar['kodeorg'];
			$listunit[$bar['tahun']][$bar['kodeorg']]=$bar['kodeorg'];
			$kgwb[$bar['tahun']][$bar['kodeorg']]=$bar['kgwb'];
		}
		
		
		
		// foreach($arrthn as $thnlist){
			// foreach($arrunit as $unitlist){
				// if($listunit[$thnlist][$unitlist]!=''){
					// for($i=1;$i<=$row;$i++){
						// $data1[$thnlist]=$kgwb[$thnlist][$unitlist];
						// $data2[$thnlist]=$kgwb[$thnlist][$unitlist];
					// }
				// }
			// }
		// }
		...
		foreach($arrthn as $thnlist){
			$no++;
			$data1[$no]=$kgwb[$thnlist]['DUKE'];
			$data2[$no]=$kgwb[$thnlist]['TEBE'];
			$data3[$no]=$kgwb[$thnlist]['SIDE'];
