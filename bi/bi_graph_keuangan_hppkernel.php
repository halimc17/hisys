...
switch($method){
	case'detailgraph':
		...
		$str="select sum(oerpk) as kgprod,left(tanggal,4) as thn,induk from ".$dbname.".pabrik_produksi a left join ".$dbname.".organisasi b
	...
	case'detail1cpo'://var dan case semua sama, hanya data CPO menjadi kernel yang di rubah
		...
		while($bar=$res->fetch()){
			...
			//$arrbln[substr($bar['bulan'],0,4).'-'.substr($bar['bulan'],4,2)]=substr($bar['bulan'],0,4).'-'.substr($bar['bulan'],4,2);	
		}
		...
		$str="select sum(oerpk) as kgprod,left(tanggal,4) as thn,left(tanggal,7) as bulan from ".$dbname.".pabrik_produksi where left(tanggal,4)='".$thn."'
		...
		$lineplot1->SetLegend($_SESSION['lang']['hppkernel']);
		$lineplot1->setCenter();
		$graph->Add($lineplot1);
	...
	case'global':
		...
		$str="select sum(oerpk) as kgprod,left(tanggal,4) as thn from ".$dbname.".pabrik_produksi where left(tanggal,4)<='".$thn."'
