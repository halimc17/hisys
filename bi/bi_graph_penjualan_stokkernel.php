...
switch($method)
{
	case'detailgraph':
		...
		$str="select max(tanggal) as tanggal,kodeorg from ".$dbname.".pabrik_masukkeluartangki where kernelquantity!=0  group by kodeorg,left(tanggal,7)";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
			$str1="select sum(kernelquantity/1000) as jumlah,kodeorg,left(tanggal,7) as bulan from ".$dbname.".pabrik_masukkeluartangki where 
