...
switch($method){
	...
	case'detailgraph':
		...
		$str="select (debet/1000000) as biaya,left(tanggal,4) as tahun,kodeorg,induk 
				...
				on a.kodeorg=b.kodeorganisasi  where a.noakun like '41101%' 
	...
	case'global':
		...
		if($pt!=''){
			...
			$no=-1;
			$str="select sum(debet) as biaya,left(tanggal,4) as tahun from ".$dbname.".keu_jurnaldt_vw where noakun like '41101%'
			      ".$sortprod." and kodevhc='' and left(tanggal,4)<='".$thn."' group by left(tanggal,4) order by left(tanggal,4) ";
			//exit('warning :'.$str);
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while($bar=$res->fetch()){
			...
			$str="select sum(rupiah) as biaya,tahunbudget as tahun from ".$dbname.".bgt_budget_detail where tahunbudget<='".$thn."'
			      ".$sortprod." and tipebudget='WS' group by tahunbudget order by tahunbudget";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$row=owlBaris($res);
			$res->setFetchMode(PDO::FETCH_ASSOC);
			if($row!=0){
