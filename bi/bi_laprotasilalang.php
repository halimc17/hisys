<?php
	...
	switch ($type) {
		case 'preview':
			...
			$tglakhir = ($periodeakhir);
			// $jumbul = intval(substr(($periodeakhir), 5, 2)) - intval(substr(($periodeawal), 5, 2));
			// $jumbul = $jumbul + 1;
			$jumbul = count(month_inbetween(($periodeawal), ($periodeakhir)));

			$per = explode('-', ($periodeawal));
			$perx = $per[0].$per[1];

			$per1 = explode('-', $tglakhir);
			$perx1 = $per1[0].$per1[1];

			// $str = "
			// 	select a.kodeorg,sum(a.hasilkerja) as hasilkerja,b.luasareaproduktif
			// 	from ".$dbname.".kebun_prestasi a
			// 	left join setup_blok b on a.kodeorg=b.kodeorg
			// 	where 1=1 ".forKebunAll($kbnarr, 'left(a.kodeorg,4)', 'in')." and kodekegiatan like '62101%' and substr(a.notransaksi,1,6) between '".$perx."' and '".$perx1."'
			// 	group by a.kodeorg
			// ";
			$str = "
				select kodeorg, sum(hasilkerja) as hasilkerja
				from ".$dbname.".kebun_prestasi
				where 1=1 ".forKebunAll($kbnarr, 'left(kodeorg,4)', 'in')." and kodekegiatan like '62101%' and substr(notransaksi,1,6) between '".$perx."' and '".$perx1."'
				group by kodeorg
			";
			try {
				$res = $owlPDO->query($str);
			...
			while ($bar = $res->fetch()) {
				...
				// $data[$bar['kodeorg']] = $bar['hasilkerja'] / $bar['luasareaproduktif'] / $jumbul;
				$hasilKerja[$bar['kodeorg']] = $bar['hasilkerja'];
			}
			...
			while ($bar = $res->fetch()) {
				...
				$data[$bar['indukblok']] = fixnan($hasilKerja[$bar['indukblok']] / $bar['luasareaproduktif']);
			}
		...
		case 'detail':
			...
			$str = "SELECT SUM(luasareaproduktif) AS luasareaproduktif FROM ".$dbname.".setup_blok WHERE indukblok = '".$blok."'";
			$luasreal = fetchData($str)[0]['luasareaproduktif'];
			...
			$str = "
				select sum(hasilkerja) as hasilkerja,sum(jumlahhk) as jumlahhk,substr(notransaksi,1,4) as tahun,substr(notransaksi,5,2) as bulan
				from ".$dbname.".kebun_prestasi
				where kodeorg='".$blok."' and kodekegiatan like '62101%' and SUBSTR(notransaksi, 1, 6) BETWEEN '".implode('', explode('-', $periodeawal))."' AND '".implode('', explode('-', $periodeakhir))."'
				group by substr(notransaksi,1,6)
			";
			$query = fetchdata($str);
			try {
				$res = $owlPDO->query($str);
			...
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while ($bar = $res->fetch()) {
				$periode = $bar['tahun'].'-'.$bar['bulan'];
				$luasha[$periode] = $bar['hasilkerja'];
			}
				...
				<table cellpading=1 cellspacing=1 border=0 class=sortable width=100%>
					<thead>
						<tr align=center>
							...
							<td width=10%>".$_SESSION['lang']['luas']."</td>
				...
				foreach ($arrper as $per) {
					$result .= "
						<tr class=rowcontent align=center>
							...
							<td align=right>".number_format($luasha[$per], 2)."</td>
							<td align=right>".number_format(fixnan($luasha[$per] / $luasreal), 2)."</td>
							<td align=right>".number_format($rotstd, 2)."</td>
							<td align=right>".number_format(fixnan($luasha[$per] / $luasreal / $rotstd * 100), 2)."</td>
						</tr>
					";
					$tluasha += $luasha[$per];
				}
			...
			if (count($query) < 1) {
				...
				$result .= "
					<tr class=rowcontent align=center style=font-weight:bold;>
						...
						<td align=right>".number_format($tluasha, 2)."</td>
						<td align=right style=background-color:".$warna.">".number_format($data, 2)."</td>
						<td align=right>".number_format($rotstd * $jumbul, 2)."</td>
						<td align=right>".number_format(fixnan($data / ($rotstd * $jumbul) * 100), 2)."</td>
					</tr>
				";
			}
			$result .= "
		...
		case 'globalreport':
			...
			*/
			...
			// 	from ".$dbname.".kebun_rekappnn where 1=1 ".forKebunAll($kbnarr, 'left(divisi,4)', 'in')." and tanggal between '".($periodeakhir)."' and '".$tglakhir."'
			// 	group by blok
			// ";
			// try {
			// 	$res = $owlPDO->query($str);
			...
			// 	$data[$bar['blok']] = fixnan($bar['luaspanen'] / $bar['luasproduksi'] / $jumbul);
			// }
			$str = "
				select kodeorg, sum(hasilkerja) as hasilkerja
				from ".$dbname.".kebun_prestasi
				where 1=1 ".forKebunAll($kbnarr, 'left(kodeorg,4)', 'in')." and kodekegiatan like '62101%' and substr(notransaksi,1,6) between '".implode('', explode('-', $periodeawal))."' and '".implode('', explode('-', $periodeakhir))."'
				group by kodeorg
			";
			try {
				$res = $owlPDO->query($str);
