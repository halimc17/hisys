...
switch($method)
{
	case'detailgraph':
		...
		$tahun = array();
		$kodept = array();
		$str=" select avg(hargasatuan) as jumlah,left(tanggalkontrak,4) as tahun,kodept,koderekanan
		...
		$str=" select avg(hargasatuan) as jumlah,left(tanggalkontrak,4) as tahun,kodept,koderekanan
				from ".$dbname.".pmn_kontrakjual  where 1=1 and kodebarang='400000001'
				and left(tanggalkontrak,4) <= '".$thn."'  group by left(tanggalkontrak,4),koderekanan ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
		...
		$form.="
				<tr class=rowcontent>
				<td colspan=2 align=center><b>".$_SESSION['lang']['ratarata']."</td>";
			foreach($tahun as $thnlist){
				foreach($koderekanan as $cus){
					$form.="		
	...
	case'global':
		...
		$str="select distinct(left(tanggalkontrak,4)) as tahun from ".$dbname.".pmn_kontrakjual 
				where left(tanggalkontrak,4) <= '".$thn."' and kodebarang='400000001' ".$sortpt." order by left(tanggalkontrak,4) asc"; 
		$not=-1;		
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){	
		...
		$str="select distinct(koderekanan) as koderekanan from ".$dbname.".pmn_kontrakjual 
			where left(tanggalkontrak,4) <= '".$thn."' and kodebarang='400000001'  ".$sortpt."";	
		...
		$str="select avg(hargasatuan) as kgwb,left(tanggalkontrak,4) as tahun,koderekanan from ".$dbname.".pmn_kontrakjual 
				where left(tanggalkontrak,4) <= '".$thn."' and kodebarang='400000001'  ".$sortpt."  group by left(tanggalkontrak,4),koderekanan";	
		$res = fetchData($str);
        $kgwb = array();
