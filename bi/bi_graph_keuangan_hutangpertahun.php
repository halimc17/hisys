...
switch($method){
	case'detailgraph':
		...
		exit ("<br> error:Om / Mas / Pak, tolong benerin dulu query di sini !");
		
		$str="select count(karyawanid) as karymasuk,left(tanggalmasuk,4) as thndt,kodeorganisasi from ".$dbname.".keu_saldobulanan
	...
	case'global':
		...
		$str="select sum(".$semuakolomdb.") as hutang,left(periode,4) as thndt from ".$dbname.".keu_saldobulanan a
			  left join ".$dbname.".organisasi b on a.kodeorg=b.kodeorganisasi
			  where left(periode,4)<= '".$thn."' ".$sortp." and left(a.noakun,5)='21111'
			  group by left(periode,4) order by left(periode,4)";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()){
			$arrthn[$bar['thndt']]=$bar['thndt'];
			$karin[$bar['thndt']]=($bar['hutang']*-1)/1000000;
		
		}

		$str="select sum(".$semuakolomdb.") as hutang,left(periode,4) as thndt from ".$dbname.".keu_saldobulanan a
			  left join ".$dbname.".organisasi b on a.kodeorg=b.kodeorganisasi
			  where left(periode,4)<= '".$thn."' ".$sortp." and left(a.noakun,5)='21112'
			  group by left(periode,4) order by left(periode,4)";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()){
			$arrthn[$bar['thndt']]=$bar['thndt'];
			@$karout[$bar['thndt']]=($bar['hutang']*-1)/1000000;
	...
	//	array_multisort($arrthnlist,SORT_ASC)
		...
		$b1plot->SetLegend($_SESSION['lang']['supplier']);
		$b2plot->SetLegend($_SESSION['lang']['kontraktor']);
