...
switch($method)
{
	...
	case'detailgraph':
				...
				<thead>
					...
					for($i=1;$i<=$jumthn;$i++){
						$form.="
							...
							<td align=center>".$_SESSION['lang']['produksi']." CPO</td>
						";
					}
					$form.="</tr></thead>";	
		...
		$str="select a.tbsdiolahton,a.tahun,b.induk,a.kodeorg,a.cpoton 
		from ".$dbname.".pabrik_produksi_vw a left join ".$dbname.".organisasi b 
		...
		while($bar=$res->fetch()){
			...
			@$prodcpopt[$bar['induk']][$bar['tahun']]+=$bar['cpoton'];
			@$prodcpounit[$bar['induk']][$bar['kodeorg']][$bar['tahun']]+=$bar['cpoton'];
			
			@$prodtbstot[$bar['tahun']]+=$bar['tbsdiolahton'];
			@$prodcpotot[$bar['tahun']]+=$bar['cpoton'];
		}
		...
		$form.="<td colspan=2><b>Total</b></td>";
		foreach($arrthn as $thn){
...
while($bar=$res->fetch()){
		...
		$str=" select sum(cpoton) as cpoton ,tahun,kodeorg from ".$dbname.".pabrik_produksi_vw  where 1=1 
					and kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where 1=1 ".$sort.")  group by tahun,kodeorg ";
        ...
        foreach ($arrthn as $thnlist => $row) {
            foreach ($optunit as $row1) {
                ...
                foreach ($res as $row2) {
                    if ($row == $row2['tahun'] && $row1 == $row2['kodeorg']) {
						$cpoton[$row1][$thnlist] = $row2['cpoton'];
                    }
                }
            }
        }
	...
	case'global':
		...
		$no=$nobgt=$nocpo=-1;
		$str=" select sum(cpoton) as cpoton,tahun from ".$dbname.".pabrik_produksi_vw  where 1=1 and tahun <= '".$thn."' ".$sortprod." group by tahun ";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch()){
			$nocpo++;
			$arrthn[$nocpo]=$bar['tahun'];
			$cpoprod[$nocpo]=$bar['cpoton'];
			$targ[$nocpo]='?method=detail1cpo&pt='.$pt.'&thn='.$thn;
		...
		while($bar=$res->fetch()){
			...
			$targ2[$no]='?method=detail1produksi&pt='.$pt.'&thn='.$thn;
		...
		$txt = new Text($_SESSION['lang']['cpo'].'('.$_SESSION['lang']['Ton'].')');
