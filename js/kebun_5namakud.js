function simpangudang() {
	...
	noakuninvestasi = document.getElementById('noakuninvestasi').options[document.getElementById('noakuninvestasi').selectedIndex].value;
	met = document.getElementById('method').value;

	param = 'kodegudang=' + kodegudang + '&afdeling=' + afdeling + '&status=' + status + '&method=' + met + '&kodeunit=' + kodeunit;
	param += '&noakuninvestasi=' + noakuninvestasi;
	tujuan = 'kebun_slave_5namakud.php';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById("kodeunit").selectedIndex = "0";
					document.getElementById("noakuninvestasi").selectedIndex = "0";
				}
			} else {
				busy_off();
...
function fillField(afdeling, kodegudang, status,kodeunit,noakuninvestasi) {
	...
	document.getElementById('noakuninvestasi').value=noakuninvestasi;
}
