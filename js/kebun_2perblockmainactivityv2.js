...
function showme(id,sumber){
	...
	jlhbrs = document.getElementById('jumlahbaris').innerHTML;
	if(id!=''){
		...
		for(e=0;e<iddet.length;e++){
			...
			if(document.getElementById(iddet[e])!= undefined){				
				...
				if(dis=="display:none" || dis=="display: none;"){
					if(e==0){ //Pastikan baris pertama
						for(i=1;i<=jlhbrs;i++){
							r = document.getElementById('row_'+i).getAttribute("style");
							if(r!="display:none" || r!="display: none;"){ //Pastikan semua di hide dulu baru di show
								document.getElementById('row_'+i).style.display="none";
								document.getElementById('row_'+i).style.backgroundColor="";
							}
						}
					}
					
					//hide baris total
					namaclas = document.getElementsByName('baristotal[]');
					for (var a = 0; a < namaclas.length; a++) {
						namaclas[a].style.display="none";
					}
					
					
					//tampilkan isi data blok
					document.getElementById(iddet[e]).style.display="";
					//document.getElementById(iddet[e]).style.backgroundColor=color;
					document.getElementById('kdblok_'+norow).style.backgroundColor=color;
					document.getElementById('kdblok_'+norow).style.fontWeight="bold";
				}else{
					//show baris total
					namaclas = document.getElementsByName('baristotal[]');
					for (var a = 0; a < namaclas.length; a++) {
						namaclas[a].style.display="";
					}
					...
					document.getElementById('kdblok_'+norow).style.backgroundColor="";
...
function getdetail(id,blok,akun) {
	...
	tujuan = 'kebun_slave_2perblockmainactivityv2_detail.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
...
function showhidev2(clas,name,thisid){
	...
	for (var i = 0; i < nnclass.length; i++) {
		...
		if(dis=="display:none" || dis=="display: none;"){
			col = document.getElementById('colhide').value;
			colhead1 = document.getElementById('colhead1').value;
			colhead1mat = document.getElementById('colhead1mat').value;
			colhead2 = document.getElementById('colhead2').value;
			if(thisid.substr(4,1)=='3'){
				jlhcol = parseFloat(col)*parseFloat(colhead1mat);
			}else{				
