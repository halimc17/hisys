...
function simpanIntraco()
{
	...
	// }
        alertify.confirm("Infomation","Are you sure?",
		function(){
			post_response_text(tujuan, param, respog);
		},
		function(){
              ...
              if(con.readyState==4)
              {
                if (con.status == 200) {
                            ...
                            if (!isSaveResponse(con.responseText)) {
                                    if((kodeorg=='')||(jenis=='')) alert('ERROR TRANSACTION, possibly caused by:\n- null fields.\n'); else 
                                    alertify.alert("Informasi",'ERROR TRANSACTION, possibly caused by:\n- duplicate entry, or \n- bad connection.\n'); 
                            }
                            else {
