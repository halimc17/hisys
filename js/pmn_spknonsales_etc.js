function delet(nospk,jenis) {
    ...
    tujuan='pmn_spknonsales_etc_slave.php';
...
function save() {
		...
		if(tanggal==''||  tanggalmuat1=='' || tanggalmuat2=='' || kuantitas=='' || kota==''){
			alert('Field Was Empty');
			return false;
		}
		
		if(transportir!='' && transportirdarat!=''){
			alert('Transportir hanya boleh salah satu darat/kapal');
			return false;
		}
		...
		param+='&transportirdarat='+transportirdarat+'&harga='+harga+'&lain='+lain+'&tandatangan2='+tandatangan2;
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('rpkg').value=ar[23];
					document.getElementById('toleransi').value=ar[24];
					document.getElementById('kgtoleransi').value=ar[25];
					document.getElementById('noakun').value=ar[26];
					document.getElementById('method').value='update';
