function getnamakaryawan(){
	...
	param = 'method=getnamakaryawan';
	tujuan = 'sdm_slave_pengajuantiket.php';
...
function carinamakaryawan(){
	namakaryawan = document.getElementById('namacari').value;
	
	param = 'method=carinamakaryawan';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('containerkarycari').innerHTML = con.responseText;
				}
			} else {
				busy_off();
				error_catch(con.status);
...
function addkarya(karyawanid,namakar,kodegol,namagolo){
	document.getElementById('idkaryawan').value=karyawanid;
	document.getElementById('namakaryawan').value=namakar;
	document.getElementById('golongan').value=kodegol;
	document.getElementById('namagolongan').value=namagolo;
	closeDialog();
}
...
function getformnopjd(){
	...
	// height   = '';
	// title    = "No Referensi";
	// content = "<fieldset style=\"width:98%;\"><div id=container style=\"width:100%;max-height:385px;overflow:auto;\"></div></fieldset>";
    // ev = 'event';
...
function addpjd(nopjd,jenis,tgldibutuhkan,karid,namakar,kdgol,nmgol,ket,tgldr,tglsd,tujuan,keperluan,nohp){
	...
	document.getElementById('ke').value=tujuan;
	document.getElementById('telp').value=nohp;
...
function simpandetail() {
	...
	if (telp == '') {
		notif('telp','Telp wajib diisi.'); return;
	}
	if (keperluan == '') {
		notif('keperluan','Keperluan wajib diisi.'); return;
	}
	if (dari == '') {
		notif('dari','Tujuan dari wajib diisi.'); return;
	}
	if (ke == '') {
		notif('ke','Tujuan ke wajib diisi.'); return;
	}
	if (tgldinasdari == '') {
...
function editdetail(id,jenis,notranspjd,idkar,nama,kodegol,gol,phone,keperluan,dari,ke,tgldinasdari,tgldinassampai,tanggal,keterangan){
...
function loaddata(page){
	...
	sumber     =document.getElementById('sumbersch').value;
...
function detailExcel(notransaksi,tipe){
	...
	tujuan = 'sdm_slave_pengajuantiket.php' + "?" + param;
	if(tipe=='pdf'){
		width = '950';
		height = '400';
	}else{		
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					alertify.popup("Detail",con.responseText).set({'resizable':true,'maximizable':true}).resizeTo('10%','40%');
				}
			} else {
				busy_off();
				error_catch(con.status);
...
function submitfile(notransaksi,id) {
	...
	busy_on();
	con.open("POST", "sdm_slave_pengajuantiket.php?method=submitfile", true);
