function loaddataexcel() {
	...
	// alert(param);
	alertify.popuppdf("title","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='vhc_slave_sipil_v2.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('90%','80%');		
}
...
function detailDataExcel(notransaksi,numRow,ev,tipe,jenis){
	...
	//alertify.popuppdf("Preview","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='vhc_slave_operasional_print_detailx.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('80%','70%');
	printnopopup("vhc_slave_operasional_print_detailx.php?"+param);
	
        // showDialog1(title,"<iframe frameborder=0 style='width:100%;min-height:400px'"+
        // " src='vhc_slave_operasional_print_detailx.php?"+param+"'></iframe>",'900','400',ev);	
...
function detailData(notransaksi,numRow,ev,tipe,jenis){
	...
	tujuan = 'vhc_slave_operasional_print_detailx.php';
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function detailPDF(notransaksi,numRow,ev,tipe) {
	...
	alertify.popuppdf("PDF","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='vhc_slave_operasional_print_detailx_pdf.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('80%','70%');
	
    // showDialog1('Print PDF',"<iframe frameborder=0 style='width:885px;height:400px'"+
        // " src='vhc_slave_operasional_print_detailx_pdf.php?"+param+"'></iframe>",'900','400',ev);
    // var dialog = document.getElementById('dynamic1');
    // dialog.style.top = '50px';
    // dialog.style.left = '15%';
}
	...
	alertify.confirm("Info","Akan dilakukan posting untuk transaksi nomor "+notransaksi+"<br><br>Data tidak dapat diubah setelah ini. Anda yakin ?",
		function(){
			if(abs=='absensi'){
				...
				post_response_text('vhc_slave_sipil_v2.php', param, respon);
			} else if (abs=='project') {
				post_response_text('vhc_slave_operasional_postingx.php', param, respon);
			}
		},
		function(){
			return;
...
function postingDataDetail(notransaksi) {
	...
	post_response_text('vhc_slave_postingbkmsipil.php', param, respon);
    function respon() {
        if (con.readyState == 4) {
            if (con.status == 200) {
                busy_off();
...
function viewPostingData(notransaksi,numRow,ev,tipe) {
	...
	tujuan = 'vhc_slave_detailview_postingbkm.php';
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function editdetail(notransaksi,karyawanid,kegiatan,blok,luas,satuan,prestasi,jhk,upah,premi,numrow, namakary,namablok,filterdivisi,keterangan,kodebarang,jumlahbarang,tipetransaksi){
	document.getElementById('filterdivisi').value=tipetransaksi;
	document.getElementById('filterdivisi').disabled=true;
...
function cleardetailall(){
	...
	document.getElementById('filterdivisi').disabled=false;
...
function cleardetail(baris){
	...
	if(row==0){
		...
		document.getElementById('kodebarang'+baris).value='';
		document.getElementById('jumlahbarang'+baris).value='';
	}
	if(baris=='x'){
		getdata(undefined,'x');
	}
...
function savedetail(currRow,maxRow){
	...
	if(row==0){
		...
		if(blok=='' && filterdivisi!='SPL'){alertify.alert("Alokasi Wajib diisi !!!"); document.getElementById('blok').focus(); return;}
		...
		jumlahbarang     =document.getElementById('jumlahbarang'+currRow).value;
	}
	...
	param +='&jumlahbarang='+jumlahbarang;
	param +='&mode='+mode;
...
function getDataDetail(baris,id){
	...
	kodeorg=document.getElementById('kodeorg').value;
    filterdivisi=document.getElementById('filterdivisi').value; 
    filterkebon=document.getElementById('filterkebon').value; 
	tgl=document.getElementById('tgl').value;
	// bahasa=document.getElementById('bahasa').value;
	if(row==0){
		karyawanid=document.getElementById('karyawanid').value;
    ...
    param='method=getDataDetail'+'&filterdivisi='+filterdivisi+'&tgl='+tgl+'&karyawanid='+karyawanid+'&blok='+blok+'&kegiatan='+kegiatan+'&kodeorg='+kodeorg+'&prestasi='+prestasi+'&filterkebon='+filterkebon;
    tujuan='vhc_slave_sipil_v2.php';
    post_response_text(tujuan, param, respog);
    function respog(){
        if(con.readyState==4){
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					satkegiatan = trim(isdt[6]);
					rpsat = parseFloat(trim(isdt[7]));
					stspremi = parseFloat(trim(isdt[8]));
					upahperhari = parseFloat(trim(isdt[9]));
					...
					if(row==0){
						...
						document.getElementById('basis').value = numberFormat(basis);
						document.getElementById('rpsat').value = numberFormat(rpsat,2);
						document.getElementById('upahperhari').value = numberFormat(upahperhari);
					} else {
						if(id=='changekeg'){
							document.getElementById('kegiatan'+baris).innerHTML = kdkeg;
						}
...
function getdata(filterdivisi,clear){
	if(clear!='x'){		
		cleardetail();
	}
	...
	kodeorg     =document.getElementById('kodeorg').value;
	filterkebon =document.getElementById('filterkebon').value;
    param='method=getdata'+'&filterdivisi='+filterdivisi+'&tgl='+tgl+'&stsawal='+stsawal+'&kodeorg='+kodeorg+'&filterkebon='+filterkebon;
	param += '&divisi=' + getValue('divisi');
...
function inputdetail(notransaksi){
    ...
    param += '&kodeorg=' + kodeorg+'&tgl=' + tgl+'&notransaksi=' + notransaksi+'&filterdivisi=' + filterdivisi+'&showpermandor=' + showpermandor+'&filterkebon=' + filterkebon;
	param += '&filterunit=' + getValue('filterunit');
	param += '&divisi=' + getValue('divisi');
	param += '&filterkebon=' + getValue('filterkebon');
    tujuan = 'vhc_slave_sipil_v2.php';
    post_response_text(tujuan, param, respog);
    function respog(){
        if (con.readyState == 4){
...
function loaddatadetailxls(notransaksi,jenis){
	...
	title="Excel";
	showDialog1(title,"<iframe frameborder=0 style='width:100%;min-height:400px'"+" src='vhc_slave_sipil_v2.php?"+param+"'></iframe>",'900','400',ev);	
	var dialog = document.getElementById('dynamic1');
	dialog.style.top = '50px';
	dialog.style.left = '15%';
}
...
function submitfile(notransaksi) {
	...
	con.open("POST", "vhc_slave_sipil_v2.php?method=submitfile", true);
