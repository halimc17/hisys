function preview(tipe) {
	...
	statuslaporan = document.getElementById('statuslaporan');
	statuslaporanV = statuslaporan.options[statuslaporan.selectedIndex].value;
    proses = document.getElementById('proses').value;

    param = 'kodept=' + kodeptV + '&kodeunit=' + kodeunitV+ '&proses=preview';
    param += '&periode=' + periodeV;
    param += '&tipe=' + tipe;
    param += '&supplier=' + supplierV;
    param += '&statuslaporan=' + statuslaporanV;
	tujuan = 'keu_2rekappembayaraninv_slave.php';
    ...
    if(statuslaporanV == '') {
        alertify.alert("Informasi",'Status Laporan Tidak Boleh Kosong!');
    }
