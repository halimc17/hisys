...
function saveData() {
  ...
  strUrl = "";
  for (i = 1; i < row; i++) {
    persetujuan = document.getElementById("persetujuan" + i).value;
    if (persetujuan == "") {
      alert("Persetujuan ke " + i + " masih kosong.");
      return;
    }
...
function viewpdf(notransaksi) {
  ...
  tujuan = "sdm_slave_dayoff.php?" + param;
  content =
...
function editdt(
  ...
  document.querySelector("#notransaksi2").value = notransaksi;
...
function getjmldayoff() {
  ...
  // param = "method=getjmldayoff&tglAwal=" + tglAwal + "&tglEnd=" + tglEnd;
