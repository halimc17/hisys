function revisiharga(tahunbudget,regional,kodebarang,namabarang,satuan,hargasatuan) {
	width = '';
	height = '';
	content = "<fieldset style=width:97%><div id=containerd style=\"min-width:480px;max-height:700px;overflow:auto;\"></div></fieldset>";
	ev = 'event';
	title = "";
	showDialog5(title, content, width, height, ev);
	
	param = 'what=revisiharga&tahunbudget=' + tahunbudget + '&regional=' + regional + '&kodebarang=' + kodebarang + '&hargasatuan=' + hargasatuan;
...
function addbarang(tahun,reg){
	document.getElementById('tahunbudget1').value = tahun;
	document.getElementById('tahunbudget1').disabled = true;
	document.getElementById('regional1').value = reg;
	document.getElementById('regional1').disabled = true;
	showById('formaddbarang');
}
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					hideById('printPanel');
					hideById('contpreview');
					showById('contlisthead');
					hideById('formaddbarang');
...
function tampolkanHarga(tahun, regional,sumberharga) {
	document.getElementById('frminput').style.display = '';
	document.getElementById('frmlist').style.display = 'none';
...
function listHarga(tahunbudget,regional,row) {
	if(tahunbudget!=undefined){
		document.getElementById('tahuntemp').value=tahunbudget;		
	}
	if(regional!=undefined){
		document.getElementById('regtemp').value=regional;		
	}
	if(tahunbudget=='' || tahunbudget==undefined){		
		tahunbudget = document.getElementById('tahuntemp').value;
	}
	if(regional=='' || regional==undefined){
		regional = document.getElementById('regtemp').value;
	}
	namabarangcari = document.getElementById('namabarangcari').value;
	
	var ada = 0;
	param2 = 'what=closing&tahunbudget=' + tahunbudget + '&regional=' + regional;
	param = 'tahunbudget=' + tahunbudget + '&regional=' + regional+ '&namabarangcari=' + namabarangcari;
	tujuancek = 'budget_slave_5hargabarang_cek.php';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					showById('printPanel');
					showById('contpreview');
					hideById('contlisthead');
					hideById('formaddbarang');
...
function TutupHarga(row, row2) {
	...
	kodeV='';
	//kodeV = kode.innerHTML;
	param = 'tahunbudget=' + tahunbudgetV + '&regional=' + regionalV + '&kodebarang=' + kodeV+ '&method=tutup';
...
function BukaHarga(row, row2) {
	...
	kodeV='';
	//kodeV = kode.innerHTML;
	param = 'tahunbudget=' + tahunbudgetV + '&regional=' + regionalV + '&kodebarang=' + kodeV+ '&method=buka';
	tujuan = 'budget_slave_5hargabarang_close.php';
	post_response_text(tujuan, param, responl);
	//    document.getElementById('barisl_'+row).style.backgroundColor='orange';
	function responl() {
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					alert('Data sudah tersimpan, silahkan click tombol List Data untuk me-refresh data.');
				}
			} else {
				busy_off();
				error_catch(con.status);
...
function editHargax() {
	tahunbudget1= document.getElementById('tahunbudget1x').innerHTML;
	regional1   = document.getElementById('regional1x').innerHTML;
	kodebarang1 = document.getElementById('kodebarang1x').innerHTML;
	namabarang1 = document.getElementById('namabarang1x').innerHTML;
	hargasatuan1= document.getElementById('hargasatuan1x').value;

	param = 'what=edit&tahunbudget=' + tahunbudget1 + '&regional=' + regional1 + '&kodebarang=' + kodebarang1 + '&hargasatuan=' + hargasatuan1;
	tujuan = 'budget_slave_5hargabarang_save.php';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					listHarga(tahunbudget1,regional1);
				}
			} else {
				busy_off();
				error_catch(con.status);
