...
function getdataIJSC(id, kolom) {
	prpopup(id, 'Approval Form', 'event');
	notransaksi = id;
	met = 'get_form_approval_IJSC';
	param = 'method=' + met + '&notransaksi=' + notransaksi + '&kolom=' + kolom;
	tujuan = 'sdm_slave_approval_ijsc.php';
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function nextapprovalIJSC(tipe) {
	...
	met = met.value = 'insert_nextapprovalIJSC';
