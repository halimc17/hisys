...
function savefile(notransaksi){
	...
	con.open("POST", "kebun_tbskud_slave.php?method=savefile", true);
...
function getrekening(divisi, sel='') {
	paramd = 'divisi='+divisi+'&method=getrekening';
	tujduan = 'kebun_tbskud_slave.php';
	post_response_text(tujduan, paramd, respogd);
// alert(divisi);
	// alert('data load');
	function respogd() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					// alert(sel);
					if(sel!='')document.getElementById('rekening').value=sel;
				}
			} else {
				busy_off();
				error_catch(con.status);
...
function saveht() {
	...
	param += '&divisi=' + divisi+'&rekening=' + rekening;
	param += '&method=' + method;
	// alert(param);
	tujuan = 'kebun_tbskud_slave.php';
	post_response_text(tujuan, param, respog);
...
function cancelht(){
	...
	document.getElementById('rekening').innerHTML="<option value=''>Pilih Data</option>";
...
function simpanApproval(notransaksi,maxaproval,page) {
	...
	if(confirm('Posting '+notransaksi+'\nThis transaction will released. are you sure?')) {
       	post_response_text('kebun_tbskud_slave.php', param, respon);
    }
    function respon() {
        if (con.readyState == 4) {
            if (con.status == 200) {
...
function excel(notransaksi)
{
	...
	param='notransaksi='+notransaksi+'&method=pdf3&type=excel';
	tujuan='kebun_tbskud_slave.php';
	// alert(param);
	// return false;
	judul='Report Ms.Excel';	
...
function printFile(param,tujuan,title,ev,notransaksi)
{
   tujuan=tujuan+"?"+param;  
   width='700';
   height='400';
...
function loopsave(currRow,maxRow) {
    ...
    if(unit=='' || tanggal=='') {
        ...
        param+='&method=savedt'+'&unit='+unit+'&divisi='+divisi+'&rekening='+rekening+'&tanggal='+tanggal+'&notransaksi='+notransaksi;
