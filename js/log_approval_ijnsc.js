...
function getdataIJNSC(id, kolom) {
	prpopup(id, 'Approval Form', 'event');
	notransaksi = id;
	met = 'get_form_approval_IJNSC';
	param = 'method=' + met + '&notransaksi=' + notransaksi + '&kolom=' + kolom;
	tujuan = 'sdm_slave_approval_ijnsc.php';
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function nextapprovalIJNSC(tipe) {
	...
	met = met.value = 'insert_nextapprovalIJNSC';
