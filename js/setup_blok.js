...
function updBlokDropdown() {
    ...
    var indukblok = document.getElementById('indukblok');
    var afdeling = document.getElementById('sAfdeling');
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    arrdata = con.responseText.split('##');
                    document.getElementById('indukblok').value=arrdata[0];
                    eval(arrdata[1]);
                }
            } else {
                busy_off();
...
function isiinduk() {
    ...
    var afdeling = document.getElementById('sAfdeling');
    var param = "afdeling="+afdeling.options[afdeling.options.selectedIndex].value;
    var param = "kodeorg="+kodeorg.options[kodeorg.options.selectedIndex].value;
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    // Success Response
                    document.getElementById('indukblok').value=con.responseText;
                }
            } else {
                busy_off();
                error_catch(con.status);
    ...
    post_response_text('setup_slave_blok_indukblok.php', param, respon);
}
...
function itungUnplan(){
    ...
    tot=parseFloat(cad)+parseFloat(umu)+parseFloat(klm)+parseFloat(jln)+parseFloat(pbrik)+parseFloat(kntr)+parseFloat(rmh)+parseFloat(sngi)+parseFloat(rndhn)+parseFloat(oksi)+parseFloat(lc);
