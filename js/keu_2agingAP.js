...
function preview(tipe){
    ...
    param='kodept='+kodept+'&periode='+periode+'&periode2='+periode2+'&tipe='+tipe+'&kodesupplier='+kodesupplier;
	param += '&method=' + method+'&nodok='+nodok+'&kodeunit='+kodeunit+'&customer='+customer+'&tanggal='+tanggal+'&noakun='+noakun;
	// alert(param);
    tujuan='keu_2agingAP_slave.php';
	// if(tipe!='html'){
		// judul=tipe;
		// ev='event';
	...
	if(tipe=='excel'){
		...
		alertify.popuppdf("title","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='keu_2agingAP_slave.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('90%','80%');
		// judul=tipe;
		// ev='event';
		// printFile(param,tujuan,judul,ev);
		// printnopopup(tujuan+'?'+param);
