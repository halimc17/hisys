...
function getJabatan(id) {
    ...
    post_response_text('pmn_slave_5ttd.php', param, respog);
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('jabatan').value = con.responseText;
                }
            } else {
                busy_off();
                error_catch(con.status);
...
function simpan() {
    ...
    validate([
        ["nama", "Nama harus dipilih."],
        ["jabatan", "Jabatan harus diisi."]
    ]);

    param = 'method=' + method + '&nama=' + nama + '&jabatan=' + jabatan + '&status=' + stat;
    tujuan = 'pmn_slave_5ttd.php';
...
function editdata(nama, jabatan, stat) {
    param = '';
    param += '&mode=update';
    param += '&nama=' + nama;
    param += '&method=create';
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    alertify.popup().destroy();
                    alertify.popup("Update", "<center>" + con.responseText + "</center>").set({ 'resizable': true, 'maximizable': false }).resizeTo('55%', '70%');
                    $(document).ready(function () {
                        $('.select2').select2({
                            dropdownAutoWidth: true,
                            width: 'auto'
                    ...
                    document.getElementById("nama").removeAttribute("onchange");
                    setValue2('nama', nama);
                    setValue2('jabatan', jabatan);
                    setValue2('status', stat);
                    setValue2('method', 'update');
                    document.getElementById('nama').disabled = true;
                    document.getElementById("nama").setAttribute("onchange", "getJabatan(this.value)");
                }
            } else {
                busy_off();
                error_catch(con.status);
