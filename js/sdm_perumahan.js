...
function save_header() {
  ...
  pintu_rmh = document.getElementById("pintu_rmh").value;
...
function save_asset() {
  ...
  oldasset = document.getElementById("oldasset").value;
  param =
    "blok=" +
    blok_asset +
    "&no_rmh=" +
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          document.getElementById("pintu_rmh").value = ar[6];
...
function fillFieldAsset(kodeOrg, blokHeadher, normhHeadher, asset, nmasset) {
  ...
  x = document.getElementById("kode_asset");
  var option = document.createElement("option");
  option.text = nmasset;
  option.value = asset;
  x.add(option);
  document.getElementById("kode_asset").value = asset;
  document.getElementById("oldasset").value = asset;
}
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          //alert(con.responseText);
          document.getElementById("blok_rmh_asset").innerHTML =
            con.responseText;
          document.getElementById("no_rmh_asset").innerHTML = "";
          load_data_asset();
        }
      } else {
        busy_off();
...
function get_asset() {
  kd_org = document.getElementById("kode_org_asset").value;
  tujuan = "log_get_data_header_rumah.php";
  param = "method=get_asset" + "&code_org=" + kd_org;
  post_response_text(tujuan, param, respog);
  //alert(param);
  function respog() {
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          //alert(con.responseText);
          document.getElementById("kode_asset").innerHTML = con.responseText;
        }
      } else {
        busy_off();
        error_catch(con.status);
...
function get_normh(blok_rmh, no_rmh, kode_org) {
  ...
  if (no_rmh != 0 && blok_rmh != 0) {
    ...
    tujuan = "log_get_data_header_rumah.php";
    param =
      "method=get_normh" + "&kode_blok=" + kd_blok + "&code_org=" + kode_org;
  }
  //alert(param);
  post_response_text(tujuan, param, respog);
  //alert(param);
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          //alert(con.responseText);
          document.getElementById("no_rmh_penghuni").innerHTML =
            con.responseText;
        }
      } else {
        busy_off();
...
function clear_save_form() {
  ...
  document.getElementById("kndsi_rmh").value = "";
  document.getElementById("pintu_rmh").value = 0;
