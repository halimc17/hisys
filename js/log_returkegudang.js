...
//set value display periode
	...
	if (gudang != '') {
		if (x == 'simpan') {
			...
			tujuan = 'log_slave_getBapbNumber_returgudang.php';
			param = 'gudang=' + gudang;
...
function Fverify()
{
	...
	kodebarang=trim(document.getElementById('kodebarang').value);
        ...
        mesin=trim(document.getElementById('mesin').value);
	gudang = document.getElementById('sloc').options[document.getElementById('sloc').selectedIndex].value;
	if (nomorlama == '' || kodebarang == '' || gudang == '') {
		alert('Storage location, Old.Document and material code are obligatory');
	}
	else {
		param = 'nomorlama=' + nomorlama + '&kodebarang=' + kodebarang + '&kodegudang=' + gudang + '&mesin=' + mesin;
...
function simpanRetur()
{
	...
	}else if(jlhretur == '' || jlhretur == '0' || jlhretur.substr(0,1) == '-'){
		alert('Please input QTY');return;
	}  
	else {
...
function searchBLOKLama(title, content, ev) {
    nolama2 = document.getElementById('nomorlama').value;
    kodebarang_ = document.getElementById('kodebarang').value;
    if(nolama2 == ''){
        alert('No.Dokumen.Lama Belum Terisi');
        return false;
    }
...
function setblokLama(kodeblok,mesin) {
