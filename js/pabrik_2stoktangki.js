...
function detail(unit,tanggal,kodetangki,tipe){
	method='detail';
    param='unit='+unit+'&tanggal='+tanggal+'&kodetangki='+kodetangki+'&tipe='+tipe+'&method='+method;
    tujuan='pabrik_2stoktangki_slave.php';
	if(tipe=='excel'){
				...
				if (con.status == 200) {
					...
					if (!isSaveResponse(con.responseText)) {
						...
						if(tipe=='html'){
							...
							alertify.popup("Detail",con.responseText).set({'resizable':true,'maximizable':true}).resizeTo('70%','50%'); 
						}
					}
				} else {
					busy_off();
