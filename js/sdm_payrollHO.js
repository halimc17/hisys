...
function getbulan() {
	...
	param = 'method=getbulan' + '&finish=' + finish + '&start=' + start + '&userid=' + userid;
	//alert(param);
	tujuan = 'sdm_slave_angsuran.php';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('lama').value = isi[0];
					document.getElementById('detailpot').innerHTML = isi[1];
					loadData();
				}
			} else {
				busy_off();
...
function hitung() {
	total = remove_comma_var(document.getElementById('total').value);
	rpbulan = document.getElementById('rpbulan').value;

	angsur = total / rpbulan;

	if (angsur === Infinity)
		angsur = 0;

	document.getElementById('lama').value = Math.ceil(angsur);
	lama = Math.ceil(angsur);

	for (i = 1; i <= lama; i++) {
		dis = document.getElementById('dis_' + i).innerHTML;
		idrp = document.getElementById('rpdetail_' + i)
		if (idrp != undefined && dis != 'disabled') {
			document.getElementById('rpdetail_' + i).value = rpbulan;
		}
	}
...
function gettot() {
	...
	if (idx == 'AngsuranJangkaWaktu') {
		rpbln = parseInt(total) / parseInt(lama);

		if (rpbln === NaN) {
			rpbln = 0;
		} else if (rpbln === Infinity) {
			rpbln = 0;
		}

		document.getElementById('rpbulan').value = rpbln;
	...
	for (i = 1; i <= lama; i++) {
		...
		idrp = document.getElementById('rpdetail_' + i)
		if (idrp != undefined && dis != 'disabled') {
			document.getElementById('rpdetail_' + i).value = rpbln;
		}
	}
}
...
function gettot1() {
	...
	ttl = (rpbulan * lama) - total;

	if (ttl < 0) {
		alert('jumlah rupiah lebih kecil dari Total Hutang, silahkan tambah jumlah bulan')
	}


}

function viewexcel(tipe) {
	kdOrg = document.getElementById('kdOrg').value;
	ev = 'event';
	param = 'method=loadData' + '&kdOrg=' + kdOrg + '&tipe=' + tipe; alert(param);
...
function editComp(id, name, jenis, type, lock, pph21, irregular) {
	...
	document.getElementById('pph21').checked = false;
	if (pph21 == '1') {
		document.getElementById('pph21').checked = true;
	}
...
function cancelComp() {
	...
	document.getElementById('pph21').checked = false;
	document.getElementById('irregular').value = 0;
...
function saveComp() {
	...
	pph21 = 1;
	if (document.getElementById('pph21').checked == false) {
		pph21 = 0;
	}

	irregular = document.getElementById('irregular').value;

	if (trim(name).length > 0 && confirm('Are you sure..')) {
		param = 'name=' + name + '&id=' + id + '&plus=' + plus + '&type=' + type + '&lock=' + lock + '&pph21=' + pph21 + '&irregular=' + irregular;
...
function editAngsuran(karyawanid, jenis, total, xstart, jlhbln, aktf, xketerangan, tahap1, tahap2, tahap3, tahap4, tahap5, rpbulan) {
	...
	if (typeof xketerangan !== 'undefined') {
		valketerangan = xketerangan;
	} else {
		valketerangan = '';
	}
	document.getElementById('rpbulan').value = rpbulan;
	document.getElementById('tahap1').value = tahap1;
	document.getElementById('tahap2').value = tahap2;
	document.getElementById('tahap3').value = tahap3;
	document.getElementById('tahap4').value = tahap4;
	document.getElementById('tahap5').value = tahap5;
	document.getElementById('method').value = 'update';
	lama = document.getElementById('lama').value = jlhbln;
	document.getElementById('keterangan').value = valketerangan;
	total = document.getElementById('total').value = total;
	document.getElementById('userid').disabled = true;
	document.getElementById('idx').disabled = true;
	document.getElementById('start').disabled = true;

	editdetail(karyawanid, jenis);
}
function editdetail(karyawanid, jenis) {
	param = 'karyawanid=' + karyawanid + '&jenis=' + jenis + '&method=editdetail';
	post_response_text('sdm_slave_angsuran.php', param, respon);
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('detailpot').innerHTML = con.responseText;
				}
			} else {
				busy_off();
				error_catch(con.status);
...
function delAngsuran(karyawanid, jenis, start) {
...
function cancelAngsuran() {
	...
	document.getElementById('tahap1').value = 0;
	document.getElementById('tahap2').value = 0;
	document.getElementById('tahap3').value = 0;
	document.getElementById('tahap4').value = 0;
	document.getElementById('tahap5').value = 0;
	document.getElementById('keterangan').value = '';
	document.getElementById('totaldetail').value = '';
	document.getElementById('detailpot').innerHTML = '';
...
function saveAngsuran() {
	...
	finish = document.getElementById('finish').options[document.getElementById('finish').selectedIndex].value;
	lama = document.getElementById('lama').value;
	rpbulan = remove_comma_var(document.getElementById('rpbulan').value);
	/*tahap1 = remove_comma_var(document.getElementById('tahap1').value);
	tahap2 = remove_comma_var(document.getElementById('tahap2').value);
	tahap3 = remove_comma_var(document.getElementById('tahap3').value);
	tahap4 = remove_comma_var(document.getElementById('tahap4').value);
	tahap5 = remove_comma_var(document.getElementById('tahap5').value);*/
	active = document.getElementById('active').options[document.getElementById('active').selectedIndex].value;
	method = document.getElementById('method').value;
	keterangan = document.getElementById('keterangan').value;
	totaldetail = document.getElementById('totaldetail').value;
	if (lama != totaldetail) {
		alert("Jumlah bulan dan jumlah detail angsuran tidak sama");
	}
	...
	if (keterangan == '') {
		alert('Please to input description coloum');
	} else {
		param = 'userid=' + userid + '&idx=' + idx + '&total=' + total + '&keterangan=' + keterangan;
		param += '&start=' + start + '&finish=' + finish + '&lama=' + lama + '&rpbulan=' + rpbulan + '&active=' + active;
		//param += '&tahap1='+tahap1+'&tahap2='+tahap2+'&tahap3='+tahap3+'&tahap4='+tahap4+'&tahap5='+tahap5;
		param += '&method=' + method;
		param += '&totaldetail=' + totaldetail;

		for (i = 1; i <= totaldetail; i++) {
			rpdetail = document.getElementById('rpdetail_' + i).value;
			bulan = document.getElementById('bulan_' + i).innerHTML;
			param += '&rpdetail_' + i + '=' + rpdetail;
			param += '&bulan_' + i + '=' + bulan;
		}
...
function saveKontribusi() {
	...
	pn0 = document.getElementById('penambah0').value;
	pn0 = replaceComma(pn0);
	pn1 = document.getElementById('penambah1').value;
	pn1 = replaceComma(pn1);
	pn2 = document.getElementById('penambah2').value;
	pn2 = replaceComma(pn2);
	pn3 = document.getElementById('penambah3').value;
	pn3 = replaceComma(pn3);
	pn4 = document.getElementById('penambah4').value;
	pn4 = replaceComma(pn4);
	...
	param += '&pn0=' + pn0 + '&pn1=' + pn1 + '&pn2=' + pn2 + '&pn3=' + pn3 + '&pn4=' + pn4;
...
function savePPh21Component(obj, id) {
	if (obj.checked)
		...
		to = 0;
	method = 'update_pph21';
	param = 'to=' + to + '&idx=' + id + '&method=' + method;
...
function thrSetup(obj, val) {
	...
	if (obj.checked) {
		param = 'action=insert&id=' + val + '&kdpt=' + kdpt;
	} else {
		param = 'action=delete&id=' + val + '&kdpt=' + kdpt;
	}
...
function showformupload(ev) {
	...
	width = '';
	height = '';
	content = "<fieldset style=width:96%><legend>Form</legend><div id=contUpload style='overflow:auto;width:100%;height:auto;' ></div></fieldset>";
...
function showupload(ev) {
	trans_no = document.getElementById('userid').value;
	idx = document.getElementById('idx').value;
	if (trans_no == '') {
		alert('Silahkan pilih Nama Karyawan terlebih dahulu');
		return;
	}
	showformupload(ev);
	param = 'method=showupload&trans_no=' + trans_no + '&idx=' + idx;
	tujuan = 'sdm_slaveSaveAngsuranUploadFile.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
			if (con.status == 200) {
...
function submitfile() {
	...
	var idx = document.getElementById("idxupload").innerHTML;
	...
	con.open("POST", "sdm_slaveSaveAngsuranUploadFile.php?method=submitfile", true);
...
function loadfiles(trans_no, idx) {
	param = 'method=loadfiles&trans_no=' + trans_no + '&idx=' + idx;
	tujuan = 'sdm_slaveSaveAngsuranUploadFile.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
...
function deletefile(trans_no, idx, namafile) {
	param = 'method=deletefile&trans_no=' + trans_no + '&idx=' + idx + '&namafile=' + namafile;
	tujuan = 'sdm_slaveSaveAngsuranUploadFile.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
...
function deletefileall(trans_no, idx) {
	param = 'method=deletefileall&trans_no=' + trans_no + '&idx=' + idx;
	tujuan = 'sdm_slaveSaveAngsuranUploadFile.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
...
function form() {
	width = '';
	height = '';
	content = "<fieldset style=width:96%><div id=containerd style=\"width:100%;max-height:700px;overflow:auto;\"></div></fieldset>";
	ev = 'event';
	title = "Detail HTML";
	showDialog5(title, content, width, height, ev);
}
function viewfile(trans_no, idx, ev) {
	form();
	param = 'method=showupload&trans_no=' + trans_no + '&idx=' + idx;
	tujuan = 'sdm_slaveSaveAngsuranUploadFile.php';
	post_response_text(tujuan, param, respog);
...
function detailangsuran(karyawanid, jenis, start, ev) {
	width = '';
	height = '';
	...
	var param = "karyawanid=" + karyawanid + "&jenis=" + jenis + '&start=' + start;

	param += '&method=detailangsuran';
	tujuan = 'sdm_slave_angsuran.php';
	post_response_text(tujuan, param, respog);
	function respog() {
		if (con.readyState == 4) {
