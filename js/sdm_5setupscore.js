function loaddata() {
    ...
    tujuan = 'sdm_slave_5setupscore.php';
...
function editdata(jenis,judul,text,nilaidari,nilaisampai,nilai,stat){
	param  = '';
	param += '&mode=update';
	param += '&method=addnew';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					setValue2('judul',judul);
					setValue2('text',text);
					setValue2('nilaidari',nilaidari);
					setValue2('nilaisampai',nilaisampai);
					setValue2('nilai',nilai);
...
function simpan(){
	...
	validate([
        ["judul",bahasa.unit+" "+bahasa.unit],
        ["text",bahasa.jabatan+" "+bahasa.jabatan],
        ["nilaidari",bahasa.tipe+" "+bahasa.tipe],
        ["nilaisampai",bahasa.tipe+" "+bahasa.tipe],
        ["nilai",bahasa.tipe+" "+bahasa.tipe]
	]);
	
	param  = '';
	param += '&judul=' + judul;
	param += '&text=' + text;
	param += '&nilaidari=' + nilaidari;
	param += '&nilaisampai=' + nilaisampai;
	param += '&nilai=' + nilai;
	param += '&status=' + stat;
	param += '&method=' + method;
