function getkebunscr() {
	...
	tujuan = 'umm_slave_2uploadpeta.php';
...
function loaddata(page) {
	...
	param += "&tglscr=" + tglscr;
	post_response_text('umm_slave_2uploadpeta.php?proses=loaddata', param, respon);
	function respon() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function del(id) {
	param = "id=" + id;
	post_response_text('umm_slave_2uploadpeta.php?proses=del', param, respon);
	function respon() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function simpan() {
	...
	con.open("POST", "umm_slave_2uploadpeta.php?proses=simpan", true);
...
function deldata(idsvg) {
	...
	if (confirm('Are you sure delete this item : ' + idsvg + '?'))
		post_response_text('umm_slave_2uploadpeta.php?proses=deldata', param, respon);
}
...
function isifile(namafile, ev) {
	...
	showDialog4(title, "<iframe frameborder=0 style='width:795px;height:395px'" +
		" src='umm_slave_2uploadpeta.php?" + param + "'></iframe>", '800', '400', ev);
