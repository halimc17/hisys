function loaddata() {
    ...
    tujuan = 'sdm_slave_5jenispesangon.php';
...
function editdata(jenis,kode,jenis,statuspajak,maxpajak,stat){
	param  = '';
	param += '&mode=update';
	param += '&method=addnew';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					setValue2('statuspajak',statuspajak);
					setValue2('maxpajak',maxpajak);
...
function simpan(){
	...
	statuspajak=document.getElementById('statuspajak').value;
	maxpajak=document.getElementById('maxpajak').value;
	stat=document.getElementById('status').value;
	method=document.getElementById('method').value;
	
	validate([
        ["jenis",bahasa.jenis+" "+bahasa.jenis]
	]);
	
	param  = '';
	param += '&kode=' + kode;
	param += '&jenis=' + jenis;
	param += '&statuspajak=' + statuspajak;
	param += '&maxpajak=' + maxpajak;
	param += '&status=' + stat;
	param += '&method=' + method;
