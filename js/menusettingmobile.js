function saveMenu(parent,caption,caption2,caption3,action,showlink,hideinput,type,newfile){
	...
	param='method=savemenu&id_parent='+id_parent+'&caption='+_caption+'&action='+_action+'&class='+clas+'&create='+createFile+'&caption2='+_caption2+'&caption3='+_caption3;
	if (clas == 'Type...'){
		alert('Choose type..!');
        objType.focus();
		...
		if(confirm('Are you sure ...'))
			post_response_text('slave_menumobile.php', param, respog);
	}
	...
	function respog(){
		if(con.readyState==4){
			if(con.status==200){
				...
				if (con.responseText.lastIndexOf('Warning') > -1){
				...
				}else if (con.responseText.lastIndexOf('xxx') > -1){
					...
					if(_ischildable=='stop'){
						...
						_in += "<select style='display:none' id=type" + _id + " onchange=checkType('" + _id + "',this)>";
						_in += "<option>click</option>";
						_in += "</select>";
						_in += "<input type=text placeholder='Caption...' maxlength=40 class=myinputtext title='Text to be shown on menu' id=newCaption" + _id + " size=12 onkeypress=\"return tanpa_kutip(event);\" "+_disabled+">";
						_in += "<input type=text placeholder='PanelName' maxlength=40 class=myinputtext title='Panel Name' id=newCaption2" + _id + "x size=12 onkeypress=\"return tanpa_kutip(event);\" "+_disabled2+">";
						_in += "<input type=text placeholder='FunctionName' maxlength=200 class=myinputtext title='Function Name and Variable' id=newCaption3" + _id + "x size=12 onkeypress=\"return tanpa_kutip(event);\" "+_disabled3+">";
						_in += "<input type=text placeholder='FormName' maxlength=40 class=myinputtext title='Form Name' id=newAction" + _id + " size=12 onkeypress=\"return tanpa_kutip(event);\" "+_disabled4+">";
						_in += "<input style='display:none' type=checkbox  title='Check to create this file' id=newFile" + _id +">";
...
function edit(_id){
	param='method=menuforedit&id='+_id;
...
function saveEditedMenu(id){
    ...
    param='method=editedmenu&id='+id+'&caption='+newCaption+'&action='+newAction+'&caption2='+newCaption2+'&caption3='+newCaption3;
...
function activate(menu_id){
	...
	if(obj.checked){
		param='method=activemenu&setHide=0&id='+menu_id;
	}else{
		param='method=activemenu&setHide=1&id='+menu_id;	
	}
...
function delet(m_id){
	...
	param='method=deletemenu&id='+m_id;
...
function showEditor(id,sub,e){
	...
	// 	Get id submenu 
	param='method=showeditor&parent='+id+'&sub='+sub;
	post_response_text('slave_menumobile.php', param, respog);
...
function change(dest,x,mx){
	...
	if(dest=='up' && (x-1)==0)
		...
		param='method=change&from='+fromId+'&to='+toId+'&orderfrom='+ox+'&orderto='+oy;
		post_response_text('slave_menumobile.php', param, respog);
...
function setMapUserMenu(ev,rowobj,uname){
	...
	pos=getMouseP(ev);
	param='method=mapusermenu&uname='+uname;
	...
	post_response_text('slave_privillagesmobile.php', param, respog);	
}

function resetDetailPrivillage(uname){
	param='method=resetdetailprivillage&uname='+uname;		
    function respog(){
		if(con.readyState==4){
			if (con.status == 200){
				busy_off();
...
function changePrivillage(menuid,uname,obj){
	...
	param='method=changeprivillage&uname='+uname+'&menuid='+menuid+'&action='+action;		
...
{
       if(val=='Caption...' || val=='Action...' || val=='PanelName' || val=='FunctionName' || val=='FormName')
       {
               obj.value='';
       }
}
...
function leaveText(val,obj)
{
       if(val=='' || val=='')
       {
               if(obj.id.lastIndexOf('Caption3')>-1)
               {
                       obj.value='FunctionName';
               }else if(obj.id.lastIndexOf('Caption2')>-1)
               {
                       obj.value='PanelName';
               }else if(obj.id.lastIndexOf('Caption')>-1)
               {
                       obj.value='Caption..';
               } 
               else
...
{
       document.getElementById('lab0').style.backgroundColor='#D4EAF9';
       document.getElementById('lab1').style.backgroundColor='#D4EAF9';
       document.getElementById('lab2').style.backgroundColor='#D4EAF9';
       document.getElementById('lab3').style.backgroundColor='#D4EAF9';
       document.getElementById('lab4').style.backgroundColor='#D4EAF9';
       x=arguments.length;
       for(z=0;z<x;z++)
