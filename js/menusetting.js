function setroleuser(idrole, username, objt){
    ...
    param = 'method=setauthrolex';
	param+= '&username=' + username;
	param+= '&idrole=' + idrole;
	param+= '&action=' + action;
	param+= '&sumber=adduser';
...
function expandsemua() {
    e = document.getElementsByName("child[]");
	for (x = 0; x <= e.length; x++) {
		e[x].style.display = '';
    }
}

function collapssemua() {
    e = document.getElementsByName("child[]");
	for (x = 0; x <= e.length; x++) {
...
function loadDetailPrivillageSetting(obj, evt,sumber) {
    hideDetailForm('globalakses', 'ctrmenu');
    hideDetailForm('ctr', 'ctrmenu'); //hide detail from
...
function setMapUserMenu(ev, rowobj, uname,sumber) {
...
function hideDetailForm() {
    ...
    document.getElementById('lab5').style.backgroundColor = '#D4EAF9';
    x = arguments.length;
    for (z = 0; z < x; z++)
					...
					if(data.length>0){
						for(i=0;i<data.length;i++){
							if(document.getElementById('cx' + data[i])!=undefined){								
								...
								if(c.checked==false && data[i]!=menuid){
									document.getElementById('cx' + data[i]).checked=true;
								}
								if(c.checked==true && data[i]!=menuid && action == 'remove'){
...
function setThis(id, cap, e, typ) {
    if (document.getElementById('parent').innerHTML == '') {
        ...
        if (id == document.getElementById('idparent').value) {
            ...
            if (document.getElementById('child').innerHTML == '') {
...
function copyPrivileges() {
    ...
    if (document.getElementById('tipe').checked) {
        ...
        typecopy = "allmenu";
    } else if (document.getElementById('tipe4').checked) {
        typecopy = "alltitle";
    } else {
        typecopy = "jabatan";
    }

    param = 'dari=' + dari + '&pengguna=' + pengguna + '&proses=user&jabatan=' + jabatan + '&typecopy=' + typecopy;
    if (dari == pengguna && typecopy != "alltitle"){
...
function getjabatan() {
    dari = document.getElementById('dari').options[document.getElementById('dari').selectedIndex].value;
    param = 'dari=' + dari + '&proses=getjabatan';
    tujuan = 'slave_copyPrivileges.php';
    post_response_text(tujuan, param, respog);
    function respog() {
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('namajabatan').innerHTML = arr[1];
                }
            } else {
                busy_off();
                error_catch(con.status);
...
function checktpyecopy() {
    if (document.getElementById('tipe').checked) {
        document.getElementById('trnewuser').style.display = "";
        document.getElementById('trjabatan').style.display = "none";
    } else if (document.getElementById('tipe2').checked) {
        document.getElementById('copyfrom').style.display = "";
		document.getElementById('trnewuser').style.display = "none";
        document.getElementById('trjabatan').style.display = "";
	}else if (document.getElementById('tipe3').checked) {
        document.getElementById('trnewuser').style.display = "";
        document.getElementById('trjabatan').style.display = "none";
