function batal(){
	document.getElementById('tipedok').selectedIndex = 0;
	document.getElementById("upload").value = "";
	document.getElementById('tipedok').disabled = false;
	document.getElementById('kegiatan').disabled = false;
	document.getElementById('nodok').disabled = false;
	document.getElementById("method").value = "insert";
	
	param='proses=batal';
	tujuan='bi_slave_5upload_photo.php';
...
function getkegiatan(kegiatan,nodok){
	tipedokumen = document.getElementById('tipedok');
    tipedokumen = tipedokumen.options[tipedokumen.selectedIndex].value;
	
	param='proses=getkegiatan&tipedokumen='+tipedokumen;
	tujuan='bi_slave_5upload_photo.php';
			...
			if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					if(kegiatan!='undefined'){
						lkegiatan = document.getElementById('kegiatan');
						for(ard=0;ard<lkegiatan.length;ard++){
							if(lkegiatan.options[ard].value==kegiatan){
								lkegiatan.options[ard].selected=true;
							}
						}
						getnodok(nodok);
					}else{
...
function adddok(){
	...
	formdata.append("nodok", document.getElementById('nodok').value);
	
	var con = createXMLHttpRequest();
	con.open("POST", "bi_slave_5upload_photo.php?proses=adddok", true);
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
					...
					newRow.innerHTML += "<td style='text-align:center'><img title='Hapus' class=resicon onclick=\"deletephoto(this,'"+con.responseText+"','delete')\" src='images/delete_32.png'/></td>";
					document.getElementById('upload').value = '';
                }
            } else {
                busy_off();
...
function deletephoto(btn,namafile,tipedel){
	param='proses=deletephoto&namafile='+namafile+'&tipedel='+tipedel;
	tujuan='bi_slave_5upload_photo.php';
...
function simpan(){
	...
	param='proses=simpan&method='+method+'&tipedokumen='+tipedokumen+'&kegiatan='+kegiatan+'&nodok='+nodok;
	tujuan='bi_slave_5upload_photo.php';
...
function fillfield(tipedok,kegiatan,nodok){
	document.getElementById('tipedok').value = tipedok;
	document.getElementById('method').value = 'update';
	document.getElementById('tipedok').disabled = true;
	document.getElementById('kegiatan').disabled = true;
	document.getElementById('nodok').disabled = true;
	
	param='proses=fillfield&tipedok='+tipedok+'&kegiatan='+kegiatan+'&nodok='+nodok;
	tujuan='bi_slave_5upload_photo.php';
...
function deletefield(nodok){
	param='proses=deletefield&nodok='+nodok;
	tujuan='bi_slave_5upload_photo.php';
	...
	if(confirm('Anda yakin hapus item ini : '+nodok+'?'))
		post_response_text(tujuan, param, respog);
}

function detailfield(nodok,ev){
	param = 'proses=detailfield&nodok='+nodok;
    title="Data Detail";
	showDialog2(title,"<iframe frameborder=0 style='width:395px;height:195px'"+
    " src='bi_slave_5upload_photo.php?"+param+"'></iframe>",'400','200',ev);	
...
function isifile(namafile,ev){
    ...
    " src='bi_slave_5upload_photo.php?"+param+"'></iframe>",'800','400',ev);	
...
// function showEdit3(id,periode,kodept,unit,tipedokumen,kegiatan,keterangan){
