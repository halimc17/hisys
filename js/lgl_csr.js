...
function edit(notransaksi,kodeorg,tanggal,kodecsr,volume,dana,kodekecamatan,kodedesa,kodekota,penerima,keterangan,status){
    //alert(kodedesa);
	
    document.getElementById('kodeorg').value=kodeorg;
    document.getElementById('tanggal').value=tanggal;
	document.getElementById('notransaksi').value=notransaksi;
    document.getElementById('kegiatan').value=kodecsr;
        
	document.getElementById('volume').value=volume;
	document.getElementById('dana').value=dana;
	document.getElementById('kec').value=kodekecamatan;
	document.getElementById('des').value=kodedesa;
	document.getElementById('kab').value=kodekota;
...
function detail(){
	...
	if(notransaksi==''||kodeorg==''||tanggal==''||instansi==''||keterangan==''||tipe==''||penerima==''){
    ...
    param += '&keterangan=' + keterangan+'&instansi=' + instansi+'&tipe=' + tipe;
...
function cancel(){
	...
	document.getElementById('kab').disabled=false;
...
function getkec(){
	kodekab= document.getElementById('kab').options[document.getElementById('kab').selectedIndex].value;
	
	
	param='kodekab='+kodekab+'&method=getkecamatan';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('kec').innerHTML=con.responseText;
					getdes();
				}
			}else {
				busy_off();
...
function getdes(){
	kodekec= document.getElementById('kec').options[document.getElementById('kec').selectedIndex].value;
	
	
	param='kodekec='+kodekec+'&method=getdesa';
		...
		if (con.readyState == 4) {
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('des').innerHTML=con.responseText;
				}
			}else {
				busy_off();
				error_catch(con.status);
