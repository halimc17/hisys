function showupload(notransaksi,ev){
	...
	tujuan = 'kebun_tbsinternal_slave.php';
...
function savefile(notransaksi){
	...
	con.open("POST", "kebun_tbsinternal_slave.php?method=savefile", true);
...
function simpanApproval(notransaksi,maxaproval,page) {
	...
	if(confirm('Posting '+notransaksi+'\nThis transaction will released. are you sure?')) {
       	post_response_text('kebun_tbsinternal_slave.php', param, respon);
    }
    function respon() {
        if (con.readyState == 4) {
            if (con.status == 200) {
...
function excel2(notransaksi) {
	param = 'method=excel2' + '&notransaksi=' + notransaksi;
	// alert(param);
	alertify.popuppdf("title","<iframe frameborder=0 style='width:100%;height:90%;overflow:none' src='kebun_tbsinternal_slave.php?"+param+"'></iframe>").set({'resizable':true, 'overflow':false}).resizeTo('90%','80%');		
}
