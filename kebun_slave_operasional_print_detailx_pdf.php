...
switch($proses) {
    case 'pdf':
        ...
        $sPres="select sum(a.insentif) as upahpremi, sum(a.umr) as umr,sum(a.jhk) as jumlahhk,kodekegiatan,
                tanggal,b.kodeorg,b.hasilkerja 
				from ".$dbname.".kebun_kehadiran a 
				left join ".$dbname.".kebun_prestasi b on a.notransaksi=b.notransaksi and a.nourut=b.nourut and a.nik=b.nikpemel
                left join ".$dbname.".kebun_aktifitas c on a.notransaksi=c.notransaksi 
				where a.notransaksi='".$param['notransaksi']."' group by a.notransaksi, kodekegiatan, b.kodeorg order by kodekegiatan asc, b.kodeorg asc";
        ...
        while($rPres=$qPres->fetch()){
			...
			$pdf->Cell(15/100*$width,$height,@hidezerodecimal($rPres['upahpremi'],0),1,0,'R',1);
    ...
    case'html':
        ...
        $tab.="</tr></thead><tbody>";
         $sPres="select sum(a.insentif) as upahpremi, sum(a.umr) as umr,sum(a.jhk) as jumlahhk,kodekegiatan,
                tanggal,b.kodeorg, sum(b.hasilkerja) as hasilkerja 
				from ".$dbname.".kebun_kehadiran a 
        ...
        while($rPres=$qPres->fetch()){
             ...
             $tab.="<td align=right>".@hidezerodecimal($rPres['jumlahhk'],2)."</td>";
