...
case 'getdetail':
	case'IJSC':
			#nama approval sama dengan IJNS
				$countApp = getCountApproval("IJSC");
				$tab.="<fieldset>
					...
					<table class='sortable' cellspacing='1' cellpadding='5' border='0'>
						...
						where a.jenispersetujuan='IJSC' and a.status='0' and a.karyawanid='".$karyawanid."' group by a.notransaksi order by b.tanggal desc";
                ...
                $tab.="<tr class=rowcontent>
                    ...
                    if ($showaction!=$level || $level==1) {
                        $tab.="<td style='text-align:center'>
                            <button class=mybutton onclick=\"getdataIJSC('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['approve']."</button>
                            </td>
                            <td style='text-align:center'>
                            <button class=mybutton onclick=\"tolakIJSC('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['ditolak']."</button>
                            </td>";
                    } else {
                        $tab.="<td colspan=2>&nbsp;</td>";
                    }
                    ...
                    for ($i = 1; $i <= $countApp; $i++) {
                        $arrDetail = detailApprove($i, $bar['notransaksi'], 'IJSC');
                        
                        $strpo="select * from ".$dbname.".setup_approval where jenispersetujuan='IJSC' and kodeunit='".$kodeorg."' and level='".$i."'";
	...
	case'get_form_approval_IJSC':
		...
		$countApp = getCountApproval('IJSC',$koderorg);

		for($i=1;$i<=$countApp;$i++){			
			$strx="select * from ".$dbname.".approval where notransaksi='".$notransaksi."' and jenispersetujuan='IJSC' and level='".$i."'";
			$resx=fetchdata($strx);
			foreach($resx as $keyx=>$valx){
				if($karyawanid==$valx['karyawanid']){
					if($i == $countApp){
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									<td colspan=3 align=center>
										<button id=Ajukan class=mybutton onclick=nextapprovalIJSC('approved') >Approved</button>
									</td>
								</tr>
							</table>
						</div>";
                        ...
                        a.karyawanid!='".$_SESSION['standard']['userid']."' and a.jenispersetujuan='IJSC' and a.level='".$level."' and a.kodeunit='".$koderorg."'  order by b.namakaryawan asc";
						...
						$tab.="<div id=test style=display:block>
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									...
									<td colspan=2></td>
									<td>
										<button class=mybutton onclick=nextapprovalIJSC() title=\" Submit to the next level\" id=Ajukan >".$_SESSION['lang']['diajukan']."</button>
									</td>
								</tr>
							</table>
	...
	case 'insert_nextapprovalIJSC':
		...
		if ($bar['approval'] == 1 || $bar['approval'] == 2 ) {
			...
			$arrDetail = detailApprove($kolom, $notransaksi, 'IJSC');
			$level = $kolom + 1;
			...
			if ($kolom != $countApp) {
				...
				if ($userid == $arrDetail['karyawanid']) {
					...
					if($tipeapp=='1'){
						...
						$str = "insert into ".$dbname.".approval values ('','".$notransaksi."','IJSC','".$level."','".$userid."','0','','','')";
						$owlPDO->exec($str);
					}
	...
	echo"<div id=rejected_form>
		...
		<tr>
            <td colspan=3 align=center>
	            <button class=mybutton onclick=\"inserttolakIJSC(".$_POST['kolom'].")\" >".$_SESSION['lang']['ditolak']."</button>
		    </td>
        </tr>
        </table>
		</div>";
