...
?>
<script src='js/sdm_tiketing.js?v=<?php echo time(); ?>'></script>
...
$optorgx="<option value=''></option>";
$str="select distinct kodeorg from ".$dbname.".sdm_tiket";
$res=fetchdata($str);
foreach($res as $bar){
	$nminduk = makeOption($dbname, 'organisasi', 'kodeorganisasi,namaorganisasi',"kodeorganisasi='".$bar['kodeorg']."'");
	$d=$nminduk[$bar['kodeorg']];
	if($d!=$n){			
		$nmorg = makeOption($dbname, 'organisasi', 'kodeorganisasi,namaorganisasi',"kodeorganisasi='".$d."'");
	...
	if($bar['kodeorg']==$_SESSION['empl']['lokasitugas']){
		$optorgx.="<option value=".$bar['kodeorg']." selected>".$bar['kodeorg']." - ".$nmorg[$bar['kodeorg']]."</option>";
	}else{		
		$optorgx.="<option value=".$bar['kodeorg'].">".$bar['kodeorg']." - ".$nmorg[$bar['kodeorg']]."</option>";
	}
...
$asumber=array('1'=>'Perjalanan Dinas','2'=>'Pengajuan Tiketing');
foreach ($asumber as $bar =>$val){
	@$optsumber.="<option value=".$bar.">".$val."</option>";	
}

OPEN_BOX('','<span class=judul>'.getMenu('sdm_tiketing').'</span><br>');
...
echo"<table border=0>
		...
		<fieldset id=formpencarianheader ><legend><b>" . $_SESSION['lang']['find'] . "</b></legend> 
         <table>
			...
			<tr>
				...
				<td>:</td>
				<td><input type='text' onkeypress='enterkey(event,loaddata)' style='width:130px;' class='myinputtext' id='suppliersch'></td>
	...
	<td>
			<fieldset id=formpencarianview style=display:none;><legend><b>" . $_SESSION['lang']['find'] . "</b></legend> 
			 <table>
				<tr>
					<td>" . $_SESSION['lang']['sumber'] . "</td> 
					<td>:</td>
					<td><select id=sumberview onchange='viewlistdata()' style=\"width:135px;\">".$optsumber."</select></td>
					
					
				   <td>".$_SESSION['lang']['jenis']."</td><td>:</td>
					<td><select id=jenisview onchange='viewlistdata()' style=\"width:135px;\">".$optjenis."</select></td>

					<td>" . $_SESSION['lang']['kodeorg'] . "</td> 
					<td>:</td>
					<td><select id=kodeorgview onchange='viewlistdata()' style=\"width:135px;\">".$optorgx."</select></td>
				
				</tr>
				<tr>
					<td>" . $_SESSION['lang']['notransaksi'] . "</td> 
					<td>:</td>
					<td><input type='text' onkeypress='enterkey(event,viewlistdata)' style='width:130px;' class='myinputtext' id='notransaksiview'>
					</td>
					
					<td>" . $_SESSION['lang']['namakaryawan'] . "</td> 
					<td>:</td>
					<td><input type='text' onkeypress='enterkey(event,viewlistdata)' style='width:130px;' class='myinputtext' id='namaview'>
					</td>
					
					<td>" . $_SESSION['lang']['tanggal'] . "</td> 
					<td>:</td>
					<td><input type='text' onchange='viewlistdata()' style='width:130px;' class='myinputtext' id='tglview' onmousemove='setCalendar(this.id)' onkeypress='return false';  />
					</td>
					
				</tr>
				<tr>
					<td><td>
					<td><button class=mybutton onclick=viewlistdata(0)>" . $_SESSION['lang']['find'] . "</button><button class=mybutton onclick=displayList(0)>" . $_SESSION['lang']['cancel'] . "</button></td>
					</td>
				</tr>
			</table>
		</fieldset>
...
$optsumber.="<option value='3'>".$_SESSION['lang']['pengajuan']." Tiket</option>";
$optsumber.="<option value='1'>Perjalanan Dinas</option>";
$optsumber.="<option value='2'>".$_SESSION['lang']['lain']."</option>";
