...
@$tanggal1 = $resPeriod[0]['tanggalmulai'];
	...
	# Get Period Range
	$qPeriod = selectQuery($dbname, 'sdm_5periodegaji_kecil', 'tanggalmulai,tanggalsampai', "periode='" . $param['periodegaji'] . "' and kodeorg='" .
			$param['kodeorg'] . "'");
	$resPeriod = fetchData($qPeriod);
	@$tanggal1 = $resPeriod[0]['tanggalmulai'];
	...
	$strgjh = "select a.karyawanid,jumlah/25 as gjperhari,jumlah as umrbulanan,b.tipekaryawan from " . $dbname . ".sdm_5gajipokok a left join 
				  " . $dbname . ".datakaryawan b on a.karyawanid=b.karyawanid where a.tahun='".substr($tanggal1, 0, 7)."' ".$tpkar." and b.lokasitugas='" . $param['kodeorg'] . "' 
				   and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') 
				   and a.idkomponen in (1) and a.tahun='".$param['periodegaji']."' group by a.karyawanid";
	...
	$strup = "select sum(a.umr) as upahabsen,sum(a.premi) as premi,
		...
		sum(a.penaltykehadiran) as penaltykehadiran,a.karyawanid,sum(a.nilaihk) as hk,a.tipekaryawan
		FROM " . $dbname . ".sdm_absensidt_vw a left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
	...
	$str1 = "select a.*,b.namakaryawan from " . $dbname . ".sdm_5gajipokok a left join " . $dbname . ".datakaryawan b on a.karyawanid=b.karyawanid where a.tahun='" . $param['periodegaji'] . "' ".$tpkar." and b.lokasitugas='" . $param['kodeorg'] . "' and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') ";
	$res1 = fetchData($str1);
	...
	if ($numrows0 > 0 || $numrows1 > 0 || $numrows2 > 0 || $numrows3 > 0 || $numrows5 > 0 ) {
	...
	$query8 = "select sum(a.premiinput) as premi,a.karyawanid
		  ...
		  where 1=1 ".$tpkar." and b.lokasitugas='" . $param['kodeorg'] . "' 
		  and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') 
		  and b.karyawanid in (select karyawanid from datakaryawan where lokasitugas = '".$param['kodeorg']."'  )  
	...
	$query202 = "select sum(a.rppremi) as premi , sum(a.rphk) as rphk,sum(a.nilai1hk) as nilai1hk,a.karyawanid
			...
			and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') 
			and a.periode = '" . $param['periodegaji'] . "'     
			and a.kontanan = 'KONTANAN'   group by a.karyawanid";
	...
	$query7="select sum(a.umr) as upah,a.nik as karyawanid,sum(a.premi) as premi,sum(a.jhk) as jhk from ".$dbname.".vhc_spl_kehadiran_vw a left join 
		...
		and a.tanggal>='".$tanggal1."' and a.tanggal<='".$tanggal2."'   group by a.nik";
	$premRes2 = fetchData($query7); 
	foreach($premRes2 as $idx => $val){
	   	if($val['premi']>0){   
		 	@$premi[$val['karyawanid']]+=$val['premi'];
	...
	$str="select a.nikmandor,a.tanggal,a.kodeorg,b.tipekaryawan,b.lokasitugas from ".$dbname.".kebun_aktifitas a 
		...
		b.karyawanid in (select karyawanid from datakaryawan where lokasitugas = '".$param['kodeorg']."'  ) ".$tpkar1." and
	...
	#########################################################################################################################################
			...
			$list0 .="<tr align=center class=rowheader>";
				...
				$list0 .= "<th rowspan=2>Total Gaji Kecil</th>";
			$list0.="</tr>";
			$list0.="<tr>";
...
switch ($proses) {
	...
	$owlPDO->beginTransaction();
		...
		$sdel = "delete from " . $dbname . ".sdm_gaji_kecil "
			...
			. " where sistemgaji='Harian' and lokasitugas='" . $param['kodeorg'] . "'  )  and sumber!='UPLOAD'";        
