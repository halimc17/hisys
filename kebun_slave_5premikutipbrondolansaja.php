...
switch($method){
	case'copy':
		...
		// }
		$str="select * from ".$dbname.".kebun_5premikutipbrondolansaja where 1=1 ".$where." and tahun='".$prdawal."' and kodeorg='".$afd."'";
		// exit("warning:$str");
		$res=fetchData($str);
		$ct=count($res);
		if ($ct==0) {
		...
		foreach($res as $bar) {
			$sIns="insert into ".$dbname.".kebun_5premikutipbrondolansaja (kodeorg,tahun,basis,brondolan,createby,createdate,updateby,lastupdate,posting) 
			values ('".$bar['kodeorg']."','".$prdakhir."','".$bar['basis']."','".$bar['brondolan']."','".$_SESSION['standard']['userid']."','".date('Y-m-d H:i:s')."','".$_SESSION['standard']['userid']."','".date('Y-m-d H:i:s')."','0')"; #exit("error".$sIns);
    ...
    case'insert':
		...
		$scek="select * from ".$dbname.".kebun_5premikutipbrondolansaja 
	    where kodeorg='".$afd."' and tahun='".$tahun."'";
		...
		try{
			...
			$sIns = insertQuery($dbname,"kebun_5premikutipbrondolansaja",$dataIns,$cols);
			$owlPDO->exec($sIns); 
    ...
    case'loadData':
            ...
            <table class=sortable cellspacing=1 cellpadding=5 border=0 >
                <thead>
					<tr class=rowheader>
						...
						<th align=center rowspan=2 width=75px>".$_SESSION['lang']['brondol']."</th>
		...
		$ql2="select count(*) as jmlhrow from ".$dbname.".kebun_5premikutipbrondolansaja where 1=1 ".$where."";
		...
		$str="select * from ".$dbname.".kebun_5premikutipbrondolansaja where 1=1 ".$where." order by tahun desc, kodeorg asc limit ".$offset.",".$limit."";
		...
		while($bar=$res->fetch()) {
			...
			<td style='text-align:right'>".$bar['brondolan']."</td>";
			...
			if($bar['posting']=='0' or $bar['posting']=='2'){				
				echo"<td align=center style=width:20px><img src='images/skyblue/submit.jpg' class='resicon' height='30' title='Ajukan' onclick=\"form_ajukan('".substr($bar['kodeorg'],0,4)."','".$bar['tahun']."','".$bar['kodeorg']."');\"></td>";
			}else{
				echo"<td align=center></td>";
			}
			if($bar['posting']=='0' or $bar['posting']=='2'){
				echo"<td align=center style=width:20px><img src=images/application/application_edit.png class=resicon  title='Edit' onclick=\"fillField('".$bar['kodeorg']."','".$bar['tahun']."', '".$bar['basis']."','".$bar['brondolan']."');\"></td>";

				echo"<td align=center style=width:20px><img src=images/application/application_delete.png class=resicon  title='Delete' onclick=\"del('".$bar['kodeorg']."','".$bar['tahun']."');\"></td>";
			}else{
				echo"<td align=center></td>";
				echo"<td align=center></td>";
			}
		...
		echo"
			<tr class=rowheader><td colspan=15 align=center>
		...
		if(count($res)==0)	{
			...
			$tab.="<table><input hidden id=unitajukan value=".$param['unit'].">
					...
					<tr>
						...
						<td><button onclick=ajukan('".$kodeapproval."','".$param['periode']."','".$param['kodeorg']."') class=mybutton style=width:200px>Ajukan</button></td> 
					</tr>
				</table>";
		}
		echo $tab;	
	...
	try {
		$owlPDO->beginTransaction();
			...
			$str = "update " . $dbname . ".kebun_5premikutipbrondolansaja set posting='9', nopengajuan ='".$param['nopengajuan']."'
				 where kodeorg like '".$param['unit']."%' and posting in ('0','2') and tahun='".$param['periode']."'
                 and kodeorg='".$param['kodeorg']."'"; 
			#EXIT("error".$str);
			$owlPDO->exec($str);
			
			
			$str = "delete from ".$dbname.".approval where jenispersetujuan='PNNBR' and status='0' and notransaksi not in (select nopengajuan from ".$dbname.".kebun_5premikutipbrondolansaja)";
			$owlPDO->exec($str);
			...
			$str = "insert into " . $dbname . ".approval (`nourut`,`notransaksi`, `jenispersetujuan`,
				...
				values ('','".$param['nopengajuan']."','PNNBR','1','" . $param['kepada']."','0','".$param['komentar']."','','')";
			$owlPDO->exec($str);
    ...
    case'update':
		...
		try{
			...
			$sUpdt = updateQuery($dbname,"kebun_5premikutipbrondolansaja",$dataUpd,"kodeorg='".$afd."' and tahun='".$tahun."'");
			$owlPDO->exec($sUpdt); 
		}catch (PDOException $e){
			echo"Gagal".$e->getMessage();
		}
    break;
    case 'deletedata':
		$sDel= deleteQuery($dbname,"kebun_5premikutipbrondolansaja","kodeorg='".$afd."' and tahun='".$tahun."'");
		try{
			$owlPDO->exec($sDel); 
			echo"";
		}catch (PDOException $e){
	...
	case'getdatapengajuan':
		$tab.="<table border=0 cellpadding=5 class=sortable cellspacing=1 width=100%>
				<thead>
					<tr class=rowheader>
						...
						<th align=center rowspan=2 width=50px>".$_SESSION['lang']['periode']."</th>
						<th align=center rowspan=2 width=75px>".$_SESSION['lang']['norma']."</th>
						<th align=center rowspan=2 width=50px>".$_SESSION['lang']['brondol']."</th>
					</tr>
				</thead>
			<tbody>";
		if($param['nopengajuan']==''){			
		...
		$str = "select * from ".$dbname.".kebun_5premikutipbrondolansaja where 1=1 ".$where." order by kodeorg,tahun asc";
		$res = fetchdata($str);
        foreach($res as $bar){
			$no++;
