...
OPEN_BOX('','<span class=judul>'.strtoupper('Inventory Debit Note/Credit Note Verification').'</span>');
$optNamaOrganisasi=makeOption($dbname, 'organisasi', 'kodeorganisasi,namaorganisasi');
$optPeriode="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
$lksiTugas=substr($_SESSION['empl']['lokasitugas'],0,4);

$sPeriode=$owlPDO->query("select distinct substring(tanggal,1,7) as periode from ".$dbname.".keu_jurnalht group by substring(tanggal,1,7) desc");
$sPeriode->setFetchMode(PDO::FETCH_ASSOC);
while($rPeriode=$sPeriode->fetch())
{
...
$sOrg=$owlPDO->query("select distinct kodeorganisasi from ".$dbname.".organisasi where length(kodeorganisasi)=4 order by kodeorganisasi asc ");	
$sOrg->setFetchMode(PDO::FETCH_ASSOC);
while($rOrg=$sOrg->fetch())
{
