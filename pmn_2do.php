...
$sPeriode2="select distinct substr(tanggalkontrak,1,4) as periode from ".$dbname.".pmn_kontrakjual where nokontrak != '' and kodebarang not in ('40000003') order by substr(tanggalkontrak,1,7) desc";
...
$sBrg="select distinct kodebarang as kodebarang from ".$dbname.".pabrik_timbangan where nokontrak != '' and kodebarang not in ('40000003') order by kodebarang";

$qBrg=$owlPDO->query($sBrg) or die(print " Gagal: ".PDOException::getMessage());
$qBrg->setFetchMode(PDO::FETCH_ASSOC);
while($rBrg=$qBrg->fetch())
{
    $optBrg.="<option value=".$rBrg['kodebarang'].">".getNamaBrg($rBrg['kodebarang'])."</option>";
    $optBrg2.="<option value=".$rBrg['kodebarang'].">".getNamaBrg($rBrg['kodebarang'])."</option>";
}

$optPt="<option value=''>".$_SESSION['lang']['all']."</option>";
$sOrg="select kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where tipe='PT' and kodeorganisasi in ('".implode("','",array_keys(getOrgDetail(3)))."')"; //echo $sOrg;
...
<script language=javascript src='js/pmn_2do.js?v=<?php echo time(); ?>'></script>
<link rel=stylesheet type=text/css href=style/zTable.css>

<?
OPEN_BOX('','<span class=judul>'.getMenu('pmn_2do').'</span>');
?>
...
<tr><td><td><button onclick="zPreview('pmn_slave_2do','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button hidden onclick="zPdf('pmn_slave_2do','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button><button onclick="zExcel(event,'pmn_slave_2do.php','<?php echo $arr?>')" class="mybutton" name="preview" id="preview">Excel</button></td></tr>
</table>
</fieldset>
...
<tr><td><td><button onclick="zPreview2('pmn_slave_2do','<?php echo $arr2?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><!-- <button onclick="zExcel2(event,'pmn_slave_2do.php','<?php echo $arr2?>')" class="mybutton" name="preview" id="preview">Excel</button> --></td></tr>
</table>
</fieldset>
<fieldset style="float: left;" hidden>
...
<tr><td><td><button onclick="zPreview3('pmn_slave_2do','<?php echo $arr3?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button onclick="zExcel3(event,'pmn_slave_2do.php','<?php echo $arr3?>')" class="mybutton" name="preview" id="preview">Excel</button></td></tr>
</table>
</fieldset>
</div>
<?php
