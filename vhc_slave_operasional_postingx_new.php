...
$noakunkeg = makeOption($dbname, 'vhc_kegiatan', 'kodekegiatan,noakun');
...
while ($bar = $res->fetch()) {
	$dataX[$bar['notransaksi']][$bar['kodekegiatan']]=$bar['kodekegiatan'];
}
// echo "<pre>";
// print_r($dataX);
// echo "</pre>";
...
$res = fetchData($str);
if(count($res)>0){
	exit("Warning: Periode gaji Unit ".$kdorgx." sudah ditutup, proses dibatalkan.");
}
...
foreach($dataX as $notranX => $valKegX){
	foreach($valKegX as $kegiatanX => $valBlokX){
		// foreach($valBlokX as $blokX){
			...
			// sama sepertii kebun_slave_operasional_printx.php(174) untuk ngambil PRESTASI KEHADIRAN ON KE nikpemel nik
			...
			if($statusfp==1){
				validasifp($tipevalidasi,$detval,'SPL',$arrUpload,$dataH[0]['tanggal'],'0');
				...
				foreach($arrUpload as $row){
					if($tipevalidasi=='subbagian'){
						...
						if($detval[$subbagian]!=''){
							$str = "select karyawanid from ".$dbname.".upload_absensi where 1=1 and karyawanid='".$row['nik']."' and tanggalabsen='".($dataH[0]['tanggal'])."' limit 1";
							$bar = fetchdata($str)[0];
				...
				if($tipevalidasi=='transaksi' and $detval['BKM']!=''){
					if($countUpload > 0){
						throw new PDOException("1. Absen fingerprint untuk karyawan dg NIK : <br>".$errorUpload."belum ada.<br>Silahkan lakukan proses melalui menu : SDM - Proses - Fingerprint.");
					}
				}else{
					if($countUpload > 0){
						throw new PDOException("2. Absen fingerprint untuk karyawan dg NIK : <br>".$errorUpload."belum ada.<br>Silahkan lakukan proses melalui menu : SDM - Proses - Fingerprint.");
					}
				} */
			}
			...
			$queryParam = selectQuery($dbname,'keu_5parameterjurnal','noakunkredit',
				"kodeaplikasi='SIPL' and jurnalid='".$kodeJurnal."'");
			...
			if(count($tmpKonter)==0){throw new PDOException("Kelompok Jurnal ".$kodeJurnal." untuk kodeorg ".$_SESSION['org']['kodeorganisasi'].", kodeunit : ".$_SESSION['empl']['lokasitugas'].", periode : ".$prdbln." silahkan tambah melalui menu : Keuangan - Setup - Kelompok Jurnal.");}
			...
			if(@count($lstUnit[$_SESSION['empl']['lokasitugas']])!=0 and ($costRawat-$totRpRK)!=0){
				foreach($dataD as $row) {
					...
					$dataRes['detail'][] = array(
						...
						'tanggal'     =>$dataH[0]['tanggal'],
						'nourut'      =>$noUrut,
						'noakun'      =>$noakunkeg[$row['kodekegiatan']],
						'keterangan'  =>'Upah Pemeliharaan '.$row['kodekegiatan'],
			...
			if(count($dataResRk)!=0){
				foreach($dataResRk['header'] as $key=>$dataDet) {
					$queryD = insertQuery($dbname,'keu_jurnalht',$dataDet);exit('error '. $queryD);
					$owlPDO->exec($queryD);
				}
			...
			if(count($lstPre)!=0){
				...
				if(count($lstUnit)!=0){
					foreach($lstUnit as $rw=>$rwUnit){
						if($rwUnit!=$_SESSION['empl']['lokasitugas']){
							$queryKonter = updateQuery($dbname,'keu_5kelompokjurnal',array('nokounter'=>$counterDt[$rwUnit]),"kodeorg='".$lstPt[$rwUnit]."' and kodekelompok='SIPL1' and kodeunit='".$rwUnit."' and periode='".$prdbln."'");
							$owlPDO->exec($queryKonter);
							
							if(count($lstPent[$rwUnit])!=0){
								$queryKonter = updateQuery($dbname,'keu_5kelompokjurnal',array('nokounter'=>$counterDtpen[$rwUnit]),"kodeorg='".$lstPt[$rwUnit]."' and kodekelompok='SIPL1' and kodeunit='".$rwUnit."' and periode='".$prdbln."'");
								$owlPDO->exec($queryKonter);
							}
							if(count($lstPre[$rwUnit])!=0){
								$queryKonter = updateQuery($dbname,'keu_5kelompokjurnal',array('nokounter'=>$counterDtpre[$rwUnit]),"kodeorg='".$lstPt[$rwUnit]."' and kodekelompok='SIPL1' and kodeunit='".$rwUnit."' and periode='".$prdbln."'");
								$owlPDO->exec($queryKonter);
							}
						}
					}    
