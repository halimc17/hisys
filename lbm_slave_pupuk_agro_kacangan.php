...
$qPosAwalThn = $owlPDO->query($sPosAwalThn) or die(print " Gagal: " . PDOException::getMessage());
$qPosAwalThn->setFetchMode(PDO::FETCH_ASSOC);
while ($rPosAwalThn = $qPosAwalThn->fetch()) {
...
foreach ($dtBarang as $lstBarang => $lstDtBarang) {
    $klmprBrg = isset($klmprBrg) ? $klmprBrg : '';
    ...
    case'pdf':
        ...
        class PDF extends FPDF {
            ...
            function Header() {
                ...
                $this->Cell(120, $height, "", 'BLR', 'C', 1);
                $this->Cell(80, $height, " ", 'BLR', 0, 'C', 1);
                $this->Cell(60, $height, " ", 'BLR', 0, 'C', 1);
                $this->Cell(50, $height, $_SESSION['lang']['bi'], 1, 0, 'C', 1);
                $this->Cell(50, $height, $_SESSION['lang']['sbi'], 1, 0, 'C', 1);
                $this->Cell(50, $height, $_SESSION['lang']['bi'], 1, 0, 'C', 1);
                $this->Cell(50, $height, $_SESSION['lang']['sbi'], 1, 0, 'C', 1);
        ...
        foreach ($dtBarang as $lstBarang => $lstBarangDt) {
            $klmprBrg2 = isset($klmprBrg2) ? $klmprBrg2 : '';
            if ($klmprBrg2 != substr($lstBarangDt, 0, 3)) {
                $klmprBrg2 = substr($lstBarangDt, 0, 3);
                $pdf->Cell(120, $height, $optKelompok[$klmprBrg2], 1, 0, 'L', 1);
                $pdf->Cell(390, $height, " ", 1, 1, 'C', 1);
            }
            $pdf->Cell(120, $height, $optNmBrg[$lstBarangDt], 1, 0, 'L', 1);
            $pdf->Cell(80, $height, @number_format($dtAwalThn[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(60, $height, @number_format($dtBlnLalu[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(50, $height, @number_format($dtTerima[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(50, $height, @number_format($dtTerimaSbi[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(50, $height, @number_format($dtKeluar[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(50, $height, @number_format($dtKeluarSbi[$lstBarangDt], 2), 1, 0, 'R', 1);
            $pdf->Cell(50, $height, @number_format($dtAkhrBln[$lstBarangDt], 2), 1, 1, 'R', 1);
        }
