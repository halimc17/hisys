...
//Ambil HK dari sdm_gaji_vw
$str="select * from ".$dbname.".sdm_gaji_vw  where 
		kodeorg='".$_SESSION['empl']['lokasitugas']."' 
		and periodegaji like '".$periode."%' ";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
	$absen[$bar['karyawanid']]['H']+=$bar['hk'];
}
...
else
{
        ...
        if($rCek>0)
        {
                ...
                while($rTot=$qTot->fetch())
                {
                        ...
                        @$jumTot[$rTot['karyawanid']]+=$rTot['jamaktual'];
                }
                ...
                //$peng2=36;
                    ...
                    {			
                                ...
                                foreach($dtAbsByr as $dtJmlhAbsDbyr){
                                    ...
                                    $stream.="<td align=right>".$absen[$dtKary][$dtJmlhAbsDbyr]."</td>";
