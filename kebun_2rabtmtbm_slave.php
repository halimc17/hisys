...
## Start Queries
		...
		#=bgt_budget TM
		$str="select *,left(noakun,5) as headakun from ".$dbname.".bgt_budget where 1=1 and ".$wh." and tahunbudget='".$tahunprd."' and tipebudget='ESTATE' and noakun like '621%' order by noakun asc";
        $res=fetchData($str);
		foreach ($res as $bar) {
        ...
        #=bgt_blok TM
        $str="select * from ".$dbname.".bgt_blok where 1=1 and ".$wh2." and tahunbudget='".$tahunprd."' and statusblok='TBM'";
        $res=fetchData($str);
        foreach ($res as $bar) {
            $tbm[$bar['tahunbudget']]+=$bar['hathnini'];
            $lad=$tbm[$bar['tahunbudget']];
        }

		#=bgt_budget TBM
		$str="select *,left(noakun,5) as headakun from ".$dbname.".bgt_budget where 1=1 and ".$wh." and tahunbudget='".$tahunprd."' and tipebudget='ESTATE' and noakun like '126%' order by noakun asc";
        $res=fetchData($str);
		foreach ($res as $bar) {
			$headakuntbm[$bar['headakun']]=$bar['headakun'];
			$isiakuntbm[$bar['headakun']][$bar['noakun']]=$bar['noakun'];
			$rptbm[$bar['headakun']][$bar['noakun']]+=$bar['rupiah'];
			$grandtbm[$bar['tahunbudget']]+=$bar['rupiah'];
            $totgrandtbm=$grandtbm[$bar['tahunbudget']];
		}
...
## Start Row content
		...
		foreach ($headakun as $head) {
		    foreach ($isiakun[$head] as $isi) {
                ...
                $tab.= "<td align=right>".number_format($rp[$head][$isi]/$lad,2)."</td>";
                $tab.= "</tr>";
                $subtotisi[$head]+=$rp[$head][$isi];
                $subtotisid7[$head]+=$rp[$head][$isi]/$lad;
            }
            $tab.= "<tr class=rowcontent>";
        ...
        $tab.= "<tr class=rowcontent>";
        $tab.= "<td align=right></td>";
        $tab.= "<td align=left><b>".$_SESSION['lang']['luas']." TBM</td>";
        $tab.= "<td align=right><b>".number_format($ladtbm,2)."</td>";
        $tab.= "<td align=center><b>Ha</td>";
        $tab.= "</tr>";

		foreach ($headakuntbm as $head) {
		    foreach ($isiakuntbm[$head] as $isi) {
                $tab.= "<tr class=rowcontent>";
                $tab.= "<td align=left>".$isi."</td>";
                $tab.= "<td align=left>".$nmheadakun[$isi]."</td>";
                $tab.= "<td align=right>".number_format($rptbm[$head][$isi],2)."</td>";
                $tab.= "<td align=right>".number_format($rptbm[$head][$isi]/$ladtbm,2)."</td>";
                $tab.= "</tr>";
                $subtotisitbm[$head]+=$rptbm[$head][$isi];
                $subtotisid7tbm[$head]+=$rptbm[$head][$isi]/$ladtbm;
            }
            $tab.= "<tr class=rowcontent>";
			$tab.= "<td align=left colspan=2><b>".$_SESSION['lang']['subtotal']." ( ".$head." - ".$nmheadakun[$head]." )</td>";
			$tab.= "<td align=right><b>".number_format($subtotisitbm[$head],2)."</td>";
			$tab.= "<td align=right><b>".number_format($subtotisid7tbm[$head],2)."</td>";
			$tab.= "</tr>";

            $grandtottbm+=$subtotisitbm[$head];
            $grandtotd7tbm+=$subtotisid7tbm[$head];
		}
        $tab.= "<tr class=rowcontent>";
        $tab.= "<td align=left colspan=2><b>Grand ".$_SESSION['lang']['total']."</td>";
        $tab.= "<td align=right><b>".number_format($grandtottbm,2)."</td>";
        $tab.= "<td align=right><b>".number_format($grandtotd7tbm,2)."</td>";
        $tab.= "</tr>";
## End Row Content
