...
<script language=javascript src='js/keu_3hpptbs.js?v=<?php echo time(); ?>'></script>
<?
include('master_mainMenu.php');		
OPEN_BOX('','<span class=judul><b>'.getMenu('keu_3hpptbs').'</b></span><br>');
$optper=$optunit="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";

// $str="SELECT * FROM ".$dbname.".organisasi where  tipe='HOLDING' and length(kodeorganisasi)=4 and induk not in (select induk from ".$dbname.".organisasi where tipe='PABRIK')";
// $str="SELECT * FROM ".$dbname.".organisasi where  tipe='KEBUN' and length(kodeorganisasi)=4 and induk not in (select induk from ".$dbname.".organisasi where tipe='PABRIK') and kodeorganisasi in (select substr(kodeorganisasi,1,4) from ".$dbname.".organisasi where kodesegment='0000000001')";
$str="SELECT * FROM ".$dbname.".organisasi where  tipe='KEBUN' and length(kodeorganisasi)=4 and induk not in (select induk from ".$dbname.".organisasi where tipe='PABRIK') and kodeorganisasi in (select substr(kodeorganisasi,1,4) from ".$dbname.".organisasi)";
// echo $str;
