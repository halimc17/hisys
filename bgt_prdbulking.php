...
?>
<script language=javascript1.2 src='js/bgt_prdbulking.js?v=<?php echo time(); ?>'></script>
...
OPEN_BOX('','<span class=judul>'.getMenu('bgt_prdbulking').'</span>');
...
$str="select * from ".$dbname.".organisasi where tipe ='PABRIK' order by induk asc";
$res = fetchdata($str);
foreach($res as $bar){
...
$optbarang="<option value='CPO'>CPO</option>";
$optbarang.="<option value='KER'>PK</option>";
#untuk inputan baru
...
OPEN_BOX();
	...
	<tbody>
		...
		<td><select id=jenis onchange=getunit(); style=\"width:100px;\">".$optjns."</select></td>
		<td><select id=kodeunit style=\"width:160px;\">".$optunit."</select></td>
		...
		<td><select id=komoditi style=\"width:100px;\">".$optbarang."</select></td>
		";
		foreach($bulan as $bln){				
			echo"<td align=center><input type=text class=myinputtextnumber style='width:55px' id=kg".$bln." onkeypress='return angka_doang(event)'></td>";
		}
		echo"<td><button class=mybutton onclick=simpan(); title=\"Save\">Save</button></td>";
	echo"</tbody>
	</table>";
CLOSE_BOX();	
echo"</div>";
