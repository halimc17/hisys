...
// $nmpek = makeOption($dbname,'vhc_kegiatan','kodekegiatan,namakegiatan');
   if ($pesan!="") {
   	# code...
	$str= "insert into ".$dbname.".log_po_chat (`nopo`,`karyawanid`,
          `pesan`)
		  values('".$nopo."',".$karyawanid.",'".$pesan."')"; 	
	try{
		$owlPDO->exec($str);
		...
		foreach ($idkar as $key => $val) {
			## CREATE NOTIFICATION
			$msgdt = "Ada pesan Chat terkait PO dengan No ".$nopo." dari ".$_SESSION['empl']['name'];
			createnotif($nopo,'CHAT',$msgdt,$val,date('Y-m-d H:i:s'));
		}
...
$str="select a.*,b.namauser from ".$dbname.".log_po_chat a left join ".$dbname.".user b
	on a.karyawanid=b.karyawanid
	where 1=1 and a.nopo='".$nopo."' order by tanggal asc";	 
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
