...
switch($proses)
{
	...
	if($unitDt!=''){
		$sUnit="select distinct kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where 
				kodeorganisasi like '".$unitDt."%' and tipe like 'GUDANG%' order by kodeorganisasi asc";
	...
	if($pt!=''){
		...
		while($rUnit=$qUnit->fetch())
		{
			$optUnit.="<option value='".$rUnit['kodeorganisasi']."'>".$rUnit['induk']." - GUDANG ".$rUnit['induk']."</option>";
		}
	}
     echo $optUnit;
    break;
	...
	if($pt!=''){
		$sUnit="select distinct kodeorganisasi,namaorganisasi from ".$dbname.".organisasi where 
				alokasi = '".$pt."' and tipe like 'GUDANG%' order by kodeorganisasi asc";
		$qUnit=$owlPDO->query($sUnit) or die(print " Gagal: ".PDOException::getMessage());
		$qUnit->setFetchMode(PDO::FETCH_ASSOC);
		while($rUnit=$qUnit->fetch())
		{
