...
while($bar=$res->fetch())
{
		...
		if($bar->photo==''){
			...
			$photo="photokaryawan/".($bar->photo==''?$defaulsrc:$bar->photo);
		}
        ...
        $stpjk=$optKetStatusPajak[$bar->statuspajak];
        $npwp=$bar->npwp;
        $bpjs=$bar->bpjs;
...
class PDF extends FPDF
{
        ...
        function Header()
        {
			...
			$arrHead = setheadreport(substr($lokasitugas,0,4));
			...
			$this->Line($this->lMargin,$this->tMargin+($height*2),
			$this->lMargin+$width,$this->tMargin+($height*2));
			$this->Ln(1);
...
//=============pdf
        ...
        $pdf->Cell(25,5,'No. SIM',0,0,'L');
                $pdf->Cell(40,5,': '.$sim,0,1,'L');	
        $pdf->SetX(60);		
        ...
        $pdf->Cell(25,5,$_SESSION['lang']['tanggalkeluar'],0,0,'L');
                    $pdf->Cell(60,5,': '.$tkeluar,0,0,'L');	
        $pdf->Cell(25,5,$_SESSION['lang']['tipekaryawan'],0,0,'L');
                    $pdf->Cell(70,5,': '.$tipekar,0,1,'L');	
        $pdf->Cell(32,5,$_SESSION['lang']['alamat'],0,0,'L');
                    $pdf->MultiCell(153,5,': '.$alamataktif.", ".$kota.", ".$provinsi.", ".$kodepos,0,'L');	
        $pdf->Cell(32,5,$_SESSION['lang']['status'].' '.$_SESSION['lang']['karyawan'] ,0,0,'L');
                    $pdf->Cell(70,5,': '.$statuskaryawan,0,1,'L');		
        ...
        $pdf->Cell(32,5,$_SESSION['lang']['suku'],0,0,'L');
                    $pdf->Cell(70,5,': '.$suku,0,1,'L');
        $pdf->Ln();	
}
//=======================Riwayat Pekerjaan
    $pdf->SetFont('Arial','B',10);		
...
$resq=$owlPDO->query($sJabat) or die(print " Gagal: ".PDOException::getMessage());
$resq->setFetchMode(PDO::FETCH_ASSOC);
while($rJabat=$resq->fetch())
{
...
$resq1=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$resq1->setFetchMode(PDO::FETCH_OBJ);
while($bar=$resq1->fetch())
    {
...
$resq1=$owlPDO->query($sJabat) or die(print " Gagal: ".PDOException::getMessage());
$resq1->setFetchMode(PDO::FETCH_ASSOC);
while($rJabat=$resq1->fetch())
{
