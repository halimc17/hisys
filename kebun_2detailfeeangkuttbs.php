...
<script language=javascript src='js/kebun_2detailfeeangkuttbs.js?ver=1.0'></script>

<?php
OPEN_BOX('','<span class=judul>'.getMenu('kebun_2detailfeeangkuttbs').'</span>');
...
## GET PENERIMA FEE
$optpenerimafee="<option value=''>".$_SESSION['lang']['all']."</option>";
if($chkebun!=''){
	$str="select id,nama from ".$dbname.".kebun_5namafee where kodeorg='".$chkebun."' order by nama";
	$res=fetchdata($str);
	foreach($res as $val){
		$optpenerimafee.="<option value='".$val['id']."'>".$val['nama']."</option>";
	}
}
...
echo"<div>
	...
	<table cellspacing='1' border='0' >
		...
		<tr>
			...
			<td>:</td>
			<td>
				<select id=penerimafee>".$optpenerimafee."</select>
				<img id='penerimafee' onclick=z.elSearch('penerimafee',event) class='resicon' src='images/onebit_02.png' style='position:relative;top:3px;left:3px;'>
			</td>
		</tr>
		<tr>
			<td colspan=2></td>
