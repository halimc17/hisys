...
switch($method){
	case 'loaddata':
		$sudahadadata=array();
		$str = "SELECT tahun FROM ".$dbname.".sdm_disiplin group by tahun"; 
        $res = fetchdata($str);
        foreach($res as $bar){
		...
		$str= "select * from ".$dbname.".sdm_5disiplin order by tahun desc, nourut asc";
		$res= fetchdata($str);
		foreach($res as $val){
	...
	case 'deletedata':
		...
		$str = "delete from ".$dbname.".sdm_5disiplin WHERE 1=1 ".$where."";
		try{
			$owlPDO->exec($str);
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5disiplin where nourut='".$param['nourut']."'  and tahun='".$param['tahun']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			if($param['tipe']=='0' or $param['tipe']=='1' or $param['tipe']=='3'){
				...
				if($param['tipe']=='0'){
					$str="select count(*) as jlhitem from ".$dbname.".sdm_5disiplin where tipe='0'  and tahun='".$param['tahun']."'";
					$res=fetchdata($str);
					if($res[0]['jlhitem'] > 0){
			...
			$str = insertQuery($dbname,'sdm_5disiplin',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':
		try {
			...
			## VALIDATE
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5disiplin where id!='".$param['id']."' and nourut='".$param['nourut']."'  and tahun='".$param['tahun']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
				throw new PDOException("No.Urut dan Tahun ini sudah terdaftar");
			}
			...
			$where = "id='".$param['id']."'";
			$str = updateQuery($dbname,'sdm_5disiplin',$data,$where);
			$owlPDO->exec($str);
