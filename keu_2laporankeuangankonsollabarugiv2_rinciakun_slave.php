...
foreach($arrnourut as $nourut){
	if($tipenourut[$nourut]=='Detail' and (@$jumlahdaftar[$nourut]>0 || @$jumlahdaftar[$nourut]!='')){
		...
		foreach($arrper as $per){
			$str="select sum(jumlah) as dtthnini,noakun from ".$dbname.".keu_jurnaldt_vw where 1=1  ".$where."  ".$listakun."  and periode='".$per."' group by noakun";
			// echo $str;
			$res=fetchdata($str);
			foreach($res as $bar){
				...
				$ltnoakun[$nourut][$bar['noakun']]=$bar['noakun'];
				@$dtthniniakun[$nourut][$bar['noakun']][$per]+=$bar['dtthnini'];
				### Rumus Pak Hadi
				if ($dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]]==0 && $dtthniniakun[$nourut][$bar['noakun']][$tahun.'-'.$i[1]]!=0) {
					$asu[$nourut][$bar['noakun']][$i[1]]=100;
				}else{
					$asu[$nourut][$bar['noakun']][$i[1]]=($dtthniniakun[$nourut][$bar['noakun']][$tahun.'-'.$i[1]]-$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]])/$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]]*100;
				}
				@$change[$nourut][$bar['noakun']][$i[1]]=$asu[$nourut][$bar['noakun']][$i[1]];
				
				### Rumus Pak Hadi
				if ($dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]]==0 && $dtthniniakun[$nourut][$bar['noakun']][$tahun.'-'.$i[1]]!=0) {
					$asux[$nourut][$bar['noakun']][$i[1]]=100;
				}else{
					$asux[$nourut][$bar['noakun']][$i[1]]=($dtthniniakun[$nourut][$bar['noakun']][$tahun.'-'.$i[1]]-$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]])/$dtthniniakun[$nourut][$bar['noakun']][$tahunlalu.'-'.$i[1]]*100;
				}
				@$changeini[$nourut][$bar['noakun']][$i[1]]=$asux[$nourut][$bar['noakun']][$i[1]];
			}
			$i=explode('-',$per);
			...
			if ($dtthnini[$nourut][$tahunlalu.'-'.$i[1]]==0 && $dtthnini[$nourut][$tahun.'-'.$i[1]]!=0) {
				...
				$asu[$nourut][$i[1]]=($dtthnini[$nourut][$tahun.'-'.$i[1]]-$dtthnini[$nourut][$tahunlalu.'-'.$i[1]])/($dtthnini[$nourut][$tahunlalu.'-'.$i[1]])*100;
			}
			@$change[$nourut][$i[1]]=$asu[$nourut][$i[1]];
			### Rumus Pak Hadi
			if ($ytdthn[$nourut][$tahunlalu]==0 && $ytdthn[$nourut][$tahun]!=0) {
				$asux[$nourut]=100;
			}else{
				$asux[$nourut]=($ytdthn[$nourut][$tahun]-$ytdthn[$nourut][$tahunlalu])/$ytdthn[$nourut][$tahunlalu]*100;
			}

			@$changeini[$nourut]=$asux[$nourut];
		}
	...
	foreach($arrnourut as $nourut){
		...
		$stream.="<tr class=rowcontent>";
			...
			foreach($arrper as $per){
				$tahunjudul=substr($per,0,4);
				if(strlen($tahunjudul)<'4'){
					$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan($change[$nourut][$per]),$dgt)."</td>";
				}else{
			...
			$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan(@$changeini[$nourut]),$dgt)."</td>"; 
		$stream.="</tr>";
		
			if($tipenourut[$nourut]=='Detail'){
				foreach($arrnoakun as $noakun){
					if($ltnoakun[$nourut][$noakun]){
						$stream.="<tr class=rowcontent>";
						$stream.="<td align=left style='width:1%'></td>"; 
						$stream.="<td align=left style='width:20%' nowrap>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$noakun." - ".$namaakun[$noakun]."</td>"; 
						foreach($arrper as $per){
							$tahunjudul=substr($per,0,4);
							if(strlen($tahunjudul)<'4'){
								$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan($changeini[$nourut][$noakun][$per]),$dgt)."</td>";
							}else{
								$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan($dtthniniakun[$nourut][$noakun][$per]),$dgt)."</td>"; 
							}
						}
					
						$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan(@$ytdthnakun[$nourut][$tahun][$noakun]),$dgt)."</td>"; 
						$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan(@$ytdthnakun[$nourut][$tahunlalu][$noakun]),$dgt)."</td>"; 
						$stream.="<td align=right ".$style.">".hidezerodecimal(fixnan(@$changeiniakun[$nourut][$noakun]),$dgt)."</td>"; 
					$stream.="</tr>";
					}
				}
			}
...
/*
				...
				// foreach($arrnoakun as $noakun){
					// $stream.="<td align=right ".$style.">".$noakun."</td>"; 
				// }
...
if($param['tipe']=='excel'){
	...
	$xls = new HtmlExcel();
	$xls->setCss($css);
	$xls->addSheet("LABARUGIKONSOLDETAILAKUN", $stream);
	...
	$dompdf->render();
	$dompdf->stream("LABARUGIKONSOLDETAILAKUN",array("Attachment"=>0));
} else {
	echo $stream;
}
