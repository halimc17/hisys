...
$tglAbsen2=tanggalnormal(checkPostGet('tglAbsen2',''));
$tglAbsen=date('Ymd');
...
switch($method){
	...
	case 'preview2':
	$nmTemp=str_replace('-','',str_replace('/','','uploadabsensi'));
	if($_FILES['file']['error']==0){    
		$filetype = strtolower('.'.substr($_FILES['file']['name'],strripos($_FILES['file']['name'],'.')+1));
		$filename = $nmTemp."_".$tglAbsen."".$filetype;
		...
		if($filetype=='.csv'){
			if($_FILES['file']['size'] <= 250000){
				...
				$ext=$ext[count($ext)-1];
				$ext=strtolower($ext);
				$path = $dir."/uploadabsensi_".$tglAbsen.".".$ext;
				@unlink($path);
				try
				{
					if(move_uploaded_file($_FILES['file']['tmp_name'], $path))
					{
						...
						for($row=1;$row<$jmlhRow;$row++){
							//echo $x[$row][0];
							$arrNik[$x[$row][0]] = $x[$row][1];
							$arrtanggal[$x[$row][0]][$x[$row][1]]=date('Y-m-d',strtotime($x[$row][0]));
							$arrTime[$x[$row][0]][$x[$row][1]]['jammasuk']=$x[$row][2];
							$arrTime[$x[$row][0]][$x[$row][1]]['jamkeluar']=$x[$row][3];
						}
						/*echo '<pre>';
						print_r($arrtanggal);
						echo '</pre>';*/
						if(count($arrtanggal)>0)
						{
							...
							<tbody>";
							$no = 0;
							foreach($arrtanggal as $valx => $valz){
								foreach($valz as $val => $val){
								...
								if($OptNama[$val]!='')
								{
									$no+=1;
									$tanggal=$valx;
									$jammasuk = $arrTime[$valx][$val]['jammasuk'];
									$jamkeluar = $arrTime[$valx][$val]['jamkeluar'];
	...
	//$tglAbsen = tanggalnormal($tglAbsen);
