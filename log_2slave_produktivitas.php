...
if($filter=='Tahunan')
{
    foreach ($arrBln as $ky => $vle) {
            ...
            " . $dbname . ".log_poht where  tanggal like '" . $periode . "-" . $ky . "%'  " . $where . "  group by purchaser";
            //exit($sCount);
            ...
            while ($rCount = $qCount->fetch()) {
                ...
                if ($rCount['purchaser'] != '') {
                    if (!isset($jmlhPo[$rCount['purchaser']][$vle]))
                        $jmlhPo[$rCount['purchaser']][$vle] = 0;
                    $jmlhPo[$rCount['purchaser']][$vle]+=intval($rCount['jmlhpo']);
    ...
    $tab = "<table cellspacing=1 border=" . $brdr . " class=sortable>
        ...
        <tr>
            ...
            <td " . $bgcoloraja . " rowspan=2  align=center>Total PP</td>";
    ...
    $tab.="</tr>";
    $tab.="<tr>";
    $tab.="<td width=35px " . $bgcoloraja . " align=center>PO</td>";
    $tab.="<td width=35px " . $bgcoloraja . " align=center>O.Std</td>";
    $tab.="</tr></thead>
        <tbody>";
    ...
    if ($dtcek != 0) {
        ...
        foreach ($purdt as $dtPur) {
            ...
            $tab.="<td  align=right onclick=\"detailPP(event,'log_2slave_produktivitas.php','" . $dtPur . "','0')\" style=cursor:pointer>".(isset($totPP[$dtPur]) ? $totPP[$dtPur] : 0)."</td>";
            foreach ($arrBln as $ky => $blns) {
                if (empty($jmlhPo[$dtPur][$blns])) {
                    $tab.="<td align=right style=cursor:pointer>0</td>";
                } else {
                    $tab.="<td align=right onclick=detailData(event,'log_2slave_produktivitas.php','".$ky."-01','" .$ky."-31','" . $dtPur . "')
                           style=cursor:pointer>" . $jmlhPo[$dtPur][$blns] . "</td>";
                }
            }
            $tab.="<td  align=right onclick=detailData(event,'log_2slave_produktivitas.php','','','" . $dtPur . "')
                       style=cursor:pointer>" . $totalPo[$dtPur] . "</td>";
            $tab.="<td  align=right onclick=detailPP(event,'log_2slave_produktivitas.php','" . $dtPur . "','1')  style=cursor:pointer >" . number_format((isset($outPP[$dtPur]) ? $outPP[$dtPur] : 0), 0) . "</td>";
            $tab.="</tr>";
        }
    } else {
...
for ($ard = 1; $ard <= $jumHari;) {
    ...
    for ($awal = $awalitung; $awal <= 7; $awal++) {
        ...
        while ($rCount = $qCount->fetch()) {
            ...
            if ($rCount['purchaser'] != '') {
                if (!isset($jmlhPo[$rCount['purchaser']][$mingguke]))
                    $jmlhPo[$rCount['purchaser']][$mingguke] = 0;
