	...
	case 'previewData':
		...
		$tab.="<table cellspacing='1' cellpadding='3' border='".$border."' class='".$class."' style='margin-top:0px;width:99.9%'>
			...
			<tr class='rowheader'>
				...
				<th style='text-align:center;vertical-align:middle' colspan=11>".$_SESSION['lang']['panen']."</th>
				<th style='text-align:center;vertical-align:middle' colspan=6>SPB</th>
				<th style='text-align:center;vertical-align:middle' rowspan='2'>".$_SESSION['lang']['kg']."</th>
			</tr>
			<tr class='rowheader'>
				...
				<th style='text-align:center;vertical-align:middle'>".$_SESSION['lang']['nik']."</th>
				<th style='text-align:center;vertical-align:middle'>".$_SESSION['lang']['namakaryawan']."</th>
		...
		$arrpemanen=array();
		$str="select tanggal,notransaksi,nikmandor,kerani,karyawanid,kodeorg,tph,sum(hasilkerja) as hasilkerja, sum(brondolan) as brondolan from ".$dbname.".kebun_prestasi_vw where 1=1 ".$where." group by tanggal,notransaksi,karyawanid,kodeorg,tph order by tanggal asc";
		$res=fetchdata($str);
		foreach($res as $val){
			...
			$arrkar[$val['tanggal']][$val['notransaksi']][$val['karyawanid']]=$val['karyawanid'];
			$arrblok[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']]=$val['kodeorg'];
			$arrtph[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']]['tph']=$val['tph'];
			$arrtph[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']]['jjg']+=$val['hasilkerja'];
			$arrtph[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']]['brondol']+=$val['brondolan'];
			...
			## GET DETAIL SPB
			$strx="select tanggal,tph,blok,pemanen,nospb,sum(jjg) as jjg, sum(brondolan) as brondolan, nokendaraan,supir,sum(kgwb) as kgwb from ".$dbname.".kebun_spb_vw where blok='".$val['kodeorg']."' and tph='".$val['tph']."' and tanggalpanen='".$val['tanggal']."' and pemanen='".$val['karyawanid']."'
			group by nospb,blok,tph,tanggalpanen,pemanen";
			//echo "<br>";
			$resx=fetchdata($strx);
			foreach ($resx as $xbv) {
				...
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbjjg']+=$xbv['jjg'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbbrondol']+=$xbv['brondolan'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbno']=$xbv['nospb'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbtanggal']=$xbv['tanggal'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbnopol']=$xbv['nokendaraan'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbsupir']=$xbv['supir'];
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['spbkg']=+$xbv['kgwb'];
			}
			
			$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['jumx']=count($resx);
			$color='';
			if (count($resx)==0)
			{
				// echo $val['karyawanid'];
				// echo "<br>";
				$arrspb[$val['tanggal']][$val['notransaksi']][$val['karyawanid']][$val['kodeorg']][$val['tph']][$xbv['nospb']]['colorx']="bgcolor='yellow'";
		...
		## GET DETAIL SPB YANG TIDAK ADA DATA PANENNNYA
			...
			if($datatph=='' or $datatanggalpanen=='' or $datanospb==''){
				...
				$strx="select tanggal,tph,blok,pemanen,nospb,sum(jjg) as jjg, sum(brondolan) as brondolan, nokendaraan,supir,sum(kgwb) as kgwb from ".$dbname.".kebun_spb_vw where blok not in (".$datablok.") and tph not in (".$datatph.") and tanggalpanen not in (".$datatanggalpanen.") and nospb not in (".$datanospb.") and pemanen not in (".$datapemanen.") ".$wehrexx."
				group by nospb,blok,tph,tanggalpanen,pemanen";
				//echo $strx;
				$resx=fetchdata($strx);
				foreach ($resx as $xbv) {
					$arrtdkcocok[$xbv['tanggal']][$xbv['nospb']][$xbv['blok']][$xbv['tph']][$xbv['pemanen']]['brondolan']+=$xbv['brondolan'];
					$arrtdkcocok[$xbv['tanggal']][$xbv['nospb']][$xbv['blok']][$xbv['tph']][$xbv['pemanen']]['jjg']+=$xbv['jjg'];
					$arrtdkcocok[$xbv['tanggal']][$xbv['nospb']][$xbv['blok']][$xbv['tph']][$xbv['pemanen']]['kgwb']+=$xbv['kgwb'];
					$arrtdkcocok[$xbv['tanggal']][$xbv['nospb']][$xbv['blok']][$xbv['tph']][$xbv['pemanen']]['spbnopol']=$xbv['nokendaraan'];
					$arrtdkcocok[$xbv['tanggal']][$xbv['nospb']][$xbv['blok']][$xbv['tph']][$xbv['pemanen']]['spbsupir']=$xbv['supir'];
				}
			...
			foreach($arrtgl as $tanggal){
				...
				foreach($arrtrk[$tanggal] as $key=>$val){
					...
					foreach($arrkar[$tanggal][$val['notransaksi']] as $karyawanid){
						...
						$nox3=0;
						foreach($arrblok[$tanggal][$val['notransaksi']][$karyawanid] as $blok){
							$nox3++;
							if($nox3 > 1){
								$tab.="<tr class='rowcontent'>";						
								$tab.="<td></td>";					
							...
							$nox4=0;
							foreach($arrtph[$tanggal][$val['notransaksi']][$karyawanid][$blok] as $tph =>$valdt){
								$nox4++;
									if($nox4 > 1){
										$tab.="<tr class='rowcontent'>";						
										$tab.="<td></td>";					
									...
									$nox5=0;
									foreach($arrspb[$tanggal][$val['notransaksi']][$karyawanid][$blok][$tph] as $spbno =>$valdt){
									$nox5++;
										if($nox5 > 1){
											$tab.="<tr class='rowcontent'>";						
											$tab.="<td></td>";					
