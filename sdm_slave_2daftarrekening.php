...
switch($method){
	...
	case'preview':
        ...
        $str="select karyawanid,nik,namakaryawan,kodejabatan,tipekaryawan,subbagian,norekeningbank,namabank,pemilikrekening from ".$dbname.".datakaryawan where lokasitugas='".$unit."' ".$where." order by namakaryawan asc";
        $res=fetchdata($str);
        $arrkary=$res;
		...
		foreach($arrkary as $val){
			...
			$tab.="<tr class='rowcontent'>
					...
					<td style='text-transform: uppercase;' align='left'>".$val['pemilikrekening']."</td>
					<td align='center'>".$val['norekeningbank']."</td>
					<td style='text-transform: uppercase;' align='center'>".$optnamabank[$val['namabank']]."</td>";
        }
