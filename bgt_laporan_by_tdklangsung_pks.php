...
OPEN_BOX('','<span class=judul>'.getMenu('bgt_laporan_by_tdklangsung_pks').'</span><br>');
#ambil tahun budget
...
#ambil kode kebun
$str="select kodeorganisasi as kodeorg from  ".$dbname.".organisasi where tipe in ('PABRIK','BULKING') order by kodeorganisasi";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
...
echo"<fieldset style='float:left;'><table>
	...
	<tr>
		...
		<button class=mybutton onclick=\"fisikKeExcel(event,'bgt_laporan_biaya_tdk_lngs_pks_excel.php')\">".$_SESSION['lang']['excel']."</button>
	</td> 
	 </tr>	
     </table>
