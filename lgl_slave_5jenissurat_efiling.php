...
switch($method){
	case 'loaddata':
		...
		<tbody>";
		$no=0;
		$str= "select * from ".$dbname.".lgl_5jenissurat_efiling";
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(jenis) as jlhitem from ".$dbname.".lgl_5jenissurat_efiling where jenis='".$param['jenis']."' ";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			$str = insertQuery($dbname,'lgl_5jenissurat_efiling',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':        
		try {
			...
			$where = " id ='".$param['idx']."' " ;
			$str = updateQuery($dbname,'lgl_5jenissurat_efiling',$data,$where);
