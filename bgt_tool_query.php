...
OPEN_BOX('','<span class=judul>'.strtoupper('BUDGET REVISION:').'</span><br>');
$opt="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
...
$qThnBud=$owlPDO->query($sThnBud) or die(print " Gagal: ".PDOException::getMessage());
$qThnBud->setFetchMode(PDO::FETCH_ASSOC);
while($rThnBud=  $qThnBud->fetch())
{
...
$qKdBudget=$owlPDO->query($sKdBudget) or die(print " Gagal: ".PDOException::getMessage());
$qKdBudget->setFetchMode(PDO::FETCH_ASSOC);
while($rKdBudget=$qKdBudget->fetch())
{
...
$qTraksi=$owlPDO->query($sTraksi) or die(print " Gagal: ".PDOException::getMessage());
$qTraksi->setFetchMode(PDO::FETCH_ASSOC);
while($rTraksi=$qTraksi->fetch())
{
