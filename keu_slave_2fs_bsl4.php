...
$tahun1 = checkPostGet('tahun21', '');
$tahun2 = checkPostGet('tahun22', '');
$tahun3 = checkPostGet('tahun23', '');
$bulan1 = checkPostGet('bulan21', '');
$bulan2 = checkPostGet('bulan22', '');
$bulan3 = checkPostGet('bulan23', '');
$tahunsd1 = checkPostGet('tahunsd21', '');
$tahunsd2 = checkPostGet('tahunsd22', '');
$tahunsd3 = checkPostGet('tahunsd23', '');
$bulansd1 = checkPostGet('bulansd21', '');
$bulansd2 = checkPostGet('bulansd22', '');
$bulansd3 = checkPostGet('bulansd23', '');

$tahun4 = checkPostGet('tahun24', '');
$tahunsd4 = checkPostGet('tahunsd24', '');
$bulan4 = checkPostGet('bulan24', '');
$bulansd4 = checkPostGet('bulansd24', '');

$tahunytd = checkPostGet('tahunytd2', '');
$tahunsdytd = checkPostGet('tahunsdytd2', '');
$bulanytd = checkPostGet('bulanytd2', '');
$bulansdytd = checkPostGet('bulansdytd2', '');
...
$str="select substr(periode,1,4) as tahun, noakun ".$sumjlh1." from ".$dbname.".keu_saldobulanan where 1=1 ".$wheresawal1." ".$where." group by noakun, periode";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
...
$str="select substr(periode,1,4) as tahun, noakun ".$sumjlh2." from ".$dbname.".keu_saldobulanan where 1=1 ".$wheresawal2." ".$where." group by noakun, periode";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
	if(!empty($dataArray))foreach($dataArray as $data){
...
$str="select substr(periode,1,4) as tahun, noakun ".$sumjlh3." from ".$dbname.".keu_saldobulanan where 1=1 ".$wheresawal3." ".$where." group by noakun, periode";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
	if(!empty($dataArray))foreach($dataArray as $data){
...
$str="select substr(periode,1,4) as tahun, noakun ".$sumjlh4." from ".$dbname.".keu_saldobulanan where 1=1 ".$wheresawal4." ".$where." group by noakun, periode";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
	if(!empty($dataArray))foreach($dataArray as $data){
...
$str="select substr(periode,1,4) as tahun, noakun ".$sumjlhytd." from ".$dbname.".keu_saldobulanan where 1=1 ".$wheresawalytd." ".$where." group by noakun, periode";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
while($bar=$res->fetch()){
	if(!empty($dataArray))foreach($dataArray as $data){
...
if(!empty($dataArray))foreach($dataArray as $data){
    if($data['tipe']=='Header'){
		...
		$stream.="
		<tr class=rowcontent style=cursor:pointer title='Click untuk melihat detail' onclick=\"viewdetail('".$data['nourut']."','".$pt."','".$kdorg."','".$periode1."','".$periode2."','".$periode3."','".$periodesd1."','".$periodesd2."','".$periodesd3."','html','bsl4','".$periode4."','".$periodesd4."','".$periodeytd."','".$periodesdytd."');\">
			<td colspan=".strlen($data['nourut']).">&nbsp;</td>
...
######PREVIEW
	...
	case'pdf':
		...
		$dompdf->render();
		$dompdf->stream("BSL4",array("Attachment"=>0));
	break;
######EXCEL	
    case 'excel':
        $tglSkrg = date("Ymd");
