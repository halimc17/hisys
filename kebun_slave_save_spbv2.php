...
$status_posting    = checkPostGet('status_posting', '');
...
switch ($proses) {
	...
	case 'amblBjr':
		...
		$sStpBlok = "select bjr from " . $dbname . ".kebun_5bjr where kodeorg LIKE '" . $blok . "%' order by periode desc limit 1";
	...
	*/
	...
	case 'donwloadmobile':
			...
			foreach ($res as $val) {
				...
				where a.nospb='" . $val['nospbref'] . "'";
					...
					foreach ($resxz as $valxz) {
						if ($valxz['syn'] == '1') {
							$dataspb1[$val['nospb']][$val['kodeorg']][$val['tanggal']][$val['penerimatbs']][$valxz['tanggalpanen']][''][$valxz['blok']][$valxz['tph']][$valxz['sesi']]['jjg'] += $valxz['jjg'];
							$dataspb1[$val['nospb']][$val['kodeorg']][$val['tanggal']][$val['penerimatbs']][$valxz['tanggalpanen']][''][$valxz['blok']][$valxz['tph']][$valxz['sesi']]['brondolan'] += $valxz['brondolan'];
						} else {
							$databelumsingkronfull[$val['nospb']] = $val['nospb'];
						}
					}
				} else {
					$dataspb1[$val['nospb']][$val['kodeorg']][$val['tanggal']][$val['penerimatbs']][$val['tanggalpanen']][''][$val['blok']][$val['tph']][$val['sesi']]['jjg'] += $val['jjg'];
					$dataspb1[$val['nospb']][$val['kodeorg']][$val['tanggal']][$val['penerimatbs']][$val['tanggalpanen']][''][$val['blok']][$val['tph']][$val['sesi']]['brondolan'] += $val['brondolan'];
				}
			}


			$dataspb2 = array();
			$str = "select a.nospb,a.tanggal,b.karyawanid,b.jjg,b.brondolan,b.sesi,b.kegiatan from " . $dbname2 . ".kebun_spbht_mobile a 
		left join " . $dbname2 . ".kebun_spbtkbm_mobile b on a.nospb=b.nospb
		where a.syn='1' and a.flag='0' and a.kodeorg in (" . getOrgDetail(2) . ")";
			$res = fetchdata($str);
			foreach ($res as $val) {
				$dataspb2[$val['nospb']][$val['karyawanid']][$val['sesi']][$val['kegiatan']][$val['tanggal']]['jjg'] += $val['jjg'];
				$dataspb2[$val['nospb']][$val['karyawanid']][$val['sesi']][$val['kegiatan']][$val['tanggal']]['brondolan'] += $val['brondolan'];
			}
		} else {
			$dataspb1 = array();
		...
		where a.tujuan='2' and a.syn='1' and a.flag='0' ";
			...
			$dataspb2 = array();
			$str = "select a.nospb,a.tanggal,b.karyawanid,b.jjg,b.sesi,b.kegiatan from " . $dbname2 . ".kebun_spbht_mobile a 
		left join " . $dbname2 . ".kebun_spbtkbm_mobile b on a.nospb=b.nospb
		where a.tujuan='2' and a.syn='1' and a.flag='0' ";
			$res = fetchdata($str);
			foreach ($res as $val) {
		...
		foreach ($dataspb2 as $nospx => $key1) {
			foreach ($key1 as $karyawanid => $key2) {
				foreach ($key2 as $sesi => $key3) {
					foreach ($key3 as $kegiatan => $key4) {
						foreach ($key4 as $tanggalx => $val) {
							if (!isset($databelumsingkronfull[$nospx])) {
								if (isset($nospbbaruxx[$nospx])) {
									$str = "insert into " . $dbname . ".kebun_spbbm (`nospb`,`karyawanid`,`kegiatan`,`tanggal`,`sesi`,`jjg_angkut`,`brondolan_angkut`,`kontanan`)
								values ('" . $nospbbaruxx[$nospx] . "','" . $karyawanid . "','" . $kegiatan . "','" . $tanggalx . "','" . $sesi . "','" . $val['jjg'] . "','" . $val['brondolan'] . "','KERJA')";
									$owlPDO->exec($str);
								}
							}
						}
	...
	case 'loadNewData':
		...
		if ($status_posting != '') {
			$wherenew .= " and posting = '" . $status_posting . "' ";
		}
		...
		<td valign=top align=left>" . $nmkary[$rlvhc['updateby']] . "</td>";
			...
			if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
				...
				$tab .= "<td " . $stylex . " valign=top align=center><img src='images/plus.png' class=zImgBtn class=zImgBtn height='30'  title='Proporsi JJG' onclick=\"proporsitahuntanam('" . $rlvhc['nospb'] . "','" . $rlvhc['noreferensi'] . "');\" > <br> <span > <b> " . $st . " </b> </span></td>";
			} else {
				$isi .= "<td></td>";
			}
	...
	case 'proporsitahuntanam':
		...
		## Cek apakah ada tiket timbang gak
		$str = "select * from " . $dbname . ".pabrik_timbangan where nospbmobile ='" . $param['noreferensi'] . "'  limit 1";
		$res = fetchdata($str);

		$notiket = $res[0]['notransaksi'];
		$tonase = $res[0]['beratbersih'];
		...
		$tab .= "<tr><td>Tiket Timbang</td><td> :</td><td> " . $notiket . "</td></tr>";
		$tab .= "<tr><td>KG WB</td><td> :</td><td> " . $tonase . "</td></tr>";
		...
		$str = "select *,sum(jjg) as jumlahjjg, sum(brondolan) as jumlahbrondolan from " . $dbname . ".kebun_spb_vw where nospb='" . $param['notransaksi'] . "' group by blok,tanggalpanen ";
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()) {

			$penampung_blok[$bar['blok']] = $bar['blok'];
			$penampung_tanggal[$bar['tanggalpanen']] = $bar['tanggalpanen'];
		...
		## Di SPB tidak ada tapi di proporsi spb sudah diinput
		$nourx = 0;

		$listBlok = "'" . implode("','", $penampung_blok) . "'";
		$listTanggal = "'" . implode("','", $penampung_tanggal) . "'";

		$strx = "select * from " . $dbname . ".kebun_proporsitahuntanam_spb where notransaksi='" . $param['notransaksi'] . "' and (kodeorg not in (" . $listBlok . ") or tanggal not in (" . $listTanggal . "))";
		$resx = fetchdata($strx);
		if (count($resx) > 0) {
			$tab .= "<tr class=rowcontent style='background-color:red;'>";
			$tab .= "<td colspan = 12 ><b>Blok yang tidak ada di SPB tapi ada di proporsi SPB </b></td>";
			$tab .= "</tr>";
			foreach ($resx as $valx) {
				$nourx++;
				...
				$tab .= "<tr class=rowcontent >";
				$tab .= "<td >" . $nourx . "</td>";
				$tab .= "<td colspan = 9 >" . $valx['kodeorg'] . " (TT: " . $valx['tanggal'] . " , JJG : " . $valx['jjg'] . ", Brondolan : " . $valx['brondolan'] . ")</td>";
	...
	case 'delData':
		try {
			...
			// Validasi ke timbangan pabrik
			$sCekTimbangan = "select count(*) as jml from " . $dbname . ".pabrik_timbangan where nospb='" . $noSpb . "'";
			$qCekTimbangan = $owlPDO->query($sCekTimbangan) or die(print " Gagal: " . PDOException::getMessage());
			$qCekTimbangan->setFetchMode(PDO::FETCH_ASSOC);
			$rCekTimbangan = $qCekTimbangan->fetch();
			if ($rCekTimbangan['jml'] > 0) {
				throw new PDOException("SPB ini sudah terhubung dengan data Timbangan. Silahkan hapus data timbangan terlebih dahulu!");
			}
			...
			// Bersihkan Data lainnya
			$owlPDO->exec("delete from " . $dbname . ".kebun_spbdt_detail where nospb='" . $noSpb . "'");
			$owlPDO->exec("delete from " . $dbname . ".kebun_proporsitahuntanam_spb where notransaksi='" . $noSpb . "'");
			$owlPDO->exec("delete from " . $dbname . ".kebun_spbbm where nospb='" . $noSpb . "'");
			$owlPDO->exec("delete from " . $dbname . ".kebun_spbpetani where nospb='" . $noSpb . "'");
	...
	case 'updateData':
		...
		and tph='" . $param['tphdt'] . "' and sesi='" . $param['sesidt'] . "' and qrcode='" . $param['noreferensidt'] . "' "; //echo "warning".$sCek;nospb
			...
			## Hapus Proporsi
			$str = "delete from " . $dbname . ".kebun_proporsitahuntanam_spb where notransaksi ='" . $param['noSpb'] . "' and tanggal  ='" . $tglpanen . "' and kodeorg ='" . $blok . "' ";
			$owlPDO->exec($str);

			## Rubah flag jadi 0
			$str = "update " . $dbname . ".kebun_proporsitahuntanam_spb set flag ='0' where notransaksi='" . $param['noSpb'] . "'";
			$owlPDO->exec($str);
	...
	case 'getDivData':
		...
		if ($idDiv == '') {
		 ...
		 where tipe='AFDELING' and kodeorganisasi LIKE '%" . $kdOrg . "%' AND status='1'";
			// exit('error'.$str);
			$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while ($bar = $res->fetch()) {
	...
	case 'delDetail':
		try {
			...
			## Hapus Proporsi
			$str = "delete from " . $dbname . ".kebun_proporsitahuntanam_spb where notransaksi ='" . $param['nospb'] . "' and tanggal  ='" . $param['tglpanen'] . "' and kodeorg ='" . $param['blok'] . "' ";
			$owlPDO->exec($str);
	...
	";
		...
		$sShwData = "select a.*,b.* from " . $dbname . ".kebun_spbht a inner join " . $dbname . ".kebun_spbdt_detail b on a.nospb=b.nospb where a.nospb='" . $noSpb . "' ";
	...
	";
		...
		foreach ($res as $bar) {
			...
			$tab .= "<td align=center>" . $bar['nik'] . "</td>";
			$tab .= "<td>" . $bar['namakaryawan'] . "</td>";
			$tab .= "<td>" . $bar['kegiatan'] . "</td>";
			$tab .= "<td align=right>" . number_format($bar['jjg_angkut']) . "</td>";
			$tab .= "<td align=right>" . number_format($bar['brondolan_angkut']) . "</td>";
			$tab .= "<td align=right>" . $bar['tanggal'] . "</td>";
			$tab .= "</tr>";
		}
