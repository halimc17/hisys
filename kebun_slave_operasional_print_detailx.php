...
// Get Tipe Transaksi
$sTipe = selectQuery($dbname, "kebun_aktifitas", "tipetransaksi", "notransaksi='" . $param['notransaksi'] . "'");
$rTipe = fetchData($sTipe);
$tipetrans = $rTipe[0]['tipetransaksi'];
// Jika Tipe transaksi bukan BKM (Kehdiran Umum) maka munculkan foto header
if ($tipetrans != "BKM") {
	$tab .= "<br /><b>Photo Header<b><br />";
	...
	// Get Photo Header
	$sPhoto = selectQuery($dbname, "kebun_aktifitas", "photo,photoakhir", "notransaksi='" . $param['notransaksi'] . "'");
	$rPhoto = fetchData($sPhoto);
	foreach ($rPhoto as $foto) {
		$tab .= "<tr class=rowcontent style='vertical-align:top;'>";
		if ($jenis == 'html') {
			$tab .= "<td align=center>";
			$tab .= "<a href='" . $foto['photo'] . "' class='popup-img'>";
			$tab .= "<img onclick=\"popupimage()\" style='height:100px;width:100px;' src='" . $foto['photo'] . "' />";
			$tab .= "</a>";
			$tab .= "</td>";

			$tab .= "<td align=center>";
			$tab .= "<a href='" . $foto['photoakhir'] . "' class='popup-img'>";
			$tab .= "<img onclick=\"popupimage()\" style='height:100px;width:100px;' src='" . $foto['photoakhir'] . "' />";
			$tab .= "</a>";
			$tab .= "</td>";
		} else {
			$tab .= "<td align=center>";
			// $tab.="<img style='height:100px;width:100px;' src='".$foto['photo']."' />";
			$tab .= "<a href='" . $foto['photo'] . "'>";
			$tab .= "" . $foto['photo'] . "";
			$tab .= "</a>";
			$tab .= "</td>";

			$tab .= "<td align=center>";
			// $tab.="<img style='height:100px;width:100px;' src='".$foto['photoakhir']."' />";
			$tab .= "<a href='" . $foto['photoakhir'] . "'>";
			$tab .= "" . $foto['photoakhir'] . "";
			$tab .= "</a>";
			$tab .= "</td>";
		}
...
$tab .= "<th align=center>" . $_SESSION['lang']['photo'] . " Awal</th>";
$tab .= "<th align=center>" . $_SESSION['lang']['photo'] . " Akhir</th>";
...
$sPres = "select sum(a.insentif) as upahpremi, sum(a.umr) as umr,sum(a.jhk) as jumlahhk,kodekegiatan,
			tanggal,b.kodeorg, sum(b.hasilkerja) as hasilkerja, b.photo, b.photoakhir
...
while ($rPres = $qPres->fetch()) {
	...
	$queryM = selectQuery($dbname, 'kebun_pakaimaterial', "*", "notransaksi='" . $param['notransaksi'] . "' and kodekegiatan='" . $rPres['kodekegiatan'] . "' and kodeorg='" . $rPres['kodeorg'] . "'");
	...
	if ($jenis == 'html') {
		$tab .= "<td " . $row . " align=left>
				<a href='" . $rPres['photo'] . "' class='popup-img'>
					<img onclick=\"popupimage()\" style='height:50px;width:50px;' src='" . $rPres['photo'] . "'
				</a>
			</td>";
		$tab .= "<td " . $row . " align=left>
				<a href='" . $rPres['photoakhir'] . "' class='popup-img'>
					<img onclick=\"popupimage()\" style='height:50px;width:50px;' src='" . $rPres['photoakhir'] . "'
				</a>
			</td>";
	} else {
		$tab .= "<td " . $row . " align=left>
				<a href='" . $rPres['photo'] . "'>" . $rPres['photo'] . "</a>
			</td>";
		$tab .= "<td " . $row . " align=left>
				<a href='" . $rPres['photoakhir'] . "'>" . $rPres['photoakhir'] . "</a>
			</td>";
	}
...
foreach ($res as $bar) {
	...
	// if (getKary($bar['karyawanid'], 'tipekaryawan') == 4 || getKary($bar['karyawanid'], 'tipekaryawan') == 5) {
	...
	@$keterangan[$bar['karyawanid']] = $bar['penjelasan'];
}
...
$sKhdrn = "select a.nik, a.absensi, a.insentif, a.umr, jhk, kodekegiatan,tanggal,b.kodeorg,b.hasilkerja 
			from " . $dbname . ".kebun_kehadiran a 
			left join " . $dbname . ".kebun_prestasi b on a.notransaksi=b.notransaksi and a.nik=b.nikpemel and a.nourut=b.nourut
			left join " . $dbname . ".kebun_aktifitas c on a.notransaksi=c.notransaksi 
...
$tab .= "<tr class=rowcontent style=background-color:#AED6F1>";
$tab .= "<td align=center colspan=12><b>Sub Total BKM</b></td>";
...
foreach (@$dataabskary as $karya) {
	...
	$tab .= "<td align=left>" . $keterangan[$karya] . "</td>";
	...
	// if(getKary($karya,'tipekaryawan')==4 || getKary($karya,'tipekaryawan')==5){
	// 	$tab.="<td align=right>".hidezerodecimal($umrabskary[$karya])."</td>";				
	// 	@$ttlumrabskary+=$umrabskary[$karya];
...
$tab .= "<tr class=rowcontent style=background-color:#AED6F1>";
$tab .= "<td align=center colspan=12><b>Sub Total Absensi</b></td>";
...
$tab .= "<tr class=rowcontent style=background-color:#A3E4D7>";
$tab .= "<td align=center colspan=12><b>Total (BKM + Absensi)</b></td>";
