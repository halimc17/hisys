...
OPEN_BOX('', '<span class=judul>' . getMenu('lm_2kegiatan') . '</span><br>');
?>
<script language="javascript" src="js/zSelect2.js?ver=1"></script>
<script language=javascript src=js/zTools.js></script>
<script language=javascript src='js/zReport.js'></script>
<script language=javascript src='js/option.js?v=<?php echo time(); ?>'></script>
<script language=javascript src='js/lm_2kegiatan.js?v=<?php echo time(); ?>'></script>
...
$str = "select distinct(left(periode,4)) as periode from " . $dbname . ".setup_periodeakuntansi order by periode desc limit 25";
$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while ($bar = $res->fetch()) {
        ...
        <legend>Form</legend>
            <table border=0 cellpadding=1 cellspacing=1>
                <tr>
                    ...
                    <td>:</td>
                    <td><select class=select2 id=prd style=\"width:84px;\" onchange='getgank()'>" . $optper . "</select></td>
                </tr>
                <tr>
                    <td colspan=16 align=center>
                        <hr>
                        <button onclick=zPreview('lm_slave_2kegiatan','" . $arr1 . "','printContainer');showheader();cekpt(); class=mybutton name=preview id=preview>" . $_SESSION['lang']['preview'] . "</button>
                        <button onclick=zExcel(event,'lm_slave_2kegiatan.php','" . $arr1 . "');cekpt(); class=mybutton name=preview id=preview>" . $_SESSION['lang']['excel'] . "</button>
                    </td>
                </tr>
            </table>
</fieldset>";
