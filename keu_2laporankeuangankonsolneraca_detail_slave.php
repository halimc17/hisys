...
switch($method){
	case'preview':
		...
		$stream.="<table cellpading=1 cellspacing=1 ".@$border." class=sortable>";
			if($param['tipe']=='html'){
		...
		if($param['tipe']=='excel'){
			$nop="NERACAKONSOL_DETAIL.xls";
			$xls = new HtmlExcel();
			$xls->setCss($css);
			$xls->addSheet("NERACAKONSOL_DETAIL", $stream);
			$xls->headers($nop);
			echo $xls->buildFile();
		}else{
			echo $stream;			
