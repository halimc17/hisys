...
echo open_body();
?>
<script language=javascript src='js/keu_2neraca_inhouse.js?v=<?php echo time(); ?>'></script>

<?
include('master_mainMenu.php');
OPEN_BOX('','<span class=judul><b>'.getMenu('keu_2neraca_inhouse').'</b></span><br>');
...
//  $optper.="<option value='2018-06'>06-2018</option>";
    ...
    $res->setFetchMode(PDO::FETCH_OBJ);
    while($bar=$res->fetch()){
        $ada=makeOption($dbname,"log_5saldobulanan","kodeorg,kodeorg","kodeorg='".$bar->kodeorganisasi."'");
        if(@count(@$ada[@$bar->kodeorganisasi])>0){
...
$opttipelaporan="<option value='default'>Default</option>";
    ...
    <legend>".$_SESSION['lang']['form']."</legend>
		<table>
			...
			<tr>
				...
				<select hidden id=periodepembanding  style='width:87px;'> ".$optper."</select>
