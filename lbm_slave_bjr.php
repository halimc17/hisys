...
$sBjr = "select distinct sum(totalkg) as kg,sum(jjg) as jjg,substr(tanggal,6,2) as bulan,left(blok,6) as afd,
       ...
       where left(tanggal,7) between '" . $tahun . "-01' and '" . $periode . "' and blok like '" . $unit . "%' and b.tahuntanam!=''
       group by left(blok,6),substr(tanggal,6,2),b.tahuntanam
       order by left(blok,6) asc,b.tahuntanam asc";
if ($afdId != '') {
    $sBjr = "select distinct sum(totalkg) as kg,sum(jjg) as jjg,substr(tanggal,6,2) as bulan,left(blok,6) as afd,
       b.tahuntanam from " . $dbname . ".kebun_spb_vw a left join " . $dbname . ".setup_blok b on a.blok=b.kodeorg
       where left(tanggal,7) between '" . $tahun . "-01' and '" . $periode . "' and blok like '" . $afdId . "%' and b.tahuntanam!=''
       group by left(blok,6),substr(tanggal,6,2),b.tahuntanam
       order by left(blok,6) asc,b.tahuntanam asc";
}

$qBjr = $owlPDO->query($sBjr) or die(print " Gagal: " . PDOException::getMessage());
$qBjr->setFetchMode(PDO::FETCH_ASSOC);
while ($rBjr = $qBjr->fetch()) {
...
$qSmtr1 = $owlPDO->query($sSmtr1) or die(print " Gagal: " . PDOException::getMessage());
$qSmtr1->setFetchMode(PDO::FETCH_ASSOC);
while ($rSmtr1 = $qSmtr1->fetch()) {
...
$qPanen = $owlPDO->query($sPanen) or die(print " Gagal: " . PDOException::getMessage());
$qPanen->setFetchMode(PDO::FETCH_ASSOC);
while ($rPanen = $qPanen->fetch()) {
...
$qParam = $owlPDO->query($sparam) or die(print " Gagal: " . PDOException::getMessage());
$qParam->setFetchMode(PDO::FETCH_ASSOC);
$rParam = $qParam->fetch();
...
$sKontanan = "select sum(jjgkontanan) as jjgkontan,left(kodeblok,6) as afd,substr(tanggal,6,2) as bln,tahuntanam
                       ...
                       where kodeblok like '" . $unit . "%'  and tanggal like '" . $periode . "%' and 
                       left(tanggal,7) between '" . $tahun . "-01' and  '" . $periode . "' and tahuntanam!=''
                       group by left(kodeblok,6),tahuntanam,substr(tanggal,6,2)";
if ($afdId != '') {
    $sKontanan = "select sum(jjgkontanan) as jjgkontan,left(kodeblok,6) as afd,substr(tanggal,6,2) as bln,tahuntanam
                       from " . $dbname . ".log_baspk a left join " . $dbname . ".setup_blok b on a.kodeblok=b.kodeorg
                       where kodeblok like '" . $afdId . "%'  and tanggal like '" . $periode . "%' and 
                       left(tanggal,7) between '" . $tahun . "-01' and  '" . $periode . "' and tahuntanam!=''
                       group by left(kodeblok,6),tahuntanam,substr(tanggal,6,2)";
}

$qKontanan = $owlPDO->query($sKontanan) or die(print " Gagal: " . PDOException::getMessage());
$qKontanan->setFetchMode(PDO::FETCH_ASSOC);
while ($rKontan = $qKontanan->fetch()) {
...
$qPanen2 = $owlPDO->query($sPanen2) or die(print " Gagal: " . PDOException::getMessage());
$qPanen2->setFetchMode(PDO::FETCH_ASSOC);
while ($rPanen2 = $qPanen2->fetch()) {
...
$tab.="<td align=center colspan=" . ($spanx + intval($bulan)) . ">BJR  (Kg/JJG)</td>";
...
$tab.="<tr><td align=center colspan=" . ($spanx2 + intval($bulan)) . ">" . $_SESSION['lang']['realisasi'] . "</td>";
...
foreach ($dtAfd as $lstAfd) {
    foreach ($dtThnTnm as $lstThnTnm) {
        if ($lstThnTnm != '' || $lstThnTnm != 0) {
            ...
            $tab.="</tr>";
            $dthntanme = isset($dthntanme) ? $dthntanme : '';
