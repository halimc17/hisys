		...
		foreach($res as $bar){
			...
			$tab.="<tr class=rowcontent>";
				...
				$tab.="<td hidden align=center>
					<img src=images/pdf.jpg class=resicon  title='Print' onclick=\"masterPDF('pmn_kontrakjual','".$bar['nokontrak']."','','pmn_kontakjual_pdf',event)\">
				</td>";
...
";
		 ...
		 echo"</td>
				...
				case 'form_ajukan':
					$stream = '';
					$query  = "SELECT kodept FROM $dbname.pmn_kontrakjual WHERE nokontrak = '".trim($param['notransaksi'])."'";
				...
				case 'ajukan':
					...
					$query = "UPDATE $dbname.pmn_kontrakjual SET posting = '9' WHERE nokontrak = '".trim($param['notransaksi'])."'";
			
					try {
						$owlPDO->exec($query);
			
						$query  = "SELECT kodeorg FROM $dbname.pmn_kontrakjual WHERE nokontrak = '".trim($param['notransaksi'])."'";
						$result = fetchData($query, 'OBJECT');
                ...
                case'insert':
                    ...
                    $sCek="select max(nokontrak) as nokontrak from ".$dbname.".pmn_kontrakjual where koderekanan='".$param['custId']."' and ".$whr."";
					...
					$sql="SELECT nokontrak_manual FROM ".$dbname.".pmn_kontrakjual where nokontrak_manual='".$param['noKtrk_M']."' ";
					$rsql=fetchData($sql);
					...
					$termbyr='BA';
                            ...
                            $sIns="insert into ".$dbname.".pmn_kontrakjual (`nokontrak`,`nokontrak_manual`, `tanggalkontrak`,`tanggalberlaku`, `koderekanan`, `kodebarang`, `satuan`, 
									...
									tipepenjualan,updateby,persenppn) 
											   ...
											   '".$param['tppenjualan']."','" . $_SESSION['standard']['userid'] . "','".$param['persenppn']."')"; 
