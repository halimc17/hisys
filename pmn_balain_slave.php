...
switch($method){
   ...
   case'loaddata':
		...
		$no = 0;
		$no=$maxdisplay;
		$str = "select sum(kgnetto) as kgnetto,sum(totalrp) as totalrp,tanggal,tanggaltbs1,tanggaltbs2,kodecustomer,unit,notransaksi,posting,createby,postingby,createtime,postingtime from ".$dbname.".".$table." where ".$where."  group by notransaksi order by tanggal desc,notransaksi desc  limit " . $offset . "," . $limit . " ";
			// echo $str;
			// exit();
		$res=fetchdata($str);
		foreach($res as $bar){
	...
	case'loaddatadt':
		...
		$str = "select * from ".$dbname.".pabrik_timbangan_vw where millcode='".$param['unit']."' and tanggal between '".tanggalsystemn($param['tanggaltbs1'])."' and '".tanggalsystemn($param['tanggaltbs2'])."' and kodecustomer='".$param['kodecustomer']."'";	
		
		$res=fetchdata($str);
		foreach($res as $bar){
			$arrnotiketd[$bar['notiket']]=$bar['notiket'];
			$dttanggalpks[$bar['notiket']]=$bar['tanggal'];
			$dtnotiket[$bar['notiket']]=$bar['notiket'];  

  
			$dtnokendaraan[$bar['notiket']]=$bar['nokendaraan'];
			$dttanggalspb[$bar['notiket']]=$bar['tanggal']; 
			$dtkgbruto[$bar['notiket']]+=$bar['kgwb']; 
			$dtkgnetto[$bar['notiket']]+=$bar['beratbersih'];
			$dtkgpotongan[$bar['notiket']]+=$bar['kgwb']-$bar['kgwbnetto']; 
			// $dtjjg[$bar['notiket']]=$bar['jjg']; 
		}
		
		
		$skontrak = "select * from ".$dbname.".pmn_kontrakjualv2 where nokontrak='".$param['nokontrak']."'";	
		$res=fetchdata($skontrak);
		foreach($res as $bar){
			$datarpkg=$bar['hargasatuan'];
		}
 
		// $str = "select * from ".$dbname.".pmn_balain where (notiket in ('".implode("','",$arrnotiket)."'))";	
		// // echo $str;
		// $res=fetchdata($str);
		// foreach($res as $bar){
		// 	$arrnotiketd[$bar['notiket']]=$bar['notiket'];
		...
		$stream.="<table cellpading=1 cellspacing=1 border=0 class=sortable>
			...
			foreach(@$arrnotiketd as $notiket){
				@$no++;
				$stream.="<tr  ".$bgcolor." class=rowcontent id=row".$no.">";
					$stream.="<td align=center>".$no."</td>";
					$stream.="<td id=notiket".$no.">".$notiket."</td>";
					$stream.="<td hidden id=nospb".$no.">".$notiket."</td>";
					$stream.="<td id=nokendaraan".$no.">".$dtnokendaraan[$notiket]."</td>";
					$stream.="<td id=tanggalpks".$no.">".tanggalnormal($dttanggalpks[$notiket])."</td>";
					$stream.="<td id=tanggalspb".$no.">".tanggalnormal($dttanggalspb[$notiket])."</td>";

					# Hitungan Old
					$stream.="<td hidden align=right><input id=oldkgbruto".$no." type=text class=myinputtextnumber value='".number_format($dtkgbruto[$notiket],8)."' onkeypress='return angka_doang(event)' /></td>";
					$stream.="<td hidden align=right><input type=text id=oldkgpotongan".$no." class=myinputtextnumber value='".number_format($dtkgpotongan[$notiket],8)."' onkeypress='return angka_doang(event)' /></td>";
					$stream.="<td hidden align=right><input id=oldkgnetto".$no." type=text class=myinputtextnumber value='".number_format($dtkgnetto[$notiket],8)."' onkeypress='return angka_doang(event)' /></td>";
					$stream.="<td hidden align=right id=oldjjg".$no.">".number_format($dtjjg[$notiket],8)."</td>";
					$stream.="<td hidden align=right id=oldbjr".$no.">".number_format($dtkgnetto[$notiket]/$dtjjg[$notiket],8)." </td>";
					# Old

					# New
					$stream.="<td hidden align=right><input id=kgbruto".$no." type=text class='myinputtextnumber kgbrutox' value='".number_format($dtkgbersih[$notiket])."' onkeypress='return angka_doang(event)' onkeyup=hitungBruto('".$no."','".$nomax."'); /></td>";
					$stream.="<td align=right><input type=text id=kgpotongan".$no." class='myinputtextnumber kgpotx' value='".number_format($dtkgpotongan[$notiket])."' onkeypress='return angka_doang(event)' onkeyup=hitungPotongan('".$no."','".$nomax."'); /></td>";
					$stream.="<td align=right><input id=kgnetto".$no." type=text class='myinputtextnumber kgnettox' value='".number_format($dtkgnetto[$notiket])."' onkeypress='return angka_doang(event)' onkeyup=hitungNetto('".$no."','".$nomax."'); /></td>";
					$stream.="<td hidden align=right id=jjg".$no.">".number_format($dtjjg[$notiket])."</td>";
					$stream.="<td hidden align=right id=bjr".$no.">".number_format($dtkgnetto[$notiket]/$dtjjg[$notiket])."</td>";
					# End New

					$stream.="<td hidden id=blok".$no.">".$arrkodeblok[$notiket]."</td>";
					$stream.="<td hidden align=right id=tahuntanam".$no.">".$dttahuntanam[$notiket]."</td>";
					$stream.="<td hidden id=intiplasma".$no." align='center'>".$dtintiplasma[$notiket]."</td>";

					$str="select * from ".$dbname.".pmn_hargajualtbs where  kodeorg='".$param['unit']."' and kodecustomer='".$param['kodecustomer']."' and tahuntanam='".$dttahuntanam[$notiket]."' and tanggal<='".$dttanggalpks[$notiket]."' and tanggal2>='".$dttanggalpks[$notiket]."'  and posting=1 order by tanggal desc limit 1";
					$res=fetchdata($str);
					foreach($res as $bar){
						$datarpkg=$bar['harga'];
					}	

					$stream.="<td align=right id=rpkg".$no.">".number_format($datarpkg,2)."</td>";
					$dttotalrp[$notiket]=$datarpkg*$dtkgnetto[$notiket];
					$stream.="<td align=right id=totalrp".$no." class='totalrpx'>".number_format($dttotalrp[$notiket],2)."</td>";
					
					if($datarpkg==0 || $datarpkg==''){
						$stream.="<td><font color=red>Belum terdapat Harga atau belum disetujui</font></td>";
						$errtampung=" Belum terdapat Harga ";
					} else{
						$stream.="<td></td>";		
					}		
				...
				$tdtkgmasuk+=$dtkgmasuk[$notiket];
				$tdtkgkeluar+=$dtkgkeluar[$notiket];
				$tdtkgbruto+=$dtkgbruto[$notiket];
				$tdtkgpotongan+=$dtkgpotongan[$notiket];
				$tdtkgnetto+=$dtkgnetto[$notiket];
				$tdtjjg+=$dtjjg[$notiket];
				$tdttotalrp+=$dttotalrp[$notiket];
			}
	
			$stream.="<tr class=rowheader>";
				$stream.="<td align=center colspan=4>".$_SESSION['lang']['total']."</td>";
				$stream.="<td hiddeh id=ttlkgbruto align=right>".@number_format($tdtkgbruto)."</td>";
				$stream.="<td hiddeh id=ttlkgpot align=right>".@number_format($tdtkgpotongan)."</td>";
				$stream.="<td id=ttlkgnetto align=right>".@number_format($tdtkgnetto)."</td>";
				$stream.="<td hidden id=ttljjg align=right>".@number_format($tdtjjg)."</td>";
	...
	case'loaddatadtori':
		...
		$res=fetchdata($str);
		foreach($res as $bar){
			$arrnotiket[$bar['nospb']]=$bar['nospb'];
			$dttanggalpks[$bar['nospb']]=$bar['tanggal'];
			$dtnotiket[$bar['nospb']]=$bar['notiket'];
		}
		
		$str = "select * from ".$dbname.".kebun_spb_penjualan_vw where (nospb in ('".implode("','",$arrnotiket)."'))";	
		// echo $str;
		$res=fetchdata($str);
		foreach($res as $bar){
