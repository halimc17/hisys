...
$str="select * from ".$dbname.".pabrikasi_salesht where kodeso='".$_GET['column']."' ";
...
$pdf->Cell(200,$height,'SALES ORDER',0,0,'C'); 
$pdf->ln(10);
$pdf->SetFont('Arial','',10);	

$pdf->Cell(35,$height,'Kode PT',0,0,'L'); 
$pdf->Cell(40,$height,': '.$bar['kodept'],0,1,'L'); 	
			
$pdf->Cell(35,$height,'Kode SO',0,0,'L'); 
$pdf->Cell(40,$height,': '.$bar['kodeso'],0,1,'L');

$pdf->Cell(35,$height,'Kode Customer',0,0,'L'); 
$pdf->Cell(40,$height,': '.$bar['kodecustomer'],0,1,'L');

$pdf->Cell(35,$height,'Tanggal Order',0,0,'L'); 
$pdf->Cell(40,$height,': '.tanggalnormal($bar['tanggalmulai']),0,1,'L');

$pdf->Cell(35,$height,'No. PO',0,0,'L'); 
$pdf->Cell(40,$height,': '.$bar['nopo'],0,1,'L');

$pdf->Cell(35,$height,'Tanggal Delivery',0,0,'L'); 
$pdf->Cell(40,$height,': '.tanggalnormal($bar['tanggalmulai']). ' s/d '.tanggalnormal($bar['tanggalsampai']),0,1,'L');
...
$pdf->Cell(50,$height,$_SESSION['lang']['keterangan'],1,0,'C',1);	
$pdf->Cell(20,$height,$_SESSION['lang']['status'],1,1,'C',1);	
$pdf->SetFillColor(255,255,255);
$pdf->SetFont('Arial','',8);
$arrst=array("0"=>"Waiting","1"=>"Open","2"=>"Cancel","3"=>"Close");
$str="select * from ".$dbname.".pabrikasi_salesdt where kodeso='".$_GET['column']."' ";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
	@$no+=1;
	...
	$pdf->Cell(13,$height,$bar['jumlah'],1,0,'R',1);	
	$pdf->Cell(50,$height,$bar['keterangan'],1,0,'L',1);
	$pdf->Cell(20,$height,$arrst[$bar['status']],1,1,'L',1);	
}
