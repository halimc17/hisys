...
/*buka tambahan*/	
	...
	case 'adddph':
			...
			// $str="select *,count(*) as jumdata from ".$dbname.".log_perintaanhargaht where 
					// nomor='".$nodph."' and flag=1 and purchaser='".$_SESSION['standard']['userid']."' ";
			// $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			// $res->setFetchMode(PDO::FETCH_ASSOC);
			// $bar=$res->fetch();
			...
			// if($bar['jumdata']==0){
				// exit("Warning:Tidak ada data, harap cek kembali nomor permintaan harga dan cek posting tender");
			// }	
			...
			#buka bentuk nopo
			$nopo="/".date('m')."/".date('Y')."/PO/".$unit."/".$kode_pt; 
			...
			$nopo=$counter."/".$bln."/".$thn."/PO/".$unit."/".$kode_pt;
			#tutup bentuk nopo	
			...
			$saveht="insert into ".$dbname.".log_poht (
					...
					('".$nopo."','".$tglSkrng."','".$tglSkrng."','".$bar['supplierid']."','".$bar['subtotal']."',
			...
			#insert dt
			$str="select * from ".$dbname.".log_permintaanhargadt where nomor='".$bar['nomor']."' and nourut='".$bar['nourut']."' ";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while($bar=$res->fetch()){
				if($persendiskon>0){
...
/*tutup tambahan*/		
	...
	case 'insert':
		...
		$strx="update ".$dbname.".log_poht set `kodesupplier`='".$supplier_id."',`subtotal`='".$sub_total."',`diskonpersen`='".$disc."',`nilaidiskon`='".$nilai_dis."',`pbbkb`='".$pbbkb."',`pph`='".$npph."',`chkppn`='".$chkppn."',`ppn`='".$nppn."',`nilaipo`='".$nilai_po."',`tanggalkirim`='".$tanggl_kirim."',
			  ".$field."='".$lokasi_kirim."',`syaratbayar`='".$cr_pembayaran."',`uraian`='".$ketUraian."',`purchaser`='".$purchaser."',`matauang`='".$mtUang."',`kurs`='".$Kurs."',`persetujuan1`='".$persetujuan."',`hasilpersetujuan1`='0',`npwp`='".$npwporg."',
			  `tglp1`='".$thisDate."',`statuspo`='0',`persetujuan2`='".$ttd2."',`hasilpersetujuan2`='0',`tglp2`='".$thisDate."',tgledit='".$thisDate."',ongkosangkutan='".$ongkirim."',lokalpusat='1',addcost='".$addcost."',rekening='".$rek."',deliverytime='".$delivtime."'
			   where nopo='".$nopo."'";
		try{
			$owlPDO->exec($strx);
	...
	case 'update_data' :
		...
		while ($res = $query->fetch()) {
			...
			echo"
			<tr class=rowcontent>
			<td align=center ".($res->stat_release==2?"bgcolor='orange' onclick=getKoreksi('".$res->nopo."')":"")." >".$no."</td>
	...
	case'ajukan':
        ...
        $body="<html>
               ...
               //exit('warning : '.$purchaser."/".$kodeorg."/".$persetujuan1."/".$persetujuan2."/".$namaorganisasi."/".$to."<br>".$subject."<br>".$body);
	...
	case'getSupplierNm':
			echo"<fieldset><legend>".$_SESSION['lang']['result']."</legend>
				<div style=\"overflow:auto;max-height:295px;width:455px;\">
				...
				";
		 $sSupplier="select namasupplier,supplierid,kodekelompok,kota from ".$dbname.".log_5supplier 
		 ...
		 while($rSupplier=$qSupplier->fetch())
		 {
			 ...
			 echo"<tr class=rowcontent onclick=setData('".$rSupplier['supplierid']."')>
				 <td align=center>".$no."</td>
				 <td>".$nmkelsup[$rSupplier['kodekelompok']]."</td>
