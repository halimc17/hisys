...
<script language=javascript src='js/sdm_2gajitransfer.js?ver=1.2'></script>
<style>
	.datarekening{
		cursor:pointer;
	}
...
OPEN_BOX('','<span class=judul>'.getMenu('sdm_2gajitransfer').'</span>');
echo"<fieldset style='width:500px'><legend>".$_SESSION['lang']['form']."</legend>
	   <table>
		  ...
		  <tr>
		   ...
		   <tr>
		      <td>".$_SESSION['lang']['tanggal']." Transfer</td>
			  <td>:</td>
			  <td>
				<input type=text class=myinputtext id=tanggal onmousemove=setCalendar(this.id) onkeypress=return false;  size=10 maxlength=10 readonly/>
			  </td>
		  </tr>
		   <tr>
		      <td>Sumber Rekening</td>
			  <td>:</td>
			  <td><select style=width:200px id=sumberrek></select>
			</td>
		  </tr>
...
echo"<fieldset><legend>".$_SESSION['lang']['list']."</legend>
		<button id='removelist' class=mybutton onclick=\"removelist()\" style='display:none;'>Remove List</button>
		<button id='exporttolist' class=mybutton onclick=\"exportlist('containerlist1')\" style='display:inline-block;'>Export to Excel </button>
		<button id='exportcsvlistbtn' class=mybutton onclick=\"exportCsvlist('containerlist1')\" style='display:inline-block;'>Export to CSV </button>
