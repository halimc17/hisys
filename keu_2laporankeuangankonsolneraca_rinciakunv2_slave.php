...
#= ambil jumlah
$str="select * from ".$dbname.".organisasi where kodeorganisasi='".$param['pt']."' or induk='".$param['pt']."'";
$res=fetchdata($str);
foreach($res as $bar){
...
$stream.="<th align=left style='".$sty."' colspan='".(($cspan*3)+2)."'><b>".strtoupper($namaorg[$param['pt']]."".$judulunit."<br>".$judullaporan)."</b></th>";
$stream.="</tr>";
	$stream.="<tr class=rowheader>";
			...
			if($tipenourut[$nourut]=='Detail'){
				foreach($arrnoakun as $noakun){
					if($ltnoakun[$nourut][$noakun]){
						...
						foreach($arrper as $per){
							$stream.="<td align=right ".$style.">".number_format(fixnan($dtthniniakun[$nourut][$noakun][$per]),$dgt)."</td>"; 
							// $stream.="<td align=right ".$style.">".number_format(fixnan($dtthniniakun[$nourut][$noakun][$per]),$dgt)."</td>"; 
						}
						$stream.="</tr>";
					}
