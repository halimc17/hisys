...
// exit("error:".$_GET['proses']);
    switch($proses){
        case'getDetail': 
            ...
            group by noakun asc";
            $qdgaji=$owlPDO->query($sdgaji) or die(print " Gagal: ".PDOException::getMessage());
            $qdgaji->setFetchMode(PDO::FETCH_ASSOC);

            while($rdgaji=$qdgaji->fetch()){
            ...
            group by noakun asc";
            $qdgajisbi=$owlPDO->query($sdgajisbi) or die(print " Gagal: ".PDOException::getMessage());
            $qdgajisbi->setFetchMode(PDO::FETCH_ASSOC);

            while($rdgajisbi=$qdgajisbi->fetch()){
        ...
        case'getDetail2':
            ...
            $sreal="select distinct sum(jumlah) as rupiah,kodebarang,kodekegiatan,keterangan,nojurnal
                    ...
                    and noakun='".$noakun."' and kodeblok like '".$kdOrg."%'  
                     group by nojurnal";
            $qReal=$owlPDO->query($sreal) or die(print " Gagal: ".PDOException::getMessage());
            $qReal->setFetchMode(PDO::FETCH_ASSOC);

            while($rreal= $qReal->fetch()){
