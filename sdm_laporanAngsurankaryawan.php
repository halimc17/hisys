...
//+++++++++++++++++++++++++++++++++++++++++++++
$str = "select * from " . $dbname . ".sdm_ho_component where name like '%Angs%'";
...
OPEN_BOX('','<span class=judul>'.strtoupper($_SESSION['lang']['angsuran']).'</span>');
echo"<div id=EList>";
//echo OPEN_THEME($_SESSION['lang']['angsuran']);
if ($_SESSION['language'] == 'EN') {
    echo"<fieldset style='width:500px;'><legend>".$_SESSION['lang']['find']."</legend>(Display installment on:<select id=bln onchange=showAngsuran(this.options[this.selectedIndex].value)><option value=''></option>" . $opt3 . "</select>)
    ...
    echo"</fieldset><br>Below installment that :<b><span id=caption>not yet settled</span></b>
                     <image src=images/pdf.jpg class=resicon title='PDF' onclick=angsuranPDF(event)>
                         <input type=hidden id=val value=''>
                         ";
} else {
    echo"<fieldset style='width:500px;'><legend>".$_SESSION['lang']['find']."</legend>(Tampilkan Angsuran Bulan:<select id=bln onchange=showAngsuran(this.options[this.selectedIndex].value)><option value=''></option>" . $opt3 . "</select>)
                     || (Tampilkan Angsuran Yang<select id=lunas  onchange=showAngsuran(this.options[this.selectedIndex].value)><option value=''></option>
                         ...
                         <option value=notactive>Not Active</option></select>)	 
                         ";
    echo"</fieldset><br>Berikut Angsuran Karyawan :<b><span id=caption>Belum Lunas</span></b>
                     <image src=images/pdf.jpg class=resicon title='PDF' onclick=angsuranPDF(event)>
                         <input type=hidden id=val value=''>
                         ";
}
