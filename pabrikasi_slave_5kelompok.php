...
switch($method)
{
	case 'insert':
		...
		$str="insert into ".$dbname.".pabrikasi_5kelompok (`kode`,`nama`,`updateby`)
	...
	break;
	case 'update':
		$str="update ".$dbname.".pabrikasi_5kelompok set nama='".$nama."' where kode='".$kode."'";
		try{$owlPDO->exec($str); }
			catch (PDOException $e) {
				print " Gagal  !: " . $e->getMessage() . "\n"; 
				die(); 
	...
	case 'delete':
		...
		$str="delete from ".$dbname.".pabrikasi_5kelompok where kode='".$kode."'";
		try{$owlPDO->exec($str); }
		catch (PDOException $e) {
			print " Gagal  !: " . $e->getMessage() . "\n"; 
			die(); 
	...
	case'loaddata':
		...
		$str="select * from ".$dbname.".pabrikasi_5kelompok";
