...
if(isTransactionPeriod())//check if transaction period is normal
{
	...
	if($optVTipe[$induk] == 'SIPIL'){
		...
		if(strlen($induk) == '6' && $optVTipe[$induk] != 'STATION'){
			$str="select distinct kodeorganisasi,namaorganisasi,tipe,indukblok,namaindukblok from ".$dbname.".organisasi where induk='".$induk."' and tipe not like '%gudang%' ".$wh." group by indukblok order by kodeorganisasi"; #exit("error".$str);
		}else{
			$str="select distinct kodeorganisasi,namaorganisasi,tipe,indukblok,namaindukblok from ".$dbname.".organisasi where induk='".$induk."' and tipe not like '%gudang%' ".$wh." order by kodeorganisasi"; #exit("error".$str);
		}
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_OBJ);
		$n='';
	...
	if(substr($induk,0,2)=='AK' or substr($induk,0,2)=='PB'){
		...
		$blehh='';
		$blehh="<option value=''></option>";
		$str="select notransaksi,project from ".$dbname.".lgl_pengajuanspkht where koderekanan='".$induk."' and posting='1' and statuspersetujuan='1' "; 
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_OBJ);
		while($bar=$res->fetch()){
	...
	#tambahan untuk transportir
	#update untuk solar yang akan masuk ke piutang kontraktor
	$str="select * from ".$dbname.".log_5supplier a
		left join ".$dbname.".log_5supkelompok b on a.supplierid = b.supplierid 
		where b.tipe='TRANSPORTIR' and b.status=1"; 
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_OBJ);
	if(count(fetchdata($str))>0){
		$blehh.="<optgroup label='TRANSPORTIR'>";
	}
