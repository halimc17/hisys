...
$sHarvest = "select a.blok, a.tanggalpanen from ".$dbname.".kebun_spb_vw4 a LEFT JOIN ".$dbname.".setup_blok b on a.blok=b.kodeorg
             where ".$whereUnit." and a.tanggalpanen between '".$tglMulai."' and '".$tglSelesai."'".$inplas."
             order by a.blok, a.tanggalpanen asc";
// exit('warning: '.$sHarvest);
...
while($rHarvest = $qHarvest->fetch()){
    ...
    $unit = substr($rHarvest['blok'],0,4);
    $divisi = substr($rHarvest['blok'],0,6);
    $groupedHarvestDates[$unit][$divisi][$rHarvest['blok']][$rHarvest['tanggalpanen']] = true;
}
ksort($groupedHarvestDates);
foreach($groupedHarvestDates as $u => $dList) {
    ksort($groupedHarvestDates[$u]);
}

// Fetch Previous Harvest
$prevHarvests = array();
$sPrev = "select a.blok, max(a.tanggalpanen) as last_date from ".$dbname.".kebun_spb_vw4 a LEFT JOIN ".$dbname.".setup_blok b on a.blok=b.kodeorg
          where ".$whereUnit." and a.tanggalpanen < '".$tglMulai."'".$inplas."
          group by a.blok";
// exit('warning: '.$sPrev);
...
if(empty($groupedHarvestDates)) {
    ...
    foreach($groupedHarvestDates as $kodeUnit => $daftarDivisi) {
        $namaUnit = isset($namaOrg[$kodeUnit]) ? $namaOrg[$kodeUnit] : '';
        $tabelHtml .= "<tr class=rowcontent><td colspan='".($jumlahHari+10)."' style='font-weight:bold; background-color: #ffe600ff;'>UNIT: ".$namaUnit." (".$kodeUnit.")</td></tr>";
        
        foreach($daftarDivisi as $kodeDivisi => $daftarBlok) {
            ...
            $tabelHtml .= "<tr class=rowcontent>
                        <td align=center>".substr($kodeDivisi,4,2)."</td>
                        <td align=center>".substr($blok,8,2)."</td>
                        <td></td>
                        <td align=center>".$putaranPanen."</td>
                        <td>Rotasi</td>";
            ...
            $tabelHtml .= "   <td align=center>".$putaranPanen." Kali</td>
                        <td align=center>".$rataRataRotasi."</td>
                    </tr>";
        ...
        $tabelHtml .= "<tr class=rowcontent>
                    <td colspan=".($jumlahHari+8)." align=center style='font-weight:bold; background-color: #d3d3d3;'>".$_SESSION['lang']['total'] ." ".strtoupper($_SESSION['lang']['divisi'])." ".substr($kodeDivisi,4,2)."</td>
        ...
        $tabelHtml .= "<tr class=rowcontent>
                    <td colspan=".($jumlahHari+8)." align=center style='font-weight:bold; background-color: #d3d3d3;'>".$_SESSION['lang']['rerata']." ".strtoupper($_SESSION['lang']['divisi'])." ".substr($kodeDivisi,4,2)."</td>
