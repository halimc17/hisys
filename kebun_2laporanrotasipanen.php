...
OPEN_BOX('', '<span class=judul>' . getMenu('kebun_2laporanrotasipanen') . '</span><br>');
...
function updateUnitId() {
    var select = document.getElementById('unitIdSelect');
    var selected = [];
    ...
    document.getElementById('unitId').value = selected.join(',');
}
...
echo "<tr><td>".$_SESSION['lang']['unit']."</td><td>:</td><td><select multiple class=select2 id=unitIdSelect onchange=\"updateUnitId()\" style=width:200px>".$optUniDt."</select><input type=text id=unitId style=\"display:none;\" /></td></tr>";
echo "<tr><td>".$_SESSION['lang']['periode']."</td><td>:</td><td><select class=select2 id=periode style=width:200px>".$optper."</select></td></tr>";
...
<button onclick=\"zPreview('kebun_slave_2laporanrotasipanen','".$arr."','printContainer2');showheader();batal2();\" class=\"mybutton\" name=\"preview\" id=\"preview\">".$_SESSION['lang']['preview']."</button>

<button onclick=\"zExcel(event,'kebun_slave_2laporanrotasipanen.php','".$arr."')\" class=\"mybutton\" name=\"excel2\" id=\"excel2\">".$_SESSION['lang']['excel']."</button>
