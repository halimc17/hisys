...
switch($method){
	case 'loaddata':
		...
		<tbody>";
		$no=0;
		$str= "select * from ".$dbname.".lgl_5jabatan_efiling";
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
			$tab.="<tr class=rowcontent>";
			$tab.="<td style='text-align:center;'>".$no."</td>";
			$tab.="<td style='text-align:center;'>".getNamaJabatan($val['jabatan'])."</td>";
			...
			$tab.="<td style='text-align:center;'>
					<img  style=margin-right: 5px; src='images/application/application_edit.png' class='resicon' title='Edit' onclick=\"editdata('edit','".$val['id']."' ,'".$val['jabatan']."')\";>
			</td>";
			$tab.="</tr>";
		}
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(jabatan) as jlhitem from ".$dbname.".lgl_5jabatan_efiling where jabatan='".$param['jabatan']."' ";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
				throw new PDOException("Jabatan sudah ada!  ");
			}
			...
			$str = insertQuery($dbname,'lgl_5jabatan_efiling',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':        
		try {
			...
			$where = " id ='".$param['idx']."' " ;
			$str = updateQuery($dbname,'lgl_5jabatan_efiling',$data,$where);
