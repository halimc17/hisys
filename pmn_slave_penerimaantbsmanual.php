...
$carisupplierid = checkPostGet('carisupplierid', '');
...
$tmplsupplierid = checkPostGet('tmplsupplierid', '');
...
switch($proses)
{
	...
	case'download':
		...
		echo $_SESSION['lang']['perusahaan'].",".$_SESSION['lang']['unit']."/".$_SESSION['lang']['pabrik'].",".$_SESSION['lang']['supplier'].",".$_SESSION['lang']['nokendaraan'].",Tanggal Masuk,Jam Masuk,Menit Masuk,Tanggal Keluar,Jam Keluar,Menit Keluar,Berat Masuk,Berat Keluar,Potongan(KG)\n";
		
		$arrSup = explode(',',$tmplsupplier);
		foreach($arrSup as $key)
		{
			echo $tmplpt.",".$tmplkodepabrik.",".$tmplsupplierid.",BH1541SKS,".$tmpltanggal.",07,45,".$tmpltanggal.",08,11,13476,3421,254\n";
		}
	break;
    ...
    case'insert':
		...
		##Get No Tiket
		$str = "select * from ".$dbname.".pabrik_timbangan where notransaksi like '".$instiket."%' order by notransaksi desc limit 1";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$bar=$res->fetch();
		...
		$str = "insert into ".$dbname.".pabrik_timbangan 
			(notransaksi,tanggal,kodecustomer,kodebarang,jammasuk,beratmasuk,jamkeluar,beratkeluar,nokendaraan,username,millcode,kgpotsortasi,beratbersih,ramp,intex) values
			('".$notiket."','".tanggalsystem($tanggalmasuk)."','".$supplierid."','40000003','".$waktumasukx."','".$beratmasuk."','".$waktukeluarx."','".$beratkeluar."','".$nokendaraan."','".$_SESSION['standard']['userid']."','".$kodepabrik."','".$potongan."','".$netto."','','0')";
		try{
			$owlPDO->exec($str); 
	...
	case'update':
		...
		// (notransaksi,tanggal,kodecustomer,kodebarang,jammasuk,beratmasuk,jamkeluar,beratkeluar,nokendaraan,username,millcode,kgpotsortasi,beratbersih)
	
		$str = "update ".$dbname.".pabrik_timbangan set 
			nokendaraan='".$nokendaraan."',
			jammasuk='".$waktumasukx."',
			jamkeluar='".$waktukeluarx."',
			beratmasuk='".$beratmasuk."',
			beratkeluar='".$beratkeluar."',
			kgpotsortasi='".$potongan."',
			beratbersih='".$netto."',
			username='".$_SESSION['standard']['userid']."'
			where notransaksi='".$instiket."".$tiket."'";
	...
	case'delData':
		$str = "delete from ".$dbname.".pabrik_timbangan 
				where notransaksi='".$notiket."'";
		try{
			$owlPDO->exec($str); 
		}catch(PDOException $e){
			echo " Gagal," . addslashes($e->getMessage());
	...
	case'posting':
		...
		$str = "update ".$dbname.".pabrik_timbangan set 
			thntm1='1',
			petugassortasi='".$_SESSION['standard']['userid']."',
			trpcode='".date('Y-m-d')."'
			where notransaksi='".$notiket."'";
		try
		{
    ...
    case'loadData':
		...
		if($caript!='')
		{
			$optPb = makeOption($dbname,'organisasi','induk,kodeorganisasi',"tipe='PABRIK' and induk='".$caript."'");
            $where.=" and millcode = '".$optPb[$caript]."'";
        }
		if($cariunit!='')
		{
            $where.=" and millcode = '".$cariunit."'";
        }
		// if($carikoderamp!='')
		// {
		...
		if($carisupplierid!='')
		{
            $where.=" and kodecustomer = '".$carisupplierid."'";
        }
		if($caritanggal!='')
		{
		...
		$str="select count(*) jmlhrow from ".$dbname.".pabrik_timbangan where notransaksi like 'XM%' ".$where."";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch())
		{
		...
		$str="select * from ".$dbname.".pabrik_timbangan where notransaksi like 'XM%' ".$where." order by tanggal desc limit ".$offset.",".$limit." ";
        $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar=$res->fetch())
		{
			$optpt = makeOption($dbname,'organisasi','kodeorganisasi,induk',"kodeorganisasi='".$bar['millcode']."'");
			$optpabrik = makeOption($dbname,'organisasi','kodeorganisasi,namaorganisasi');
			$optsupid = makeOption($dbname,'log_5supplier','kodetimbangan,supplierid',"kodetimbangan='".$bar['kodecustomer']."'");
			$optsupnama = makeOption($dbname,'log_5supplier','supplierid,namasupplier');
			...
			$tab.="<tr class=rowcontent>
				...
				<td>".$optpabrik[$optpt[$bar['millcode']]]."</td>
				<td>".$optpabrik[$bar['millcode']]."</td>
				<td>".$optsupnama[$optsupid[$bar['kodecustomer']]]." - ".$optsupid[$bar['kodecustomer']]."</td>
				<td style='text-align:center'>".$bar['nokendaraan']."</td>
				<td style='text-align:center'>".tanggalnormal($bar['tanggal'])."</td>
				<td style='text-align:right'>".number_format($bar['beratmasuk'])."</td>
				<td style='text-align:right'>".number_format($bar['beratkeluar'])."</td>
				<td style='text-align:right'>".number_format($bar['kgpotsortasi'])."</td>
				<td style='text-align:right'>".number_format($bar['beratbersih'])."</td>
				<td style='text-align:center'>".$optkaryawan[$bar['username']]."</td>
				<td style='text-align:center'>".($bar['trpcode']==null?'-':tanggalnormal($bar['trpcode']))."</td>
				<td style='text-align:center'>".$optkaryawan[$bar['petugassortasi']]."</td>";
			if($bar['thntm1']==0)
			{
				$tglmasuk = tanggalnormal($bar['tanggal']);
				$jammasuk = substr($bar['jammasuk'],0,2);
				$menitmasuk = substr($bar['jammasuk'],3,2);
				$tglkeluar = tanggalnormal($bar['tanggal']);
				$jamkeluar = substr($bar['jamkeluar'],0,2);
				$menitkeluar = substr($bar['jamkeluar'],3,2);
				$tab.="<td style='text-align:center'>
					<img src=images/application/application_edit.png class=zImgBtn title='Edit' onclick=\"edit('".str_replace('XM','',$bar['notransaksi'])."','".$optpt[$bar['millcode']]."','".$bar['millcode']."','".$bar['kodecustomer']."','".$bar['nokendaraan']."','".number_format($bar['beratmasuk'])."','".number_format($bar['beratkeluar'])."','".number_format($bar['kgpotsortasi'])."','".number_format($bar['beratbersih'])."','".$tglmasuk."','".$jammasuk."','".$menitmasuk."','".$jamkeluar."','".$menitkeluar."');\">
				</td>";            
                $tab.="<td style='text-align:center'>
					<img src=images/application/application_delete.png class=zImgBtn  title='Delete' onclick=\"del('".$bar['notransaksi']."');\" >
				</td>";
				if(count($optPosting)==0)
				{
					$tab.="<td align=center>
						<img src='images/".$_SESSION['theme']."/posting.png' class=zImgBtn title='Posting' style='cursor:default'>
					</td>";
				}
				else
				{
					$tab.="<td align=center>
						<img src='images/".$_SESSION['theme']."/posting.png' class=zImgBtn  title='Posting'   onclick=\"posting('".$bar['notransaksi']."','".tanggalnormal($bar['tanggal'])."','".$bar['kodecustomer']."')\">
					</td>";
					// $tab.="<td align=center>
