...
case 'getdetail':
	case'KONTAN':
		...
		$countApp = getCountApproval('KONTAN');
		...
		$str = "select * from ".$dbname.".approval a
			left join ".$dbname.".kebun_realkontanan b on a.notransaksi = b.notransaksi
			where a.jenispersetujuan='KONTAN' and a.status='0' and a.karyawanid='".$karyawanid."' order by a.tanggal asc";
		...
		while ($bar = $res->fetch()) {
			...
			$tab.="<tr class=rowcontent>
				...
				<td align=center>
				<img src=images/zoom.png class=resicon height='30' title='Preview' onclick=\"htmlkontan('".$bar['notransaksi']."','1','event','html');\">
				</td>";
			$showaction = 0;
			$countubahjumlah = 0;
			$level = 1;
			...
			if ($showaction!=$level || $level==1) {
				$tab.="<td style='text-align:center'>
					<button class=mybutton onclick=\"getdatakontan('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['approve']."</button>
					</td>
					<td style='text-align:center'>
					<button class=mybutton onclick=\"tolakkontan('".$bar['notransaksi']."','".$level."')\">".$_SESSION['lang']['ditolak']."</button>
					</td>";
			} else {
				$tab.="<td colspan=2>&nbsp;</td>";
			}
			for ($i = 1; $i <= $countApp; $i++) {
				$arrDetail = detailApprove($i, $bar['notransaksi'], 'KONTAN');
				
				$strpo="select * from ".$dbname.".setup_approval where jenispersetujuan='KONTAN' and kodeunit='".$kodeorg."' and level='".$i."'";
	...
	case'get_form_approval':
		...
		$koderorg=$temporg[1];
		$countApp = getCountApproval('KONTAN',$koderorg); 
		for($i=1;$i<=$countApp;$i++){
			...
			foreach($resx as $keyx=>$valx){
				if($karyawanid==$valx['karyawanid']){
					if($i == $countApp){
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									<td colspan=3 align=center>
										<button id=Ajukan class=mybutton onclick=nextapprovalkontan('approved') >Approved</button>
									</td>
								</tr>
							</table>
						</div>";
						...
						$level = $i+1;
						$arrListApp = listApprove($level,'KONTAN',$koderorg);
						...
						$tab.="<div id=test style=display:block>
							...
							<table cellspacing=1 border=0>
								...
								<tr>
									...
									<td colspan=2></td>
									<td>
										<button class=mybutton onclick=nextapprovalkontan() title=\" Submit to the next level\" id=Ajukan >".$_SESSION['lang']['diajukan']."</button>
									</td>
								</tr>
							</table>
	...
	if ($bar['statuspersetujuan'] == 1) {
		...
		$arrDetail = detailApprove($kolom, $notransaksi, 'KONTAN');
		$level = $kolom + 1;
		if ($kolom != $countApp) {
			if ($user_id == $arrDetail['karyawanid']) {
				...
				$str="select * from ".$dbname.".setup_approval where 
						jenispersetujuan='".$jenisApp."' and level='".$level."' and kodeunit='".$koderorg."'"; #exit("error".$str);
				$res=fetchData($str);
				$tipeapp = $res[0]['tipe'];
				...
				if($tipeapp=='1'){
					...
					$str = "insert into ".$dbname.".approval values ('','".$notransaksi."','KONTAN','".$level."','".$user_id."','0','','','')";
						try {
							$owlPDO->exec($str);
			...
			$strx = "update ".$dbname.".kebun_realkontanan set persetujuan='1' where `notransaksi`='".$notransaksi."'";
	...
	case 'tolak':
		...
		<button class=mybutton onclick=\"inserttolakkontan(".$_POST['kolom'].")\" >".$_SESSION['lang']['ditolak']."</button>
		</td></tr></table>
		</div>";
	break;
	case 'inserttolak':
		...
		$arrDetail = detailApprove($kolom,$notransaksi,'KONTAN');
		$tglskrng=date("Y-m-d H:i:s");
		$str="update ".$dbname.".kebun_realkontanan set persetujuan='3' where notransaksi='".$notransaksi."'" ;
		try{$owlPDO->exec($str); 
