...
OPEN_BOX('','<span class=judul>'.getMenu('setup_approval').'</span><br>');
...
// 	}
// }
$str=$owlPDO->query("select distinct namagolongan as namagolongan from ".$dbname.".sdm_5golongan where  aktif='1' order by kodegolongan desc");
$str->setFetchMode(PDO::FETCH_OBJ);
while($bar=$str->fetch()){
			...
			<td valign=top>
				...
				<table border=0 style='display: inline-block;vertical-align:top'>
					...
					<tr>
						...
						<td>:</td>
						<td>
							<input type='text' class='myinputtext' id='nilaidari' size='10' style='text-align:right' placeholder='0' onkeypress=\"return angka_doang(event);\" onkeyup=\"z.numberFormat('nilaidari',2);\">
							 &nbsp;s/d&nbsp;
							<input type='text' class='myinputtext' id='nilaisampai' size='10' style='text-align:right' placeholder='0' onkeypress=\"return angka_doang(event);\" onkeyup=\"z.numberFormat('nilaisampai',2);\">
						</td>
					</tr>
					<tr>
...
<button class=mybutton onclick=\"loaddata('','excel')\">".$_SESSION['lang']['excel']."</button>
<div style='clear:both'></div>
<div class='table-scroll' id=container>
