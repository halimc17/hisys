...
switch ($method) {
    ...
    case 'insert':
        $i = "insert into " . $dbname . ".log_5delivtime (kode,nama,jumlahhari,updateby)
            values ('" . $kode . "','" . $ket . "','".$jumlahhari."','" . $_SESSION['standard']['userid'] . "')";
		try{
			$owlPDO->exec($i); 
    ...
    case 'update':
        $i = "update " . $dbname . ".log_5delivtime set updateby='" . $_SESSION['standard']['userid'] . "',nama='" . $ket . "', jumlahhari='".$jumlahhari."' 
             where kode='" . $kode . "'";
        try{
			$owlPDO->exec($i); 
		}catch(PDOException $e){
    ...
    case'loadData':
        ...
        $ql2 = "select count(*) as jmlhrow from " . $dbname . ".log_5delivtime"; // echo $ql2;notran
        ...
        $i = "select * from " . $dbname . ".log_5delivtime  limit " . $offset . "," . $limit . "";
        ...
        while ($d = $n->fetch()) {
            ...
            echo "<td align=center>" . $d['jumlahhari'] . "</td>";
