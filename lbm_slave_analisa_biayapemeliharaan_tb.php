...
$qRealLuas = $owlPDO->query($sRealLuas) or die(print " Gagal: " . PDOException::getMessage());
$qRealLuas->setFetchMode(PDO::FETCH_ASSOC);
$rRealLuas = $qRealLuas->fetch();
...
$qRealLuas2 = $owlPDO->query($sRealLuas2) or die(print " Gagal: " . PDOException::getMessage());
$qRealLuas2->setFetchMode(PDO::FETCH_ASSOC);
$rRealLuas2 = $qRealLuas2->fetch();
...
while ($rData2 = $qData2->fetch()) {
    if ($rData2['jumlah'] != '' || $rData2['jumlah'] != 0) {
        ...
        $dtSatuan[$rData2['kodekegiatan']] = $rData2['satuan'];
    }
}
...
$qDbgt = $owlPDO->query($sDbgt) or die(print " Gagal: " . PDOException::getMessage());
$qDbgt->setFetchMode(PDO::FETCH_ASSOC);
while ($rDbgt = $qDbgt->fetch()) {
...
$qVol = $owlPDO->query($sVol) or die(print " Gagal: " . PDOException::getMessage());
$qVol->setFetchMode(PDO::FETCH_ASSOC);
while ($rVol = $qVol->fetch()) {
    if ($rVol['volreal'] != 0 || $rVol['volreal'] != '') {
        @$dtVolReal[$rVol['kodekegiatan']]+=$rVol['volreal'];
        $dtKegiatan[$rVol['kodekegiatan']] = $rVol['kodekegiatan'];
        $dtSatuan[$rVol['kodekegiatan']] = $rVol['satuan'];
    }
}
...
$qVol2 = $owlPDO->query($sVol2) or die(print " Gagal: " . PDOException::getMessage());
$qVol2->setFetchMode(PDO::FETCH_ASSOC);
while ($rVol2 = $qVol2->fetch()) {
    if ($rVol2['volreal'] != '' || $rVol2['volreal'] != 0) {
        @$dtVolReal[$rVol2['kodekegiatan']]+=$rVol2['volreal'];
        $dtSatuan[$rVol2['kodekegiatan']] = $rVol2['satuan'];
        $dtKegiatan[$rVol2['kodekegiatan']] = $rVol2['kegiatan'];
    }
}
##si volume realisasi##
$sVol = "select  sum(hasilkerja) as volreal,a.kodekegiatan,b.satuan from 
...
while ($rVol2 = $qVol2->fetch()) {
    if ($rVol2['volreal'] != 0 || $rVol2['volreal'] != '') {
        ...
        $dtSatuan[$rVol2['kodekegiatan']] = $rVol2['satuan'];
        $dtKegiatan[$rVol2['kodekegiatan']] = $rVol2['kodekegiatan'];
    }
}
