...
$qbgt = $owlPDO->query($sRotasiBudget) or die(print " Gagal: " . PDOException::getMessage());
$qbgt->setFetchMode(PDO::FETCH_ASSOC);
while ($rbgt = $qbgt->fetch()) {
...
if (!empty($dAfd)) {
    foreach ($dAfd as $sltAfd) {
        foreach ($dThntnm as $lstThnTanam) {
            $dtLuas[$sltAfd . $lstThnTanam] = isset($dtLuas[$sltAfd . $lstThnTanam]) ? $dtLuas[$sltAfd . $lstThnTanam] : '';
