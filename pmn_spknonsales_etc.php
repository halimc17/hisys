...
<script language=javascript1.2 src='js/pmn_spknonsales_etc.js?v=<?php echo time(); ?>'></script>
...
$optbuyer=$optpelayaran=$optfranco1=$optfranco2=$optttd=$optsurveyor=$optbarang=$opttransportirdarat=$optkapal=$optponton=$optpt=$optnoakun="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
...
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
	$optfranco1.="<option value='".$bar['id_franco']."'>".$bar['franco_name']."</option>";
	$optfranco2.="<option value='".$bar['id_franco']."'>".$bar['franco_name']."</option>";
}
    ...
    echo"<legend>".$_SESSION['lang']['form']."</legend>";
        echo"<table border=0 cellpadding=1 cellspacing=1>
				...
				<tr>
					...
					<td>:</td>		
					<td>
						<input type=text id=nospk size=20 placeholder='Otomatis' disabled class=myinputtext style=\"width:146px;\">
					</td>
					...
					<td  rowspan=4 ><textarea rows='4' id=lain type=text onkeypress=\"return tanpa_kutip(event)\" style=\"width:150px;\">".$isitext."</textarea>
					</td>
				</tr>
				<tr>
				 ...
				 <tr>
					...
					<td>:</td>		
					<td>
						<input type=text class=myinputtext readonly id=tanggal size=12 onmousemove=setCalendar(this.id) maxlength=10 onkeypress=\"return false;\" style=\"width:146px;\">
					</td>
				...
				<tr>
					...
					<td>
						...
						<img id='tujuan' onclick=z.elSearch('tujuan',event) class='resicon' src='images/skyblue/zoom.png' style='position:relative;top:3px;left:3px;'>&nbsp;
					</td>
...
//ISI UNTUK DAFTAR 
echo "<fieldset style='float:left'>
		<legend>".$_SESSION['lang']['cari']."</legend>
		<table>
		<tr>
			<td>".$_SESSION['lang']['kodept']." </td>
			<td>:</td>		
