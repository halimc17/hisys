...
function getPeriode() {
	...
	post_response_text('kebun_slave_3postingtransaksi_view.php', param, respon);
	function respon() {
		if (con.readyState == 4) {
			if (con.status == 200) {
				busy_off();
...
function loopClosingFisik(currRow, maxRow) {
	...
	if (xtipe == 'PNN') {
		post_response_text('kebun_slave_panen_posting_view.php', param, respon);
	} else {
		post_response_text('kebun_slave_operasional_posting_view.php', param, respon);
	}
		...
		if (con.readyState == 4) {
			...
			if (con.status == 200) {
				...
				if (!isSaveResponse(con.responseText)) {
					...
					document.getElementById('status_' + currRow).innerHTML = 'OK';
					currRow += 1;
					if (currRow > maxRow) {
						document.getElementById('revTmbl').disabled = false;
						tutupProses('simpan');
...
<tr><td></td><td colspan="2"><button onclick="zPreview('kebun_slave_3postingtransaksi_view','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button></td></tr>
</table>
</fieldset>
</div>
