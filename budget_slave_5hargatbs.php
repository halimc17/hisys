...
switch ($method) {
    case'html':
        ...
        $no = 0;
        $str = "select * from " . $dbname . ".bgt_hargatbs where tahun='".$param['tahun']."' and pabrik like '".$param['kodeorg']."%' and kodebarang='".$param['namabarang']."'";
        $res = fetchdata($str);
        foreach($res as $bar){
            $no+=1;
    ...
    case'insert':
		try {
			...
			$sql = "select * from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and pabrik='".$_SESSION['empl']['lokasitugas']."' and posting=1 and kodebarang='" . $param['namabarang'] . "'";
			$res = fetchdata($sql);
			$jlhbrs = count($res);
			if ($jlhbrs > 0) {
			...
			$sql = "select * from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and pabrik='".$_SESSION['empl']['lokasitugas']."' and kodeorg ='" . $param['kodeorg'] . "' and  kodebarang='" . $param['namabarang'] . "' and tahuntanam='".$param['tahuntanam']."'";
			$res = fetchdata($sql);
			$jlhbrs = count($res);
			if ($jlhbrs > 0) {
			...
			$query = insertQuery($dbname,'bgt_hargatbs',$data,$cols);
			$owlPDO->exec($query);
	...
	break;
    case'delete':
		$sql = "select * from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and pabrik ='" . $param['kodeorg'] . "' and posting=1 and kodebarang='" . $param['namabarang'] . "'";
		$res = fetchdata($sql);
		$jlhbrs = count($res);
		if ($jlhbrs > 0) {
			throw new PDOException("Data sudah di posting / tutup.");
        ...
        $str = "delete from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and pabrik ='" . $param['kodeorg'] . "' and  kodebarang='" . $param['namabarang'] . "'";
		try {$owlPDO->exec($str);} catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n";die();}
	break;
    case'deletedetail':
		$str = "delete from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and kodeorg ='" . $param['kodeorg'] . "' and  kodebarang='" . $param['namabarang'] . "' and tahuntanam='".$param['tahuntanam']."' and pabrik='".$param['pabrik']."'"; #exit("error".$str);
		try {$owlPDO->exec($str);} catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n";die();}
	break;
	case'updatedetail':
	   try {
			...
			$sql = "select * from " . $dbname . ".bgt_hargatbs where tahun='" . $param['tahun'] . "' and pabrik='".$_SESSION['empl']['lokasitugas']."'  and posting=1 and kodebarang='" . $param['namabarang'] . "' and pabrik='".$_SESSION['empl']['lokasitugas']."'";
			$res = fetchdata($sql);
			$jlhbrs = count($res);
			if ($jlhbrs > 0) {
				throw new PDOException("Data sudah di posting / tutup.");
			...
			$query = updateQuery($dbname,'bgt_hargatbs',$data,$where);
			$owlPDO->exec($query);
    ...
    case'posting':
		...
		$str = "update " . $dbname . ".bgt_hargatbs set posting='1' where tahun='" . $param['tahun'] . "' and pabrik ='" . $param['kodeorg'] . "' and  kodebarang='" . $param['namabarang'] . "'";
		$owlPDO->exec($str);
	...
	break;
	case'unposting':
		$str = "update " . $dbname . ".bgt_hargatbs set posting='0' where tahun='" . $param['tahun'] . "' and pabrik ='" . $param['kodeorg'] . "' and  kodebarang='" . $param['namabarang'] . "'";
        try {$owlPDO->exec($str);} catch (PDOException $e) {print " Gagal  !: " . $e->getMessage() . "\n";die();}
	break;
    case'loaddatadetailori':
        ...
        $no = 0;
        $str = "select * from " . $dbname . ".bgt_hargatbs where tahun='".$param['tahun']."' and pabrik like '".$_SESSION['empl']['lokasitugas']."%'";
        $res = fetchdata($str);
        foreach($res as $bar){
            $no+=1;
            $tab.="<tr class=rowcontent style=height:25px>";
    ...
    case'loaddata':
        ...
        $sql = "select count(*) as jmlhrow from " . $dbname . ".bgt_hargatbs where 1=1 " . $where . " group by tahun, pabrik, kodebarang";
        $res = fetchdata($sql);
        $jlhbrs = count($res);
		...
		$str = "SELECT * FROM " . $dbname . ".bgt_hargatbs where 1=1 " . $where . " group by tahun,pabrik, kodebarang order by tahun desc, kodeorg asc, kodebarang asc limit " . $offset . "," . $limit . "";
        $res = fetchdata($str);
        foreach($res as $bar){
            $no+=1;
