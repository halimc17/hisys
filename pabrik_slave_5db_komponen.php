...
$opttipe=makeOption($dbname,'pabrik_5db_ksf','kode,nama');

switch ($proses) {
	case 'simpan':
		if($tipe == ''){
			exit("Warning : Tipe laporan harus dipilih.");
		}
		
		if($nama == ''){
			exit("Warning : Nama Laporan harus diisi.");
		}
		
		if($kode == ''){
			exit("Warning : kode File harus diisi.");
		}
		...
		if($method == 'insert'){
			$str = "insert into ".$dbname.".pabrik_5db_komponen (tipe,kode,nama,updateby) values ('".$tipe."','".$kode."','".$nama."','".$_SESSION['standard']['userid']."')";
		
			try{
				$owlPDO->exec($str);
				foreach($_SESSION['mswarna'] as $row){
					$str = "insert into ".$dbname.".pabrik_5db_warna (tipe,kode,opawal,nilaiawal,opakhir,nilaiakhir,warna,keterangan,
						updateby) VALUES ('".$tipe."','".$kode."','".$row['operationawal']."','".$row['nilaiawal']."','".$row['operationakhir']."','".$row['nilaiakhir']."','".$row['kodefill']."','".$row['keterangan']."','".$_SESSION['standard']['userid']."')";
					try{
						$owlPDO->exec($str);
			...
			$str = "delete from ".$dbname.".pabrik_5db_warna where tipe = '".$tipe."' and kode='".$kode."'";
			try{
				$owlPDO->exec($str);
			}catch(PDOException $e){
				echo "error : ".$e->getMessage();
			...
			$str = "update ".$dbname.".pabrik_5db_komponen set nama='".$nama."', updateby = '".$_SESSION['standard']['userid']."' where tipe = '".$tipe."' and kode='".$kode."'";
			try{
				$owlPDO->exec($str);
				
				foreach($_SESSION['mswarna'] as $row){
					$str = "insert into ".$dbname.".pabrik_5db_warna (tipe,kode,opawal,nilaiawal,opakhir,nilaiakhir,warna,keterangan, updateby) VALUES ('".$tipe."','".$kode."','".$row['operationawal']."','".$row['nilaiawal']."','".$row['operationakhir']."','".$row['nilaiakhir']."','".$row['kodefill']."','".$row['keterangan']."','".$_SESSION['standard']['userid']."')";
					try{
						$owlPDO->exec($str);
					}catch(PDOException $e){
						echo "error : ".$e->getMessage();
	...
	case 'fillfield':
		$result = "";
		$_SESSION['mswarna'] = array();
		$str = "select * from ".$dbname.".pabrik_5db_warna where tipe = '".$tipe."' and kode='".$kode."' order by idwarna";
	...
	case 'deletefield':
		$str = "delete from ".$dbname.".pabrik_5db_komponen where tipe = '".$tipe."' and kode = '".$kode."' ";
		try{
			$owlPDO->exec($str);
		}catch (PDOException $e){
			echo "error : ".$e->getMessage();
	...
	case 'loadData':
        ...
        $str="select * from ".$dbname.".pabrik_5db_komponen";
        $res=fetchdata($str);
        $jlhbrs=count($res);
        if($jlhbrs==0){
            ...
            $no=$maxdisplay;
            $str="SELECT * from ".$dbname.".pabrik_5db_komponen order by tipe asc limit ".$offset.",".$limit."";
            $tab="";
            $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
            ...
            while($bar=$res->fetch()){
                ...
                $tab.="<tr class=rowcontent>
                     ...
                     <td>".$opttipe[$bar->tipe]."</td>
                     <td>".$bar->kode."</td>
                     <td>".$bar->nama."</td>
                     <td align=center>
                     	<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar->tipe."','".$bar->kode."','".$bar->nama."')\">
                     	<img src='images/skyblue/delete.png' class='resicon' title='Delete' onclick=\"deletefield('".$bar->tipe."','".$bar->kode."')\">
                     	<img src='images/skyblue/zoom.png' class='resicon' title='Detail' onclick=\"detailfield('".$bar->tipe."','".$bar->kode."',event)\">
                     </td>
                 	</tr>";
            }
