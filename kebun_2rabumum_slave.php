...
## Start Queries
		...
		#=bgt_budget TM
		$str="select *,left(noakun,5) as headakun from ".$dbname.".bgt_budget where 1=1 and ".$wh." and tahunbudget='".$tahunprd."' and tipebudget='ESTATE' and noakun like '7%' order by noakun asc";
        $res=fetchData($str);
		foreach ($res as $bar) {
        ...
        #=bgt_blok
        $str="select * from ".$dbname.".bgt_blok where 1=1 and ".$wh2." and tahunbudget='".$tahunprd."'";
        $res=fetchData($str);
        foreach ($res as $bar) {
...
## Start Row content
        ...
        $tab.= "<td align=left><b>Luas Areal Diusahakan</td>";
        ...
        $d7=$totgrand/$lad;
        $tab.= "<td align=right></td>";
        $tab.= "<td align=left><b>Cost/Ha</td>";
		...
		foreach ($headakun as $head) {
		    foreach ($isiakun[$head] as $isi) {
                ...
                $tab.= "<td align=right>".number_format($rp[$head][$isi]/$d7,2)."</td>";
                $tab.= "</tr>";
                $subtotisi[$head]+=$rp[$head][$isi];
                $subtotisid7[$head]+=$rp[$head][$isi]/$d7;
            }
            $tab.= "<tr class=rowcontent>";
...
## End Row Content
		...
		if($tipeprint=='html'){
			...
			$nop = "Laporan RAB Biaya Umum ".$unit.".xls";
			$xls = new HtmlExcel();
			$xls->setCss($css);
			$xls->addSheet("1", $tab);
			$xls->headers($nop);
