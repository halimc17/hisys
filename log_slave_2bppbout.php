...
$gudang=checkPostGet('gudangout','');
$tgl=tanggalsystemn(checkPostGet('tglout',''));
$wherebrg='';
...
$stream=" <b>".$_SESSION['lang']['pengeluaranbarang']."</b><br><br>	";
...
// $str=" 	SELECT a.jumlah,a.satuan,a.notransaksi,b.debet,b.kredit FROM ".$dbname.".log_transaksi_vw a 
		// left join ".$dbname.".keu_jurnaldt_vw b on a.notransaksi=b.noreferensi 
		// WHERE a.tanggal = '".$tgl."'  and a.kodegudang='".$gudang."' and a.tipetransaksi='5' ";
$str=" 	SELECT * FROM ".$dbname.".log_transaksi_vw WHERE tanggal = '".$tgl."'  and kodegudang='".$gudang."' and tipetransaksi='5' ";		
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){      
	$notransaksi[$bar['notransaksi']]=$bar['notransaksi'];
	$kodebrg[$bar['kodebarang']]=$bar['kodebarang'];
	$kodebarang[$bar['notransaksi']][$bar['kodebarang']]=$bar['kodebarang'];
	$jumlah[$bar['notransaksi']][$bar['kodebarang']]=$bar['jumlah'];
	$satuan[$bar['notransaksi']][$bar['kodebarang']]=$bar['satuan'];
}

$str="select noakun,noreferensi,kodebarang from ".$dbname.".keu_jurnaldt_vw where noreferensi in 
		(SELECT notransaksi FROM ".$dbname.".log_transaksi_vw WHERE tanggal = '".$tgl."'  
		and kodegudang='".$gudang."' and tipetransaksi='5') and debet!=0 ";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){ 
	$debet[$bar['noreferensi']][$bar['kodebarang']]=$bar['noakun'];
}		


$str="select noakun,noreferensi,kodebarang from ".$dbname.".keu_jurnaldt_vw where noreferensi in 
		(SELECT notransaksi FROM ".$dbname.".log_transaksi_vw WHERE tanggal = '".$tgl."'  
		and kodegudang='".$gudang."' and tipetransaksi='5') and kredit !=0 ";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){ 
	$kredit[$bar['noreferensi']][$bar['kodebarang']]=$bar['noakun'];
}	

	foreach($notransaksi as $notran){
		foreach($kodebrg as $kdbrg){
			if($kodebarang[$notran][$kdbrg]!=''){
				$wherebrg=" kodebarang='".$kodebarang[$notran][$kdbrg]."' ";
			@$no+=1;      
			$stream.="<tr class=rowcontent>
					<td align=center>".$no."</td>
					<td>".$debet[$notran][$kdbrg]."</td>    
					<td>".$kredit[$notran][$kdbrg]."</td>    
					<td>".$satuan[$notran][$kdbrg]."</td>
					<td align=right>".number_format($jumlah[$notran][$kdbrg])."</td>
					<td>".$kodebarang[$notran][$kdbrg]."</td>
					<td>".$nmbrg[$kodebarang[$notran][$kdbrg]]."</td>
					<td>".$notran."</td>
				 </tr>";
			}
		}
...
######EXCEL	
	case 'excel':
		...
		$tglSkrg=date("Ymd");
		$nop_="bukti_pengeluaran_barang_".$tglSkrg;
		if(strlen($stream)>0)
		{
