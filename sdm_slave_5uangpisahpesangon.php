...
switch($method){
	case 'loaddata':
		...
		$no=0;
		$str= "select * from ".$dbname.".sdm_5uangpisahpesangon order by jenispesangon asc, status desc ";
		//echo $str;
		$res= fetchdata($str);
		foreach($res as $val){
			$no+=1;
	...
	case 'insert':
		try{
			...
			## VALIDATE
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5uangpisahpesangon where kodept='".$param['kodept']."' and jenispesangon='".$param['jenispesangon']."' and tipekaryawan='".$param['tipekaryawan']."'  and masakerjadari<='".$param['masakerjadari']."' and masakerjasampai>='".$param['masakerjadari']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5uangpisahpesangon where kodept='".$param['kodept']."' and  jenispesangon='".$param['jenispesangon']."' and tipekaryawan='".$param['tipekaryawan']."'  and masakerjadari<='".$param['masakerjasampai']."' and masakerjasampai>='".$param['masakerjasampai']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
			...
			$str = insertQuery($dbname,'sdm_5uangpisahpesangon',$data,$cols);
			$owlPDO->exec($str);
	...
	case 'update':
		try {
			...
			## VALIDATE
			$str="select count(*) as jlhitem from ".$dbname.".sdm_5uangpisahpesangon where kodept='".$param['kodept']."' and jenispesangon='".$param['jenispesangon']."' and tipekaryawan='".$param['tipekaryawan']."'  and masakerjadari<='".$param['masakerjadari']."' and masakerjasampai>='".$param['masakerjadari']."' and kode!='".$param['kode']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
				throw new PDOException("Masa Kerja Dari ini sudah pernah terdaftar pada rentang waktu masa dari dan masa sampai sebelumnya.");
			}

			$str="select count(*) as jlhitem from ".$dbname.".sdm_5uangpisahpesangon where kodept='".$param['kodept']."' and jenispesangon='".$param['jenispesangon']."' and tipekaryawan='".$param['tipekaryawan']."'  and masakerjadari<='".$param['masakerjasampai']."' and masakerjasampai>='".$param['masakerjasampai']."' and kode!='".$param['kode']."'";
			$res=fetchdata($str);
			if($res[0]['jlhitem'] > 0){
				throw new PDOException("Masa Kerja Sampai ini sudah pernah terdaftar pada rentang waktu masa dari dan masa sampai sebelumnya.");
			}
			...
			$str = updateQuery($dbname,'sdm_5uangpisahpesangon',$data,$where);
			$owlPDO->exec($str);
