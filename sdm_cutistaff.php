...
<script language=javascript src=js/sdm_cutistaff.js?v=<?php echo time(); ?>></script>
<script>
	// getSelect2();
</script>
<?php
	OPEN_BOX('','<span class=judul>'.getMenu('sdm_cutistaff').'</span>');
	echo"<div>";
...
$query = "SELECT nilai FROM setup_parameterappl WHERE kodeaplikasi = 'HR' AND kodeparameter = 'HRSDMCUTI' AND typenilai = '1'";
$stmt = $owlPDO->prepare($query);
$stmt->execute();
...
if (strpos($nilai, $kodejabatan) !== false) {
    $str="select karyawanid, namakaryawan, nik, lokasitugas from ".$dbname.".datakaryawan where
		(tanggalkeluar = '0000-00-00' or tanggalkeluar>= '".date("Y-m-d")."')  and statuskaryawan != 'Keluar' and tipekaryawan ='0' and lokasitugas in (".$arrorgdet.") order by namakaryawan,lokasitugas asc";
} else {
    $str="select karyawanid, namakaryawan, nik, lokasitugas from ".$dbname.".datakaryawan where
		(tanggalkeluar = '0000-00-00' or tanggalkeluar>= '".date("Y-m-d")."')  and statuskaryawan != 'Keluar' and tipekaryawan ='0' and karyawanid='".$_SESSION['standard']['userid']."' order by namakaryawan,lokasitugas asc";
}
...
while($bar=$res->fetch())
{
	$optkaryawan.="<option value=".$bar['karyawanid']." selected>".$bar['nik']." - ".$bar['namakaryawan']."</option>";
}
...
$str="select karyawanid, namakaryawan,nik, lokasitugas from ".$dbname.".datakaryawan where (tanggalkeluar = '0000-00-00' or tanggalkeluar>= '".date("Y-m-d")."') and lokasitugas='".$_SESSION['empl']['lokasitugas']."'  and statuskaryawan != 'Keluar' and tipekaryawan IN ('0','1','2','3') order by namakaryawan,lokasitugas asc";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch())
{
	$optpengganti.="<option value=".$bar['karyawanid'].">".$bar['nik']." - ".$bar['namakaryawan']."</option>";
}
...
## Ambil setup approval
	$str="select * from ".$dbname.".setup_approval where karyawanid = '".$_SESSION['standard']['userid']."'";
	$res = fetchdata($str);
	$jlhbrs = count($res);

	if($jlhbrs > '1'){
		$str2=$owlPDO->query("select karyawanid from ".$dbname.".user where karyawanid != '".$_SESSION['standard']['userid']."' and namauser not like '%tim.owl%' and karyawanid in (select karyawanid from datakaryawan where tipekaryawan = '0' and tanggalkeluar = '0000-00-00') group by karyawanid ");
		$str2->setFetchMode(PDO::FETCH_OBJ);
		while($bar=$str2->fetch()){
			$optApp.="<option value='".$bar->karyawanid."'>".getNamaKaryawan($bar->karyawanid)."</option>";
		}
	}else{
		$optApp.="<option value=''></option>";  
	}
## End setup approval
...
<table cellspacing='1' cellpadding=2 border='0'>
	...
	<tr>
		...
		<td valign=top>:</td>
		<td valign=top>
			<input type='text' class='myinputtext' style='width:242px;' id='alamatcuti' name='alamatcuti' onkeypress='return tanpa_kutip(event);' maxlength='30' style='width:150px;' />
		</td>	

		<td>Approval Pengganti</td>
		<td>:</td>
		<td>
			<select id=apppengganti style=width:245px>".$optApp."</select>
			<img id='apppengganti' onclick=z.elSearch('apppengganti',event) class='resicon' src='images/onebit_02.png' style='position:relative;top:3px;left:3px;'>
		</td>	
...
<legend>" . $_SESSION['lang']['list'] . "</legend>
				<div>
					<table class=sortable style='width:100%;' cellspacing=1 cellpadding=7 border=0>
						<thead>
							<tr class=rowheader>
