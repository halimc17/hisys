...
?>
<script language=javascript1.2 src='js/bgt_prdkebun_parameter.js?v=<?php echo time(); ?>'></script>
		...
		var parseAndPivot = function(f) {
			...
			Papa.parse(f, {
				...
				complete: function(parsed){
					...
					tujuan = 'bgt_slave_prdkebun_parameter.php';
...
$str="select distinct tahunbudget from ".$dbname.".bgt_produksi_kebun_parameter order by tahunbudget desc";
$res = fetchdata($str);
foreach($res as $bar){
...
OPEN_BOX('','<span class=judul>'.getMenu('bgt_prdkebun_parameter').'</span>');
