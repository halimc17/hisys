...
$periode = checkPostGet('periodeId2','');
$unit = checkPostGet('unitId2','');
$afd = checkPostGet('afdId2','');
$intiplasma = checkPostGet('intiplasma2',''); 
...
$where="";
// if($unit!=''){
//     $where=" and blok like '".$unit."%'";
// }
if($afd!=''){
    $where.=" and blok like '".$afd."%'";
}else{
    $where.=" and blok like '".$unit."%'";
}
...
$tab.="<th align=center ".$bgdt.">".$_SESSION['lang']['supir']."</th>";
...
if($rowDta>0)
{
    ...
    while($rData=$qData->fetch())
    {
        ...
        if($nospbatas!=$rData['nospb']){
            $jumlahBarisdt=0;
            $sEspebe="select count(*) as jumlahbarisdt from (select distinct * from ".$dbname.".kebun_spb_vw4 where tanggal like '%".$periode."%' and nospb='".$rData['nospb']."' ".$where." ".$inplas.") as tbl";
        ...
        if($nospbatas!=$rData['nospb']){    
           $tab.="<td align=center rowspan='".$jumlahBarisdt."' valign='top'>".$dtNo."</td>";
            ...
            @$bjrwbBrutto=fixnan(@$rData['kgwb']/@$rData['jjg']);
            ...
            $tab.="<td align=right>".number_format($bjrwbBrutto,2)."</td>";
...
{
    $tab.="<tr class=rowcontent><td colspan=16>".$_SESSION['lang']['dataempty']."</td>";
}
