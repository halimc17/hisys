...
$str="select * from ".$dbname.".kebun_spb_vw2 where (tanggal like '".$prd."%' or tanggalpanen like '".$prd."%')  and kodeorg='".$unit."' and divisi like '".$afd."%' and posting='1'"; 
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$Listkeg=array();
$str="select a.*,b.lokasitugas,b.kodeorganisasi from ".$dbname.".kebun_3premibmtbs a left join ".$dbname.".".$db_karyawan." b on a.karyawanid=b.karyawanid where a.notransaksi = '".$notransaksi."' and a.kegiatan='".$keg."' and a.kontanan='".$kontanan."' ".$where_kar."";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
	$trpplb+=$bar['rppremi'];
...
#=========================== Nomor Jurnal ==============================
###tambah try catch coba arahan abdul
try {
	...
	if($konter>999){
		throw new PDOException('Nomor transaksi Jurnal sudah melebihi batas, silahkan reset nomor melalui menu Keuangan - Setup - Kelompok Jurnal, Kode Org = '.$_SESSION['org']['kodeorganisasi'].', Kode Kelompok = BM01');
	...
	// try{
	$owlPDO->exec($str); 
	// } catch(PDOException $e){$errorDB.= "Detail Header 1 : ".$e->getMessage()."\n";}
	if($errorDB!=''){
		$str="delete from ".$dbname.".keu_jurnalht where nojurnal='".$nojurnal."' and noreferensi = '".$notransaksi."' ";
		// try{
		$owlPDO->exec($str); 
		// } catch(PDOException $e){print " Roll Back !: " . $e->getMessage() . "\n"; die();}
	}
	//echo 'TOTAL.'.$trplb.'<br>';
	...
	foreach ($listkar as $spb => $ky1){
		foreach ($ky1 as $kary => $vl) {
			foreach($blokx[$spb] as $blok =>$val){
				...
				//echo '1.'.$kary.'-2.'.$spb.'-3.'.$blok.'-4.DB.'.$jrplb[$spb][$blok].'<br>';
				...
				// try{
				$owlPDO->exec($str); 
				// } catch(PDOException $e){$errorDB.= "Detail Debet 1 : ".$e->getMessage()."\n";}
				if($errorDB!=''){
					$str="delete from ".$dbname.".keu_jurnalht where nojurnal='".$nojurnal."' and noreferensi = '".$notransaksi."' ";
					// try{
					$owlPDO->exec($str); 
	...
	if(count($all_organisasi_rk) > 0){
		foreach ($all_organisasi_rk as $organisasi) {
			...
			## Jurnal RK
				...
				$formatNo = $tmpNoJurnal[0]."/".$tmpKodeOrgRK."/".$kodeJurnalRK."/";
				$nojurnalRK = $tmpNoJurnal[0]."/".$tmpKodeOrgRK."/".$kodeJurnalRK."/".$konterRK;

				$bla = getCountRows($dbname, 'keu_jurnalht', " kodejurnal = '" . $kodeJurnalRK . "' AND nojurnal LIKE '%" . $nojurnalRK . "%'");
				if ($bla > 0) {

					$nilaikounter = $tmpKonterRK[0]['nokounter'];

					$strjurnalht = selectQuery(
						$dbname,
						'keu_jurnalht',
						'nojurnal',
						" kodejurnal = '" . $kodeJurnalRK . "' AND nojurnal LIKE '%".$formatNo."%' ORDER BY CAST(SUBSTRING_INDEX(nojurnal, '/', -1) AS UNSIGNED) DESC LIMIT 1"
					);
					...
					if ($konterht > $nilaikounter) {
						...
						kodeunit='" . $organisasi . "' and kodekelompok='" . $kodeJurnalRK . "' and periode='" . $prd . "' ";
						$owlPDO->exec($str);
						$tmpKonterRK = fetchData($queryRK);
					}
				...
				// try{
				$owlPDO->exec($str); 
				// } catch(PDOException $e){$errorDB.= "Detail Header 2 : ".$e->getMessage()."\n";}
				if($errorDB!=''){
					$str="delete from ".$dbname.".keu_jurnalht where nojurnal='".$nojurnal."' and noreferensi = '".$notransaksi."' ";
					// try{
					$owlPDO->exec($str); 
					...
					foreach ($ky1 as $kary => $vl) {
						foreach($blokx[$spb] as $blok =>$val){
							...
							if($listkar_lokasitugas[$spb][$kary] == $organisasi){
								...
								// try{
								$owlPDO->exec($str); 
								// } catch(PDOException $e){$errorDB.= "Detail Debet 2 : ".$e->getMessage()."\n";}
								if($errorDB!=''){
									$str="delete from ".$dbname.".keu_jurnalht where nojurnal='".$nojurnal."' and noreferensi = '".$notransaksi."' ";
									// try{
									$owlPDO->exec($str); 
				...
				// try{
				$owlPDO->exec($str); 
				// } catch(PDOException $e){$errorDB.= "Detail Kredit RK 1".$tmpKodeOrg." : ".$e->getMessage()."\n";}
				...
				// try{
				$owlPDO->exec($str); 
				// } catch(PDOException $e){$errorDB.= "Detail Debit RK 1 ".$organisasi." : ".$e->getMessage()."\n";}
				...
				// try{
				$owlPDO->exec($str); 
				// } catch(PDOException $e){$errorDB.= " Update Konter !: " . $e->getMessage() . "\n"; die();}
		}
		...
		// try{
		$owlPDO->exec($str); 
		// } catch(PDOException $e){$errorDB.= "Detail Kredit 1 : ".$e->getMessage()."\n";}
		...
		// try{
		$owlPDO->exec($str); 
		// } catch(PDOException $e){$errorDB.= "Detail Kredit 2 : ".$e->getMessage()."\n";}
		//echo $str;
	...
	if($errorDB!=''){
		...
		throw new PDOException('ERROR : Posting gagal di lakukan => '.$errorDB);
	}
	...
	// try{
	$owlPDO->exec($str); 
	// } catch(PDOException $e){$errorDB.= " Update Flag !: " . $e->getMessage() . "\n"; die();}
