...
try{
	...
	foreach($res as $bar){
		...
		$gudangx      = $bar['gudangx'];
		$untukpt      = $bar['untukpt'];
		$gudang       = $bar['kodegudang'];
		$nopo         = $bar['nopo'];
		$supplier     = $bar['idsupplier'];
		$pengguna     = $bar['untukunit'];
		$untukunit    = $bar['untukunit'];
		$statusjurnal = $bar['statusjurnal'];
		$post         = $bar['post'];		
	...
	## CEK PERIODE GUDANG
	$str = "select tutupbuku, periode,tanggalmulai,tanggalsampai from ".$dbname.".setup_periodeakuntansi where kodeorg='".$gudang."' and periode='".$periode."' order by periode asc limit 1";
	$res = fetchdata($str);
	if(count($res) <= 0){
		throw new PDOException("Periode gudang central ".$gudang." tidak ada.");
	}
	if($res[0]['tutupbuku']=='1'){
		throw new PDOException("Periode gudang sudah ditutup.");
	}
	
	if ($statusjurnal == '1' or $post == '1'){
	...
	if ($gudangx != '' and (substr($gudang,0,4) != substr($gudangx,0,4))){
		...
		if($close == '1' and $tipetransaksi != '3'){
			...
			if($_SESSION['language']=='ID'){
				throw new PDOException("Departement keuangan gudang tujuan sudah tutup buku");
			}else{
				throw new PDOException("Receiver Accounting Period has been closed.");
			}
		}
	}
	$text=1;
	#ambil data dari dt
	$kodemesin=$kodekegiatan=$nopp=$hargasatuan=$blok=$jumlah=$satuan=$kodebarang="";
	$statussaldo = 0;
	$str = "select * from ".$dbname.".log_transaksidt where notransaksi='".$notransaksi."'";
	$res = fetchdata($str);
	foreach($res as $bar){
		...
		$statussaldo = ($bar['statussaldo']==''?'0':$bar['statussaldo']);
		$ongkir = ($bar['ongkir']==''?'0':$bar['ongkir']);
		if ($statussaldo==0){
			## Periksa transaksi yang belum diposting di tanggal sebelumnya:
			$str = "select * from ".$dbname.".log_transaksi_vw where kodebarang='".$kodebarang."' and kodegudang='".$gudang."' and tanggal < '".tanggalsystemn($tanggal)."' and statussaldo='0' and notransaksi not in (select notransaksi from ".$dbname.".approval where status='2' and jenispersetujuan='GR')";
			$res = fetchdata($str);
			if (count($res) > 0) {
				if($_SESSION['language']=='ID'){
					throw new PDOException("Masih ada barang yang sama belum di posting pada tanggal yang lebih kecil.");
				}else{
					throw new PDOException("There is material has not been posted on previous date.");				
				}
			}
			...
			if($namabarang == ''){$namabarang = $kodebarang;}
			...
			switch ($tipetransaksi){
				case'1':
					...
					$close = $res[0]['tutupbuku'];
					if ($close == '1'){
						throw new PDOException($ronya." sudah tutup buku");
					}

					//throw new PDOException($ronya." ".$akuncacoro."/".$akuncacoes." ".$close);
					...
					if ((substr($kodebarang, 0, 3) < '400') and trim($akunbarang) != '') {
						...
						$cek = "select * from ".$dbname.".log_5masterbarangdt where kodegudang='".$gudang."' and kodebarang='".$kodebarang."' and kodeorg='".$kodept."'";
						$rcek = fetchdata($cek);
						if(count($rcek)>0){
				...
				case'6':
					...
					if ($newsaldoakhirqty < 0){
						if($_SESSION['language']=='ID'){
							throw new PDOException("Saldo tidak mencukupi (retur:".$jumlah." saldo:".$cursaldo.")");
						}else{
				...
				case'2':
					...
					if ($akunbarang == ''){
						...
						if ($pengguna == substr($gudang, 0, 4)) {
							...
							## Data Detail
							$keterangan = "ReturGudang barang " . $namabarang . " " . $jumlah . " " . $satuan . " " . substr($tanggal, 0, 7);
							$keterangan = substr($keterangan, 0, 150);
							$noUrut = 1;
				...
				case'3':
					...
					if ($akunbarang == ''){
						...
						## Data Detail
						$keterangan = "Terima Mutasi barang " . $namabarang . " " . $jumlah . " " . $satuan . " " . substr($tanggal, 0, 7);
						$keterangan = substr($keterangan, 0, 150);
						$noUrut = 1;
				...
				case'7':
					...
					## Prepare rollback penerimaan
					$strrollback = "update ".$dbname. ".log_5saldobulanan set saldoakhirqty='".$saldoakhirqty."',nilaisaldoakhir='".$nilaisaldoakhir."',lastuser='".$user."',qtykeluar='".$qtykeluar."',qtykeluarxharga='".$qtykeluarxharga."' where periode='".$periode."' and kodegudang='".$gudang."' and kodebarang='".$kodebarang."' and kodeorg='".$kodept."'";
				...
				case'5':
					...
					//throw new PDOException("Error");
						...
						if ($pengguna == substr($gudang, 0, 4)) {
							...
							## Data Detail
							$keterangan = "Pemakaian barang " . $namabarang . " " . $jumlah . " " . $satuan . " " . substr($tanggal, 0, 7);
							$keterangan = substr($keterangan, 0, 150);
							$noUrut = 1;
				...
				break;
				default:
					throw new PDOException("Tipe transaksi ini tidak tersedia.");
				break;
			}#tutup switch
		}#tutup foreach $statussaldo
		
		$text+=1;
	} #tutup foreach log_transaksidt
...
$res = fetchData($str);
if(count($res)>0){#jika belum maka jangan ubah flag
	throw new PDOException("There is still unsucceed transaction, please re-process");
}else{  #jika sudah, maka ubah flag      
	$str = "update ".$dbname.".log_transaksiht set post='1', postedby=".$user.",statusjurnal='1' where notransaksi='".$notransaksi."'  and kodegudang='".$gudang."'";
	$owlPDO->exec($str);
}	
