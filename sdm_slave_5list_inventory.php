...
$nmkat=makeOption($dbname,'sdm_5kategori_inv','idjenis,jeniskategori');
?>
...
switch ($method) {
    ...
    case 'insert':
        $str = "insert into " . $dbname . ".sdm_5hrd_inv (unit,station,kodebarang,updateby)
            values ('" . $unit . "','" . $kategori . "','" . $kodebarang . "','" . $_SESSION['standard']['userid'] . "')";
		try{
			$owlPDO->exec($str); 
    ...
    case'loaddata':
        ...
        $ql2 = "select count(*) as jmlhrow from " . $dbname . ".sdm_5hrd_inv"; // echo $ql2;notran
		...
		//  limit " . $offset . "," . $limit . "
        $str = "select * from " . $dbname . ".sdm_5hrd_inv";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
        $no = $maxdisplay;
        while ($bar = $res->fetch()) {
            ...
            echo "<td align=left>" . $nmkat[$bar['station']] . "</td>";
            ...
            echo "<td align=center>
						   ...
						   onclick=\"del('".$bar['unit']."','".$bar['kategori']."','".$bar['kodebarang']."');\">
                            </td>";
            echo "</tr>"; //
        }
    ...
    case 'delete':
        $str = "delete from " . $dbname . ".sdm_5hrd_inv where unit='" . $unit . "' and station='".$kategori."' and kodebarang='".$kodebarang."' ";
		try{
			$owlPDO->exec($str); 
		}catch(PDOException $e){
			echo " Gagal," . addslashes($e->getMessage());
