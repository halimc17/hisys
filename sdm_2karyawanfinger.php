...
<script language="javascript" src="js/sdm_2karyawanfinger.js?v=<?php echo time(); ?>"></script>

<?php

OPEN_BOX('','<span class=judul>'.getMenu('sdm_2karyawanfinger').'</span><br>');
...
## GET SHIFT KARYAWAN
$optShift="<option value=''>".$_SESSION['lang']['all']."</option>";
$str="select * from ".$dbname.".sdm_5mastershift ";
$res=fetchdata($str);
foreach($res as $val){
	$optShift.="<option value='".$val['shift']."'>".$val['namashift']."</option>";
}
...
<fieldset style=float:left>
	<table cellspacing=1 cellpadding=2>
		...
		<tr>
			...
			<td>:</td>
			<td>
				<select class=select2 id='periode' style='min-width:245px'>".$optPeriode."</select>
			</td>
		</tr>
		<tr hidden>
			<td>Nama ".$_SESSION['lang']['shift']."</td>
			<td>:</td>
			<td>
				<select class=select2 id='shiftkar' style='min-width:245px'>".$optShift."</select>
			</td>
		</tr>
		<tr>
			<td colspan=2></td>
