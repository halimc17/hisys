...
$str="SELECT noakun, namaakun,namaakun1 FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=5 and  (left(noakun,5) between '12601' and '12605')
    ORDER BY noakun";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
#bentuk akun 7
$str="SELECT noakun, namaakun,namaakun1 FROM ".$dbname.".keu_5akun
    WHERE length(noakun)=7 and  (left(noakun,7) between '1260101' and '1260599')
    ORDER BY noakun";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select kegiatan,noakun,".$addstr." as sdbi,rp".$bulan." as bi,rupiah,tahunbudget,kodeorg,".$addstrvol." as sdbivol,fis".$bulan." as bivol,volume
		from ".$dbname.".bgt_budget_detail where kodeorg like '".$sortunit."%' and tahunbudget = '".$tahun."' and (left(noakun,5) between '12601' and '12605')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select * from ".$dbname.".kebun_perawatan_dan_spk_vw where unit='".$unit."' 
	and kodeorg like '".$sortunit."%' and tanggal like '".$periode."%' and (left(kodekegiatan,5) between '12601' and '12605')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
...
$str=" select * from ".$dbname.".kebun_perawatan_dan_spk_vw where unit='".$unit."' 
	...
	and (left(kodekegiatan,5) between '12601' and '12605')";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
while($bar=$res->fetch()){
