...
<script language=javascript src='js/sdm_2laporanpendapatan.js?v=<?php echo time(); ?>'></script>

<?php

OPEN_BOX('','<span class=judul>'.getMenu('sdm_2laporanpendapatan').'</span><br>');
...
$str="select distinct idkomponen from ".$dbname.".sdm_pendapatanlainht";
$res=fetchdata($str);
foreach($res as $val){
	$optKom.="<option value='".$val['idkomponen']."'>".getNamaKomponenGaji($val['idkomponen'])."</option>";
}
...
<fieldset style=float:left>
	<table cellspacing=1 cellpadding=2>
		...
		<tr>
			<td>Jenis ".$_SESSION['lang']['pendapatan']."</td>
			<td>:</td>
			<td>
				<select class=select2 id='pendapatan' style='width:246px'>".$optKom."</select>
			</td>
		</tr>
		<tr>
			<td colspan=2></td>
