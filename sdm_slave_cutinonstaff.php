...
$jenispersetujuan="IJNS";
switch ($method) {
    case'loadperiodecuti':
        $optPeriodec="";
        $str="select periodecuti from ".$dbname.".sdm_cutiht where karyawanid='".$param['karyawanid']."' and periodecuti in ('".(date('Y')-1)."','".date('Y')."','".(date('Y')+1)."') group by periodecuti ";
        $res=fetchdata($str);
        foreach($res as $val){
	...
	case'insert':
        ...
        if(count($resApp) > 0 and trim($param['pengganti']) == ""){
            ...
            if (trim($param['tglijin']) == '') {
                $param['tglijin'] = date('d-m-Y');
            }
    ...
    case'loadData':
        ...
        if($param['karyawanidsch'] != ''){
            $where.=" and karyawanid = '".$param['karyawanidsch']."'";
        }
    ...
    case'update': 
        ...
        if(count($resApp) > 0 and trim($param['pengganti']) == ""){
            ...
            foreach($rangetgl as $tglxz){
                ...
                // $str="select * from ".$dbname.".sdm_cutibersamadt where karyawanid='".$param['karyawanid']."' and tanggal='".$tglxz."' ";
                // $res=fetchData($str);
                // $jmlhbaris=count($res);

                // if($jmlhbaris>0)
                // {
                //     $brax.='tanggal cuti bersama '.tanggalnormal($tglxz).',';
                // }
            }
            if($brax!=''){
	...
	case 'preview':
        ...
        $str="select * from ".$dbname.".approval_return where notransaksi='".$param['notransaksi']."' and jenispersetujuan='IJNS' order by level asc";
        $res=fetchdata($str);

        // $tabx="<legend>Persetujuan Izin/Cuti Reconfirm</legend><br><br><table cellspacing=0 border=1 width=100% style='text-align:center'>";
        // $tabx.="<tr>";
        ...
        $str="select * from ".$dbname.".approval where notransaksi='".$param['notransaksi']."' and jenispersetujuan='IJNS' order by level asc";
        $res=fetchdata($str);
        ...
        // echo "Jumlah Baris Cuti Lainnya: ".$brax;
        // echo "<br>";
        // echo "Status Batal Cuti: ".$statusbatalcuti;
        // echo "<br>";
        // echo "Status Persetujuan Cuti: ".$statuscuti;
            $str="select * from ".$dbname.".approval where notransaksi='".$param['notransaksi']."' and jenispersetujuan='IJNSC' order by level asc";
            $res=fetchdata($str);
        ...
        if($brax==0 and ($statusbatalcuti=='0' or $statusbatalcuti=='2') and $statuscuti=='1'){
            ...
            ##CEK PER DEPARTEMEN
            $str="select count(kodeunit) as kodeunit from ".$dbname.".setup_approval where kodeunit='".$lokasitugas."' and jenispersetujuan='IJNSC' and departemen='".$departemen."'";
            $res=fetchdata($str);
            ...
            ##CEK PER GOLONGAN
            $str="select count(kodeunit) as kodeunit from ".$dbname.".setup_approval where kodeunit='".$lokasitugas."' and jenispersetujuan='IJNSC' and golongan='".$golongan."'";
            $res=fetchdata($str);
            ...
            $str="select * from ".$dbname.".setup_approval 
                    where jenispersetujuan='IJNSC' and kodeunit='".$lokasitugas."' and karyawaniduser='".$karyawanid."' ".$where." order by level";  
            $res=fetchData($str);
            if(count($res) > 0){
                ...
                $str="select * from ".$dbname.".setup_approval 
                where jenispersetujuan='IJNSC' and kodeunit='".$lokasitugas."' and karyawaniduser='' ".$where." order by level";  
                $res=fetchData($str);
                if(count($res)>0){
                    foreach($res as $key => $bar){
                        $whr        =" karyawanid='".$bar['karyawanid']."'";
                     ...
                     $str="select * from ".$dbname.".setup_approval 
                    where jenispersetujuan='IJNSC' and kodeunit='".$lokasitugas."' and karyawaniduser=''  order by level";  
                    $res=fetchData($str);
                     foreach($res as $key => $bar){
                        $whr        =" karyawanid='".$bar['karyawanid']."'";
                        $optnama    = makeOption($dbname,'datakaryawan','karyawanid,namakaryawan',$whr);
    ...
    case 'pdf':
        ...
        if($statuspotongan=='1' or $statuspotongan=='2'){
            ...
            $str="select sum(jumlahhari) as diambil from ".$dbname.".sdm_ijin where karyawanid ='".$karyawancuti."' and periodecuti = '".$periodecuti."'and statuspotongan in (1,2) and tanggal<='".$bar['tanggal']."' and notransaksi!='".$param['notransaksi']."' and statuspersetujuan!=2 and statuspersetujuan_cancel!=1";
            $res=fetchData($str);
            if($res[0]['diambil']>0){
    ...
    case'form_ajukan';
        ...
        ##CEK PER DEPARTEMEN
        $str="select count(kodeunit) as kodeunit from ".$dbname.".setup_approval where kodeunit='".$lokasitugas."' and jenispersetujuan='IJNS' and departemen='".$departemen."'";
        $res=fetchdata($str);
        $perdepartemen=$res[0]['kodeunit'];
        $where="";
        if($perdepartemen>0){
        ...
        ##CEK PER GOLONGAN
        $str="select count(kodeunit) as kodeunit from ".$dbname.".setup_approval where kodeunit='".$lokasitugas."' and jenispersetujuan='IJNS' and golongan='".$golongan."'";
        $res=fetchdata($str);
        $pergolongan=$res[0]['kodeunit'];
        if($pergolongan>0){
            $where.=" and golongan='".$golongan."'";
        ...
        $optper4=$optper3=$optper2=$optper1="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
        $str="select * from ".$dbname.".setup_approval 
                where jenispersetujuan='IJNS' and kodeunit='".$lokasitugas."' and karyawaniduser='".$karyawanid."' ".$where." order by level";  
        $res=fetchData($str);
        if(count($res) > 0){
            foreach($res as $key => $bar){
                $whr        =" karyawanid='".$bar['karyawanid']."'";
            ...
            $str="select * from ".$dbname.".setup_approval 
                where jenispersetujuan='IJNS' and kodeunit='".$lokasitugas."' and karyawaniduser='' ".$where." order by level";  
            $res=fetchData($str);
            if(count($res)>0){
                foreach($res as $key => $bar){
                    $whr        =" karyawanid='".$bar['karyawanid']."'";
                 ...
                 $str="select * from ".$dbname.".setup_approval 
                where jenispersetujuan='IJNS' and kodeunit='".$lokasitugas."' and karyawaniduser=''  order by level";  
                $res=fetchData($str);
                 foreach($res as $key => $bar){
                    $whr        =" karyawanid='".$bar['karyawanid']."'";
                    $optnama    = makeOption($dbname,'datakaryawan','karyawanid,namakaryawan',$whr);
    ...
    case'ajukanbatalcuti':
        ...
        if($alasanbatal==''){
            exit('Warning : Alasan pembatalan cuti tidak boleh kosong');
        }
