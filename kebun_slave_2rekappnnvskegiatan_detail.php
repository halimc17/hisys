...
switch($proses)
{
	...
	echo" Print Excel : <img style=cursor:pointer; "
	 . " onclick=\"parent.lihatdetail('" . $blok . "','" . $tgl . "','excel','KegiatanPanen',event)\" src=images/excel.jpg  
		title='MS.Excel'>
	...
	$stream = "<table " . $border . " class=sortable cellspacing=1 style=width:100%>
				 <thead>
					<tr>
					  ...
					  <td align=center>" . $_SESSION['lang']['premlebihbasis'] . "</td>  
	...
	$no = 0;
	$str = "select * from " . $dbname . ".kebun_prestasi_vs_hk where kodeorg='" . $blok . "' and tanggal='" . $tgl . "'  ";
	$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	while ($bar = $res->fetch()) {
		$no+=1;
		$stream.="<tr class=rowcontent>
			...
			<td align=right>" . number_format($bar['tupah']) . "</td>   
			<td align=right>" . number_format($bar['upahpremi']) . "</td>   
			<td align=right>" . number_format($bar['upahpremilebihbasis']) . "</td>   
			<td align=right>" . number_format($bar['rupiahpenalty']) . "</td>   
			<td align=right>" . number_format($bar['tpremi']-$bar['rupiahpenalty']) . "</td>   
			 </tr>";
			 @$tjjg+=$bar['hasilkerja'];
			 @$thk+=$bar['hkpanenperhari'];
			 @$tluas+=$bar['luaspanen'];
			 @$tupah+=$bar['tupah'];
			 @$tsb+=$bar['upahpremi'];
			 @$tlb+=$bar['upahpremilebihbasis'];
			 @$tdenda+=$bar['rupiahpenalty'];
			 @$tpremi+=$bar['tpremi']-$bar['rupiahpenalty'];
	}
			$stream.="<tr class=rowcontent>
				...
				<td align=right>".number_format($tsb)."</td>  
