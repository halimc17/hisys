 ...
 class PDF extends FPDF
        {
            function Header() {
                ...
                $this->SetFont('Arial','BU',10);
                                $this->Cell($width,$height,  strtoupper("Surat Pengiriman Tandan Buah Segar"),'',0,'C');
                                $this->Ln(20);
                ...
                $this->Cell(15/100*$width,$height,'Hamparan',1,0,'C',1);	
                $this->Cell(15/100*$width,$height,'Kavling',1,0,'C',1);	
				$this->Cell(15/100*$width,$height,'Nama',1,0,'C',1);	
...
//kebun_spbdt
				$no=0;
                $str="select * from ".$dbname.".kebun_spbpetani where nospb='".$noSpb."'"; //echo $str;exit();
                $qry=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
				$qry->setFetchMode(PDO::FETCH_ASSOC);
				while($res=$qry->fetch()){
					$no+=1;
					$pdf->Cell(3/100*$width,$height,$no,1,0,'C',1);
                    $pdf->Cell(15/100*$width,$height,$res['no_hamp'],1,0,'L',1);   
					$pdf->Cell(15/100*$width,$height,$res['no_kavl'],1,0,'L',1);	
					$pdf->Cell(15/100*$width,$height,$res['nama'],1,0,'L',1);
					$pdf->Cell(10/100*$width,$height,number_format($res['janjang']),1,0,'R',1);
				...
				// $pdf->Cell(10/100*$width,$height,number_format($tkgbjr/$tjjg,2),1,0,'R',1);
