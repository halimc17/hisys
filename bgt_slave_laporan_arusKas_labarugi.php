...
$qLastsum=$owlPDO->query($sLastsum) or die(print " Gagal: ".PDOException::getMessage());
$qLastsum->setFetchMode(PDO::FETCH_ASSOC);
while($hasil=$qLastsum->fetch())
{
...
$tab.="<table cellpadding=1 cellspacing=1 border=".$brdr." class=sortable width=3000px><thead>";
...
function printline6(){
    ...
    $tabz.="<tr class=rowcontent>";
    $tabz.="<td align=left ".$bg." ></td>";
    $tabz.="<td align=left ".$bg." >Jumlah Laba Rugi</td>";
    $tabz.="<td align=left ".$bg." ></td>";
    //$tabz.="<td align=right ".$bg." >".number_format($jumlah[0])."</td>";
    for ($i = 1; $i <= 12; $i++) {
        if(strlen($i)==1)$ii='0'.$i; else $ii=$i;
