...
class Master extends OWL_Controller{
	...
	function index(){
	 ...
	 function checkPriviledge(){
		...
		$priv  = array();
		$query = "SELECT r,w,u,d,p FROM ".$this->dbname.".auth WHERE namauser = '".$_SESSION['standard']['username']."' AND menuid = '".$this->get('id')."' LIMIT 1";
	 ...
	 function load_childmenu(){
		if(!empty($this->get('jumper')) or !empty($this->get('refresh'))){
			if(!empty($this->get('jumper'))){
				$listMenu = $this->Menuharvest->jumper($_SESSION['standard']['username'],$this->get('jumper')); 
			}else{
				$listMenu = $this->Menuharvest->refresh($_SESSION['standard']['username'],$this->get('refresh')); 
			}
			if(count($listMenu) > 0){
				echo json_encode(array_shift($listMenu));
			}
		}else{
			$listMenu = $this->Menuharvest->loadMenu($_SESSION['standard']['username'],$this->get('parent')); 
