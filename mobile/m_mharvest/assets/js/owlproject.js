		...
		if(methodDef == 'POST'){
			...
			if(sendWith__){
				if(this.Utils().isElement(eleform) && eleform.tagName.toLowerCase() == 'form'){
					...
					let dataform = this.jsonToFormData(eleform);
					//tambahan 
					dataform.append('par', par);
					conPost.send(dataform);
				}else{
			...
			if(el.disabled==false) {
				...
				for(let x=0;x<el.options.length; x++) {
					...
					var elText = eleOpt.text,
						...
						cont += '<tr idx="'+x+'" style="cursor:pointer;" title="Choose.."  class="rowcontent" onclick="$.z.passParam(\''+name+'\',this,\'\')"><td>'+elValue+'</td><td>'+elText+'</td></tr>';
				}
				cont += "</tbody></table>";
				cont += "<div>";
