...
(function(root, factory) {
    ...
    var TouchSideSwipe = function(config) {
        ...
        var opt = { //default options
            ...
            opacityBackground: config.opacityBackground || 0.6,
            shiftForStart: config.shiftForStart || 50, // px
            windowMaxWidth: config.windowMaxWidth || 2024, // px
        };
        //------------------------------------------------------------------
        ...
        function tssInitStates() {
			...
			listmenuside = document.getElementsByClassName('list-menuside');
            //-------------------------------
        ...
        function tssTouchend(event) {
            ...
            elBg.style.transitionDuration = opt.moveSpeed + 's';
            if (!open && touchendCoordX > touchstartCoordX && !menuTouches) {
            ...
            else if (open && (touchendCoordX < touchstartCoordX) && (touchendCoordX <= elSubmainWidth) && !menuTouches){
        ...
        function closeByClick(event){
			...
			if(event.path.length>0){
				for(var x00i=0; x00i<event.path.length; x00i++){
					if(event.path[x00i].getAttribute("onclick")){
						eval(event.path[x00i].getAttribute("onclick"));
					}else if(event.path[x00i].className == "list-menuside"){
						if (open === true) {
							tssClose();
							menuTouches = true;
						}
        ...
        function tssOpen() {
			...
			if(window.location.href.search("map.html") != -1){
				elLabel.style.opacity = 0;
				elLabel.style.left = "44px";
				elLabel.style.right = null;
			}
        ...
        function tssClose() {
			...
			if(window.location.href.search("map.html") != -1){
				elLabel.style.opacity = null;
				elLabel.style.right = "-44px";
				elLabel.style.left = null;
			}
        ...
        function winOnresizeEngine(event) {
            winInnerWidth = window.innerWidth;
            if(winInnerWidth < 2024 && !init){
                tssActionsEngine();
            }
            else if(winInnerWidth >= 2024 && init){
                tssClear();
            }
        }
        //------------------------------------------------------------------
        ...
        function tssActionsEngine(){
            if(!init){
				...
				touchSideSwipe.addEventListener('touchstart', closeByClick, false);
				 //Array.from(listmenuside).forEach(function(element) {
				//  element.addEventListener('click', elLabelClick, false);
				//});
