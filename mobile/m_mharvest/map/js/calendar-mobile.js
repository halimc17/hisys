...
calendar = {
	...
	makeCalendar:function(year, month, day) {
		...
		this.wrt("<div class='calendar-warp'><div class='headcalendar'>");
		this.wrt("<a class='lt' onclick='javascript:calendar.makeCalendar("+(previous_month_year)+","+(previous_month)+");' title='"+this.month_names[previous_month]+" "+(previous_month_year)+"'></a>");
		this.wrt("<div class='mid-box-date'><select name='calendar-month' class='calendar-month' onChange='calendar.makeCalendar("+year+",this.value);'>");
		...
		this.wrt("</select></div>");
		this.wrt("<a class='gt' onclick='javascript:calendar.makeCalendar("+(next_month_year)+","+(next_month)+");' title='"+this.month_names[next_month]+" "+(next_month_year)+"'></a></div>");
		this.wrt("<table border=0 cellspacing=1px><tr class='header'>");
		...
		for(var i=0;i<=5;i++) {
			...
			for(var j=0;j<7;j++) {
				...
				if(flag) {
					...
					var hariIni = parseInt(this.today.getDate());
					if(yea < 1900) yea += 1900;

					if(yea == year && this.today.getMonth() == month && this.today.getDate() == d) class_name = "today";
					if(day == d && day!=hariIni) class_name += "selected";
					
					//class_name += " " + this.weekdays[j].toLowerCase();

					this.wrt("<td class='days"+class_name+"' onclick='javascript:calendar.selectDate(\""+year+"\",\""+mon+"\",\""+w+"\")'>"+w+"</td>");
					d++;
				} else {
					this.wrt("<td class='days-null'>&nbsp;</td>");
				}
			}
			this.wrt("</tr>");
		}
		this.wrt("</table>");
		this.wrt("<a class='button-calendar' onclick='calendar.hideCalendar();'>Cancel</a><div class='clearfix'></div></div>");
	...
	showCalendar: function() {
		...
		//div.style.top=(xy[1]+25)+"px";
		var wh = (window.innerHeight-310)/2;
		div.style.left=0+"px";
		div.style.top=0+"px";
		div.style.bottom=0+"px";
		//div.style.paddingTop=wh+"px";
		div.style.right=0+"px";
		//div.style.zIndex=1000006;
		...
		document.getElementById(this.opt['calendar']).style.zIndex = 1000006;
		document.getElementById(this.opt['calendar']).style.transform = "translateY(0px)";
		document.getElementById(this.opt['calendar']).style.opacity = 1;
	...
	hideCalendar: function(instance) {
		...
		if(active_calendar_id){
			document.getElementById(active_calendar_id).style.zIndex = -1000006;
			document.getElementById(active_calendar_id).style.transform = "translateY(-100px)";
			document.getElementById(active_calendar_id).style.opacity = 0;
		}
