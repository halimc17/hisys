...
class Log_5masterbarang extends OWL_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Master_brg');
        $this->load->model('Master_brg');
    }
    ...
    function slave()
    {
        ...
        switch ($case) {
            ...
            default:
                ...
                $getData = $this->Master_brg->selectQuery();
    ...
    function Datalist($pageLimit)
    {
        ...
        $r = $this->Master_brg->selectdata($pageLimit);
        $table = "";
        if (count($r) > 0) {
            $no = 0;
            foreach ($r as $k => $v) {
                ...
                $table .= "<td align=\"left\">{$v['namabarang']}</td>";
                $table .= "</tr>";
            }
        } else {
            $table .= "<tr>";
