...
class Pmn_kontrakjual extends OWL_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Kontrak_jual');
        $this->load->model('Kontrak_jual');
    }
    ...
    function slave()
    {
        ...
        switch ($case) {
            ...
            default:
                ...
                $getData = $this->Kontrak_jual->selectQuery();
    ...
    function Datalist($pageLimit)
    {
        ...
        $thead =  "<thead>
        <tr class=\"rowheader\">
        <th align=center>Kode PT</th>	
        <th align=center>Kode Org</th>	
        <th align=center>No Kontrak</th>	
        <th align=center>No Kontrakinduk</th>	
        <th align=center>Kode Rekanan</th>	
        <th align=center>Kode Barang</th>	
        <th align=center>Satuan</th>	
        <th align=center>Franco</th>	
        ...
        $r = $this->Kontrak_jual->selectdata($pageLimit);
        $table = "";
        if (count($r) > 0) {
            // $no = 0;
            foreach ($r as $k => $v) {
                ...
                $table .= "<td align=\"left\">{$v['kodept']}</td>";
                $table .= "<td align=\"left\">{$v['kodeorg']}</td>";
                $table .= "<td align=\"left\">{$v['nokontrak']}</td>";
                $table .= "<td align=\"left\">{$v['nokontrakinduk']}</td>";
                $table .= "<td align=\"left\">{$v['koderekanan']}</td>";
                $table .= "<td align=\"left\">{$v['kodebarang']}</td>";
                $table .= "<td align=\"left\">{$v['satuan']}</td>";
                $table .= "<td align=\"left\">{$v['franco']}</td>";
                $table .= "<td align=\"left\">{$v['franconame']}</td>";
                $table .= "</tr>";
            }
        } else {
            $table .= "<tr>";
