...
class Pmn_kontrakbeli extends OWL_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Kontrak_beli');
        $this->load->model('Kontrak_beli');
    }
    ...
    function slave()
    {
        ...
        switch ($case) {
            ...
            default:
                ...
                $getData = $this->Kontrak_beli->selectQuery();
    ...
    function Datalist($pageLimit)
    {
        ...
        $r = $this->Kontrak_beli->selectdata($pageLimit);
        $table = "";
        if (count($r) > 0) {
            $no = 0;
            foreach ($r as $k => $v) {
                ...
                $table .= "<td align=\"left\">nama rekanan</td>";
                $table .= "<td align=\"center\">{$v['nokontrak']}</td>";
                $table .= "<td align=\"center\">{$v['bsart']}</td>";
                $table .= "<td align=\"center\">{$v['tanggalawal']}</td>";
                $table .= "<td align=\"center\">{$v['tanggalakhir']}</td>";
                $table .= "</tr>";
            }
        } else {
            $table .= "<tr>";
