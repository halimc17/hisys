...
class Setuptph extends OWL_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Setup_tph');
    }
    ...
    function slave()
    {
        ...
        switch ($case) {
            ...
            default:
                ...
                $where = "WHERE kodeorg like '".substr($_SESSION['empl']['lokasitugas'],0,4)."%'";
                $getData = $this->Setup_tph->getTph($where);
    ...
    function Datalist($pageLimit,$where)
    {
        ...
        $r = $this->Setup_tph->selectdata($pageLimit,$where);
        $table = "";
        if (count($r) > 0) {
            // $no = 0;
            foreach ($r as $k => $v) {
                ...
                $table .= "<td align=\"left\">{$v['kode']}</td>";
                $table .= "<td align=\"left\">{$v['keterangan']}</td>";
                $table .= "<td align=\"left\">{$v['kodeorg']}</td>";
                $table .= "<td align=\"left\">{$v['latitude']}</td>";
                $table .= "<td align=\"left\">{$v['logitude']}</td>";
                $table .= "<td align=\"left\">{$v['luas']}</td>";
                $table .= "</tr>";
            }
        } else {
            $table .= "<tr>";
