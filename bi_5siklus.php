...
<script language=javascript1.2 src='js/bi_5siklus.js'></script>

<?php
include('master_mainMenu.php');
OPEN_BOX('','<span class=judul>'.strtoupper('Business inteligent cycle setup').'</span>');

//Get No Akun
$optNoAkun = $optKegiatan = "";
$optNoAkun = "<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
$str = "select distinct(noakun) as noakun, namaakun from ".$dbname.".keu_5akun where (noakun like '126%' or noakun like '621%' or noakun like '611%') and length(noakun) > 5 order by noakun asc";
...
echo"<fieldset>
	...
	<table>
		...
		<tr>
			...
			<td>:</td>
			<td>
				<select id='noakun' onchange='getkegiatan()'>".$optNoAkun."</select>
			</td>
		</tr>
		<tr>
			...
			<td>:</td>
			<td>
				<select id='kegiatan'>".$optKegiatan."</select>
			</td>
		</tr>
		<tr>
