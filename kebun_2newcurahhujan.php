...
<script language=javascript src='js/kebun_2newcurahhujan.js?v=<?php echo time(); ?>'></script>

<?php

OPEN_BOX('','<span class=judul>'.getMenu('kebun_2newcurahhujan').'</span><br>');
...
## GET PERIODE 2
$optPeriode2="";
$str="select DATE_FORMAT(tanggal, '%Y')  as periode from ".$dbname.".kebun_curahhujan group by periode order by periode desc";
$res=fetchdata($str);
foreach($res as $val){
	$optPeriode2.="<option value='".$val['periode']."'>".$val['periode']."</option>";
}
...
<legend>HARIAN</legend>
	...
	<table cellspacing=1 cellpadding=2>
		<tr>
			...
			<td>:</td>
			<td>
				<select class=select2 id='unit2' style='width:246px' onchange=\"getsubunit2()\">".$optUnit."</select>
			</td>
		</tr>
		<tr>
			<td>Sub ".$_SESSION['lang']['unit']."</td>
			<td>:</td>
			<td>
				<select class=select2 id='subunit2' style='width:246px'>".$optSubUnit."</select>
			</td>
		</tr>
		<tr>
			<td>".$_SESSION['lang']['periode']."</td>
			<td>:</td>
			<td>
				<select class=select2 id='periode2' style='width:246px'>".$optPeriode2."</select>
			</td>
		</tr>
		<tr>
			<td colspan=2></td>
