...
<script language=javascript src='js/sdm_2laporanpotongan.js?v=<?php echo time(); ?>'></script>

<?php

OPEN_BOX('','<span class=judul>'.getMenu('sdm_2laporanpotongan').'</span><br>');
...
$str="select distinct tipepotongan from ".$dbname.".sdm_potonganht";
$res=fetchdata($str);
foreach($res as $val){
	$optKom.="<option value='".$val['tipepotongan']."'>".getNamaKomponenGaji($val['tipepotongan'])."</option>";
}
...
<fieldset style=float:left>
	<table cellspacing=1 cellpadding=2>
		...
		<tr>
			<td>Jenis ".$_SESSION['lang']['potongan']."</td>
			<td>:</td>
			<td>
				<select class=select2 id='potongan' style='width:246px'>".$optKom."</select>
			</td>
		</tr>
		<tr>
			<td colspan=2></td>
