...
if($proses=='getkodews'){
	$str = "select * from ".$dbname.".organisasi where induk='".$_SESSION['empl']['lokasitugas']."' and tipe='WORKSHOP'"; #exit("error".$str);
	$res=fetchdata($str);
	foreach($res as $bar){
...
if($proses=='getjumlahws'){
	$str="select distinct rpperjam from ".$dbname.".bgt_biaya_ws_per_jam where tahunbudget='".$param['tahunbudget']."' and kodews='".$param['kodews']."'";
	$res=fetchdata($str)[0];
	if(count($res)>0){
...
if($cekapa=='kendaraan'){
    ...
    $str="select * from ".$dbname.".bgt_biaya_jam_ken_vs_alokasi
        where tahunbudget = '".$tahunbudget."' and kodetraksi in (select kodeorganisasi from ".$dbname.".organisasi where alokasi='".$kodept."') "; //exit("error".$str);
    $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
    $res->setFetchMode(PDO::FETCH_OBJ);
...
//kamus namabarang
    ...
    $hkef.="<tr class=rowcontent>
            ...
            <td align=center>".$bar->tipebudget."</td>
            <td align=left>".$bar->kodebudget." - ".$nmkode[$bar->kodebudget]."</td>
			<td align='center'>".$bar->aruskas." - ".$nmaruskas[$bar->aruskas]."</td>
...
if($cekapa=='tab2'){
    ...
    $hkef.="<table id=container7 class=sortable cellspacing=1 border=0 >
     ...
     <tbody>";
    $str="select * from ".$dbname.".bgt_budget
        where kodebudget in ('PKSM','SERVICE') and tipebudget = '".$tipebudget."' and tahunbudget = '".$tahunbudget."' and kodeorg = '".$mesin."'";
    $res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
    $res->setFetchMode(PDO::FETCH_OBJ);
    $no=1;
    while($bar= $res->fetch())
...
$str="select a.*, b.tutup from ".$dbname.".bgt_budget_detail a
		...
		and a.kodeorg like '".substr($_SESSION['empl']['lokasitugas'],0,4)."%'  
		".$where."
		and (a.kodebudget like 'EXPL%' or a.kodebudget like 'M%' or a.kodebudget like 'TOOL%' or a.kodebudget like 'VHC%' or a.kodebudget = 'PKSM' or a.kodebudget='SERVICE') limit ".$offset.",".$limit."";
...
$hkef.="
        <tr class=rowcontent>
			<td colspan=22><button class=mybutton onclick=sebarkanall(".$ttlbrs.") title=\"Sebarkan Seluruhnya\">Sebarkan Seluruhnya</button></td>
		</tr>";
while($bar= $res->fetch()){
$nmaruskas=makeOption($dbname,'keu_5aruskas','noaruskas,nama_aruskas',"noaruskas='".$bar->aruskas."'");
$no+=1;
...
$hkef.="
		...
		<td align=center>".$bar->kodeorg." - ".$nmorg[$bar->kodeorg]."</td>
