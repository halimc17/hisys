...
$operationawal = checkPostGet('operationawal', '');
$nilaiawal = checkPostGet('nilaiawal', '');
$operationakhir = checkPostGet('operationakhir', '');
$nilaiakhir = checkPostGet('nilaiakhir', '');
$keterangan = checkPostGet('keterangan', '');

switch ($proses){
	case 'simpan':
		if($noakun == ''){
			exit("Warning : No Akun harus dipilih.");
		}
		
		if($_SESSION['mswarna'] == array()){
			exit("Warning : Master warna harus diisi.");
		}
		
		if($method == 'insert'){
			$str = "select * from ".$dbname.".bi_5siklusht where noakun='".$noakun."' and kegiatan='".$kegiatan."'";
			...
			if($kegiatan == ''){
				$str = "select * from ".$dbname.".bi_5siklusht where noakun='".$noakun."'";
				$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
				$res->setFetchMode(PDO::FETCH_ASSOC);
				$jlhbrs=owlBaris($res);
				
				if($jlhbrs <= 1){
					exit("Warning : Kegiatan untuk No Akun min harus 2 kegiatan.");
				}
			}
			
			$idlap = getID();
			$str = "insert into ".$dbname.".bi_5siklusht (idsiklus,noakun,kegiatan,updateby) values ('".$idlap."','".$noakun."','".$kegiatan."','".$_SESSION['standard']['userid']."')";
		
			try{
				$owlPDO->exec($str);
				foreach($_SESSION['mswarna'] as $row){
					$str = "insert into ".$dbname.".bi_5siklusdt (idsiklus,opawal,nilaiawal,opakhir,nilaiakhir,warna,keterangan) VALUES ('".$idlap."','".$row['operationawal']."','".$row['nilaiawal']."','".$row['operationakhir']."','".$row['nilaiakhir']."','".$row['kodefill']."','".$row['keterangan']."')";
					try{
						$owlPDO->exec($str);
			...
			$str = "delete from ".$dbname.".bi_5siklusdt where idsiklus = '".$idlap."'";
			try{
				$owlPDO->exec($str);
			}catch(PDOException $e){
				echo "error : ".$e->getMessage();
			...
			$str = "update ".$dbname.".bi_5siklusht set noakun='".$noakun."', kegiatan = '".$kegiatan."', updateby = '".$_SESSION['standard']['userid']."' where idsiklus = '".$idlap."'";
			try{
				$owlPDO->exec($str);
	...
	case 'addwarna':
		if($kodefill == ''){
			exit("Warning : Kode warna harus diisi");
		}
		...
		$newdata = array(
			'kodefill'=>$kodefill,
			'operationawal'=>$operationawal,
			'nilaiawal'=>$nilaiawal,
			'operationakhir'=>$operationakhir,
			'nilaiakhir'=>$nilaiakhir,
			'keterangan'=>$keterangan
		);
		
		if($_SESSION['mswarna'] != array()){
			foreach($_SESSION['mswarna'] as $key=>$row){
				if($row['kodefill'] == $kodefill && $row['operationawal'] == $operationawal && $row['nilaiawal'] == $nilaiawal && $row['operationakhir'] == $operationakhir && $row['nilaiakhir'] == $nilaiakhir){
					exit("Warning : Item ini sudah pernah diinput sebelumnya.");
				}
			}
			array_push($_SESSION['mswarna'],$newdata);
		}else{
			array_push($_SESSION['mswarna'],$newdata);
		}
		
		echo $kodefill."####".$operationawal."####".$nilaiawal."####".$operationakhir."####".$nilaiakhir."####".$keterangan;
		break;
		
	case 'deletewarna':
		foreach($_SESSION['mswarna'] as $key=>$row){
			if($row['kodefill'] == $kodefill && $row['operationawal'] == $operationawal && $row['nilaiawal'] == $nilaiawal && $row['operationakhir'] == $operationakhir && $row['nilaiakhir'] == $nilaiakhir){
				unset($_SESSION['mswarna'][$key]);
			}
		}
		break;
	...
	case 'loaddata':
		...
		$str = "select * from ".$dbname.".bi_5siklusht order by noakun asc, kegiatan asc";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$jlhbrs=owlBaris($res);
		...
		if($jlhbrs == 0){
			...
			while($bar = $res->fetch()){
				...
				$result .= "<tr class=rowcontent>
					...
					<td>".$bar['idsiklus']."</td>
					<td>".$bar['noakun']."-".$optNamaAkun[$bar['noakun']]."</td>
					<td>".($bar['kegiatan']=='' ? $_SESSION['lang']['all'] : $bar['kegiatan']."-".$optNamaKegiatan[$bar['kegiatan']])."</td>
					<td style='text-align:center'>
						<img src='images/skyblue/edit.png' class='resicon' title='Edit' onclick=\"fillfield('".$bar['idsiklus']."','".$bar['noakun']."','".$bar['kegiatan']."')\">
					</td>
					<td style='text-align:center'>
						<img src='images/skyblue/delete.png' class='resicon' title='Delete' onclick=\"deletefield('".$bar['idsiklus']."')\">
					</td>
					<td style='text-align:center'>
						<img src='images/skyblue/zoom.png' class='resicon' title='Detail' onclick=\"detailfield('".$bar['idsiklus']."',event)\">
					</td>
				</tr>";
			}
		}
	...
	case 'fillfield':
		$result = "";
		$_SESSION['mswarna'] = array();
		$str = "select * from ".$dbname.".bi_5siklusdt where idsiklus = '".$idlap."' order by idwarna";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$no = -1;
		while($bar = $res->fetch()){
			$no++;
			$_SESSION['mswarna'][$no]['kodefill'] = $bar['warna'];			
			$_SESSION['mswarna'][$no]['operationawal'] = $bar['opawal'];			
			$_SESSION['mswarna'][$no]['nilaiawal'] = $bar['nilaiawal'];			
			$_SESSION['mswarna'][$no]['operationakhir'] = $bar['opakhir'];			
			$_SESSION['mswarna'][$no]['nilaiakhir'] = $bar['nilaiakhir'];
			$_SESSION['mswarna'][$no]['keterangan'] = $bar['keterangan'];
			$result .= "<tr class=rowcontent>
				<td bgcolor='".$bar['warna']."'>".$bar['warna']."</td>
				<td style='text-align:center'>".$bar['opawal']."</td>
				<td style='text-align:right'>".$bar['nilaiawal']."</td>
				<td style='text-align:center'>".$bar['opakhir']."</td>
				<td style='text-align:right'>".$bar['nilaiakhir']."</td>
				<td>".$bar['keterangan']."</td>
				<td style='text-align:center'><img title='Hapus' class=resicon onclick=\"deletewarna(this,'".$bar['warna']."####".$bar['opawal']."####".$bar['nilaiawal']."####".$bar['opakhir']."####".$bar['nilaiakhir']."')\" src='images/delete_32.png'/></td>
			</tr>";
		}	
	...
	case 'deletefield':
		$str = "delete from ".$dbname.".bi_5siklusht where idsiklus = '".$idlap."'";
		try{
			$owlPDO->exec($str);
		}catch (PDOException $e){
			echo "error : ".$e->getMessage();
	...
	case 'getdetailfield':
		...
		$result = "<table style='padding:5px;' cellspacing=3 cellpading=10>
		<thead>
		<tr>
	...
	case 'getkegiatan':
		...
		$str = "select distinct(kodekegiatan) as kodekegiatan, namakegiatan, kelompok from ".$dbname.".setup_kegiatan where status='1' and noakun = '".$noakun."' order by kodekegiatan asc";
		$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while($bar = $res->fetch()){
			if($kegiatan==$bar['kodekegiatan']){
				$optsik .= "<option value='".$bar['kodekegiatan']."' selected>".$bar['kodekegiatan']." ".$bar['namakegiatan']."</option>";
			}else{
				$optsik .= "<option value='".$bar['kodekegiatan']."'>".$bar['kodekegiatan']." ".$bar['namakegiatan']."</option>";
			}
		}
...
function getID(){
	...
	$str = "select idsiklus from ".$dbname.".bi_5siklusht order by idsiklus DESC LIMIT 1";
	$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
	$res->setFetchMode(PDO::FETCH_ASSOC);
	$numrow=owlBaris($res);
	...
	if($numrow == 0){
		...
		$bar = $res->fetch();
		$expNo = explode('SIK',$bar['idsiklus']);
		$idno = 'SIK'.addZero(intval($expNo[1]) + 1,7);
	}
