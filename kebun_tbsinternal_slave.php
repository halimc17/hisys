...
switch ($method) {
	...
	case'excel2':
		...
		$tab.="<tr>";
		if($noreferensi==''){
			$tab.="<td align=center colspan=7><b>PEMBAYARAN TBS INTI ".$namasupplier." PERIODE TGL : ".tanggalnormal($tanggaltbs1)." s/d ".tanggalnormal($tanggaltbs2)." </td>"; 	
		}else{
			$tab.="<td align=center colspan=7><b>PEMBAYARAN TBS PETANI ".$namasupplier." PERIODE TGL : ".tanggalnormal($tanggaltbs1)." s/d ".tanggalnormal($tanggaltbs2)." </td>"; 	
		}
		...
		$tab.="<table width=100% cellpadding=".$cellpadding." cellspacing=0 border=1 style='font-size:12px'>";
		$tab.="<tr bgcolor=lightgray>";
			$tab.="<td style='text-align:center;width:10px;'><b>".$_SESSION['lang']['nourut']."</td>"; 
	...
	case'pdf2':
		...
		*/	
		...
		//exit($tab);
		$tab.=getketeranganttd($table,'pdf',$unit);	
			$dompdf = new Dompdf();
		$dompdf->loadHtml($tab);
		$dompdf->setPaper('A4', 'potrait');
		$dompdf->render();
	...
	case 'persetujuan':
        ...
        $str = "UPDATE ".$dbname.".".$table." set posting = 9, postingby = '".$_SESSION['standard']['userid']."'
                WHERE notransaksi = '".$notransaksi."'";
        try{
            $owlPDO->exec($str); 
        }catch(PDOException $e){
        ...
        for($i=1; $i<=$maxaproval; $i++){
            ...
            try{
                $owlPDO->exec($str); 
               	...
               	$str = "UPDATE ".$dbname.".".$table." set posting = 0
                		WHERE notransaksi = '".$notransaksi."'";
                try{
                    $owlPDO->exec($str); 
                }catch(PDOException $e){
	...
	*/
	...
	case'notransaksi':
		...
		$str = "select count(*) as jumlah,notransaksi from ".$dbname.".".$table." 
				where unit='".$unit."' and substr(tanggaltbs1,1,10)='".$tanggaltbs1."' and  substr(tanggaltbs2,1,10)='".$tanggaltbs2."' and divisi='".$divisi."'";	
		...
		$notransaksi = generatenotransaksitbsinternal();	
        ...
        while ($bar = $res->fetch()){
			...
			$pg = $page + 1;
				...
				if($bar['posting']==0){
					...
					$tab.="<td align=center width=25px><img src=images/icons/04/16/01.png class=resicon class=zImgBtn height='30'  title='Posting' onclick=\"posting('".$bar['notransaksi']."','".$pg."');\"></td>";	
				...
				$tab.="<td align=center width=25px><img src=images/excel.jpg class=resicon  caption='excel'  title='Print Rekap  ".$bar['notransaksi']."' onclick=\"excel2('".$bar['notransaksi']."');\">";	
	...
	case'loaddatadt':
		...
		if($datatransaksi==0){
			#= harga tbs
			// $str = "select * from ".$dbname.".pmn_hargabelitbs  where supplierid='".$divisi."'  and tanggal='".$tanggaltbs1."' 
			$str = "select * from ".$dbname.".pmn_hargabelitbs  where supplierid='".$divisi."'  and tanggal like '".$tanggaltbs1."%' 
				and kodeorg='".$unit."'";
				// echo $str;
			$res = $owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
