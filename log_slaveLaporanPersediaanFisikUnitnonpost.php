...
if ($proses != 'getGudang') {
    ...
    if ($gudang != '') {
        ...
        $wherex.="and kodegudang like '" . $gudang . "%'";
    } else {
        exit("Error : Gudang Tidak Boleh Kosong");
    }
    if ($periode != '') {
        $where.=" and periode='" . $periode . "'";
        $wherex.=" and tanggal like '" . $periode . "%'";
    }
    if ($proses == 'excel') {
    ...
    $sData = "select * from " . $dbname . ".log_transaksi_vw a 
    where kodegudang!='' " . $wherex . " and post='0' ".$wh."";
    $qData = $owlPDO->query($sData) or die(print " Gagal: " . PDOException::getMessage());
    $qData->setFetchMode(PDO::FETCH_ASSOC);
    while ($rData = $qData->fetch()) {
        $dtPeriode[substr($rData['tanggal'],0,7)] = substr($rData['tanggal'],0,7);
        $lstKdBrg[$rData['kodebarang']] = $rData['kodebarang'];
        if($rData['tipetransaksi']=='1' or $rData['tipetransaksi']=='3' or $rData['tipetransaksi']=='6'){
            $dtMasuk[substr($rData['tanggal'],0,7) . $rData['kodebarang']] += $rData['jumlah'];
            $dtAkhir[substr($rData['tanggal'],0,7) . $rData['kodebarang']] += $rData['jumlah'];
        }
        if($rData['tipetransaksi']=='2' or $rData['tipetransaksi']=='5' or $rData['tipetransaksi']=='7'){
            $dtKeluar[substr($rData['tanggal'],0,7) . $rData['kodebarang']] += $rData['jumlah'];
            $dtAkhir[substr($rData['tanggal'],0,7) . $rData['kodebarang']] = $dtAkhir[substr($rData['tanggal'],0,7) . $rData['kodebarang']]-$rData['jumlah'];
        ...
        foreach ($lstKdBrg as $dtBrg) {
            if (!empty($dtKdBarang[$dtIsi][$dtBrg])) {
				...
				if($showhide=='show' && $showhide2=='show' && $showhide3=='show' && $showhide4=='show'){
					...
					$tab.="<td align=right ".$pdf.">".hidezerodecimal($vstokmin,2)."</td>";
