...
$jenispanen    = checkPostGet('jenispanen', '');


$karyawanid    = checkPostGet('karyawanid', '');
$jjgpanenbesar = checkPostGet('jjgpanenbesar', '');
$jjgpanenkecil = checkPostGet('jjgpanenkecil', '');
...
$luasblok      = checkPostGet('luasblok', '');
...
@$param['jjgpanenbesar']  = str_replace(",", "", $param['jjgpanenbesar']);
@$param['jjgpanenkecil']  = str_replace(",", "", $param['jjgpanenkecil']);
...
$whereKary .= " and subbagian in (select kodeorganisasi from " . $dbname . ".organisasi where  tipe = 'AFDELING')";

$whereKary .= " and tipekaryawan !=0 and (tanggalkeluar = '0000-00-00' or tanggalkeluar>= '" . tanggalsystemn($tgl) . "')";
...
foreach ($res as $bar) {
	...
	// kalau subbagian berubah
	if ($d != $n) {
		// kalau bukan pertama kali, tutup dulu optgroup lama
		if ($n != '') {
			$optKary .= "</optgroup>";
		}
...
if ($filterdivisi != '') {
	...
	$whereBlok .= " and (" . date("Y") . " - b.tahuntanam >= 3 or statusblok='TM')";
} else {
	@$whereBlok = " and substr(a.kodeorganisasi,1,4) ='" . $kodeorg . "'";
	$whereBlok .= " and (" . date("Y") . " - b.tahuntanam >= 3 or statusblok='TM') and substr(a.indukblok,1,4)='" . $kodeorg . "'";
}

$optBlok = "<option value=''>" . $_SESSION['lang']['pilihdata'] . "</option>";
$str = "select distinct a.indukblok, a.namaindukblok from " . $dbname . ".organisasi a 
			left join " . $dbname . ".setup_blok b on a.indukblok=b.indukblok
			where a.tipe='BLOK' " . $whereBlok . " order by substr(a.kodeorganisasi,1,6), a.kodeorganisasi asc";
$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_ASSOC);
$m = ''; // penanda awal kosong
...
switch ($method) {
	case 'detail':
		// Kodeorg harus sama dengan lokasi tugas nya untuk mode edit
		if ($mode == 'edit') {
			$sCek = selectQuery($dbname, "kebun_aktifitas", "kodeorg", "notransaksi='" . $notransaksi . "'");
			$rCek = fetchData($sCek);
			$unitCek = $rCek[0]['kodeorg'];

			if ($unitCek != $_SESSION['empl']['lokasitugas']) {
				exit("Warning: Kode organisasi di transaksi dengan lokasi tugas user tidak sama.<br>
				Silakan pindah lokasi tugas ke <b>" . $unitCek . " (" . getNamaOrg($unitCek) . ")</b> terlebih dahulu.");
			}

			if ($kodeorg == '' || $tgl == '') {
				exit("Warning: Tanggal dan atau Kode Organisasi harus di isi !");
			}

			if ($mandor == '') {
				exit("Warning: Mandor harus di isi !");
			}
		}
		...
		if ($tgl < '2021-12-01') {
			exit("Warning : Menu ini hanya bisa digunakan untuk transaksi tanggal 01 Desember 2021 ke atas.");
		}
		...
		if ($mode == 'edit') {
			...
			cekmaxnilaihk($mandor, $tgl, '1', '0', 'new', $exit = '1');
			cekmaxnilaihk($mandor1, $tgl, '1', '0', 'new', $exit = '1');
			cekmaxnilaihk($kerani, $tgl, '1', '0', 'new', $exit = '1');

			$str = "update " . $dbname . ".kebun_aktifitas set `nikmandor`='" . $mandor . "', `nikmandor1`='" . $mandor1 . "',`nikasisten`='" . $kerani . "', divisi='" . $param['divisi'] . "', `kontanan`='" . $kontan . "', `jenispanen`='" . $jenispanen . "' where `notransaksi`='" . $notransaksi . "'";
			try {
				$owlPDO->exec($str);
			...
			#validasi maksimal HK BHL
			cekmaxnilaihk($mandor, $tgl, '1', '1', 'edit', $exit = '1');
			cekmaxnilaihk($mandor1, $tgl, '1', '1', 'edit', $exit = '1');
			cekmaxnilaihk($kerani, $tgl, '1', '1', 'edit', $exit = '1');
			...
			$str = "insert into " . $dbname . ".kebun_aktifitas (`notransaksi`, `tipetransaksi`, `tanggal`, `nobkm`, `kodeorg`, `nikmandor`, `nikmandor1`, `nikasisten`, `keranimuat`, `jurnal`, `nospk`, `kontanan`, `updateby`, `divisi`,`jenispanen`)
			values ('" . $notransaksi . "','PNN','" . $tgl . "','" . $notransaksi . "','" . $kodeorg . "','" . $mandor . "','" . $mandor1 . "','" . $kerani . "','','0',null,'" . $kontan . "','" . $_SESSION['standard']['userid'] . "', '" . $param['divisi'] . "','" . $jenispanen . "')";
			try {
				$owlPDO->exec($str);
			} catch (PDOException $e) {
				print " Gagal  !: " . $e->getMessage() . "\n";
		...
		$arrDivisi = [];
		$str = "select namaorganisasi,kodeorganisasi from " . $dbname . ".organisasi where length(kodeorganisasi)=6 and tipe='AFDELING' and (kodeorganisasi like '" . $kodeorg . "%' OR induk IN (".getOrgDetail(2)."))";
		...
		while ($res = $resstr->fetch()) {
			if (!isset($arrDivisi[$res['kodeorganisasi']])) {
				$arrDivisi[$res['kodeorganisasi']] = $res['namaorganisasi'];
		...
		$str = "select * from " . $dbname . ".kebun_5asistensi where kodeorgtujuan ='" . $param['kodeorg'] . "' and divisitujuan='" . $param['divisi'] . "' and tanggal<='" . tanggalsystemn($param['tgl']) . "' and tanggalsampai>='" . tanggalsystemn($param['tgl']) . "' and posting='1' and (tipetrans='PNN' or tipetrans='')";
		$resx = fetchdata($str);
		foreach ($resx as $res) {
			...
			if (!isset($arrDivisi[$res['divisiasal']])) {
				$arrDivisi[$res['divisiasal']] = $nmorg[$res['divisiasal']];
		...
		$tab .= "<th align=center " . $rows . " width=20px>No</th>
				...
				<th align=center colspan=4>" . $_SESSION['lang']['blok'] . "</th>
				<th align=center " . $rows . "><font color=red><b>* </font></b> Sesi</th>
			...
			<tr class=rowheader height=25px>
				<th align=center colspan=2><font color=red><b>* </font></b>" . $_SESSION['lang']['nomor'] . "</th>
				<th align=center><font color=red><b>* </font></b> TPH</th>
				<th align=center>Luas</th>

				<th align=center width=45px><font color=red><b>* </font></b>" . $_SESSION['lang']['ha'] . "</th>
				<th align=center width=45px>" . $_SESSION['lang']['jjg'] . " Basis Besar</th>
				<th align=center width=45px>" . $_SESSION['lang']['jjg'] . " Basis Kecil</th>
				<th align=center width=45px>" . $_SESSION['lang']['total'] . " " . $_SESSION['lang']['jjg'] . "</th>
	...
	case 'inputdetail':
		...
		$optnospb = "<option value=''>" . $_SESSION['lang']['all'] . "</option>";
		$where = " and blok like '" . $param['kodeorg'] . "%'";
		$where .= " and tanggalpanen = '" . tanggalsystemn($tgl) . "'";
		$str = "select distinct nospb from " . $dbname . ".kebun_spbdt where 1=1 " . $where . " order by nospb asc "; #exit("error".$str);
		$res = fetchdata($str);
		foreach ($res as $bar) {
			$s = "";
			if ($param['nospb'] == $bar['nospb']) {
				$s = "selected";
			}
			$optnospb .= "<option value=" . $bar['nospb'] . " " . $s . ">" . $bar['nospb'] . "</option>";
		}

		$optTpBuah = "<option value=''>" . $_SESSION['lang']['pilihdata'] . "</option>";
		$arrTpBuah = getEnum($dbname, "kebun_5basispanen2", "tipebuah");
		foreach ($arrTpBuah as $buah) {
			$optTpBuah .= "<option value='" . $buah . "'>" . $buah . "</option>";
		}

		$optSesi = "<option value=''>" . $_SESSION['lang']['pilihdata'] . "</option>";
		for ($i = 1; $i <= 10; $i++) {
			$optSesi .= "<option value='" . $i . "'>" . $i . "</option>";
		}
		$optTph = "<option value=''>" . $_SESSION['lang']['pilihdata'] . "</option>";
		...
		echo "<td id=no align=center>1</td>
			...
			<td colspan=2><select style=width:120px onchange=getDataDetail() id=blok class=select2>" . $optBlok . "</select></td>
			<td><select id=notph class=select2>" . $optTph . "</select></td>
			<td><input class=myinputtextnumber style=width:50px id=luasblok disabled></td>
			<td><select id=sesi class=select2>" . $optSesi . "</select></td>
			...
			<td><input id=jjgpanenbesar onkeyup=\"getTotJjg()\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
			<td><input id=jjgpanenkecil onkeyup=\"getTotJjg()\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
			<td><input id=jjgpanen disabled onkeyup=\"getDataDetail()\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
	...
	case 'getdatamandor':
		...
		$whereKary = '';
		$whereKary = " and statuskaryawan != 'Keluar' and (tanggalkeluar = '0000-00-00' or tanggalkeluar>= '" . tanggalsystemn($tgl) . "')";
		...
		while ($bar = $res->fetch()) {
			...
			<td colspan=2><select style=width:120px onchange=getDataDetail(" . $no . ") id=blok" . $no . " class=select2>" . $optBlok . "</select></td>
			<td><select id=notph" . $no . " class=select2>" . $optTph . "</select></td>
			<td><input class=myinputtextnumber style=width:50px id=luasblok" . $no . " disabled></td>
			<td><select id=sesi" . $no . " class=select2>" . $optSesi . "</select></td>
			<td><input id=hapanen" . $no . " onkeyup=\"z.numberFormat('hapanen',2)\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:40px;\"></td>
			<td><input id=jjgpanenbesar" . $no . " onkeyup=\"getTotJjg(" . $no . ")\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
			<td><input id=jjgpanenkecil" . $no . " onkeyup=\"getTotJjg(" . $no . ")\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
			<td><input id=jjgpanen" . $no . " disabled onkeyup=\"getDataDetail()\" nkeypress=\"return_tanpa_kutip(event);\" class=myinputtextnumber onkeypress=\"return angka_doang(event);\" style=\"width:60px;\"></td>
			...
			<input style=display:none id=kodeiddenda" . $no . " value='" . implode("##", $iddendapnn) . "'>";
	...
	case 'loaddatadetail':
		...
		$tab = "<table id=tabledt cellpadding=5 cellspacing=1 border=0 class=sortable>
			...
			<th align=center colspan=3>" . $_SESSION['lang']['blok'] . "</th>
			<th align=center " . $rows . " width=30px>Sesi</th>
		...
		<tr>
			...
			<th align=center width=50px>Luas</th>

			<th align=center>" . $_SESSION['lang']['ha'] . "</th>
			<th align=center>" . $_SESSION['lang']['jjg'] . " Basis Besar</th>
			<th align=center>" . $_SESSION['lang']['jjg'] . " Basis Kecil</th>
		...
		if ($row == 0) {
			...
			while ($bar = $res->fetch()) {
				...
				$nmorg = makeOption($dbname, 'organisasi', 'indukblok,namaindukblok', "indukblok='" . $bar['kodeorg'] . "'");
				// $strluas = "SELECT SUM(luasareaproduktif) AS luasareaproduktif, indukblok FROM ".$dbname.".setup_blok WHERE indukblok='".$bar['kodeorg']."' AND statusblok='TM'";
				$strluas = "SELECT SUM(luasareaproduktif) AS luasareaproduktif, indukblok FROM " . $dbname . ".setup_blok WHERE indukblok='" . $bar['kodeorg'] . "' AND (" . date("Y") . " - tahuntanam >= 3 or statusblok='TM') AND status='A'";
				$resluas = fetchdata($strluas);
				foreach ($resluas as $val) {
					$luaswblok = $val['luasareaproduktif'];
				}
				...
				$tab .= "<td align=right>" . $bar['tph'] . "</td>";
				$tab .= "<td align=right>" . $luaswblok . " </td>";
				$tab .= "<td align=right>" . $bar['sesi'] . "</td>";
				$tab .= "<td hidden align=center>" . $bar['keterangan'] . "</td>";
				$tab .= "<td align=right>" . @hidezerodecimal($bar['luaspanen'], 2) . "</td>";
				$tab .= "<td align=right>" . @hidezerodecimal($bar['jjgbuahbesar'], 2) . "</td>";
				$tab .= "<td align=right>" . @hidezerodecimal($bar['jjgbuahkecil'], 2) . "</td>";
				$tab .= "<td align=right>" . @hidezerodecimal($bar['hasilkerja'], 2) . "</td>";
				$tab .= "<td align=right>" . @hidezerodecimal($bar['brondolan'], 2) . "</td>";
				...
				$tab .= "<img src=images/application/application_edit.png class=zImgBtn  title='Edit' 
					onclick=\"editdetail('" . $bar['notransaksi'] . "','" . $bar['nik'] . "','" . $bar['kodeorg'] . "','" . $bar['luaspanen'] . "','" . $bar['jjgbuahbesar'] . "','" . $bar['jjgbuahkecil'] . "','" . $bar['hasilkerja'] . "','" . $bar['brondolan'] . "','" . $bar['upahkerja'] . "','" . $bar['upahpremi'] . "','" . $bar['upahpremilebihbasis'] . "','" . $bar['rupiahpenalty'] . "','" . $bar['keterangan'] . "','" . $bar['tph'] . "','" . $bar['sesi'] . "','" . $no . "','" . $jlhdenda . "','" . $edit . "');\" >
					</td>
					<td align=center width=25px>
					<img src=images/application/application_delete.png class=zImgBtn  title='Delete' 
					onclick=\"deletedetail('" . $bar['notransaksi'] . "','" . $bar['nik'] . "','" . $bar['kodeorg'] . "','" . $bar['tph'] . "');\" >
			...
			$tab .= "<td align=right>" . @hidezerodecimal(@$tjjgbesar, 2) . "</td>";
			$tab .= "<td align=right>" . @hidezerodecimal($tjjgkecil, 2) . "</td>";
			$tab .= "<td bgcolor=#A3E4D7 align=right><b>" . @hidezerodecimal($tjjg, 2) . "</b></td>";
			$tab .= "<td bgcolor=#A3E4D7 align=right><b>" . @hidezerodecimal($tbrd, 2) . "</b></td>";
			...
			while ($bar = $res->fetch()) {
				...
				$tab .= "<td align=right>" . hidezerodecimal($bar['jjgbuahkecil'], 2) . "</td>";
				$tab .= "<td align=right>" . hidezerodecimal($bar['hasilkerja'], 2) . "</td>";
				$tab .= "<td align=right>" . hidezerodecimal($bar['brd'], 2) . "</td>";
	...
	case 'getDataDetail':
		...
		#============================= BJR Per Blok ===========================
		#=============================== Get UMR ==============================
		$str = "select sum(jumlah) as nilai from " . $dbname . ".sdm_5gajipokok where karyawanid='" . $karyawanid . "' and tahun='" . $perLalu . "' and idkomponen in (1)";
		// exit('error'.$str);
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		$Umr = $res->fetch();
	...
	*/
		...
		#=============================== Get TPH ===============================
		// $optTph="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
		$str = "SELECT kode FROM $dbname.kebun_5tph WHERE kodeorg like '" . $blok . "'";
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()) {
			$s = "";
			if ($param['notph'] == $bar['kode']) {
				$s = "selected";
			}
			$optTph .= "<option value='" . $bar['kode'] . "' " . $s . ">" . $bar['kode'] . "</option>";
		}
		#=============================== Get TPH ===============================

		#=============================== Get Luas Blok ===============================
		$luasblok = '0';
		// $str = "SELECT SUM(luasareaproduktif) as luasareaproduktif FROM $dbname.setup_blok WHERE indukblok='".$blok."' and statusblok='TM'"; 
		$str = "SELECT SUM(luasareaproduktif) as luasareaproduktif FROM $dbname.setup_blok WHERE indukblok='" . $blok . "' and (" . date("Y") . " - tahuntanam >= 3 or statusblok='TM') AND status='A'";
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()) {
		...
		// }
		// $optnospb.="<option value=''>&nbsp;</option>";
		// $optnospb.="<option value='SPB_Sementara_01'>SPB Sementara 01</option>";
		// $optnospb.="<option value='SPB_Sementara_02'>SPB Sementara 02</option>";


		echo $umrHarian . "######" . $rpnn . "######" . $rpsatuan . "######" . $optTph . "######" . $luasblok;
		break;
	...
	// case 'getTotJjg':
	// 	$resultjjg = ($param['jjgpanenbesar'] + $param['jjgpanenkecil']);
	...
	case 'insert':
		...
		if ($param['karyawanid'] != '' and $param['blok'] != '' and ($param['hapanen'] != '' or $param['hapanen'] != '0') and ($param['jjgpanen'] != '' or $param['jjgpanen'] != '0') and ($param['brdpanen'] != '' or $param['brdpanen'] != '0')) {
			...
			try {
				...
				// Cek Apakah TPH Ada
				if ($param['notph'] == "") {
					throw new PDOException("TPH Wajib Diisi");
				}

				// Cek Apakah Sesi Ada
				if ($param['sesi'] == "") {
					throw new PDOException("Sesi Wajib Diisi");
				}

				// Cek Apakah JJG & Brondolan Kosong
				if (($param['jjgpanen'] == 0 || $param['jjgpanen'] == '') && ($param['brdpanen'] == 0 || $param['brdpanen'] == '')) {
					throw new PDOException("Hasil Kerja Jjg atau Brondolan Wajib Diisi");
				}

				# Hapus dulu data yang lama
				if ($param['modedt'] == 'edit') {

					$str = "delete from " . $dbname . ".kebun_prestasi where notransaksi='" . $notransaksi . "' and nik='" . $param['karyawanid'] . "' and kodeorg='" . $param['blok'] . "' and tph='" . $param['notphold'] . "' and sesi = '" . $param['sesi'] . "'";
					$owlPDO->exec($str);
					...
					if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
						...
						## Hapus Proporsi
						$str = "delete from " . $dbname . ".kebun_proporsitahuntanam where notransaksi ='" . $notransaksi . "' and karyawanid ='" . $param['karyawanid'] . "'  and kodeorg='" . $param['blok'] . "' ";
						$owlPDO->exec($str);
					...
					$str = "delete from " . $dbname . ".kebun_prestasi where notransaksi='" . $notransaksi . "' and nik='" . $param['karyawanid'] . "' and kodeorg='" . $param['blok'] . "' and tph='" . $param['notph'] . "' and sesi = '" . $param['sesi'] . "'";
					$owlPDO->exec($str);
				...
				if (getKary($param['karyawanid'], 'tipekaryawan') == 4) {
					cekmaxnilaihk($param['karyawanid'], $arrtgl[$param['notransaksi']], '1', '0', 'new', $exit = '0');
				}
				...
				$data = array(
					...
					'jjgbuahbesar'       => $param['jjgpanenbesar'],
					'jjgbuahkecil'       => $param['jjgpanenkecil'],
	...
	case 'delete':
		try {
			...
			if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
				$str = "delete from " . $dbname . ".kebun_proporsitahuntanam where notransaksi='" . $notransaksi . "'";
				$owlPDO->exec($str);
			}

			## Hapus seluruh detail prestasi BKM panen
			$str = "delete from " . $dbname . ".kebun_prestasi where notransaksi='" . $notransaksi . "'";
			$owlPDO->exec($str);

			## Terakhir hapus header BKM panen
	...
	case 'deletedetail':
		...
		if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
			...
			## Hapus Proporsi
			$str = "delete from " . $dbname . ".kebun_proporsitahuntanam where notransaksi ='" . $notransaksi . "' and karyawanid ='" . $karyawanid . "'  and kodeorg='" . $blok . "'";
			try {
				$owlPDO->exec($str);
			} catch (PDOException $e) {
				print " Gagal  !: " . $e->getMessage() . "\n";
		...
		$str = "delete from " . $dbname . ".kebun_prestasi where notransaksi ='" . $notransaksi . "' and kodeorg='" . $blok . "' and nik='" . $karyawanid . "'  and tph='" . $param['notph'] . "'";
		try {
			$owlPDO->exec($str);
		} catch (PDOException $e) {
			print " Gagal  !: " . $e->getMessage() . "\n";
	...
	case 'loaddata':
		...
		if ($param['nospb'] != '') {
			$where .= " and b.nospb like '%" . $param['nospb'] . "%' ";
		}

		if ($param['upload'] == '1') {
			$where .= " and a.notransaksi in (select notransaksi from " . $dbname . ".listfileupload where kriteriaefil='PNN')";
		}
		if ($param['upload'] == '0') {
			$where .= " and a.notransaksi not in (select notransaksi from " . $dbname . ".listfileupload where kriteriaefil='PNN')";
		}
		...
		$fileup = array();
		$sql = "select distinct(notransaksi) as notransaksi from " . $dbname . ".listfileupload where 1=1 and notransaksi like '%" . $_SESSION['empl']['lokasitugas'] . "%' and notransaksi like '" . str_replace("-", "", $periodesch) . "%' and kriteriaefil='PNN'";
		$res = fetchdata($sql);
		foreach ($res as $bar) {
		...
		if ($_SERVER['SERVER_ADDR'] == $_SERVER['SERVER_NAME']) {
			// Jika URI yang array [1] panjang string <= 7, Maka munculkan
			if (strlen($expri[1]) <= 7) {
				$url = $_SERVER['HTTP_ORIGIN'] . '/' . $expri[1] . '/mobile/index.php/api/module/mharvest/getHeader/send';
			} else {
				$url = $_SERVER['HTTP_ORIGIN'] . '/mobile/index.php/api/module/mharvest/getHeader/send';
			}
		} else {
			// Jika Server local / localhost maka munculkan URL localhost
			$url = $urlocal . 'mobile/index.php/api/module/mharvest/getHeader/send';
		}
		...
		foreach ($data->response['result']['data'] as $key => $val) {
			...
			$tab .= "<td colspan=8 align=center>
			<img src=images/skyblue/zoom.png class=zImgBtn class=zImgBtn height='30' title='Preview Data' onclick=\"previewData('" . $val['notransaksi'] . "','PNN')\">
			</td>";
			$tab .= "</tr>";
		}
		...
		$res = fetchdata($sql);
		$jlhbrs = $res[0]['notr'] + count($data->response['result']);
		if ($jlhbrs == 0) {
			$tab .= "<tr class=rowcontent>";
		...
		foreach ($res as $bar) {
			...
			$tab .= "<td align=center hidden>" . $bar['noreferensi'] . "</td>";
			$tab .= "<td align=center hidden>" . $bar['deviceid'] . "</td>";
			...
			if ($bar['jurnal'] == 0) {
				$isi .= "<td align=center><img src=images/application/application_edit.png class=zImgBtn  title='Edit' 
					onclick=\"edit('" . $bar['notransaksi'] . "','" . tanggalnormal($bar['tanggal']) . "','" . $bar['kodeorg'] . "','" . $bar['nobkm'] . "','" . $bar['nikmandor'] . "',
					'" . $bar['nikmandor1'] . "','" . $bar['nikasisten'] . "','" . $no . "','" . $bar['divisi'] . "','" . $bar['kontanan'] . "','" . $bar['jenispanen'] . "');\" ></td>";
				...
				$isi .= "<td></td><td></td>";
				if (in_array($_SESSION['empl']['jabatan'], $jab)) {
					$isi .= "<td align=center><img src=images/icons/04/16/04.png class=zImgBtn class=zImgBtn height='30'  title='unposting' onclick=\"unpostingData('" . $bar['notransaksi'] . "','" . $no . "');\" ></td>";
				} else {
			...
			if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
				...
				if ($flagsimpan[$bar['notransaksi']] == '1') {
					$st = "Saved";
				...
				$isi .= "<td " . $stylex . " align=center><img src='images/plus.png' class=zImgBtn class=zImgBtn height='30'  title='Proporsi JJG' onclick=\"proporsitahuntanam('" . $bar['notransaksi'] . "','" . $no . "','event','PNN');\" ><br> <span > <b> " . $st . " </b> </span></td>";
			} else {
				$isi .= "<td></td>";
			}
			...
			$isi .= "<td align=center><img src=images/skyblue/zoom.png class=zImgBtn class=zImgBtn height='30'  title='Print Data Detail' onclick=\"detailData('" . $bar['notransaksi'] . "','" . $no . "','event','PNN','" . $bar['jenispanen'] . "');\" ></td>";
		...
		$footd = "";
		$footd .= "</tr><tr><td colspan=23 align=center>";
	...
	case 'loaddataexcel':
		...
		$str = "SELECT a.*, substr(b.kodeorg,1,6) as divisipres, sum(b.hasilkerja) as jjg, b.pekerjaanpremi, b.keterangan  FROM " . $dbname . ".kebun_aktifitas a left join " . $dbname . ".kebun_prestasi b on a.notransaksi=b.notransaksi where 1=1 and a.tipetransaksi='PNN' and ((a.noreferensi !='' and a.deviceid !='') or ((a.noreferensi ='' or a.noreferensi is null) and a.deviceid is null)) " . $where . " group by a.notransaksi order by a.notransaksi desc";
		$res = fetchdata($str);
		foreach ($res as $bar) {
			$isi = $cl = '';
			$no += 1;
		...
		$tab .= "
			...
			<table class='sortable' cellspacing='1' cellpadding=5 border='0' width=100%>
				...
				<tr class=rowheader>
					...
					<td align='center' colspan=2 width=20px>Action</td>
				</tr>
				</thead>
				<tbody id='listfiles'>
				</tbody>
	...
	case 'getbasispnn':
		...
		$tab .= "<table class=sortable cellpadding=5 cellspacing=1 border=0 >
                <thead>
					<tr class=rowheader>
						...
						<th align=center rowspan=2 width=50px>" . $_SESSION['lang']['tahun'] . "</th>
						<th align=center rowspan=2 width=50px>" . $_SESSION['lang']['tahuntanam'] . "</th>
						<th align=center rowspan=2 width=50px>" . $_SESSION['lang']['jenis'] . "</th>
						<th align=center rowspan=2>" . $_SESSION['lang']['norma'] . "</th>
						<th align=center rowspan=2>" . $_SESSION['lang']['premlebihbasis'] . " Rp/Kg</th>
						<th align=center rowspan=2>" . $_SESSION['lang']['topografi'] . "</th>
						<th align=center rowspan=2>Premi Kehadiran</th>
		...
		$str = "SELECT *  FROM " . $dbname . ".kebun_5basispanen2 where 1=1 and afdeling='" . $kodeorg . "' and tahun=(select max(tahun) as tahun FROM " . $dbname . ".kebun_5basispanen2 where 1=1 and afdeling='" . $kodeorg . "') order by tahuntanam asc";
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()) {
			$no += 1;
	...
	case 'posting':
		...
		## Selain palma harus input proporsi panen dulu
		if (getindukPT($_SESSION['empl']['lokasitugas']) != 'PPP') {
			$strx = "select * from " . $dbname . ".kebun_proporsitahuntanam where notransaksi='" . $param['notransaksi'] . "' and flag = 1";
			$resx = fetchdata($strx);
			if (count($resx) == 0) {
				exit("Warning : Proporsi panen belum ada, silahkan input proporsi panen terlebih dahulu");
			}

			## Cek proporsi nya udah sama atau belum
			$strx = "select * from " . $dbname . ".kebun_prestasi_vw where notransaksi='" . $param['notransaksi'] . "' order by karyawanid asc";
			$resx = fetchdata($strx);
			foreach ($resx as $valx) {

				$totJjgPro += $valx['hasilkerja'];
				$totBrdPro += $valx['brondolan'];
			}


			$strx = "select * from " . $dbname . ".kebun_proporsitahuntanam where notransaksi='" . $param['notransaksi'] . "'";
			$resx = fetchdata($strx);
			foreach ($resx as $valx) {
				$totJjgProx += $valx['jjg'];
				$totBrdProx += $valx['brondolan'];
			}

			if (round($totJjgPro, 2) != round($totJjgProx, 2)) {
				exit("Warning : JJG proporsi : " . number_format($totJjgProx) . " tidak sama dengan kegiatan panen : " . number_format($totJjgPro) . ", cek lagi proporsi pemanen nya ");
			}

			if (round($totBrdPro, 2) != round($totBrdProx, 2)) {
				exit("Warning : BRONDOLAN proporsi :" . number_format($totBrdProx, 2) . " tidak sama dengan kegiatan panen : " . number_format($totBrdPro, 2) . ", cek lagi proporsi pemanen nya ");
			}
		}
		...
		try {
			...
			#validasi posting
			// validasiInput($dataH[0]['kodeorg'],$dataH[0]['divisi'],'PNNPOST',$dataH[0]['tanggal'],$exit='1');
			...
			if ($statusfp == 1) {
				validasifp($tipevalidasi, $detval, 'PNN', $arrUpload, $dataH[0]['tanggal'], '1');
			} else {
				if ($statusfp == '') {
			...
			$kodeBlok = array();
			$query808 = "select notransaksi,nobkm,nik,nikpemel,kodekegiatan,kodeorg,tahuntanam,sum(hasilkerja) as hasilkerja,sum(hasilkerjakg) as hasilkerjakg,
			sum(jumlahhk) as jumlahhk,bjr,sum(norma) as norma,sum(outputminimal) as outputminimal,sum(upahkerja) as upahkerja,sum(upahpenalty) as upahpenalty,
			sum(upahpremi) as upahpremi, sum(premibasis) as premibasis, sum(premibasis2) as premibasis2,sum(upahpremilebihbasis) as upahpremilebihbasis,
			sum(upahpremilebihbasis2) as upahpremilebihbasis2,sum(premibrondol) as premibrondol,sum(umr) as umr,statusblok,sum(pekerjaanpremi) as pekerjaanpremi,
			sum(rupiahpenalty) as rupiahpenalty,sum(luaspanen) as luaspanen,kodesegment,noreferensi,sum(brondolan) as brondolan,sum(jjgpenalty) as jjgpenalty,
			keterangan from " . $dbname . ".kebun_prestasi where notransaksi='" . $param['notransaksi'] . "' group by kodeorg";
			...
			foreach ($res808 as $row) {
				...
				$tersedia = $terkirim = 0;
				$s = "select sum(jjg) as jjgkirim from " . $dbname . ".kebun_spb_vw where  blok='" . $row['kodeorg'] . "' and tanggalpanen<='" . $dataH[0]['tanggal'] . "'";
				$b = fetchdata($s);
				...
				// Cek setup BJR
				$sbjr = "SELECT kodeorg,bjr FROM $dbname.kebun_5bjr WHERE kodeorg='" . $row['kodeorg'] . "' AND periode='" . substr($dataH[0]['tanggal'], 0, 7) . "'";
				$rbjr = fetchData($sbjr);
				if ($rbjr[0]['bjr'] == 0 || $rbjr[0]['bjr'] == null) {
					$bjrrow = 0;
				} else {
					$bjrrow = $rbjr[0]['bjr'];
				}
				...
				if (count($req) > 0) {
					...
					*/
					...
					$data = array(
						...
						'bjr'        => $bjrrow,
						'kgkebun'    => $req[0]['kgkebun'] + (round($bjrrow * $row['hasilkerja'], 0)),
						'posting'    => '1',
					...
					*/
					...
					// $str = "select SUM(luasareaproduktif) AS luasareaproduktif from ".$dbname.".setup_blok where indukblok='".$row['kodeorg']."' AND statusblok='TM'";
					$str = "select SUM(luasareaproduktif) AS luasareaproduktif from " . $dbname . ".setup_blok where indukblok='" . $row['kodeorg'] . "' AND (((" . date("Y") . " - tahuntanam)+1) >= 3 or statusblok='TM') AND status='A'";
					$res = fetchdata($str);
					...
					$data = array(
						...
						'bjr'         => $bjrrow,
						'kgkebun'     => round($bjrrow * $row['hasilkerja'], 0),
						'jjgafkir'    => 0,
						'keterangan'  => '',
						'posting'     => '1',
						'updateby'    => $_SESSION['standard']['userid'],
	...
	case 'unposting':
		try {
			...
			$str_cekdata = "SELECT kodeorg
				FROM " . $dbname . ".`kebun_prestasi`
				WHERE `notransaksi` = '" . $param['notransaksi'] . "'
				group by kodeorg;";
			$res_cekdata = fetchData($str_cekdata);
			if (count($res_cekdata) > 0) {
				// hapus transaksi rekap panen
				foreach ($res_cekdata as $bar) :

					$str_del = "delete from " . $dbname . ".kebun_rekappnn where blok='" . $bar['kodeorg'] . "' and tanggal='" . $dataH[0]['tanggal'] . "'";
					$owlPDO->exec($str_del);

				endforeach;

				// update flag jurnal
				$str_jurnal = "update " . $dbname . ".kebun_aktifitas set jurnal='0' where notransaksi='" . $param['notransaksi'] . "'";
				$owlPDO->exec($str_jurnal);
			}
...
function cekPrestasi($param)
{
	...
	$qAbs = selectQuery(
		$dbname,
		'kebun_kehadiran_vw',
		'karyawanid,sum(jhk) as jhk, sum(umr) as umr,notransaksi',
	...
	$qPnn = selectQuery(
		$dbname,
		'kebun_prestasi_vw',
		'karyawanid,notransaksi',
		"karyawanid='" . $param['karyawanid'] . "' and tanggal='" . $tgl . "' and notransaksi!='" . $param['notransaksi'] . "' and kodeorg = '" . $param['blok'] . "' "
	);
	...
	if ($param['blok'] == '') {
		$warning = "Blok";
		throw new PDOException("Silahkan mengisi " . $warning . ".");
	}
	if ($param['karyawanid'] == '') {
		$warning = "Karyawan";
		throw new PDOException("Silahkan mengisi " . $warning . ".");
	...
	// $query = "SELECT SUM(luasareaproduktif) AS luasareaproduktif FROM ".$dbname.".`setup_blok`
	// 	WHERE `indukblok` = '".$param['blok']."' and statusblok='TM'";
	$query = "SELECT SUM(luasareaproduktif) AS luasareaproduktif FROM " . $dbname . ".`setup_blok`
		WHERE `indukblok` = '" . $param['blok'] . "' and (" . date("Y") . " - tahuntanam >= 3 or statusblok='TM') AND status='A'";
	$qDetail = $owlPDO->query($query) or die(print " Gagal: " . PDOException::getMessage());
	$qDetail->setFetchMode(PDO::FETCH_ASSOC);
	while ($rDetail = $qDetail->fetch()) {
		$luasbloknya = $rDetail['luasareaproduktif'];
	...
	if (floatval(trim($luaspanennya)) > floatval(trim($luasbloknya))) {
		// $warning="Total Luas Panen ".$luaspanennya." (Ha), melebihi Luas Blok ".$luasbloknya." (Ha)";
...
function proporsiUpah($param)
{
	...
	$bulan = substr($tmpTgl[0], 4, 2);
	$prdgj = $tahun . "-" . $bulan;
	$tgl = $tmpTgl[0];

	# Get UMR
	$qUMR = selectQuery($dbname, 'sdm_5gajipokok', 'sum(jumlah) as nilai', "karyawanid=" . $param['karyawanid'] . " and tahun='" . $prdgj . "' and idkomponen in (1)");
	$Umr = fetchData($qUMR);
