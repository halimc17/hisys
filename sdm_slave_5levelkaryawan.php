...
{
case 'update':	
	$str="update ".$dbname.".sdm_5levelkaryawan set nama='".$nama."',aktif='".$aktif."'
	       where kode='".$kode."'";
	try{
		$owlPDO->exec($str); 
	}
	...
	break;
case 'insert':
	$str="insert into ".$dbname.".sdm_5levelkaryawan (kode,nama,aktif)
	...
	break;
case 'delete':
	$str="delete from ".$dbname.".sdm_5levelkaryawan 
	where kode='".$kode."'";
	try{
		$owlPDO->exec($str); 
	}
	...
	break;
case'loaddata':
	$str1="select * from ".$dbname.".sdm_5levelkaryawan order by aktif desc, kode";
	...
	while($bar1=$res1->fetch()){
		...
		$ceck=makeOption($dbname,'sdm_5levelkaryawan_detail','kode,kode',"kode='".$bar1->kode."' and unittipe='GLOBAL'");
		$no=1;
		if($ceck[$bar1->kode]!=''){
		...
		$res = fetchData($str);
		foreach($res as $bar){
			$ceck=makeOption($dbname,'sdm_5levelkaryawan_detail','kode,kode',"kode='".$bar1->kode."' and unittipe='".$bar['tipe']."'");
			$no++;
			if($ceck[$bar1->kode]!=''){
...
case'simpandetail':
		if($kode!=''){
			if($check=='1'){
				$str="insert into ".$dbname.".sdm_5levelkaryawan_detail (kode,unittipe)
				  values('".$kode."','".$tipe."')";
				try{$owlPDO->exec($str); }catch (PDOException $e){echo " Gagal,".addslashes($e->getMessage());}			
			}else{
				$str="delete from ".$dbname.".sdm_5levelkaryawan_detail where kode='".$kode."' and unittipe='".$tipe."'";
				try{$owlPDO->exec($str); }catch (PDOException $e){echo " Gagal,".addslashes($e->getMessage());}			
			}
		}else{
			$str="select * from ".$dbname.".sdm_5levelkaryawan where aktif='1'";
			$res = fetchData($str);
			foreach($res as $bar){
				if($check=='1'){
					$str="delete from ".$dbname.".sdm_5levelkaryawan_detail where kode='".$bar['kode']."' and unittipe='".$tipe."'";
					try{$owlPDO->exec($str); }catch (PDOException $e){echo " Gagal,".addslashes($e->getMessage());}	
