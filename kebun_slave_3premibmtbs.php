...
# Ambil TK BM
if($prd > '2024-10'){
	if(getNamaOrg($unit,'induk')=='PPP'){
		$str="select *,a.tanggal as tanggalx,a.kontanan as kontananx,b.tanggal as tanggalspb from ".$dbname.".kebun_spbbm a 
		  left join ".$dbname.".kebun_spb_vw3 b on a.nospb=b.nospb
		  where	a.tanggal like '".$prd."%' and b.kodeorg='".$unit."' and b.divisi like '%".$afd."%' and posting='1' and b.kontanan='".$kontanan."' ".$where."  order by a.tanggal asc, a.nospb,a.kegiatan, a.sesi asc";
	}else{
		$str="select *,a.tanggal as tanggalx,a.kontanan as kontananx,b.tanggal as tanggalspb from ".$dbname.".kebun_spbbm a 
		  left join ".$dbname.".kebun_spb_vw4 b on a.nospb=b.nospb
		  where	a.tanggal like '".$prd."%' and b.kodeorg='".$unit."' and b.blok like '%".$afd."%' and posting='1' and b.kontanan='".$kontanan."' ".$where."  order by a.tanggal asc, a.nospb,a.kegiatan, a.sesi asc";
	}
	...
	$str="select *,a.kontanan as kontananx from ".$dbname.".kebun_spbbm a 
	  left join ".$dbname.".kebun_spb_vw3 b on a.nospb=b.nospb
	  where	b.tanggal like '".$prd."%' and b.kodeorg='".$unit."' and b.divisi like '%".$afd."%' and posting='1' and b.kontanan='".$kontanan."' ".$where."  order by a.tanggal asc, a.nospb,a.kegiatan, a.sesi asc";
	//echo $str;
	//exit("warning " . $str);
	...
	while($bar=$res->fetch()){
		...
		$list[$bar['tanggal']][$bar['nospb']]=1;
		$listtgl[$bar['tanggal']][$bar['nospb']][$bar['kegiatan']][$bar['sesi']][$bar['karyawanid']][$bar['kontananx']]=$bar['kegiatan'];
		
		$jlhhk[$bar['karyawanid']][$bar['tanggal']]=1;
		$jlhkarykg[$bar['karyawanid']][$bar['nospb']][$bar['kegiatan']][$bar['sesi']]=floatval($bar['jjg_angkut']);
		@$jlhkary[$bar['nospb']][$bar['kegiatan']][$bar['sesi']]+=floatval($bar['jjg_angkut']);
	}
}
...
if(getNamaOrg($unit,'induk')=='PPP'){
	...
	## Periode selanjutnya
	$datexx = DateTime::createFromFormat("Y-m", $prd);
	$datexx->modify("+1 month");

	// $prd_selanjutnya = $datexx->format("Y-m");	

	
	$str = "SELECT * FROM ".$dbname.".kebun_spb_vw3 
				WHERE (
						(tanggal >= '".$MinDate->format('Y-m-d H:i:s')."' 
						AND tanggal <= '".$MaxDate->format('Y-m-d H:i:s')."')
						OR tanggal LIKE '".$prd."%'
					)
				AND kodeorg = '".$unit."' 
				AND divisi LIKE '".$afd."%' 
	...
	$str="select * from ".$dbname.".kebun_spb_vw4 where tanggal like '".$prd."%' and kodeorg='".$unit."' and blok like '".$afd."%' and posting='1'";
}
...
while ($dspb = $ttp->fetch()) {
	...
	$stream.="<tr class=rowheader>";
		...
		$stream.="<td align=center rowspan=2>".$_SESSION['lang']['bjr']."</td>";$stream.="</tr>";
	$stream.="<tr></tr>";
	$stream.="</thead>";
				...
				foreach ($key3 as $sesi => $key4) {
					foreach ($key4 as $kary => $key5) {
						foreach ($key5 as $kontanan => $val) {
							...
							if($jabkar[$kary] != '16' and getKary($kary,'sistemgaji') == 'Bulanan'){
								$jmlhk[$kary][$ltgl]=1;
							}
...
// echo '<pre>';
// print_r($jumlahhk['611020202']['0000001173']);
// echo '</pre>';
foreach($listtgl as $ltgl => $key ){
		$jenispremi = getjenisharikerja($unit,$ltgl);
				...
				foreach ($key3 as $sesi => $key4) {
					foreach ($key4 as $kary => $key5) {
						foreach ($key5 as $kontanan => $val) {
							...
							$stream.="<td align=right id=rpkg".$no.">".$rpkg[$keg][$jenispremi]."</td>";
