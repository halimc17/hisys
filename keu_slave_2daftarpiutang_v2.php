...
function dpOrgScope($owlPDO, $dbname, $root)
{
    $scope = array($root);
    $queue = array($root);
    while(!empty($queue)){
...
if($unit==''){
    $orgScope = dpOrgScope($owlPDO, $dbname, $pt);
    $unitx = $pt;
}else{
    $orgScope = array($unit);
    $unitx = $unit;
}
$whereOrgList = dpSqlList($orgScope);
    ...
    $str = "select x.noakun, x.tipepihak, x.kodepihak, x.namapihak,
        ...
        from (
            ...
            where d.tanggal<='".$akhirBulan."' and d.kodeorg in(".$whereOrgList.")
                and d.noakun in(".dpSqlList($akunList).")
        ) x
...
$stream = "<style>
    .dp{border-collapse:collapse;font-family:Arial;font-size:12px;min-width:900px}
    .dp td{border:1px solid #222;padding:4px;height:22px;vertical-align:middle}
    .dp .center{text-align:center;font-weight:bold}
    .dp .head{font-weight:bold;text-align:center;background:#fff}
    .dp .right{text-align:right;white-space:nowrap}
    .dp .group td{background:#fce4d6;font-weight:bold}
    .dp .subtotal td{font-weight:bold}
    .dp .grand td{font-weight:bold;border-top:2px solid #000;border-bottom:3px double #000}
    .dp .noborder td{border:0}
    .dp .muted{color:#1f4e79}
</style>";
$stream .= "<table class='dp' border='".$border."' cellspacing=0 cellpadding=3>";
$stream .= "<tr class='noborder'><td colspan=6 class=center>".strtoupper($namaOrg)."</td></tr>";
$stream .= "<tr class='noborder'><td colspan=6 class=center>DAFTAR PIUTANG</td></tr>";
...
foreach($groups as $groupUrut=>$group){
            ...
            foreach($detail['akun'] as $akun){
                ...
                foreach($saldoRows[$akun] as $bar){
                    ...
                    $stream .= "<tr>
                        ...
                        <td class=center>".dpDate($bar['tanggal'])."</td>
                        <td class=right>".dpMoney($bar['saldo'])."</td>
                    </tr>";
                }
            }
        }
    ...
    $stream .= "<tr class=subtotal><td></td><td colspan=4>Total ".$group['keterangan']."</td><td class=right>".dpMoney($totalGroup)."</td></tr>";
}
$stream .= "<tr class=grand><td></td><td colspan=4>TOTAL PIUTANG</td><td class=right>".dpMoney($grand)."</td></tr>";
$stream .= "</table>";

if($tipe=='excel'){
    $stream .= "Print Time:".date('Y-m-d H:i:s')."<br />By:".$_SESSION['empl']['name'];
    $nop = "DaftarPiutang-".$pt."_".$unit."_".$periode."___".date("YmdHis").".xls";
