...
$tplData2=    isset($_POST['tplData2'])? $_POST['tplData2']: '';
...
#lalu
if($periode1=='akhir')$periodPRF=substr($periodesaldo,0,4)."-12"; else $periodPRF=$tahunlalu."-".$bulan;
if($periode1=='akhir')$periodPRF2=substr($periodesaldo,0,4)."12"; else $periodPRF2=$tahunlalu.$bulan;
if($periode1=='akhir')$kolomPRF="awal12"; else $kolomPRF="awal".$bulan; #"awal".substr($periodesaldo,4,2);
...
$periodeDpn=substr($periodCUR2,0,4).'-'.substr($periodCUR2,4,2);;
...
// $stream.="<div style='position:fixed;width:995px;'>";
// $stream.="<table class=sortable border=0 cellspacing=1>
//     <thead>
//         <tr class=rowheader>
//         <td rowspan=2 align=center width='365px;' >Keterangan</td>    

//         <td rowspan=2 align=center width='200px;'>".$captionCUR."</td>
//         <td rowspan=2 align=center width='200px;'>".$captionPRF."</td>
//         <td colspan=2 align=center width='200px;'  ".$styDt.">KENAIKAN/PENURUNAN</td>
//         </tr>";
...
//     $stream.="<br />";
//     }
//     $stream.="<div style='width:995px;height:359px;overflow:auto;'><table class=sortable border=0 cellspacing=1><thead><tr><td colspan=7 width='800px;'></td></tr></thead><tbody>";

$addEx="";
$stream.="<table class=sortable border=0 cellspacing=1>
    <thead>
        <tr>
        <th rowspan=2 align=center width='395px;'  colspan=5 ".$bgclr.">Keterangan</td>    

        <th rowspan=2 align=center width='200px;' ".$bgclr.">".$captionCUR."</th>
        <th rowspan=2 align=center width='200px;' ".$bgclr.">".$captionPRF."</th>";
    if($tplData2==0){
        $stream.="<th colspan=2 align=center width='200px;' ".$bgclr."  ".$styDt.">KENAIKAN/PENURUNAN</th>";
    }
        $stream.="</tr>";
    if($tplData2==0){
    $stream.="<tr  ".$styDt.">
        <th align=center width='155px;'   ".$bgclr.">Rupiah</th>
        <th align=center width='45px;'   ".$bgclr.">%</th>
        </tr>";
    }
$stream.="</thead><tbody>";
...
$st12="select sum(".$kolomPRF.") as kemarin,noakun
       ...
       and periode<='".$periodPRF2."' and ".$where." order by noakun asc";
...
$st12="select sum(jumlah) as kemarin,noakun,kodeorg,right(kodeorg,1) as tipeunit
       ...
       and left(tanggal,7)<='".$periodPRF."' and ".$where." ".$addRev."  and noakun not in ('3110400','3110600','3120100') 
...
if(!empty($dzArr)){
            ...
            if($data['tampil']==0){
                if(!empty($arrTampilanTotal[$data['nourut']])){
                        ...
                        case'210019':
                            ...
                            //exit('warning:'.$rpCpoSkrg."___".$rpKerSkrg);
                        ...
                        //exit('warning:'.$dtRp['cpo']['jumlahlalu']."___".$dtRp['ker']['jumlahlalu']."___".$dtRp['cpo']['jumlahsekarang']."__".$dtRp['ker']['jumlahsekarang']);
                            ...
                            if($unit=='' and (($periodeDpn > '2014-12') and ($periodeDpn != '2016-01'))){ // eksepsi 2014-12
                                $scek="";
                                ...
                                if(mysql_query($scek)){
                                    ...
                                    exit('gagal :'.$scek."__sini".mysql_error($conn));
                                }
                            }
                        break;
                    }
                    ...
                    if($data['nourut']=='216999'){
                         ...
                         $data['naikturun']= $data['jumlahsekarang']-$data['jumlahlalu'];
                         @$dtCpo['persen']=($dtCpoSkrg['naikturun']/$data['jumlahlalu'])*100;
                            if(($unit=='') and ($periodeDpn != '2016-01')){
                                $sKd="select kodeorganisasi from ".$dbname.".organisasi where induk='".$pt."' and tipe='HOLDING'";
                                $qKd=mysql_query($sKd) or die(mysql_error($conn));
                                $rKd=mysql_fetch_assoc($qKd);
                                $unit=$rKd['kodeorganisasi'];
                                $sinsert="update ".$dbname.".keu_4hpp set lababersih='".$temPdt."' where kodeorg='".$unit."' and periode='".$periodeDpn."'";
                                if(!mysql_query($sinsert)){
                                 exit('gagal :'.$sinsert."__".mysql_error($conn));
                                }
                                $unit="";
        ...
        $stream.="<tr class=rowcontent><td colspan=".$jlhkolom." ".$stDet.">.</td></tr>
            ...
            // PENY = PENYUSUTAN
            if(($data['nourut']=='214002')and($lstAkun2=='82212')){
                $jumlahbi=$dtRupiah[$lstAkun2]['jumlahsekarang'];
                $jumlahb0=$dtRupiah[$lstAkun2]['jumlahlalu'];
                masukinkerasio('PENY',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
        ...
        if($unit==''){
            ...
            if($rCek!=0){
                $sUpdate="update ".$dbname.".keu_4rasio set 
                          pendapatan='".floatval($dzArr['100009']['jumlahsekarang'])."',
                          lababersih='".floatval($lababersih)."',
                          bebanpokok='".floatval($rpBaru['212005']['jumlahsekarang'])."',
                          labakotor='".floatval(($dzArr['100009']['jumlahsekarang']+$rpBaru['212005']['jumlahsekarang']))."',
                          labausaha='".floatval($dzArr['214999']['jumlahsekarang'])."',
                          beban_keuangan='".floatval(($dzArr['215005']['jumlahsekarang']+$dzArr['216001']['jumlahsekarang']))."'
                          where periode='".$periode."' and kodeorg='".$unit."'";
            }else{
                $sUpdate="insert into ".$dbname.".keu_4rasio (`kodeorg`,`periode`,`pendapatan`,`lababersih`,`bebanpokok`,`labakotor`,`labausaha`,`beban_keuangan`) values 
                         ('".$unit."','".$periode."','".floatval($dzArr['100009']['jumlahsekarang'])."','".floatval($lababersih)."','".floatval($rpBaru['212005']['jumlahsekarang'])."','".floatval(($dzArr['100009']['jumlahsekarang']+$rpBaru['212005']['jumlahsekarang']))."',
                          '".floatval($dzArr['214999']['jumlahsekarang'])."','".floatval(($dzArr['215005']['jumlahsekarang']+$dzArr['216001']['jumlahsekarang']))."')";
            }
...
//        if($periode>'2014-12'){
            ...
            if($data['nourut']=='216999'){
                ...
                masukinkerasio('LABE',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }
            // PEND = Pendapatan
            if($data['nourut']=='100009'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('PEND',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // HAPP = Harga Pokok Penjualan
            if($data['nourut']=='212005'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('HAPP',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // BIB1 = Biaya bunga (BEBAN LAIN-LAIN)
            if($data['nourut']=='215003'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('BIB1',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // BIB2 = Biaya bunga (BEBAN KEUANGAN)
            if($data['nourut']=='215004'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('BIB2',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // LSPP = LABA (RUGI) SEBELUM PAJAK
            if($data['nourut']=='215999'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('LSPP',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // PEAM = PENYUSUTAN DAN AMORTISASI
            if($data['nourut']=='210014'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('PEAM',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // PEUS = Pendapatan Usaha
            if($data['nourut']=='214999'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('PEUS',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // PEUS = Pendapatan Usaha
            if($data['nourut']=='214999'){
                $jumlahbi=$data['jumlahsekarang'];
            ...
            // end of buat ikhtisar                
            
            // buat rasio
            // BTLK = BIAYA TIDAK LANGSUNG KEBUN
            if($data['nourut']=='210003'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('BTLK',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // BTLP = BIAYA TIDAK LANGSUNG PABRIK
            if($data['nourut']=='210013'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('BTLP',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // BADU = BEBAN ADMINISTRASI DAN UMUM
            if($data['nourut']=='214002'){
                $jumlahbi=$data['jumlahsekarang'];
                $jumlahb0=$data['jumlahlalu'];
                masukinkerasio('BADU',$dzkodeorg,$periode,$kodelaporan,$jumlahbi,$jumlahb0); // dzlib.php
            }                    
            // end of buat rasio
//        }
...
//
//        // masukin dz
//        echo "error:".$dzArr['216999']['jumlahsekarang'];
//        exit;
//        // end of masukin dz
