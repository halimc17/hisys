...
# Get Period Range
$qPeriod = selectQuery($dbname, 'sdm_5periodegaji_kecil', 'tanggalmulai,tanggalsampai', "periode='" . $param['periodegaji'] . "' and kodeorg='" .$param['kodeorg'] . "'");
$resPeriod = fetchData($qPeriod);
@$tanggal1 = $resPeriod[0]['tanggalmulai'];

// $str = "select karyawanid,nik,namakaryawan,nourut from ".$dbname.".datakaryawan where approval_status='9'  and lokasitugas = '".$param['kodeorg']."' and periodegaji ='".$param['periodegaji']."' and sistemgaji='Bulanan' "; 
// $res = fetchdata($str);
// if(count($res)>0){ 
// $datatmpl="";
// $nodasa=0;
...
// $str = "select karyawanid,nik,namakaryawan,nourut from ".$dbname.".datakaryawan where approval_status='7'  and lokasitugas = '".$param['kodeorg']."' and periodegaji ='".$param['periodegaji']."' and tipekaryawan='".$param['tipekar']."' "; 
// $res = fetchdata($str);
// if(count($res)>0){ 
...
// $datahist=array();
// $str = "select karyawanid from ".$dbname.".datakaryawan where 1=1 ".$tpkar1." and alokasi in ('0','1') and approval_status='8' and version_type='B' and lokasitugas='".$param['kodeorg']."' and periodegaji='".$param['periodegaji']."'"; 
// $res = fetchdata($str); 
...
// $jumlahkaryhist=0;
// $str = "select count(karyawanid) as jlh from ".$dbname.".datakaryawan where approval_status='8' and version_type='B' and lokasitugas='".$param['kodeorg']."' and periodegaji='".$param['periodegaji']."' "; 
// $res = fetchdata($str);
...
// $str = "select karyawanid from ".$dbname.".datakaryawan where approval_status='8' and version_type='B' and lokasitugas='".$param['kodeorg']."' and periodegaji='".$param['periodegaji']."'"; 
// $res = fetchdata($str);
// if(count($res)==0){
...
// }
	...
	where 1=1 ".$tpkar." and b.karyawanid in (select karyawanid from datakaryawan where lokasitugas = '".$param['kodeorg']."'  )
	...
	and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') and b.alokasi in ('0','1')
	and a.tanggal>='" . $tanggal1 . "' and a.tanggal<='" . $tanggal2 . "'     
	and sistemgaji='Bulanan'  group by a.tanggal, a.karyawanid";
	$res = fetchData($str);
	foreach($res as $bar){
		// $cekabsen[$bar['karyawanid']][$bar['tanggal']]=$bar['tanggal'];
		// if(!isset($arrayhkxx[$bar['karyawanid']][$bar['tanggal']])){
	...
	where 1=1 ".$tpkar." and b.karyawanid in (select karyawanid from datakaryawan where lokasitugas = '".$param['kodeorg']."'  ) and
	...
	$tipekaryawan=array();
	$str1 = "select a.*,b.namakaryawan,b.tipekaryawan from ".$dbname.".sdm_5gajipokok a left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
	...
	$query3="select a.nik as karyawanid,sum(jumlahpotongan) as potongan,tipepotongan from ".$dbname.".sdm_potongandt a left join 
			".$dbname.".datakaryawan b on a.nik=b.karyawanid  where 1=1 ".$tpkar." and b.lokasitugas='".$param['kodeorg']."' 
	...
	left join " . $dbname . ".datakaryawan b on a.karyawanid=b.karyawanid 
				   where 1=1 ".$tpkar." and b.lokasitugas='" . $param['kodeorg'] . "' 
				   and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') and b.alokasi in ('0','1')   
				   and a.status=1  and bulan='".$param['periodegaji']."' group by jenis,karyawanid";
	...
	$query5xx = "select a.* from " . $dbname . ".sdm_pesangon a left join 
			  ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid 
			   ...
			   and a.tanggal>='" . $tanggal1 . "' and a.tanggal<='" . $tanggal2 . "'     
			   and b.sistemgaji='Bulanan'   and posting='1' ";
	//echo $query5xx;
	...
	$query6 = "select a.tanggal, sum(a.upahkerja) as upahkerja,a.karyawanid,sum(a.tpremi) as premi,sum(a.rupiahpenalty) as penalty,
			  ...
			  and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') and b.alokasi in ('0','1') 
			  and a.tanggal>='" . $tanggal1 . "' and a.tanggal<='" . $tanggal2 . "'     
			  and sistemgaji='Bulanan'  and a.keterangan!='KONTAN' and a.noreferensi like '%PNN%'  group by a.tanggal, a.karyawanid";
	...
	$query66 = "select a.tanggal, sum(a.upahkerja) as upahkerja,a.karyawanid,sum(a.tpremi) as premi,sum(a.rupiahpenalty) as penalty,
			  ...
			  and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') and b.alokasi in ('0','1')
			  and a.tanggal>='" . $tanggal1 . "' and a.tanggal<='" . $tanggal2 . "'     
			  and sistemgaji='Bulanan'  and a.kontanan='KONTAN' and a.noreferensi like '%PNN%' group by a.tanggal, a.karyawanid"; 
	...
	$query20 = "select sum(a.rppremi) as premi , sum(a.rphk) as rphk,a.nilai1hk as nilai1hk,sum(a.hk) as hk,a.karyawanid,a.tanggal
		  ...
		  and b.sistemgaji='Bulanan'    and a.kontanan !='KONTAN' group by a.karyawanid,a.tanggal";
	...
	$query202 = "select sum(a.rppremi) as premi , sum(a.rphk) as rphk,sum(a.nilai1hk) as nilai1hk,a.karyawanid
		  ...
		  and  (b.tanggalkeluar>='" . $tanggal1 . "' or b.tanggalkeluar='0000-00-00') and b.alokasi in ('0','1')
		  and a.periode = '" . $param['periodegaji'] . "'     
		  and b.sistemgaji='Bulanan'    and a.kontanan ='KONTANAN' group by a.karyawanid"; 
	...
	$i="select * from ".$dbname.".sdm_pendapatanlaindt a where kodeorg='" . $param['kodeorg'] . "' and a.periodegaji='".$param['periodegaji']."' "
		." and karyawanid in (select karyawanid from ".$dbname.".datakaryawan b where 1=1 ".$tpkar1." and"
	...
	###################################################################################################################################################
			...
			$list0 .="<tr align=center class=rowheader>";
				...
				$list0 .= "<th rowspan=2 >".$_SESSION['lang']['karyawanid']."</th>";
		...
		$no=0;
			...
			foreach ($readyData as $dat => $bar) {
				$adagaji[$bar['karyawanid']]+=$bar['jumlah'];
				...
				if ($sisa[$val[0]] < 0) {
					...
					$list1 .="<tr class=rowcontent>";
						...
						$list1 .= "<td align=center>  ".getNamaTipeKary($tpkarid[$val[0]])."</td>";
				...
				## Jika Gaji Tidak Minus
					...
					$list2 .="<tr class=rowcontent>";
						...
						$list2 .= "<td align=center>   ".getNamaTipeKary($tpkarid[$val[0]])."</td>";
...
switch($proses) {
	...
	$owlPDO->beginTransaction();
          ...
          $sdel="delete from ".$dbname.".sdm_gaji_kecil "
           ...
           . " and karyawanid in (select distinct karyawanid from ".$dbname.".datakaryawan "
            . " where lokasitugas='".$param['kodeorg']."'   )  and sumber!='UPLOAD'";
        ...
        foreach($readyData as $row) {
            ...
            if($row['jumlah']==0 or $row['jumlah']==''){
				...
				$str="delete from ".$dbname.".sdm_gaji_kecil where idkomponen='".$row['idkomponen']."' 
						and karyawanid='".$row['karyawanid']."' and periodegaji='".$row['periodegaji']."' and sumber !='UPLOAD'";
				$owlPDO->exec($str);
				
                $queryIns = insertQuery($dbname,'sdm_gaji_kecil',$row);
				$owlPDO->exec($queryIns);
    
				$queryUpd = updateQuery($dbname,'sdm_gaji_kecil',$row,
					"kodeorg='".$row['kodeorg'].
				...
				## Komponen Gaji Kecil masuk ke SDM Gaji
				$ids = array('3', '61', '67', '44', '81', '70', '71', '72', '73', '80','131');
