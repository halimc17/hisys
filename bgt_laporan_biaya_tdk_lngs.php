...
OPEN_BOX('','<span class=judul>'.getMenu('bgt_laporan_biaya_tdk_lngs').'</span><br>');
#ambil tahun budget
...
#ambil kode kebun
$str="select kodeorganisasi as kodeorg,namaorganisasi from  ".$dbname.".organisasi where tipe!='KEBUN' and tipe!='PABRIK' and length(kodeorganisasi)='4' order by kodeorganisasi";
$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
$res->setFetchMode(PDO::FETCH_OBJ);
$optunit="<option value=''>".$_SESSION['lang']['pilihdata']."</option>";
while($bar=$res->fetch()){
	$induk = makeOption($dbname, 'organisasi', 'kodeorganisasi,induk',"kodeorganisasi='".$bar->kodeorg."'");
	$d=$induk[$bar->kodeorg];
	if($d!=$n){			
