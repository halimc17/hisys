...
// $addrp_muat  =str_replace(",","",$addrp_muat);
// $rp_muat     =str_replace(",","",$rp_muat);
// $rp_muat2    =str_replace(",","",$rp_muat2);
// $rp_muat3    =str_replace(",","",$rp_muat3);
// $rp_muat4    =str_replace(",","",$rp_muat4);
// $addrp_angkut=str_replace(",","",$addrp_angkut);
// $rp_angkut   =str_replace(",","",$rp_angkut);
// $rp_angkut2  =str_replace(",","",$rp_angkut2);
// $rp_angkut3  =str_replace(",","",$rp_angkut3);
// $rp_angkut4  =str_replace(",","",$rp_angkut4);
// $kgwbdet     =str_replace(",","",$kgwbdet);
// $kgwb        =str_replace(",","",$kgwb);
// $potonganrp  =str_replace(",","",$potonganrp);
if ($kgwbdet == '') {
	$kgwbdet = 0;
}
...
switch ($method) {
	...
	case 'insert':
		try {
			...
			$values = array(
				...
				'Rp Muat 6' => $rp_muat6,
				'Rp Muat 7' => $rp_muat7,
				'Tambahan Muat' => $addrp_muat,
				'Rp Angkut 1' => $rp_angkut,
				'Rp Angkut 2' => $rp_angkut2,
				'Rp Angkut 3' => $rp_angkut3,
				'Rp Angkut 4' => $rp_angkut4,
				'Rp Angkut 5' => $rp_angkut5,
				'Rp Angkut 6' => $rp_angkut6,
				'Rp Angkut 7' => $rp_angkut7,
				'Tambahan Angkut' => $addrp_angkut,
				'Potongan Rupiah' => $potonganrp
			);

			foreach ($values as $namaNilai => $nilai) {
				$nilaiCek = str_replace(',', '', trim((string)$nilai));
				if ($nilaiCek === '') {
					continue;
				}
				if (!is_numeric($nilaiCek) || floatval($nilaiCek) < 0) {
					throw new PDOException($namaNilai . " tidak boleh minus.");
				}
			}

			$kgwbdetCek = floatval(str_replace(',', '', $kgwbdet));
			$kgwbpksCek = floatval(str_replace(',', '', $kgwbpks));
			$kgbrdCek = floatval(str_replace(',', '', $kgbrd));
			if ($kgwbdetCek > $kgwbpksCek) {
				throw new PDOException("KG WB tidak boleh lebih besar dari KG PKS.");
			}
			if ($kgbrdCek > $kgwbpksCek) {
				throw new PDOException("KG Brondolan tidak boleh lebih besar dari KG PKS.");
			}

			// Potongan hanya disimpan pada baris angkut. Karena itu potongan
			// tidak boleh melebihi total rupiah angkut agar nilai angkut tidak minus.
			$totalAngkutCek =
				floatval(str_replace(',', '', $rp_angkut)) +
				floatval(str_replace(',', '', $rp_angkut2)) +
				floatval(str_replace(',', '', $rp_angkut3)) +
				floatval(str_replace(',', '', $rp_angkut4)) +
				floatval(str_replace(',', '', $rp_angkut5)) +
				floatval(str_replace(',', '', $rp_angkut6)) +
				floatval(str_replace(',', '', $rp_angkut7)) +
				floatval(str_replace(',', '', $addrp_angkut));
			$potonganCek = floatval(str_replace(',', '', $potonganrp));
			if ($potonganCek > $totalAngkutCek) {
				throw new PDOException("Potongan rupiah tidak boleh lebih besar dari total rupiah angkut.");
			}
			if ($param['tujuan'] != '') {
				...
				if (count(fetchdata($str)) == 0) {
					...
					$kgwb = 0;
					$str = "select * from " . $dbname . ".kebun_spb_vw2 where nospb='" . $nospb . "'";
				...
				foreach ($arrdata as $key => $value) {
					...
					if ($pekerjaanz[$param['tujuan']][$key] != '' and $param['tujuan'] != '' and $kgwbdet != '0' and ($rpz[$param['tujuan']][$key] > 0 or $addrpz[$param['tujuan']][$key] > 0)) {
						...
						$datadt = array(
							...
							'kgwb'         => $kgwbdet,
							'potonganrp'   => $potrp,
							'rupiah'       => (floatval($rpz[$param['tujuan']][$key]) + floatval($addrpz[$param['tujuan']][$key])),
	...
	case 'getharga':
		...
		foreach ($res as $bar) {
			if ($bar['tujuan'] == $tujuan) {
				if ($bar['posting'] != '1') {
					exit("Warningsystem : Harga belum disetujui untuk blok " . $bar['blok'] . " - (" . getNamaOrg($bar['blok']) . ") dan tanggalberlaku " . tglnmbln($bar['tanggalberlaku'], 'I', 'long') . ".");
				}
				if ($bar['postingadd'] != '1') {
		...
		if (count($jlh) < 1 and $tujuan != '') {
			...
			exit("Warningsystem : Upah " . ucfirst($bar['jenis']) . " " . strtoupper($tujuan) . " di Blok " . getIndukBlok($blok) . " belum disetupkan di menu KONTRAK > SETUP > HARGA LOADING DAN ANGKUT TBS.");
			// exit("Warningsystem : Kode kegiatan : ".$kegmuat." atau ".$kegangkut." tidak terdaftar di Setup - Kegiatan");
		}


		//exit("Warningsystem : Kode kegiatan : ".$rpmuat." atau ".$rpangkut." tidak terdaftar di Setup - Kegiatan");
	...
	case 'getformnospb':
		...
		$tab .= "<table class='sortable' cellspacing=1 cellpadding=3 border=0 width=100%>
			...
			<tr class=rowheader>
				...
				<th align=center style='width:150px'>" . $_SESSION['lang']['jeniskend'] . "</th>
				<th align=center>" . $_SESSION['lang']['nopol'] . "</th>
				<th align=center>" . $_SESSION['lang']['sopir'] . "</th>
	...
	case 'getnospb':
		...
		$where = "";
		if ($param['tiket'] != '') {
			$where .= " and b.notransaksi like '%" . $param['tiket'] . "%'";
		}
		if ($param['nospb'] != '') {
			$where .= " and a.nospb like '%" . $param['nospb'] . "%'";
		}
		if ($param['unit'] != '') {
			$where .= " and a.kodeorg like '%" . $param['unit'] . "%'";
		}
		if ($param['tanggal'] != '') {
			$where .= " and a.tanggal = '" . tanggalsystemn($param['tanggal']) . "'";
		}

		if ($param['nopol'] != '') {
			$where .= " and lower(REPLACE(b.nokendaraan,' ','')) like '%" . strtoupper(str_replace(' ', '', $param['nopol'])) . "%'";
		}
		if ($param['sopir'] != '') {
			$where .= " and b.supir like '%" . $param['sopir'] . "%'";
		}

		$tglbl = tglbulanlalu($tanggal1);

		//$where.=" and a.angkutan = 'kontrak'";
		//$where.=" and b.reject = '0'";
		$where .= " and substr(b.tanggal,1,10) between '" . $tglbl . "' and '" . $tanggal2 . "'";
		$where .= " and lower(REPLACE(b.nokendaraan,' ','')) in ('" . implode("','", $nopol) . "')";

		$sql = "select a.*,b.supir,b.beratbersih,b.beratmasuk,b.beratkeluar,b.kgpotsortasi, b.notransaksi as tiket, nokendaraan from " . $dbname . ".kebun_spbht a 
		left join " . $dbname . ".pabrik_timbangan b on a.nospb=b.nospb
		where a.kodeorg = '" . $kodeorg . "' and substr(a.tanggal,1,10) between '" . $tglbl . "' and '" . $tanggal2 . "' and a.posting='1' and a.nospb not in (select nospb from " . $dbname . ".kebun_rekapangkutantbsht) and b.millcode like 'EXT%' " . $where . " order by tanggal";
		$res = fetchdata($sql);
		if (count($res) == 0) {
			$sql = "select a.*,b.supir,b.beratbersih,b.beratmasuk,b.beratkeluar,b.kgpotsortasi, b.notransaksi as tiket, nokendaraan from " . $dbname . ".kebun_spbht a 
			left join " . $dbname . ".pabrik_timbangan b on a.nospb=b.nospb
			where a.kodeorg = '" . $kodeorg . "' and substr(a.tanggal,1,10) between '" . $tglbl . "' and '" . $tanggal2 . "' and a.posting='1' and a.nospb not in (select nospb from " . $dbname . ".kebun_rekapangkutantbsht) and b.millcode in (select kodeorganisasi from " . $dbname . ".organisasi where tipe ='PABRIK') " . $where . " order by tanggal";
			$res = fetchdata($sql);
		}
		foreach ($res as $bar) {
			$no++;
			...
			$tab .= "<td align=center id=tiket_" . $no . " name=tiket[]>" . $bar['tiket'] . "</td>";
			$tab .= "<td align=center id=nospb_" . $no . " name=nospb[]>" . $bar['nospb'] . "</td>";
			$tab .= "<td align=center>" . tglnmbln($bar['tanggal'], 'I', 'long') . "</td>";
			$tab .= "<td align=center>" . $bar['kodeorg'] . "</td>";
			$tab .= "<td align=center><select style=\"width:150px;\" onchange=getjnskendall(" . $no . "); id=jnskendcr_" . $no . " name=jnskendcr[]>" . $optjns . "</select></td>";
			$tab .= "<td align=center>" . $bar['nokendaraan'] . "</td>";
			$tab .= "<td>" . $bar['supir'] . "</td>";
			$tab .= "<td align=right>" . ($bar['beratmasuk'] - $bar['beratkeluar']) . "</td>";

			$tab .= "<td align=center><input hidden name=cekharga[] value=''><input type=checkbox id=click" . $no . " name=click[] onclick='hitungclick()'></td>";
			$tab .= "</tr>";
		}
	...
	case 'getdetailspb':
		...
		// $arrmuat=array('tphpks'=>'TPH - PMKS','rampks'=>'RAMP - PMKS');//yang lama
		...
		// echo"<pre>";
		// print_r($param['jeniskend']['A084907']);
		...
		while ($bar = $res->fetch()) {
			...
			// #======== ini diambil dari case getharga tapi langsung dimasukan ke phpnya ga dilempar lagi ke js===========#
			// $str = "select * from " . $dbname . ".kebun_spbht where nospb  = '".$bar['nospb']."'";
			// $resi = fetchdata($str);
			// foreach($resi as $ber){
			// 	$tanggal=$ber['tanggal'];
			// 	$periode=substr($ber['tanggal'],0,7);
			// }

			// $addrpmuat=$addrpangkut=$rpangkut=$rpmuat=0;

			// // if($tujuan !=''){
			// 	$str = "select * from " . $dbname . ".kebun_5hargaangkut where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku = (SELECT MAX(`tanggalberlaku`) FROM `kebun_5hargaangkut` where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku <= '".$tanggal."' and tanggalberlaku like '".$periode."%' and nospk='".$spk."') and nospk='".$spk."'";
			// 	$resx = fetchData($str);
			// 	if(count($resx)<=0){
			// 		$str = "select * from " . $dbname . ".kebun_5hargaangkut where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku = (SELECT MAX(`tanggalberlaku`) FROM `kebun_5hargaangkut` where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku <= '".$tanggal."' and tanggalberlaku like '".$periode."%' and nospk='') and nospk=''";
			// 		$resx = fetchData($str);
			// 		if(count($resx)<=0){
			// 			$str = "select * from " . $dbname . ".kebun_5hargaangkut where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku = (SELECT MAX(`tanggalberlaku`) FROM `kebun_5hargaangkut` where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku <= '".$tanggal."' and nospk='".$spk."') and nospk='".$spk."'";
			// 			$resx = fetchData($str);
			// 			if(count($resx)<=0){
			// 				$str = "select * from " . $dbname . ".kebun_5hargaangkut where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku = (SELECT MAX(`tanggalberlaku`) FROM `kebun_5hargaangkut` where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku <= '".$tanggal."' and nospk='') and nospk=''";
			// 				$resx = fetchData($str);
			// 				if(count($resx)<=0){
			// 					exit("Warningsystem : Harga blok ".$bar['blok']." - ".getIndukBlok($bar['blok']).",<br>PKS Tujuan ".$bar['penerimatbs'].",<br>Jenis Kendaraan ".$jeniskendnya.",<br>belum ada, silahkan di tambah melalui menu : Kebun - Setup - Harga Loading dan Angkut TBS");
			// 				}
			// 			}
			// 		}
			// 	}
			...
			// 					case 'tphpks1':
			// 						$rpmuat=$b['harga'];
			// 						$rupiahmuat=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'tphpks2':
			// 						$rpmuat2=$b['harga'];
			// 						$rupiahmuat2=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'tphpks3':
			// 						$rpmuat3=$b['harga'];
			// 						$rupiahmuat3=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'rampks':
			// 						$rpmuat4=$b['harga'];
			// 						$rupiahmuat4=$b['harga']*$bar['kgwb'];
			// 						break;
			// 				}
			// 				$ttlrupiahmuat = $rupiahmuat+$rupiahmuat2+$rupiahmuat3+$rupiahmuat4;
			// 			}
			// 			if($b['jenis']=='angkut'){
			// 				$kegangkut=$b['kodekeg'];
			// 				switch ($b['tujuan']) {
			// 					case 'tphpks1':
			// 						$rpangkut=$b['harga'];
			// 						$rupiahangkut=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'tphpks2':
			// 						$rpangkut2=$b['harga'];
			// 						$rupiahangkut2=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'tphpks3':
			// 						$rpangkut3=$b['harga'];
			// 						$rupiahangkut3=$b['harga']*$bar['kgwb'];
			// 						break;
			// 					case 'rampks':
			// 						$rpangkut4=$b['harga'];
			// 						$rupiahangkut4=$b['harga']*$bar['kgwb'];
			// 						break;
			// 				}
			// 				$ttlrupiahangkut = $rupiahangkut+$rupiahangkut2+$rupiahangkut3+$rupiahangkut4;
			// 			}
			// 		// }

			// 		$sql = "select * from " . $dbname . ".kebun_5hargaangkut_additional where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku='".$bar['tanggalberlaku']."' and tglawal<='".$tanggal."' and tglakhir>='".$tanggal."' and nospk='".$spk."'";
			// 		if(count(fetchdata($sql))==0){
			// 			$sql = "select * from " . $dbname . ".kebun_5hargaangkut_additional where blok = '".$bar['blok']."' and pkstujuan='".$bar['penerimatbs']."' and jenisvhc='".$jeniskendnya."' and tanggalberlaku='".$bar['tanggalberlaku']."' and tglawal<='".$tanggal."' and tglakhir>='".$tanggal."' and nospk=''";
			// 		}
			// 		$req = fetchdata($sql);
			// 		foreach($req as $val){
			// 			if($val['posting']!='1'){
	...
	case 'loaddatadetail':
		...
		/* $tab.="<th align=center width=100px>" . $_SESSION['lang']['nama'] . "</th>
				<th align=center width=100px>" . $_SESSION['lang']['noakun'] . "</th>
				<th align=center width=40px>" . $_SESSION['lang']['rp'] . "</th>"; */
		...
		$potonganrp = array();
		$str = "select a.*,b.jenis,b.tujuan,b.rupiah,b.kgtotal,b.kgbrd,b.potonganrp from " . $dbname . ".kebun_rekapangkutantbsht a 
		left join " . $dbname . ".kebun_rekapangkutantbsdt b on a.nospb=b.nospb 
		where kodeorg = '" . $kodeorg . "' and a.periode='" . $periode . "' and 
		spk='" . $spk . "' and a.periodebyr='" . $periodebyr . "' ";
		$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
		$res->setFetchMode(PDO::FETCH_ASSOC);
		while ($bar = $res->fetch()) {
		...
		foreach ($dataspb as $nospb) {
			...
			$tab .= "<td align=center width=25px>
					<img src=images/application/application_delete.png class=zImgBtn  title='Delete' onclick=\"deletedetail('" . $kodeorg . "','" . $periode . "','" . $nospb . "');\" ></td>";
			$tab .= "<td align=center width=25px>
		...
		$tab .= "<td align=right><b>" . @number_format($tkgnet) . "</b></td>";
		$tab .= "<td align=right><b>" . @number_format($tkgbrd) . "</b></td>";
		$tab .= "<td align=right><b>" . @number_format($tkgnet) . "</b></td>";
		foreach ($arrjenis as $jenis) {
			foreach ($arrmuat as $keytujuan => $valtujuan) {
		...
		$tab .= "<td align=right><b>" . @number_format($gtrp - $tpotonganrp) . "</b></td>";
	...
	case 'loaddata':
		...
		if (count($rescek) > 0) {
			...
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while ($bar = $res->fetch()) {
				@$rupiah[$bar['kodeorg']][$bar['periode']][$bar['spk']][$bar['periodebyr']][$bar['jenis']][$bar['tujuan']] += $bar['rupiah'];
		...
		// $footd.="</tr>
		//              <tr><td colspan=".(22+(count($arrmuat)*2))." align=center>";
		// if ($page == '0') {
	...
	case 'html':
		...
		/* 
		$tab.="<td align=center width=100px>" . $_SESSION['lang']['nama'] . "</td>
				<td align=center width=100px>" . $_SESSION['lang']['noakun'] . "</td>
				<td align=center width=40px>" . $_SESSION['lang']['rp'] . "</td>"; */
			...
			foreach ($dataspb as $nospb) {
				...
				/* $tab.="<td colspan=3>";
					...
					if($jenistampil=='html'){
						...
						$tab.="<table border=1 cellpadding=1  width=100%>";
					}
					$strx = "select * from " . $dbname . ".kebun_rekapangkutantbsdtfee where nospb = '" . $nospb . "'";
					$resx = $owlPDO->query($strx) or die(print " Gagal: " . PDOException::getMessage());
					$resx->setFetchMode(PDO::FETCH_ASSOC);
					while ($barx = $resx->fetch()) {
						$brs++;$g="";
						if($brs>1){
							$g="border-top:0.5px solid #A5C1D6;";
						}
							$optnamafee = makeOption($dbname,"kebun_5namafee",'id,nama',"id='".$barx['id']."'");
							#$optjnsfee = makeOption($dbname,"keu_5akun",'noakun,namaakun',"noakun='".$barx['noakun']."'");
							$tab.="<tr class=rowcontent style=vertical-align:top>";
							$tab.="<td align=left style='width:100px;border-right:0.5px solid #A5C1D6;".$g."'>".$optnamafee[$barx['id']]."</td>";
							$tab.="<td align=left style='width:100px;border-right:0.5px solid #A5C1D6;".$g."'>".$optjnsfee[$barx['noakun']]." - <b><i>".$barx['jenisfee']."</i></b></td>";
							$tab.="<td align=right style='".$g."'>".@number_format($barx['rupiah'],0)."</td>";
	...
	case 'posting':
		...
		left join " . $dbname . ".kebun_rekapangkutantbsdt b on a.nospb=b.nospb 
		where 1=1 and a.kodeorg = '" . $kodeorg . "' and a.periode='" . $periode . "' and spk='" . $spk . "' and periodebyr='" . $periodebyr . "' ";
			#exit("error".$str);
			$res = $owlPDO->query($str) or die(print " Gagal: " . PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
	...
	case 'previewexcelall':
		...
		// $optkodeorg = makeOption($dbname,'log_spkht','notransaksi,kodeorg',"notransaksi = '".$param['notransaksi']."'");
		// $optkoderek = makeOption($dbname,'log_spkht','notransaksi,koderekanan',"notransaksi = '".$param['notransaksi']."'");
		// $optjenispk = makeOption($dbname,'lgl_pengajuanspkht','notransaksi,jenis',"notransaksi = '".$param['notransaksi']."'");
		// $where = "notransaksi='".$param['notransaksi']."'and termin='".$param['termin']."'";	
		// $jumlahblok =$nomor= 0;
		// $str ="SELECT kgwbnetto,blok,nospb,indukblok,totalkg,tanggalpanen FROM $dbname.kebun_spbdt_detail WHERE nospb IN (SELECT keterangan2 FROM $dbname.log_baspkdt WHERE keterangan='".$param['nobapp']."') ";
		// $res = fetchData($str);
		// foreach ($res as $val) {
		// 	$blokkecil[$val['nospb']][$val['indukblok']][$val['blok']]=$val['blok'];
		// 	$ttlblokkecil[$val['nospb']][$val['indukblok']][$val['blok']]+=$val['totalkg'];
		// 	$ttlblokbesar[$val['nospb']][$val['indukblok']]+=$val['kgwbnetto'];
		// 	$totalkg[$val['nospb']][$val['tanggalpanen']][$val['indukblok']][$val['blok']]+=$val['totalkg'];
		// 	$jumlahblok++;
		// }

		// $str = "select notransaksi, kodeblok, kodekegiatan, tanggal, hasilkerjarealisasi, hkrealisasi, jumlahrealisasi, sum(jjgkontanan) as jjgkontanan, posting, statusjurnal, statuspengajuan, nopengajuan, blokspkdt, kodesegment, termin, keterangan, jenis from ".$dbname.".log_baspk where ".$where."  order by termin asc";

		// $res = fetchdata($str);
		// foreach ($res as $v) {
		// 	$kdkeg[$v['notransaksi']]=$v['kodekegiatan'];
		// 	$jlhreal[$v['notransaksi']][$v['termin']]=$v['hasilkerjarealisasi'];
		// 	$rphreal[$v['notransaksi']][$v['termin']]=$v['jumlahrealisasi'];
		// }
		// $no=0;$ttl=0;$tempspb='';
		// $optsup = makeOption($dbname,'lgl_pengajuanspkht','notransaksi,koderekanan',"notransaksi = '".$param['notransaksi']."'");


		// $str = selectQuery($dbname,'log_baspkdt_detail','*',"keterangan='".$param['nobapp']."' and notransaksi='".$param['notransaksi']."'");
		// $res = fetchdata($str);
		// foreach($res as $val){
		// 	$optAct = makeOption($dbname,'setup_kegiatan','kodekegiatan,namakegiatan',"kodekegiatan = '".$val['kodekegiatan']."'");
		// 	$optsup = makeOption($dbname,'lgl_pengajuanspkht','notransaksi,koderekanan',"notransaksi = '".$param['notransaksi']."'");
		// 	$nama_blokkecil = makeOption($dbname,'organisasi','kodeorganisasi,namaorganisasi');
		...
		// 		$tab.="<td align=left id=nobapppost".$no.">".$val['notransaksi']."</td>";
		// 		$tab.="<td align=left id=nospb".$no.">".$val['keterangan2']."</td>";
		// 	// }else{
		...
		// 	// 	$tab.="<td align=center></td>";
		// 	// }
		// 	$tab.="<td align=left >".$optAct[$val['kodekegiatan']]."</td>";
		// 	$tab.="<td align=left>".$val['kodeblok']." - ".$nama_blokkecil[$val['kodeblok']]."</td>";
		// 	$tab.="<td align=right id=hslkerjapost".$no.">".number_format($val['hasilkerjarealisasi'],2)."</td>";
		// 	$tab.="<td align=right  id=realpost".$no.">".number_format($val['jumlahrealisasi'],2)."</td>";
		// 	$tab.="</tr>";

		// 	$tempspb = $spb;

		// 	$total_hsl1 += $val['hasilkerjarealisasi'];
		// 	$total_hsl2 += $val['jumlahrealisasi'];
