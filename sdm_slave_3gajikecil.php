...
$param=$_POST;
    ...
    switch ($proses) {
        ...
        case 'preview':
            ...
            if(strlen($param['unitId'])>4){
                ...
                // $dimanaPnjng3=" substr(b.kodeorg,1,4)='".substr($kodeOrg,0,4)."'";
					$dimanaPnjng3=" substr(b.kodeorg,1,4) in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$kodept."')";
			...
			$str="select * from ".$dbname.".kebun_prestasi_vs_hk  
					where unit in (select kodeorganisasi from ".$dbname.".organisasi where induk='".$kodept."')  
					and tanggal between '".$tgl1."' and '".$tgl2."'  ";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while($bar=$res->fetch()){
				$resData[$bar['karyawanid']]=$bar['karyawanid'];
				@$hasilAbsn[$bar['karyawanid']]+=$bar['hkpanenperhari'];
			}
			/*
			...
			*/
			...
			while($bar=$res->fetch()){
				$resData[$bar['karyawanid']]=$bar['karyawanid'];
                @$hasilAbsn[$bar['karyawanid']]+=$bar['jhk'];
			}
			/*
			...
			*/
			...
			while($bar=$res->fetch()){
				$resData[$bar['karyawanid']]=$bar['karyawanid'];
                @$hasilAbsn[$bar['karyawanid']]+=$bar['nilaihk'];
			}
			/*	
			...
			*/
			...
			$str="select a.*,b.lokasitugas,b.subbagian,tipekaryawan from ".$dbname.".vhc_runhk_vw a left join ".$dbname.".datakaryawan b
					...
					and tanggal between '".$tgl1."' and '".$tgl2."' and a.hk>0  ";
			$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
			$res->setFetchMode(PDO::FETCH_ASSOC);
			while($bar=$res->fetch()){
				$resData[$bar['idkaryawan']]=$bar['idkaryawan'];
                @$hasilAbsn[$bar['idkaryawan']]+=$bar['hk'];
			}
			
			/*
			$sTraksi="select distinct * from ".$dbname.".vhc_runhk_vw where tanggal between  '".$tgl1."' and '".$tgl2."' and ".$dimanaPnjng2."
						and hk>0";
            $rTraksi=fetchdata($sTraksi);
            foreach($rTraksi as $rowTrk=>$resData2){
                $hasilAbsn[$resData2['idkaryawan']][$resData2['tanggal']]='H';
                $resData[$resData2['idkaryawan']]=$resData2['idkaryawan'];
            }
			*/
			...
			foreach ($dtmandor as $karid){
				foreach($dttgl as $tgl){
					if(@$dtpejabatbkm[$karid][$tgl]!=''){
						...
						// kamus hari libur
						$strorg="select * from ".$dbname.".sdm_5harilibur where tanggal = '".$tgl."' and (kebun='GLOBAL' or kebun='".substr($unit,0,4)."')";
						...
						if($libur==false){
							$resData[$karid]=$karid;
							@$hasilAbsn[$karid]+=$counttgl[$karid][$tgl];
						}
					}
				}
			}
            ...
            // echo"</pre>";
                    ...
                    foreach($dtKary as $lstKary){
                        if(@$hasilAbsn[$lstKary]>=4){
                            ...
                            $tab.="<td align=right><input type=hidden id=hkDt_".$rowSb."_".$rowSbdt." value='".$hasilAbsn[$lstKary]."' />".@number_format($hasilAbsn[$lstKary],2)."</td>";
        ...
        case'saveAll':
			if (isset($param['dtKary']))
			{
				$str="select posting  from ".$dbname.".sdm_potonganht where kodeorg='".$_POST['dtKirim'][0]."' and periodegaji='".$param['periode']."' and tipepotongan='59'";
				$res=$owlPDO->query($str) or die(print " Gagal: ".PDOException::getMessage());
				$res->setFetchMode(PDO::FETCH_ASSOC);
				$bar=$res->fetch();
				if($bar['posting']==1)
				{
					exit("Warning:Data untuk unit : ".$_POST['dtKirim'][0]." dengan periode ".$param['periode']." sudah terposting ");
				}
				...
				foreach($param['dtKary'] as $rw=>$rwDt)
				{
					if($temp!=$kodeorg)
					{
						...
						$sIns="insert into ".$dbname .".sdm_potonganht (`kodeorg`,`periodegaji`,`tipepotongan`,`updateby`) values ('".$kodeorg."','".$param['periode']."',59,'".$_SESSION['standard']['userid']."')";
						try
						{
							$owlPDO->exec($sIns);
