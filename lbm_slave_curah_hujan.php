...
$qCurah = $owlPDO->query($sCurah) or die(print " Gagal: " . PDOException::getMessage());
$jumlahdiv = owlBaris($qCurah);
$qCurah->setFetchMode(PDO::FETCH_ASSOC);
while ($rCurah = $qCurah->fetch()) {
...
for ($dtThn5; $dtThn5 <= $thn[0]; $dtThn5++) {
    ...
    for ($awaBln = 1; $awaBln < 13; $awaBln++) {
        ...
        $qDtMm = $owlPDO->query($sDtMm) or die(print " Gagal: " . PDOException::getMessage());
        $qDtMm->setFetchMode(PDO::FETCH_ASSOC);
        $rDtMm = $qDtMm->fetch();
        ...
        $qDtCurah = $owlPDO->query($sDtCurah) or die(print " Gagal: " . PDOException::getMessage());
        $qDtCurah->setFetchMode(PDO::FETCH_ASSOC);
        $rDtCurah = $qDtCurah->fetch(); {
