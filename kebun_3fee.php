...
<script language=javascript1.2 src='js/kebun_3fee.js?v=<?php echo time(); ?>'></script>
...
foreach(getOrgDetail(11) as $key => $val){
	...
	if($_SESSION['empl']['lokasitugas']==$key){		
		$optorgx.="<option value=".$key." selected>".$key." - ".$val."</option>";
	}else{
...
OPEN_BOX('','<span class=judul>'.getMenu('kebun_3fee').'</span>');
...
echo"<table>
	...
	<tr>
		...
		<td>:</td>
		<td><select id=divsch class=select2>" . $optorgx . "</select></td>
		
		<td>" . $_SESSION['lang']['bulan'] . "</td> 
		<td>:</td>
		<td><select id=tglsch  class=select2>" . $optprdscr . "</select></td>
...
echo "
        ...
        echo"<table cellpadding=5 cellspacing=1 border=0 class=sortable>
            ...
            <th align=center rowspan='2'>Rupiah Fee</th>
...
OPEN_BOX();
$res=array('0'=>'Sebulan (Tanggal : 1 s/d 30)','1'=>'Pertama (Tanggal : 1 s/d 15)','2'=>'Kedua (Tanggal : 16 s/d 30)','3'=>'Lainnya (Tanggal ditentukan)');
...
$res=array('lainnya'=>'Lainnya','tempunak'=>'Tempunak (SDK3)');
...
<table cellspacing=1 border=0>
    <tr>
		...
		<td>:</td>
		<td><select style=\"width:160px;\"  class=select2 onchange=getDivisiX(this.value,'divisi',''); id=kodeorg>" . $optorg . "</select></td>
    
		<td>" . $_SESSION['lang']['divisi'] . "</td> 
		<td>:</td>
		<td><select style=\"width:150px;\"  class=select2 id=divisi>" . $optdiv . "</select></td>
    </tr> 
    <tr>
		<td>" . $_SESSION['lang']['bulan'] . "</td> 
		<td>:</td>
		<td><select style=\"width:160px;\" onchange=gettanggal()  class=select2 id=periode>" . $optprd . "</select></td>
    
		<td>" . $_SESSION['lang']['periode'] . "</td> 
		<td>:</td>
		<td><select style=\"width:150px;\" onchange=gettanggal() class=select2 id=periodebyr>" . $optbyr . "</select></td>
    </tr>
	<tr>
		<td>" . $_SESSION['lang']['tanggal'] . "</td> 
		<td>:</td>
		<td>
			<input type=text class=myinputtext disabled id=tgl onmousemove=setCalendar(this.id) onkeypress=return false;   style=\"width:75px;\"  readonly/>
			<input type=text class=myinputtext disabled id=tglsd onmousemove=setCalendar(this.id) onkeypress=return false;   style=\"width:75px;\"  readonly/>
		</td>
		
		<td>" . $_SESSION['lang']['jenis'] . "</td> 
		<td>:</td>
		<td><select style=\"width:150px;\"  class=select2 id=jenisH>" . $optjenis . "</select></td>
    </tr>
	<tr>
		<td colspan=2></td>
		<td>
