...
switch($method){
    case 'previewdata':
		if($param['mode']=='baru'){
			$str = "SELECT max(convert(substring_index(notransaksi,'/',-1),unsigned integer)) as nomor FROM " . $dbname . ".sdm_pengajuantiket where kodeorg='".$param['kodeorg']."' and substr(tanggal,1,4) order by notransaksi desc";
			$res = fetchdata($str)[0];
			if($res['nomor']==0){
				$notransaksi=tanggalsystem($param['tgl'])."/".$param['kodeorg']."/PTKT/0001";
			}else{
				$nmr=addZero($res['nomor']+1,4);
				
				$notransaksi=tanggalsystem($param['tgl'])."/".$param['kodeorg']."/PTKT/".$nmr;
			}
			...
			$query = insertQuery($dbname,'sdm_pengajuantiket',$data,$cols);
		...
		if($param['sumber']=='1'){
			#karyawan
			$onclick="onclick=getnamakaryawan();";
			$style="";
			$read="readonly";
		}else{
			$read="";
        ...
        $tab.="<fieldset>
			<legend>Detail</legend>
				...
				<tr>
					...
					<td>:</td>
					<td><input ".$style." onmousemove=hapuswarna(this.id); placeholder='Nomor Pengajuan Cuti.' id=nopjd style='width:150px;' onclick=getformnopjd(); readonly class='myinputtext'/></td>
				</tr> 
				<tr>	
					<td>".$_SESSION['lang']['nama'] . "</td> 
					<td>:</td>
					<td><input hidden id=idkaryawan style='width:150px;' class='myinputtext'/>
						<input ".$onclick." ".$read." onmousemove=hapuswarna(this.id); id=namakaryawan style='width:150px;' class='myinputtext'/></td>
					
					<td>".$_SESSION['lang']['kodegolongan'] . "</td> 
					<td>:</td>
					<td><input hidden id=golongan style='width:150px;' class='myinputtext'/>
						<input ".$read." onmousemove=hapuswarna(this.id); id=namagolongan style='width:150px;' class='myinputtext'/>
				...
				<tr>
					...
					<td>:</td>
					<td><input onmousemove=hapuswarna(this.id); id=telp style='width:150px;' class='myinputtext'/></td>
					
					<td>".$_SESSION['lang']['keperluan'] . "</td> 
					<td>:</td>
					<td><input onmousemove=hapuswarna(this.id); id=keperluan style='width:150px;' class='myinputtext'/></td>
				</tr> 
				<tr>
					<td>".$_SESSION['lang']['tujuan']." ".$_SESSION['lang']['dari']."</td> 
					<td>:</td>
					<td><input onmousemove=hapuswarna(this.id); id=dari style='width:150px;' class='myinputtext'/></td>
					
					<td>".$_SESSION['lang']['tujuan']." ".$_SESSION['lang']['ke'] . "</td> 
					<td>:</td>
					<td><input onmousemove=hapuswarna(this.id); id=ke style='width:150px;' class='myinputtext'/></td>
				</tr> 
				<tr>
					<td>".$_SESSION['lang']['tanggal'] . "</td> 
					<td>:</td>
					<td><input readonly type=text class=myinputtext style='width:70px;' id=tgldinasdari onmousemove=setCalendar(this.id);hapuswarna(this.id); onkeypress=return false; maxlength=10 />
					<input readonly type=text class=myinputtext style='width:73px;' id=tgldinassampai onmousemove=setCalendar(this.id);hapuswarna(this.id); onkeypress=return false; maxlength=10 />
					</td>
				...
				<tr>
					...
					<td>:</td>
					<td colspan=4><input onmousemove=hapuswarna(this.id); id=keterangan style='width:395px;' class='myinputtext'/></td>
				</tr> 
    ...
    break;
	case'getnamakaryawan':
		$tab.="<table cellspacing=1 border=0>
				<tr>
					<td>".$_SESSION['lang']['nama'] . "</td> 
					<td>:</td>
					<td><input id=namacari style='width:150px;' onkeypress='enterkey(event,carinamakaryawan)' class='myinputtext'/></td>
					
					<td>
						<button class=mybutton onclick=carinamakaryawan()>" . $_SESSION['lang']['find'] . "</button>
					</td>
				</tr>
			</table><hr><div id=containerkarycari></div>";
		echo $tab;
    break;
	case'carinamakaryawan':
		...
		$tab.="<table border=0 cellpadding=1 cellspacing=1 class=sortable>
			...
			$tab.="<td align=center width=20px>No</td>
				...
				<tbody>";
				if(strlen($param['namakaryawan'])<'3'){
					exit("Warning : Isikan nama minimal 3 huruf.");
				}
				
				$str = "select *  from ".$dbname.".datakaryawan where namakaryawan like '%".$param['namakaryawan']."%' and tanggalkeluar='0000-00-00' order by namakaryawan asc limit 100";
				$res = fetchdata($str);
				$no=0;
				foreach ($res as $bar){
					$nmtipe=makeOption($dbname,'sdm_5tipekaryawan','id,tipe',"id='".$bar['tipekaryawan']."'");
					$nmjab=makeOption($dbname,'sdm_5jabatan','kodejabatan,namajabatan',"kodejabatan='".$bar['kodejabatan']."'");
					$nmgol=makeOption($dbname,'sdm_5golongan','kodegolongan,namagolongan',"kodegolongan='".$bar['kodegolongan']."'");
					
					$no++;
					$tab.="<tr class=rowcontent style=cursor:pointer; title=Click... onclick=\"addkarya('".$bar['karyawanid']."','".$bar['namakaryawan']."','".$bar['kodegolongan']."','".$nmgol[$bar['kodegolongan']]."')\";>";
					$tab.="<td align=center>".$no."</td>";
					$tab.="<td align=left>".$bar['nik']."</td>";
					$tab.="<td align=left>".$bar['namakaryawan']."</td>";
					$tab.="<td align=left>".$bar['lokasitugas']."</td>";
					$tab.="<td align=left>".$nmtipe[$bar['tipekaryawan']]."</td>";
					$tab.="<td align=left>".$nmjab[$bar['kodejabatan']]."</td>";
					$tab.="<td align=left>".$nmgol[$bar['kodegolongan']]."</td>";
				}
	...
	case'getformnopjd':
		...
		$tab.="<table border=0 cellpadding=3 cellspacing=1 class=sortable>
				<thead><tr class=rowheader>";
			$rows="rowspan=2";	
	...
	case'getnopjd':
		switch($param['sumber']){
			case'1':
				...
				$where="";
				$where.=" and sampaijam>='".tanggalsystemn($param['tgl'])."'";
				$where.=" and karyawanid in (SELECT karyawanid  FROM " . $dbname . ".datakaryawan where lokasipenerimaan!='LOKAL')";
				...
				$str = "SELECT distinct notransaksi  FROM " . $dbname . ".sdm_ijin where 1=1 ".$where." order by notransaksi desc";
				
				$res = fetchdata($str);
				$jlhbrs = count($res);
				if(count($res)==0){
					echo"<tr class=rowcontent><td align=center colspan=11>Data tidak ditemukan.</td></tr>";
					exit();
				}
				$limit = 10;
				$page = 0;
				...
				$no = 0;
				$no = $maxdisplay;
				$str = "SELECT *  FROM " . $dbname . ".sdm_ijin where 1=1 ".$where." order by notransaksi desc limit " . $offset . "," . $limit . "";
				$res = fetchdata($str);
				foreach ($res as $bar){
					$no++;
					...
					$tab.="<td align=left ".$rows.">".$bar['alamatcuti']."</td>";
					$tab.="<td align=left ".$rows.">".$bar['keperluan']."</td>";
					$tab.="<td align=left ".$rows.">".$bar['keterangan']."</td>";
					$tab.="<td align=center ".$rows.">".tanggalnormal($bar['darijam'])." s/d ".tanggalnormal($bar['sampaijam'])."</td>";
					
					$e=0;
					$t = "select * from ".$dbname.".sdm_pengajuantiketdt a where a.notranspjd='".$bar['notransaksi']."' and a.jenis='".$param['jenis']."' and a.nama='".$bar['karyawanid']."'";
					$e = fetchdata($t);
					$color=$nomor="";
					if(count($e)>0){
						foreach($e as $a){
						...
						$tab.="<td align=center ".$color."></td>";
					}

					
					$tab.="<td align=center ".$rows." colspan=2 style=width:20px><img src=images/plus.png class='zImgBtn' title=Add onclick=\"addpjd('".$bar['notransaksi']."','".$param['jenis']."','','".$bar['karyawanid']."','".$nmkar[$bar['karyawanid']]."','".$kdgol[$bar['karyawanid']]."','".$nmgol[$kdgol[$bar['karyawanid']]]."','".$bar['keterangan']."','".tanggalnormal($bar['darijam'])."','".tanggalnormal($bar['sampaijam'])."','".$bar['alamatcuti']."','".$bar['keperluan']."','".$bar['nohp']."');\"></td>";
				...
				$footd.="</tr><tr><td colspan=11 align=center>";
	...
	try {
		$owlPDO->beginTransaction();
			...
			$data = array(
				...
				'dari'       => $param['dari'],
				'ke'         => $param['ke'],
				'keperluan'  => $param['keperluan'],
				'keterangan' => $param['keterangan'],
				'jenis'      => $param['jenis'],
				'tanggal'    => tanggalsystemn($param['tgldibutuhkan']),
				'tgldari'    => tanggalsystemn($param['tgldinasdari']),
				'tglsampai'  => tanggalsystemn($param['tgldinassampai']),
			...
			$query = insertQuery($dbname,'sdm_pengajuantiketdt',$data,$cols);#exit("error".$query);
			$owlPDO->exec($query);
	...
	try {
		$owlPDO->beginTransaction();
			...
			$where = "id='".$param['id']."'";
			$query = updateQuery($dbname,'sdm_pengajuantiketdt',$data,$where);
			$owlPDO->exec($query);
	...
	case'loaddatadetail':
		...
		$tab.="<fieldset><legend>".$_SESSION['lang']['list']."</legend>
			...
			$tab.="<th align=center ".$rows." width=20px>No</th>
				...
				<th align=center ".$rows." >".$_SESSION['lang']['nama']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['kodegolongan']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['telp']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['keperluan']."</th>
				<th align=center colspan=2 >".$_SESSION['lang']['tujuan']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['tanggal']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['tgldibutuhkan']."</th>
				<th align=center ".$rows." >".$_SESSION['lang']['keterangan']."</th>
			...
			<tr class=rowheader>
				...
				<th align=center>".$_SESSION['lang']['ke']."</th>
			</tr>
			</thead>
				<tbody>";
		$str = "SELECT *  FROM " . $dbname . ".sdm_pengajuantiketdt where notransaksi='".$param['notransaksi']."' order by id desc ";
		$res = fetchdata($str);
		$no=0;
		foreach ($res as $bar){
			$no++;
			...
			$tab.="<td align=left>".$bar['phone']."</td>";
			$tab.="<td align=left>".$bar['keperluan']."</td>";
			$tab.="<td align=left>".$bar['dari']."</td>";
			$tab.="<td align=left>".$bar['ke']."</td>";
			$tab.="<td align=center>".tanggalnormal($bar['tgldari'])." s/d ".tanggalnormal($bar['tglsampai'])."</td>";
			$tab.="<td align=center>".tanggalnormal($bar['tanggal'])."</td>";
			$tab.="<td align=left>".$bar['keterangan']."</td>";
			
			$tab.="<td align=center width=20px><img src=images/application/application_edit.png class=zImgBtn  title='Edit' onclick=\"editdetail('".$bar['id']."','".$bar['jenis']."','".$bar['notranspjd']."','".$bar['nama']."','".$nama."','".$bar['golongan']."','".$gol."','".$bar['phone']."','".$bar['keperluan']."','".$bar['dari']."','".$bar['ke']."','".tanggalnormal($bar['tgldari'])."','".tanggalnormal($bar['tglsampai'])."','".tanggalnormal($bar['tanggal'])."','".$bar['keterangan']."');\" ></td>";
	...
	case'loaddata':
		...
		if($param['nopjd']!=''){
			$where.=" and notransaksi in (select notransaksi from ".$dbname.".sdm_pengajuantiketdt where notranspjd like '%".$param['nopjd']."%')";
		}
		if($param['nama']!=''){
			$where.=" and notransaksi in (select notransaksi from ".$dbname.".sdm_pengajuantiketdt where nama like '%".$param['nama']."%' or nama in (select karyawanid from ".$dbname.".datakaryawan where namakaryawan like '%".$param['nama']."%'))";
		}
		if($param['sumber']!=''){
			$where.=" and sumber = '".$param['sumber']."'";
		}
		
		$str = "SELECT distinct notransaksi  FROM " . $dbname . ".sdm_pengajuantiket where 1=1 ".$where." order by createdtime desc";
		$res = fetchdata($str);
		$jlhbrs = count($res);
		if(count($res)==0){
		...
		$arsumber=array('1'=>$_SESSION['lang']['karyawan'],'2'=>$_SESSION['lang']['lain']);
		...
		$str = "SELECT *  FROM " . $dbname . ".sdm_pengajuantiket where 1=1 ".$where." order by notransaksi desc limit " . $offset . "," . $limit . "";
		$res = fetchdata($str);
		foreach ($res as $bar){
			$no++;
			...
			if($bar['status']=='0' or $bar['status']=='3'){				
				...
				$n = "SELECT *  FROM " . $dbname . ".sdm_pengajuantiketdt where notransaksi='".$bar['notransaksi']."'";
				$r = fetchdata($n);
	...
	try {
		$owlPDO->beginTransaction();
		$str="delete from ".$dbname.".sdm_pengajuantiketdt where id='".$param['id']."' and notransaksi='".$param['notransaksi']."'";
		$owlPDO->exec($str);
	...
	try {
		...
		$str="delete from ".$dbname.".sdm_pengajuantiket where notransaksi='".$param['notransaksi']."'";
		$owlPDO->exec($str);
	...
	case'preview':
		...
		$str = "SELECT *  FROM " . $dbname . ".sdm_pengajuantiket where notransaksi='".$param['notransaksi']."'";
		$bar = fetchdata($str)[0];
		...
		if($param['tipe']=='pdf'){				
			...
			$style="cellpadding=5 cellspacing=1";
		}
		...
		$tab.="<table ".$style.">
			...
			<tr class=rowheader>
				...
				<tbody>";
		$str = "SELECT *  FROM " . $dbname . ".sdm_pengajuantiketdt where notransaksi='".$param['notransaksi']."' order by id asc ";
		$res = fetchdata($str);
		$no=0;
		foreach ($res as $bar){
			$no++;
		...
		<tr class=rowheader>";
			$tab.="
			<th align=center style=font-weight:bold; ".$row." width=20px>No</th>
			<th align=center style=font-weight:bold; ".$row." >".$_SESSION['lang']['nama']."</th>
			<th align=center style=font-weight:bold; ".$row." >".$_SESSION['lang']['status']."</th>
			<th align=center style=font-weight:bold; ".$row." >".$_SESSION['lang']['tanggal']."</th>
			<th align=center style=font-weight:bold; ".$row." >".$_SESSION['lang']['note']."</th>
		</tr>";
		$tab.="</thead>";
		$tab.="<tbody>";
		...
		$kdorg = makeOption($dbname, 'sdm_pengajuantiket', 'notransaksi,kodeorg',"notransaksi='".$param['notransaksi']."'");
	...
	case'form_ajukan':
		...
		$kodeorg=makeOption($dbname,'sdm_pengajuantiket','notransaksi,kodeorg',"notransaksi='".$param['notransaksi']."'");
	...
	$tab = "<table cellspacing=1 border=0 width=100%>
				...
				<tr class=rowcontent>
					...
					<td width=5px>:</td>
					<td><select id=kepada >".$optKry."</select></td>
				</tr>
				<tr class=rowcontent>
	...
	case'ajukan':
		...
		$str = "update " . $dbname . ".sdm_pengajuantiket set status='9', postingtime='" . date('Y-m-d H:i:s') . "', "."diajukanoleh='" . $_SESSION['standard']['userid'] . "' where notransaksi = '" . $notransaksi . "'"; 
		#exit("error".$str);
		$owlPDO->exec($str);
